#!/usr/bin/env bash
# ledger-vitals.sh — 원장 활력 4축 (슬림 복귀 1호: ①②③ · 2호: ④)
# 근거 사고 — ①②③ = BL-643 「다음 행동」 중복 · RESOLVED 13건 역류 / ④ = 2026-08-31 방향 재설정(status.md ⓬)
# ① status.md 살아 있는 `다음 행동 =` ≤1 (파일 전체 — 취소선·인라인코드·코드펜스 제외)
# ② status.md ⓪ 표 데이터 행 ≥1 (2026-08-25 사용자 결정 ≥3→≥1 — 실재 후보가 2건뿐인데 3행을
#    강제하니 도래 안 한 행이 억지로 올라왔다. 표의 존재 이유는 「고르는 자리」가 아니라 「진입점 실재」다)
# ③ backlog.md 에 상태줄 RESOLVED 섹션 0건 (끝난 것은 **지운다** — git 이 원문을 갖는다)
# ④ 살아 있는 「다음 행동 =」이 PRD §5 를 겨냥한다 (방향 게이트 — ①②③ 은 전부 **양**만 잰다)
...
# ── ② ⓪ 표 데이터 행 ≥1 — 빈 표는 진입점이 아니다 (≥3→≥1: 2026-08-25 사용자 결정, BL-650 종결 회차) ──
rows=$(awk '
  /^[ \t>]*(```|~~~)/ { fence = !fence; next }   # 들여쓰기·인용부(>) 뒤 백틱/물결 펜스도 토글 (bl-audit 계약)
  fence  { next }
...
' "$STATUS")
if [ "$rows" -lt 1 ]; then
  echo "✗ ② ⓪ 다음 후보 표의 데이터 행이 ${rows}개다 (계약 ≥1) — 빈 표는 진입점이 아니다"
  FAIL=1
fi
...
# ── ④ 진입점이 PRD §5 를 겨냥하는가 — 방향 게이트 (슬림 복귀 2호) ──
# 근거 사고 = 2026-08-31 방향 재설정(`docs/status.md` ⓬). 14 회차 연속으로 결함 수리가 진입점에
# 앉았는데 ①②③ 은 **전부 양(量)의 게이트**라 한 번도 그것을 못 봤다. status.md ⓻ 절이 한 층
# 아래에서 이미 같은 문장을 적어 뒀다 — 「상한만 있고 하한이 없다」.
#
# ★★**이 축이 재는 것은 「§5 를 명시했는가」뿐이다.** 「실제로 §5 를 전진시키는가」는 기계가 못 잰다 —
#   §5 라고 적기만 하면 통과하므로 **게임 가능**하고 그 판정은 사람이 한다. 그래도 값이 있는 이유는
#   지금까지의 실패가 게임이 아니라 **망각**이었기 때문이다(14 회차 동안 아무도 묻지 않았다).
#   ⇒ **이 게이트의 초록을 「방향이 맞다」로 읽지 마라.** 「물었다」까지가 이것이 아는 전부다.
# ★부수 효과 = ① 의 하한. 살아 있는 「다음 행동 =」이 0 개면 참조도 0 개라 여기서 걸린다.
NEXT_LINE=$(awk '
  /^[ \t>]*(```|~~~)/ { fence = !fence; next }
  fence  { next }
  {
    ...
    while (match(s, /다음 행동[ 	]*[=＝]/)) {
      ...
      t = pre; nt = gsub(/~~/, "", t)
      b = pre; nb = gsub(/`/, "", b)
      if (nt % 2 == 0 && nb % 2 == 0) { print NR; exit }
      off += RSTART + RLENGTH - 1
      s = substr($0, off + 1)
    }
  }
' "$STATUS")
if [ -z "$NEXT_LINE" ]; then
  echo "✗ ④ 살아 있는 「다음 행동 =」이 0개다 — 진입점이 없는 상태는 초록이 아니다(① 은 상한만 잰다)"
  FAIL=1
else
  # 진입점 줄부터 5줄 안에서 §5 참조를 찾는다 — 근거를 바로 옆에 적게 하려는 폭이다.
  if ! sed -n "${NEXT_LINE},$((NEXT_LINE + 4))p" "$STATUS" | grep -q '§5'; then
    echo "✗ ④ 진입점이 PRD §5 를 겨냥하지 않는다 (${NEXT_LINE}행부터 5줄 안에 「§5」 없음)"
    echo "    결함을 고르는 회차라면 **왜 §5 를 전진시키는 일보다 먼저인지**를 그 줄에 적어라."
    FAIL=1
  fi
fi

[ "$FAIL" -eq 0 ] && echo "✓ ledger-vitals 4축 통과 (다음 행동=${live} · ⓪ 행=${rows} · 역류=0 · §5 겨냥=${NEXT_LINE}행)"
exit "$FAIL"
