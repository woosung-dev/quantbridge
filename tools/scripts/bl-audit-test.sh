...
#   ★⑰ 은 둘째 파일 첫 줄이 `### BL-` 이라 「`FNR == 1` 자체가 펜스인 줄」을 한 번도 안 밟는다.
#     그래서 `if (FNR == 1 && index($0,"```") == 0)` 로 초기화를 건너뛰는 변이가 **20/20 으로
#     통과했다**(리뷰어가 실증). 앞 파일이 펜스를 **열어 둔 채** 끝나고 둘째 파일 첫 줄이 ``` 면,
#     초기화가 없을 때 그 줄이 앞 파일의 펜스를 **닫아** 양쪽 다 오류 0 이 된다 — 조용한 초록이다.
R2_FENCE_FIRST='```

### BL-003

**우선순위:** P3
**상태:** ✅ **Resolved** (첫 줄 펜스 뒤 — 삼켜진다)
'
run_fixture "$R2_FENCE_FIRST" <<'EOF'
### BL-001
...
**우선순위:** P3
**상태:** ⬜ Open — 첫 파일이 펜스를 열어 둔 채 끝난다.
EOF
_why=""
[ "$RC" -eq 1 ] || _why="${_why}종료코드=$RC(기대 1) — 둘째 파일 첫 줄이 앞 파일의 펜스를 닫았다 "
printf '%s' "$OUT" | grep -q 'backlog.md 의 ``` 코드펜스가 닫히지 않았다' || _why="${_why}★첫 원장의 미닫힌 펜스를 안 잡았다 "
printf '%s' "$OUT" | grep -q 'backlog-resolved.md 의 ``` 코드펜스가 닫히지 않았다' || _why="${_why}★둘째 원장의 첫 줄 펜스를 안 잡았다(FNR==1 초기화가 펜스 줄을 건너뛴다) "
report "⑲ 둘째 원장 첫 줄이 펜스 → 양쪽 다 red" "$_why"

# ── ⑳ `<details>` 가 파일 경계를 넘는다 — 펜스와 **다른 변수**다 ──────────────────
#   ★⑰⑲ 는 `fence` 만 잰다. `details` 기록을 지우는 변이는 그 셋을 전부 통과했다(리뷰어 실증).
#     두 구간은 같은 자리에서 초기화되지만 **다른 배열에 기록**되므로 케이스도 따로 있어야 한다.
#   ★음성 대조를 같은 fixture 에 넣었다 — 둘째 원장은 `<details>` 를 **제대로 닫으므로**
#     그 파일 이름은 보고에 나오면 안 된다. 「항상 서식 오류를 낸다」는 파서를 여기서 배제한다.
R2_DETAILS='### BL-003
...
EOF
_why=""
[ "$RC" -eq 1 ] || _why="${_why}종료코드=$RC(기대 1) — 앞 파일의 미닫힌 <details> 가 기록되지 않았다 "
printf '%s' "$OUT" | grep -q 'backlog.md 의 <details>' || _why="${_why}★첫 원장의 미닫힌 <details> 를 안 잡았다 "
