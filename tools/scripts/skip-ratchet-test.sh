...
#   위반이 사는 `apps/api/tests` 가 통째로 안 스캔돼도 `apps/api/src` 혼자 합계 하한을 넘겨
...
#
# ★fixture 는 임시 트리에 만든다 — 실제 `apps/api/` 를 절대 건드리지 않는다.
...
#
# 사용법: tools/scripts/skip-ratchet-test.sh

set -uo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd -P)"
RATCHET="$ROOT/tools/scripts/skip-ratchet.sh"
[ -f "$RATCHET" ] || {
...
#   TESTS_N 은 `apps/api/tests` 하한과 **정확히 같다** — 케이스 ①(하한 = 통과)과
#   ⑥(하한−1 = rc=3)이 경계를 양쪽에서 동시에 재기 때문이다.
#   SRC_N 은 `apps/api/src` 하한보다 크되 **옛 합계 하한 200 을 단독으로 넘는다**(위 ★★).
...
build_base() {
  rm -rf "$TMP/base"
  mkdir -p "$TMP/base/apps/api/tests" "$TMP/base/apps/api/src"

  # 깨끗한 트리에도 **음성 대조를 실물로** 심는다. 케이스 ①이 「빈 트리라 조용한 것」이
  # 아니라 「봤는데 셀 것이 없는 것」이 되게 한다.
  cat >"$TMP/base/apps/api/tests/test_conditional.py" <<'FIX'
...
def test_ok() -> None:
    pass
FIX
  cat >"$TMP/base/apps/api/tests/test_quoted.py" <<'FIX'
...
def test_quoted() -> None:
    pass
FIX
  cat >"$TMP/base/apps/api/src/marker_factory.py" <<'FIX'
...
FIX
  ...
  cat >"$TMP/base/apps/api/tests/notes.md" <<'FIX'
과거 기록
@pytest.mark.skip(reason="이건 파이썬 파일이 아니다")
FIX
  : >"$TMP/base/apps/api/tests/fixtures.json"
...
for scope, want in (("apps/api/tests", tests_n), ("apps/api/src", src_n)):
...
echo "  fixture: apps/api/tests $TESTS_N · apps/api/src $SRC_N (tmp 트리)"
echo ""
...
new_tree
put "apps/api/tests/test_dead.py" '# 3개월째 꺼져 있다
import pytest
...
[ -z "$why" ] && ! has "apps/api/tests/test_dead.py" && why="위반 경로를 출력에 안 적었다"
report "② 데코레이터 위반 1건 → rc=1 + 경로 표기" "$why"

# ③ 모듈 레벨 위반 1건 → rc=1 (형태 축이 2개인지)
new_tree
put "apps/api/src/dead_module.py" '# 파일을 통째로 끈다.
import pytest
...
why=""
[ "$RC" -eq 1 ] || why="rc=$RC (기대 1)"
[ -z "$why" ] && ! has "apps/api/src/dead_module.py" && why="위반 경로를 출력에 안 적었다"
report "③ 모듈 레벨 위반 1건 → rc=1" "$why"

# ④ ★BL-705 본체 — `apps/api/tests` 통째 소실. 옛 코드는 rc=0 이었다.
new_tree
rm -rf "$TMP/tree/apps/api/tests"
run
why=""
...
report "④ apps/api/tests 통째 소실 → rc=3 + 「경로 부재」" "$why"

# ⑤ 반대 축도 대칭인가 — `apps/api/src` 통째 소실
new_tree
rm -rf "$TMP/tree/apps/api/src"
run
why=""
[ "$RC" -eq 3 ] || why="rc=$RC (기대 3) — 반대 스코프에는 하한이 없다"
report "⑤ apps/api/src 통째 소실 → rc=3" "$why"

# ⑥ 경계 — tests 를 하한−1 로. `<` 인가 `<=` 인가.
new_tree
victim="$(find "$TMP/tree/apps/api/tests" -name 'pad_*.py' | head -1)"
rm -f "$victim"
run
why=""
...
new_tree
put "apps/api/tests/test_more_conditional.py" '# 조건부는 정상이다.
import pytest
...
new_tree
put "apps/api/tests/test_dead.py" 'import pytest
...
[ -z "$why" ] && ! has "apps/api/tests/test_dead.py:4" && why="위치(파일:줄)를 안 냈다"
