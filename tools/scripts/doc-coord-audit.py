...
@lru_cache
def existing_markdown_basenames() -> frozenset[str]:
    paths = (
        *REPO_ROOT.glob("*.md"),
        *(REPO_ROOT / "docs").rglob("*.md"),
        *(REPO_ROOT / "apps" / "api").rglob("*.md"),
        *(REPO_ROOT / "apps" / "web").glob("*.md"),
    )
    return frozenset(path.name for path in paths)


def reference_exists(candidate: str, source_path: Path) -> bool:
    candidate_path = Path(candidate)
    if candidate.startswith(("./", "../")):
        return (source_path.parent / candidate_path).resolve().is_file()
    if any(path.is_file() for path in (source_path.parent / candidate_path, REPO_ROOT / candidate_path)):
        return True
    return "/" not in candidate and candidate_path.name in existing_markdown_basenames()
...
def find_dead_path_findings() -> list[Finding]:
    ...
    for source_path, line_number, line in iter_documentation_source_lines():
        for match in MARKDOWN_PATH_RE.finditer(line):
            ...
            if is_historical_reference(line, match.start()) or reference_exists(candidate, source_path):
                continue
...
def run_dead_paths() -> int:
    ...
    print(f"✗ 죽은 문서 경로 {len(findings)}건")
    for finding in findings:
        print(f"  {relative_to_root(finding.path)}:{finding.line}: {finding.text}")
    return 1
...
def parse_args() -> argparse.Namespace:
    ...
    mode.add_argument("--dead-paths", action="store_true", help="존재하지 않는 문서 경로를 검사한다")
