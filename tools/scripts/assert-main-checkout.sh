...
# 슬롯 번호(`QB_SLOT`)로 판정하지 않는다. 그건 `make QB_SLOT=0`(구 러너) 한 줄로 꺼졌다.
# ★mise 로 옮긴 뒤에도 같다 — `--skip-deps`·`MISE_TASK_SKIP_DEPENDS=true` 가 depends 를 통째로
#   끄므로, 가드는 각 task 의 `run` **첫 줄**에 인라인으로 있다(2026-08-16 실측 [ADR-036]).
#
...
✗ '${_target}' 은 메인 체크아웃 전용이다 (여기는 워크트리).
  ...
  → 메인에서 실행해라: cd $(dirname "$_gc") && mise run ${_target}
  (근거: docs/development/worktree-parallel.md §2.1)
EOF
exit 1
