...
★★**커밋은 step 당 2개가 아니라 4개다** (2026-08-15 스모크 실측 — `smoke/` 참조).
`_build_preamble` 규칙 6 이 세션에게 「모든 변경사항을 커밋하라」고 지시하므로 **세션이 먼저 커밋**하고
러너가 또 커밋한다 — 같은 메시지의 `feat` 이 두 번 난다. 그리고 세션이 `index.json` 을 자기 feat
커밋에 넣어 버리므로 **`_commit_step` 의 2단 분리가 실물에서 발화하지 않는다.** 상류 설계의 모순이다.
