# Step 3: 자기 변이 검증 + 회귀 — strategy · optimizer · waitlist React Query 훅
...
3. `python3 tools/harness/assert_fe.py apps/web/coverage/fe6-hooks-strategy-opt --min-cases 22 --target src/features/strategy/hooks.ts --min-cov 50 --target src/features/optimizer/hooks.ts --min-cov 50 --target src/features/waitlist/hooks.ts --min-cov 50`
...
7. `cd apps/web && pnpm exec biome check src/features/strategy/__tests__/hooks.core.test.tsx src/features/optimizer/__tests__/hooks.core.test.tsx src/features/waitlist/__tests__/hooks.core.test.tsx`
