# Step 2: 에러·경계 분기 — celery 워커 수명주기 · ws 스트림 태스크
...
3. `cd apps/api && uv run coverage report --include=src/tasks/celery_app.py --fail-under=64`
4. `cd apps/api && uv run coverage report --include=src/tasks/websocket_task.py --fail-under=76`
5. `cd apps/api && uv run coverage report --include=src/tasks/_ws_lease.py --fail-under=90`
