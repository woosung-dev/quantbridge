...
3. `python3 tools/harness/assert_fe.py apps/web/coverage/fe6-api-strategy-opt --min-cases 24 --target src/features/strategy/api.ts --min-cov 85 --target src/features/optimizer/api.ts --min-cov 85 --target src/features/alert-rules/api.ts --min-cov 85`
...
7. `cd apps/web && pnpm exec biome check src/features/strategy/__tests__/api.test.ts src/features/optimizer/__tests__/api.test.ts src/features/alert-rules/__tests__/api.test.ts`
