...
3. `python3 tools/harness/assert_fe.py apps/web/coverage/fe6-api-strategy-opt --min-cases 16 --target src/features/strategy/api.ts --min-cov 55 --target src/features/optimizer/api.ts --min-cov 55 --target src/features/alert-rules/api.ts --min-cov 55`
