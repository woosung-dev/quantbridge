# Step 1: strategy-contract — `features/strategy` 의 REST 계약을 고정한다
...
- `apps/web/src/features/strategy/api.ts` — 이 step 의 대상 (export **8개**)
...
`apps/web/src/features/strategy/__tests__/api-contract.test.ts` 를 **새로** 만든다.
...
- **`api.ts` 를 수정하지 마라.** 이 step 은 **현재 동작을 고정**하는 것이다. AC 가
  `git diff --quiet -- apps/web/src/features/strategy/api.ts` 로 집행한다.
...
- **fixture 의 필드를 임의로 만들지 마라.** BE 응답 스키마를 근거로 삼아라 —
  `apps/api/src/strategy/` 의 `schemas.py` 가 정본이다(경로가 다르면 찾아서 확인해라). **읽기만 해라.**

### 최소 커버리지

export 8개 중 **최소 6개 함수**에 `it(...)` 이 있어야 한다(AC 가 센다).
남은 것을 뺐다면 **왜 뺐는지** `summary` 에 적어라.

## Acceptance Criteria

```bash
test -f apps/web/src/features/strategy/__tests__/api-contract.test.ts
cd apps/web && pnpm exec vitest run src/features/strategy/__tests__/api-contract.test.ts
test "$(grep -cE '\bit\(' apps/web/src/features/strategy/__tests__/api-contract.test.ts)" -ge 6
cd apps/web && pnpm exec biome check src/features/strategy/__tests__/api-contract.test.ts
cd apps/web && pnpm exec tsc --noEmit
git diff --quiet -- apps/web/src/features/strategy/api.ts
```
