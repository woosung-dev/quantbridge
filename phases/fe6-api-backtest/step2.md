# Step 2: 에러·경계 분기 — backtest REST 클라이언트
...
케이스 하한 26 · 커버리지 하한 80%.

## Acceptance Criteria

1. `test -f apps/web/src/features/backtest/__tests__/api.test.ts`
2. `cd apps/web && pnpm exec vitest run src/features/backtest/__tests__/api.test.ts --coverage --coverage.include='src/features/backtest/api.ts' --coverage.thresholds.perFile --coverage.thresholds.lines=80`
