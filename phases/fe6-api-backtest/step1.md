# Step 1: 정상 경로 전수 — backtest REST 클라이언트
...
케이스 하한 17 · 대상 커버리지 하한 55% 는 AC 가 잰다.

## Acceptance Criteria

1. `test -f apps/web/src/features/backtest/__tests__/api.test.ts`
2. `cd apps/web && pnpm exec vitest run src/features/backtest/__tests__/api.test.ts --coverage --coverage.include='src/features/backtest/api.ts' --coverage.thresholds.perFile --coverage.thresholds.lines=55`
