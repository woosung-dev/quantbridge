# Step 5: `trading/` 패키지 3파일 — 가드 밖 metric mutation 감싸기

## 읽어야 할 파일

- `apps/api/src/trading/providers.py` — 수리 대상
- `apps/api/src/trading/realtime_publisher.py` — 수리 대상
- `apps/api/src/trading/websocket/bybit_private_stream.py` — 수리 대상
...
거래소 provider · 실시간 publisher · Bybit private 스트림이다. 셋 다 **거래소 왕복 경로**라 metric 실패가 주문·포지션 반영을 막으면 안 된다.
