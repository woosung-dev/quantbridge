# Step 2: `tasks/conditional_entry_janitor.py` — 가드 밖 metric mutation 감싸기

## 읽어야 할 파일

- `apps/api/src/tasks/conditional_entry_janitor.py` — 수리 대상
...
조건부 진입 청소기다. 여기서 metric 예외가 새면 **고아 조건부 주문이 정리되지 않은 채 루프가 멈춘다.**
...
cd apps/api && uv run ruff check src/tasks/conditional_entry_janitor.py
```
