# Step 4: `tasks/trading.py` — 가드 밖 metric mutation 감싸기

## 읽어야 할 파일

- `apps/api/src/tasks/trading.py` — 수리 대상
...
주문 발주·체결 반영 경로다. **머니-패스의 한복판**이라 metric 예외가 체결을 취소 실패로 오기록할 수 있다 — 이 회차의 원래 동기([BL-520])가 바로 이 축이다.
...
cd apps/api && uv run ruff check src/tasks/trading.py
```
