# Step 3: `tasks/_ws_circuit_breaker.py` — 가드 밖 metric mutation 감싸기

## 읽어야 할 파일

- `apps/api/src/tasks/_ws_circuit_breaker.py` — 수리 대상
...
WebSocket 서킷 브레이커다. 여기서 예외가 새면 **차단 판정 자체가 실패**해 스트림 복구 로직이 어긋난다.
...
cd apps/api && uv run ruff check src/tasks/_ws_circuit_breaker.py
```
