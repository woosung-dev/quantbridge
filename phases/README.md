...
★**결과 — 8/8 completed · 병합 충돌 0 · 변이 8/8 red**(복구 8/8 바이트 동일) · **대상 소스 전건 0줄 변경** ·
`xfail`/`skip`/`only` **0건**. `apps/api` **5,165 → 5,292 collected**(신규 8파일 · 92 테스트 함수 → **+127 케이스**).
PR = #755(① 사전 배치) · #756~#762(lane 7벌) · **#763**([BL-819] 결함 수리) · #764(lane 4).
★이 묶음은 닫혔다. **열려 있는 묶음은 지금 없다.**

★★**blocked 2건이 이 회차의 값이었다 — 둘의 성격이 정반대다:**

1. ★★★**`be5-di-assembly` 가 진짜 제품 결함을 잡았다 → [BL-819].**
   `build_backtest_service_for_worker()` 가 **기본 설정 `fixture` 에서 `UnboundLocalError`** 로 죽는다 —
   `else`(timescale) 갈래의 **함수-지역 재import** 가 `OHLCVRepository` 를 **함수 전체에서 지역화**해,
   else 를 안 타는 경로의 반환부가 미바인딩 지역변수를 읽었다. lane 은 **소스를 고치지 않고 `blocked` + 사유**를 남겼고
   CONTROL 이 코드 대조 + 실증으로 확인해 1줄 수리(#763)했다. **그 lane 의 `[backtest]` 파라미터 2건이 회귀 테스트**다.
   ⇒ **[LESSON-122] 계열 3번째** — `blocked` 를 자격증명 문제로 읽지 마라.
2. **`be5-dispatch-trio` 는 내 환경이었다.** `quantbridge_w7_test`·`w8_test` 가 **이전 회차가 남긴 낡은 스키마**였고
   (`strategies` FK 부재 → `drop_all` 이 없는 제약을 DROP), `worktree-bootstrap.sh` 는 「이미 있으면 안 만든다」라
   그대로 물려받았다. w4·w10 은 정상 — **4벌 중 2벌**이었다. ⇒ ★**슬롯을 재사용하면 테스트 DB 도 재사용된다.
   워크트리를 지워도 DB 는 남는다**(주인 없는 `quantbridge_w11_test`·`w12_test` 가 그 증거다).

## ★★★5차 회고 — 병렬 구조 자체에 대한 반성 (사용자 지적, 2026-08-21)

> 「병렬에 너무 집중한 나머지 효율이 안 나오는 구조 아니었나? 병렬인데 각각 step 1씩만 있어서.
> 좀 더 길게 병렬을 하거나, 어느 정도는 싱글로 묶고 중간부터 병렬로 해도 됐던 것 아닌가?」

**실측으로 보면 그 지적이 맞다.**

| 구간                               | 실제 소요                                                                                      |
| ---------------------------------- | ---------------------------------------------------------------------------------------------- |
| 러너 8벌 병렬 실행                 | **약 20분** (17:34:03 → 17:53:38, 6 lane 동시)                                                 |
| 같은 것을 순차로 돌렸다면 `[가정]` | 8 × 3~5분 ≈ **25~40분**                                                                        |
| **CONTROL(나)의 저작·측정·검수**   | **수 시간** — 재료 실사 · 커버리지 3회 재측정 · step 8벌 저작 · AC red · 변이 8벌 · PR·CI·머지 |

⇒ ★**병렬이 최적화한 것은 전체에서 가장 싼 구간이다.** 1~5차 내내 `phases/README.md` 가
「**저작이 상한이다**」라고 적어 뒀는데, 그 문장이 뜻하는 바를 이번에 숫자로 확인했다.

★**「step 1개짜리 lane 8벌」은 러너의 핵심 기능을 통째로 안 쓴 것이다.** 러너가 제공하는 것은
step **순차 실행 + 앞 step 의 `summary` 누적 전달 + 실패 시 피드백 재시도**인데, step 이 하나면
그 셋이 전부 죽고 남는 것은 「워크트리에서 에이전트 1회 호출」뿐이다.

**다음 회차에 쓸 두 구조 (이번 회차가 근거를 제공한다):**

1. ★★★**step 0 = 재료 자가검증, step 1 = 작성** — 이번 회차 최대 낭비는 **초판 8 lane 중 6개가 유령 재료**였고
   그것을 **내가 CONTROL 에서** 커버리지로 잡은 것이다. step 0 을 「대상의 커버리지를 직접 재고 미커버 줄을
   `summary` 에 적어라. 85% 초과면 `blocked`」로 두면 **lane 이 스스로 재료를 검증**하고, 그 결과가
   `summary` 로 step 1 에 넘어간다. **step 이 둘이면 러너가 비로소 러너답게 쓰인다.**
2. ★★**변이는 step 2 로 내려라** — 이번에 변이 8벌을 내가 손으로 심고 돌리고 복구했다. 「너 자신의 테스트에
   변이를 심어 red 를 확인하고 복구한 뒤 `summary` 에 적어라」는 lane 이 할 수 있는 일이다.
   ★단 **판정은 여전히 CONTROL 이 재실행해야 한다**(코드를 쓴 쪽이 자기 채점을 하면 판정이 무너진다).

★**「싱글로 묶고 중간부터 병렬」도 이번 구조에 이미 있었다 — 다만 러너 밖이었다.** 전량 스위트 커버리지
1회(17분 48초)가 8 lane 전부의 재료를 정한 **공유 선행 계산**이었고, 그것을 내가 손으로 돌렸다.
`phases/index.json` 은 **의존을 표현할 수단이 없어서**(README 첫 절) 그 단계를 회차 안에 넣을 수 없다.
⇒ **fan-out 앞에 공유 step 을 두려면 러너에 「선행 phase」 개념이 필요하다** — 없으면 지금처럼 CONTROL 이 진다.
...
★**결과 — 8/8 completed · retry 0 · blocked 0 · 병합 충돌 0 · 변이 8/8 red · 6분.**
`apps/web` **247 files/1,780 → 253 files/1,896 passed**(신규 6파일 **+116**) · `apps/api` 신규 **29 passed** ·
전이 폐포 미도달 `web` **33 → 15** · `api` **2 → 1** · **대상 소스 전건 무변경** · `xfail`/`skip`/`only` **0건**.
★이 묶음은 닫혔다. **열려 있는 묶음은 지금 없다** — 다음 재료는 `docs/status.md` 의 살아 있는 「다음 행동」이 든다.
★**FE 축은 바닥에 닿았다** — 남은 15 중 실질 후보는 `src/app/api/auth/[...all]/route.ts` 하나다.

★**착수 전 AC red 8/8 확인**(python 으로 각 AC 를 `bash -c` 서브셸에 넣어 측정) · **변이는 값·분기 수준으로만**
심었다(정렬 인자 · 정규식 `i` 플래그 · `userId={null}` · 갈래 반전 · 제목 충돌 · `encodeURIComponent` 제거 ·
fail-closed 무력화 · `dispose()` 제거) — 8/8 red · 복구 8/8 바이트 동일.

★★**4차가 더한 다섯 번째 줄 — 도구를 python 으로 바꿔도 「엉뚱한 축을 재는」 병은 안 낫는다**([LESSON-125]).
이 회차에서 내 판정 도구가 **세 번** 틀렸고 셋 다 python 이었다: ⑴ 리졸버가 패키지 **재export** 를
`__init__.py` 로 귀속시켜 거짓 재료를 냈다(AC red 가 잡았다) · ⑵ **`gh pr merge --squash --delete-branch` 의
rc 를 머지 판정으로 읽어** 8/8 머지를 **「0/8」로** 보고했다(실패한 것은 워크트리에 붙잡힌 **로컬 브랜치 삭제**
뿐이었다 — `state=MERGED` 로 대조해 갈랐다) · ⑶ 원장 섹션 슬라이스가 다음 `###` 까지 잡아 **인덱스 표 96행
(95KB)을 함께 삭제**했다(내 검증이 `### BL-` 헤더만 세고 표 행을 안 봐 한 번 놓쳤고, **95,321자라는 이상
신호를 내가 해명해 버렸다**). ⇒ **⑴ 복합 명령의 rc 를 한 부분의 판정으로 쓰지 마라 ⑵ 파괴적 편집에는
「폭 가드」(길이 상한 assert)와 **불변량 대조**(표 행 수·섹션 수)를 함께 걸어라 ⑶ 이상 수치는 해명하지 말고 재라.**
...
★이 묶음은 닫혔다. **다음 묶음 = `fe4-*`/`be4-*` 8벌**(위 — 2026-08-21 완주).
