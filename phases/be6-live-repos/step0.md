...
3. `python3 tools/harness/assert_be.py apps/api/.coverage.harness.json --target src/trading/repositories/live_signal_session_repository.py --min-cov 55 --target src/trading/repositories/live_signal_event_repository.py --min-cov 62 --target src/waitlist/repository.py --min-cov 64`
4. `cd apps/api && test "$(uv run --env-file .env.local pytest tests/trading/test_live_signal_repositories.py tests/waitlist/test_waitlist_repository.py --collect-only -q 2>/dev/null | grep -c '::')" -ge 6`
