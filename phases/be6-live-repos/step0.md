# Step 0: 재료 실사 + 테스트 파일 신설 — live-signal 세션·이벤트 저장소 + waitlist 저장소
...
3. `cd apps/api && uv run coverage report --include=src/trading/repositories/live_signal_session_repository.py --fail-under=55`
4. `cd apps/api && uv run coverage report --include=src/trading/repositories/live_signal_event_repository.py --fail-under=62`
5. `cd apps/api && uv run coverage report --include=src/waitlist/repository.py --fail-under=64`
6. `cd apps/api && test "$(uv run --env-file .env.local pytest tests/trading/test_live_signal_repositories.py tests/waitlist/test_waitlist_repository.py --collect-only -q 2>/dev/null | grep -c '::')" -ge 6`
