...
## 이미 지원 완료 (Week 3까지, 재구현 금지)
- **stdlib ta.***: sma / ema / atr / rsi / highest / lowest / change / crossover / crossunder / **pivothigh / pivotlow** + na / nz
- **strategy.***: entry(long/short, 시장가, **stop=** OCO pending fill), close, close_all, position_size, position_avg_price
- **built-in**: open / high / low / close / volume / bar_index / na + **syminfo.mintick** (0.01)
- **Pine 구문**: var / varip, user 변수 series subscript (`myvar[n]`), if-else, ternary, BoolOp 단축평가, chained Compare
- **Alert Hook v1**: collect_alerts() + classify_message() + condition-trace + discrepancy 자동 감지
...
## 진행 순서 (권장)
1. **Tier-1 가상 strategy 래퍼 먼저** → i1_utbot.pine E2E 검증 (렌더링 불필요, 단독 완결 가능)
2. **Tier-0 렌더링 scope A** → i2_luxalgo.pine E2E 검증 (line.get_price() 필수)

독립적이지만 위 순서가 각 단계 검증 명확. 병렬 작업 원하면 plan 단계에서 재조정 가능.
