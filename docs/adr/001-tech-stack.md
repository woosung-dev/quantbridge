...
| Auth            | ~~**Clerk**~~ → **Better Auth (자체 호스팅)** | ★2026-08-17 [ADR-034] 로 교체. proxy.ts 미들웨어는 유지, 클라이언트 훅은 `useAuthCtx()` 하나 |
...
| Auth            | ~~**Clerk JWT 검증**~~ → **JWKS 공개키 검증** | ★2026-08-17 [ADR-034]. `pyjwt[crypto]` 로 검증만 수행 — 백엔드 시크릿 0개 |
...
| 대안                     | 거부 이유                                                                                                                                                                                           |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Custom JWT (python-jose) | Clerk가 인증 위임으로 구현 비용 절감. 거래소 키는 별도 암호화 ★2026-08-17 재판정 — 인증은 self-host 로 갔지만 **직접 구현은 여전히 기각**이다(Better Auth 가 세션·해싱·회전을 갖고 온다, [ADR-034]) |
...
- DB 스키마에서 users.hashed_password 제거, id를 Clerk user_id 포맷으로 변경
  → ★Sprint 4 에 뒤집혔다: `users.id` 는 **UUID PK** 이고 외부 ID 는 별도 컬럼이다. 그 분리 덕에 2026-08-17 인증 공급자 교체가 컬럼 1개로 끝났다([ADR-034])
- Auth 관련 API 엔드포인트 (register/login/refresh) Clerk으로 대체 → ★2026-08-17 [ADR-034] 로 Next 앱의 `/api/auth/*` 가 그 자리를 가져갔다(FastAPI 는 검증만)
