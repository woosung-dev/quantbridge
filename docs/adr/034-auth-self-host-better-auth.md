...
절차는 [`better-auth-setup.md`](../operations/better-auth-setup.md) §5.
...
★★**초판 설계는 「클라이언트가 우리 API 를 먼저, 그다음 `deleteUser`」였고 그것이 틀렸다.**
2026-08-17 codex 적대 리뷰가 P1 으로 잡았다 — **엔드포인트와 회귀 테스트는 있는데 부르는 쪽이
0건**이었고(FE 에 호출 없음 · `deleteUser` 도 꺼져 있었다), 설령 배선했더라도 순서를 지키는지
아무도 보증하지 않는다. ⇒ **`deleteUser.beforeDelete` 가 서버에서 부르고, 204 가 아니면
throw 해서 삭제를 중단한다**(fail-closed). 우리 API 가 실패하면 인증 사용자도 남는다.
★이 회차가 같은 회차에 쓴 [LESSON-114] — 「가드는 소비자가 아니라 **생산자**를 세라」 —를
자기 코드에서 그대로 위반했다 — 외부 눈이 없었으면 「돈을 멈추는 경로」가 닿을 수 없는 채로 갔다.
