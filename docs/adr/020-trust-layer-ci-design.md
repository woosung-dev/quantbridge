...
> **ADR-020 (구 ADR-013, 2026-05-29 정식 renumber):** 본 문서는 원래 ADR-013 으로 작성됐으나, Sprint 54 의 Optimizer grammar ADR(`docs/dev-log/2026-05-12-sprint54-bayesian-genetic-grammar-adr.md`) 도 동일 ID 를 사용해 충돌했다 ("ID 재사용 금지" global §3). 광범위하게 참조되는 Optimizer 측이 ADR-013 을 유지하고, 본 Trust Layer CI 문서를 **ADR-020** 으로 renumber 했다. 과거 sprint 로그/코드의 "ADR-013 §4·§10·§11" (parity / mutation oracle / stage 2c) 은 모두 본 문서(현 ADR-020)를 가리킨다.

> **ADR ONLY:** 본 문서는 결정 근거다. 현재 Trust Layer 계약은 [`trust-layer-architecture.md`](../architecture/trust-layer-architecture.md)와 코드·테스트가 맡는다. 당시 요구사항/SLO 초안은 `docs/archive/product/requirements/2026-04-23-trust-layer-requirements.md`에 보존한다.
>
...
> **상위 문서 (SSOT):** [`docs/architecture/trust-layer-architecture.md`](../architecture/trust-layer-architecture.md) (Path β Stage 0 산출)
> **관련 Sprint:** Path β (Stage 0 문서 → Stage 1 설계 → Stage 2 구현) + Sprint Y1 Coverage Analyzer prerequisite
> **참조 아키텍처 서베이:** `docs/archive/superpowers/reports/2026-04-23-architecture-survey.html`
...
- `docs/architecture/trust-layer-architecture.md` (신규, 이 ADR 의 상위 아키텍처 요약)
...
3. **단일 ADR 필수**: v2 도입은 ADR-020 amendment 또는 신규 ADR 로 근거 기록
...
| **2026-04-23** | **Gate-3 1차 CONDITIONAL PASS (codex 8/10 + opus 8/10)**              | **C-1 즉시 해소: `conftest.py` 에 `pytest_addoption` 추가로 `--run-mutations` 플래그 실 구현 + `pytest_collection_modifyitems` 로 기본 pytest 실행 시 `@pytest.mark.mutation` 자동 skip (ADR-020 §10.1 Q2 실 구현). `.github/workflows/trust-layer-nightly.yml` 신규 (18:00 UTC cron + workflow_dispatch + failure 시 issue 자동 생성). W-2/W-3 (M2 명칭 불일치, M4 skip 구분) 은 Stage 2c 2차로 이연 명시**                                                                                                                                                                                                                                                                          |
...
| **2026-04-23** | **Stage 2c 2차 구현 완료 — Mutation 8/8 감지 (SLO TL-E-5 초과 달성)** | **T1 M3 `StrategyState.entry` no-op cascade (PASS) / T2 M5 `fill_price` ABS_TOL drift (PASS) / T3 M6 PnL Decimal × 1.0001 amplifier (PASS) / T4 M8 `VirtualStrategyWrapper.process_bar` duplicate fire (PASS) 4개 mutation 전부 감지. M1/M2/M7 기존 PASS + M4 `xfail(strict=False)` 현 corpus 에서 XPASS → 실질 8/8 감지. T5 W-2 M2 rename + W-3 M4 xfail marker 적용 — "미감지" vs "N/A" 의미론 구분. mutation suite 실행 2:31 (nightly 5분 예산 여유). backend 985 pass / 17 skip / 0 fail 유지 (regression 0). SLO TL-E-5 ≥7/8 **GREEN\*\* → Path β 완료 선언 준비. Gate-4 검증 pending                                                                                            |
