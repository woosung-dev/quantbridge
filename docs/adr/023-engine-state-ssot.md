...
> **상류에서** 닫았다. **이 머리말이 재판정 요약이다**(전문 = `git show 4c65bc0e:docs/adr/023-engine-state-ssot.md` §재판정) — 못 읽으면
...
> **결론은 바로 위 두 줄이다**(선행 측정 전문 = `git show 4c65bc0e:docs/adr/023-engine-state-ssot.md` §선행 측정) — 이 문장을 「지금 병이 그쪽에서 난다」로 읽으면
...
> **잘라낸 것: 6절 · 364줄 · 30,579B.** 원문 = `git show 4c65bc0e:docs/adr/023-engine-state-ssot.md`.
>
...
>
> ★**살아 있는 코드가 이 중 §폭발 반경 3 을 §이름으로 인용한다** — `apps/api/tests/strategy/pine_v2/test_conditional_fill_authority_isolation.py:118`.
> 그 주석을 고치지 않았다(이 회차는 코드 무변경). **절 본문이 필요하면 위 git 좌표를 열어라.**
>
> 잘라낸 절:
...
> - ★2차 발화 기록 (2026-08-07) — 재개 조건이 발화했다. 그러나 표적은 이 축이 아니었다 (50줄·### 2개)
