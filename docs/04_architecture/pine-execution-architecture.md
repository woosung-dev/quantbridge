...
- `SUPPORTED_FUNCTIONS` (91 항목): TA/Strategy/Utility/Declaration/Plot/Input/String/Math/V4 aliases
- `SUPPORTED_ATTRIBUTES` (21 항목): 시계열 + strategy state + syminfo + barstate
- `SUPPORTED_ENUM_CONSTANTS` (40+ 항목): `color.*`, `shape.*`, `plot.style_*`, `line.style_*`
...
**SSOT 동기화 규약:** 새 stdlib 함수 추가 시 **3 파일 동시 갱신**:

1. `stdlib.py` 구현
2. `interpreter.py` dispatch 등록
3. `coverage.py` 해당 group frozenset 에 추가

이 규약의 준수는 **Path β P-2 (Coverage SSOT Sync)** 가 CI 에서 자동 검증 — 자세한 설계는 [`dev-log/013-trust-layer-ci-design.md`](../dev-log/013-trust-layer-ci-design.md) §4.2 + [ADR-016 Sprint Y1](../dev-log/016-sprint-y1-coverage-analyzer.md) 참조.

**Trust Layer 2 축 관계:**

- 축 1 (사용자 facing): Y1 Coverage Analyzer
- 축 2 (엔지니어 facing): Path β 3-Layer Parity CI
