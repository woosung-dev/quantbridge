...
#### 2.0.2 PyneCore 런타임 참조 이식 (1주)

- [PyneSys/pynecore](https://github.com/PyneSys/pynecore) (Apache 2.0, 121★, v6.4.2) 참조
- 이식 대상:
  - `var` / `varip` 영속 변수 구현
  - bar-by-bar rollback 메커니즘 (realtime bar 재진입 시 committed state 복원)
  - `na` 3-value logic (na + 5 = na, na == na → na)
  - `[n]` history 연산자 구현
- **이식 전략:** PyneCore 코드 직접 copy가 아닌 **로직 참조** 후 QB AST 구조에 맞춰 재구현. Apache 2.0 허용하나 NOTICE 파일 의무 준수.
...
**왜 TV가 아닌 PyneCore가 오라클인가:**
- TV는 공식 지표값 export API 부재 → 자동화 불가
- 헤드리스 브라우저는 TV ToS 회색지대
- PyneCore는 Apache 2.0 로컬 CI로 매 PR 돌 수 있음
- TV 수동 간헐 검증은 분기 1회 (샘플 10개)
...
| 리스크 | 심각도 | 확률 | 완화책 |
|------|:---:|:---:|--------|
| PyneCore `trail_points` 미지원 | 🔴 High | 30% | Phase -1 Day 1-2 즉시 확인. 미지원 시 Tier-3 자체 구현 +2주 |
