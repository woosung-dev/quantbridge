...
- **상황:** 2026-08-07 [BL-618]. 공백 재동기 판정이 원장보다 먼저 뛰어 정상 세션을 죽였고,
...
- **1차 누적.** ★상한은 새 상수로 만들지 마라 — 이번엔 `conditional_entry_janitor` 의
  `_SCAN_STUCK_THRESHOLD_MINUTES` 가 이미 그 역할을 했고(문턱을 넘기면 주문이 terminal 로 가
  판별자가 비므로 미룸이 유한하다), 새 상수를 뒀다면 **같은 불변식의 두 번째 진실**이 됐다.
