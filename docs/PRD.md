...
| **사용 마찰 건수** | (분모: 여정 완주 6회 · 후보 1벌) | **신규 1건 + 기존 BL 재발현 0건** — 온보딩 진입 링크 0건(수리됨). 코드 대조 산물(임포트 `unsupported` 배선)은 마찰 카운트 밖 | 🟢 첫 분모 확보 |
...
| 지표 | 분모가 생기는 조건 |
| --- | --- |
| Pine 파싱 성공률 | ~~실제 임포트 후보 N벌이 등록됐을 때 (지금 분모 = 테스트 코퍼스 9벌뿐)~~ → ★**2026-09-06 시도했고 세우지 못했다.** `strategies.parse_status` 로는 **구조적으로 영원히 100%** 다 — 위저드가 파스 실패·미지원을 **저장 자체에서 거부**하고(`new-strategy-wizard.tsx:106-110`) `POST /strategies/parse` 는 행을 안 남긴다(로컬 실측 6/6 = 100%). 분모는 DB 가 아니라 **사용자가 가져온 원본 `.pine` N벌**이어야 하고, 그때도 **두 수**(파스 성공률 / 실행 가능률)로 갈라야 한다 — 한 수로 적으면 「파스는 됐는데 못 돌린다」가 100% 뒤에 숨는다. ★**2026-09-06 절반이 열렸다** — 저장 경로가 `parse_status="unsupported"` 를 실제로 set 하게 되어 **「실행 가능률」은 이제 DB 로 잴 수 있다**(임포트된 것 중 몇 %가 돌아가나). 남은 절반(파스 성공률)은 여전히 저장 전에 걸러지므로 **원본 `.pine` N벌**이 있어야 한다 |
| ~~임포트 → 첫 백테스트 결과 시간~~ | → **2026-09-06 §⑴ 로 승격**(33.58s → 5.69s) |
| ~~사용 마찰 (막힌 지점 건수)~~ | → **2026-09-06 §⑴ 로 승격**(신규 1건) |

### 최신 실사용 관측 — 2026-09-10

기존 Pine 코퍼스 5벌을 실제 로그인·파일 임포트·백테스트·Monte Carlo 화면으로 검증했다.
요청 구간은 2026-04-01 00:00~06-30 00:00 UTC, Bybit USDT 무기한선물 BTC/USDT 1h다.
전부 실제 2,161봉·누락 0봉, 파스/실행 가능/여정 완료 각각 5/5다. **기존 코퍼스의 성공률이며 외부 Pine 전체로 일반화하지 않는다.**
수수료·슬리피지·사이징은 각 실행 config와 원본 Pine에 따르므로 서로 다른 사이징의 수익률을 동일 위험 순위로 읽지 않는다.

| 전략 | 수익률 | 판정 |
| --- | --- | --- |
| Pivot Reversal | −2.09% | 이번 조건의 운용 후보 제외 |
| UT Bot | −3.74% | 이번 조건의 운용 후보 제외 |
| RSI Divergence | −242.39% | 자본 음수·샤프 계산 불가, 제외 |
| HMA Curvature | −45.29% | 이번 조건의 운용 후보 제외 |
| EMA Trend | −1.02% | 이번 조건의 운용 후보 제외 |

표본 하나를 채택하는 대신 **다섯 모두 탈락시키는 판단**까지 완료했다. Monte Carlo는 완료 거래의 재표집이며
별도 기간 검증이 아니다. 실행별 config·지표·원장 ID 원문은 로컬 `runs/verified-strategy-journey/decisions.json`,
재실행 방법은 `development/ci-cd.md`의 실제 전략 여정 절이다. CI는 고정 CSV로 대표 여정을 검증하므로 이 시장 실측과 구분한다.
