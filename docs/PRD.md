...
| ~~단일 심볼 1Y/1H 백테스트~~ | ★**2026-08-25 재기술 — 1Y 코퍼스와 전략 무관 단일 목표는 근거·측정 수단이 없어 성립하지 않는다.** |
| 백테스트 처리량 (6M/1H corpus, `run_backtest_v2`) | **실측: 277.18~3,689.36 bar/s** (`s2_utbot`~`s5_ema_trend`; 측정 = `apps/api/tests/strategy/pine_v2/test_execution_speed.py`). **[가정] 1Y(8,760 bar) 선형 환산: 약 2.4~31.6초** — 6M 실측의 선형 외삽이며 1Y 실측이 아니다. ★**2026-08-25 이 환산은 과대 추정임이 확인됐다** — 분모 `run_backtest_v2` 가 **파스를 포함**하는데 파스는 bar 수와 무관한 고정비다(`s3_rsid` 21.752초 = parse 16.432 + execute 5.185 + 후처리 0.132). bar 수를 2배로 늘리면 커지는 것은 execute 뿐이라 선형 환산 43.6초 대신 **약 27.1초**가 맞다 ⇒ **`bar/s` 는 데이터가 길수록 저절로 좋아지는 수다.** 고정비를 분모에서 빼거나 cold/warm 을 나눠 두 수로 내는 것이 다음 회차 과제다 |
| 데모 주문 체결 레이턴시 | < 2초 |
| ~~백테스트 정확도 (vectorbt 직접 실행 대비)~~ | ★**측정 불가 — 대조군이 없다.** vectorbt 는 2026-08-06 에 **의존성째 제거**됐다(`backtest/engine/__init__.py`). 이 줄은 목표가 아니라 **죽은 문장**이었다. 정확도를 재려면 새 대조군을 먼저 정해라 |
| 임포트 → 첫 백테스트 결과 | < 5분 |
| 백테스트 → 데모 트레이딩 | 3클릭 이내 |

★**이 표는 목표지 실측이 아니다.** 실측하려면 그 회차에 재고 `lessons.md` 에 남겨라.
★★**2026-08-24 실측 — 6줄 중 단 하나도 잰 적이 없다**(`grep` 로 `lessons.md` 대조 = 0건). 그리고 그중 하나는 위처럼 **측정 자체가 불가능**했다. 목표를 적어 두고 한 번도 재지 않으면 그 표는 제품 문서가 아니라 희망 목록이다.
