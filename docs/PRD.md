...
| 백테스트 처리량 (6M/1H corpus, `run_backtest_v2`) | **실측: 277.18~3,689.36 bar/s** (`s2_utbot`~`s5_ema_trend`; 측정 = `apps/api/tests/strategy/pine_v2/test_execution_speed.py`). **[가정] 1Y(8,760 bar) 선형 환산: 약 2.4~31.6초** — 6M 실측의 선형 외삽이며 1Y 실측이 아니다 |
| 데모 주문 체결 레이턴시 | < 2초 |
