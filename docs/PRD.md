...
> ★★★**2026-08-31 재건.** 2026-08-30 에 이 표를 닫은 근거는 「6줄 중 한 줄도 잰 적이 없다」였다
> (원문 = `git show ec26e28d:docs/PRD.md`). **재건 규칙은 하나** — **지금 잴 수 있는 것만 적고,
> 적는 회차에 한 번 잰다**([LESSON-129]). 못 재는 줄은 §⑶ 으로 내려가고 표에 올라오지 않는다.
> **다시 희망 목록이 되면 다시 닫아라.**

### ⑴ 재고 있는 것 — 2026-08-31 서버 원장 실측 + **2026-09-06 d1 실사용 루프**

| 지표 | 목표 | 실측 (2026-08-31) | 판정 |
| --- | --- | --- | --- |
| **데모 시장가 체결 레이턴시** | < 2초 (2026-04 원안) | **p50 1.829s · p95 2.509s · max 18.910s** (n=356 · `trading.orders` 전량 중 `trigger_price IS NULL`) | 🟡 **p50 통과 · p95 미달** |
| **백테스트 → 라이브 성과 괴리** | (원안에 없음 — 핵심가치 ③ 의 수) | 백테스트 PF **0.6069** → 라이브 PF **0.373** = **−38.5%** (`PbR Pivot Reversal` · 백테스트 n=1,028 · 라이브 n=1,578 / 16일) | 🔴 **격차 미해명** |
| **임포트 → 첫 백테스트 결과 시간** | < 5분 (`onboarding/types.ts:26` TTL 이 정한 자기 목표) | **33.58s → 5.69s (−83.1%)** · median of 3 · 스프레드 33.42~33.89 → 4.97~5.74 · 신규 가입부터는 **16.3s**(n=1) · 측정 = `apps/web/e2e/friction-run.mjs`(2026-09-06) | 🟢 **목표의 1/50** |
| **사용 마찰 건수** | (분모: 여정 완주 6회 · 후보 1벌) | **신규 1건 + 기존 BL 재발현 0건** — 온보딩 진입 링크 0건(수리됨). 코드 대조 산물([BL-855])은 마찰 카운트 밖 | 🟢 첫 분모 확보 |
| 백테스트 처리량 (6M/1H corpus, `run_backtest_v2`) | — | **277.18~3,689.36 bar/s** (`s2_utbot`~`s5_ema_trend`) · 측정 = `apps/api/tests/strategy/pine_v2/test_execution_speed.py` | ⚠ cold/warm 미분리 |

★**체결 레이턴시는 시장가만 잰다.** 조건부 주문을 섞으면 p50 이 **355.8초**가 되는데 그것은 레이턴시가
아니라 **트리거 대기 시간**이다(전량 n=4,141). 분모를 안 가르면 잰 적 없는 수보다 나쁘다.

★**처리량 수를 그대로 1Y 로 환산하지 마라.** 분모 `run_backtest_v2` 가 **파스를 포함**하는데 파스는 bar 수와
...
길수록 저절로 좋아지는 수다.** cold/warm 을 나눠 **두 수**로 내는 것이 남은 일이다([BL-830]).

### ⑵ 실사용 축 — **2026-08-31 사용자 결정: 「만든 사람이 진짜 사용자가 된다」**

결정 ⑴⑵⑶ 을 **하나도 건드리지 않는다.** 나침반 셋(실자금 · 외부 사용자 · 멀티 거래소)을 다 뗀 상태에서
방향을 되찾는 가장 싼 경로로 이것을 택했다.

**왜 이 축인가 — 2026-08-31 서버 원장 실측:**
...
⇒ **§3 이 약속한 「검증 확장」(Monte Carlo · Walk-Forward · 파라미터 안정성)은 한 번도 쓰인 적이 없다.**
같은 10일에 코드는 **소스 +115,150줄 · 테스트 +216,427줄**이 늘었다(최근 300커밋 실측).
**만든 것을 만든 사람조차 쓰지 않는다** — 방향이 흐려진 지점이 여기다.

**이 축이 낳을 지표 3종** — 첫 루프를 한 바퀴 돌아야 분모가 생긴다. **그전에는 §⑴ 표에 올리지 않는다.**

| 지표 | 분모가 생기는 조건 |
| --- | --- |
| Pine 파싱 성공률 | ~~실제 임포트 후보 N벌이 등록됐을 때 (지금 분모 = 테스트 코퍼스 9벌뿐)~~ → ★**2026-09-06 시도했고 세우지 못했다.** `strategies.parse_status` 로는 **구조적으로 영원히 100%** 다 — 위저드가 파스 실패·미지원을 **저장 자체에서 거부**하고(`new-strategy-wizard.tsx:106-110`) `POST /strategies/parse` 는 행을 안 남긴다(로컬 실측 6/6 = 100%). 분모는 DB 가 아니라 **사용자가 가져온 원본 `.pine` N벌**이어야 하고, 그때도 **두 수**(파스 성공률 / 실행 가능률)로 갈라야 한다 — 한 수로 적으면 「파스는 됐는데 못 돌린다」가 100% 뒤에 숨는다. 선행 = [BL-855] |
| ~~임포트 → 첫 백테스트 결과 시간~~ | → **2026-09-06 §⑴ 로 승격**(33.58s → 5.69s) |
| ~~사용 마찰 (막힌 지점 건수)~~ | → **2026-09-06 §⑴ 로 승격**(신규 1건) |

### ⑶ 아직 못 세우는 것 — 선행 조건

1. **UX 지표**(클릭 수 · 온보딩 이탈)는 **외부 사용자**가 있어야 한다. 결정 ⑵ 가 열려야 한다.
   ★단 「첫 결과까지 시간」은 **§⑵ 가 자기 자신으로 잰다** — 결정 ⑵ 를 열지 않아도 된다.
2. **정확도 지표**는 대조군이 있어야 한다. vectorbt 를 지운 뒤(2026-08-06) 그 자리가 비어 있다.
...
**열린 결함** — [`backlog.md`](./backlog.md) 의 **미완 14건**(ACTIVE ∪ PARTIAL · 2026-08-31 실측 `grep -c '^### BL-'`)
+ `backlog-deferred.md` **24건**. ~~미완 10건~~ ~~DEFERRED 151건~~ 은 2026-08-30 원장 트리아주 전 수치다.
**P1 은 0건**이다(2026-08-30 아키텍처 감사 8건 전건 처리).
...
**다음에 할 일** — [`status.md`](./status.md) 의 살아 있는 `다음 행동 =` **하나**가 유일한 진입점이다.
★★**2026-08-31 부터 그 한 줄은 §5⑵ 실사용 축을 전진시켜야 한다.** ⓪ 표(결함)는 **후보 풀이지 진입점이 아니다** —
14 회차 연속으로 결함 수리가 진입점에 앉았고 어떤 게이트도 그것을 못 봤다(`ledger-vitals` 는 **개수만** 잰다).
결함을 고를 때는 **왜 §5 를 전진시키는 일보다 먼저인지**를 그 줄에 적어라.
