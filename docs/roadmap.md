...
- [ ] **BL-818** [P2] `apps/api` 미커버 모듈 커버리지 — **밤샘 루프 5차 · 8 lane 병렬**. 4차가 FE 축을 바닥까지 닫아(미도달 15 중 실질 후보 1건) BE 차례다. ★★**재료를 AST 가 아니라 커버리지가 정했다** — 초판 8 lane 중 **둘이 실제로는 100% 커버**였고 셋은 89~94% 였다. 확정 재료(착수 전 전량 스위트 **5,130 passed · TOTAL 90%** 기준): `run_alembic_with_lock` **0%** · `optimizer/stress_test_tasks` 35% · `health/router` **54%** · DI 4종 52~62% · `convert/service` 61% · `alert_rule_repository` 67% · `tasks/backtest`+dispatcher 3종 · `waitlist` 서비스 3종. 착수 전 **AC red 8/8 · 인접 회귀 8/8 green** · 대상 소스 0줄 변경. ★부수 발견 — `[tool.coverage.run]` 에 `concurrency` 가 없어 **async 커버리지가 낮게 나온다**(BL-308/309 래칫 동반 영향, 별도 판단 필요)
