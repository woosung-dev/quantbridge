...
- [ ] **BL-720** [P2] `docs-audit` 지식 정본 축 결손 3종 — LESSON ID 유일성 검사 부재 · [ADR-029] 이동 경로가 `legacy_paths` 에 없음 · 백틱 코드 스팬이 링크 검사 밖. 셋이 겹쳐 `lessons.md` 승격 표 7행이 존재하지 않는 파일을 가리키고 있었다 (2026-08-14 gate-surface-close 등재)
- [ ] **BL-722** [P3] `assert-main-checkout.sh` 판별력 덮개 부재 — 공유 자원(`make up/migrate/seed`)을 지키는 가드인데 하네스·`--selftest`·다른 층 테스트가 없다. §8.7 성문화 스윕에서 확정 (2026-08-14 gate-2stage)
