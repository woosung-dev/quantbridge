...
- [ ] **BL-816** [P3] `/not-available` 만 `metadata` 를 export 하지 않는다 — geo-block L2 **착지점**의 `<title>` 이 비어 나간다(법무 3종은 전부 있다). 처방 = 4줄. ★지금 동작은 `fe3-public-legal-pages` 테스트가 고정해 뒀다 — 고치면 red 로 뒤집히고 그것이 종결 신호다
