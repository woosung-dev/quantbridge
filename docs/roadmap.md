...
~~★**현재 최대 리스크는 미지정이다**~~ → **2026-08-03 지정됨: [BL-580] 잔여 129곳**이다
(`order_service.py` 10곳 · `trading.py` closed_pnl 7곳 — 아직 **코드 독해로만** 판정됐다).
직전 두 회차가 같은 종류의 산문 근거를 **전건 반증**했으므로 남은 곳도 고장 주입으로 재야 한다.
다음 스프린트 = `metric-guard-residual-close`(진입점 = `docs/status.md`).
