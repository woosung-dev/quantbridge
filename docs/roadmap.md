...
- [ ] **BL-817** [P2] `app/**` 라우트 조립층에 테스트가 0건이다 — [BL-786] 프리페치 키 동일성 · UUID→`notFound()` 가드 · `getServerAuth` 배선 · `metadata` 계약이 무증거다. 전이 폐포 미도달 `apps/web` **33/344**(실질 = `page.tsx` 14 + `layout` 2 + og 1) · `apps/api` ★**2/193**. ★★착수 프롬프트가 든 BE 후보 4 중 **둘은 이미 테스트가 있었다**(「BE 미도달 6」은 실측 2) ⇒ 8 lane = **FE 6 + BE 2**. ★프로브가 `next/font/google` top-level throw 를 잡아 ① PR 이 `resolve.alias` 로 뚫었다
