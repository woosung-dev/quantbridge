...
★**현재 최대 리스크는 [BL-544] 다** — **엔진이 자기 주문의 체결을 모른다.** 거래소는 stop 을 bar **안에서 실시간** 트리거하고 엔진 재생은 **bar 종가 기준**이라, 조건부 진입이 체결돼도 재생은 그 진입을 만들지 않는다 → `exchange_only` 발산 → **>5분 공백마다 세션 사망**. 즉 리스크의 성질이 "계측기를 믿을 수 없다" 에서 **"세션이 오래 살지 못해 장시간 검증 자체가 불가능하다"** 로 옮겨갔다(2026-07-30 실측: 세션이 **18분** 만에 죽었다).
