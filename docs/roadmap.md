...
> ★**P별 ACTIVE 내역 = P0 1 · P1 6 · P2 66 · P3 83** (2026-08-05 재측정 —
> `bash scripts/bl-audit.sh` 실측. 같은 실행의 전체 판정 = **ACTIVE 156 · PARTIAL 7 ·
> RESOLVED 90 · UNKNOWN 0 / 전체 253**). ★**위 §3 제목 줄의 수치는 2026-07-30 스냅샷이고
> 이 실측이 최신이다** — 어느 쪽도 손으로 세지 말고 스크립트를 다시 돌려라. ★**직전 표기와의 차이
> 대부분은 그 사이 여러 스프린트가 쌓은 것이고 이번 회차 몫은 P1 −1 / P2 +1 하나뿐이다** —
> [BL-591] 강등(근거 = [`ADR-023 §재판정`](decisions/023-engine-state-ssot.md)). 나머지 증감은
> 귀속하지 않았다.
