...
- [ ] **BL-813** [P2] FE 순수 판정 모듈 테스트 0건 — **인증 경계가 무증거로 산다**. 전이 폐포 실측(2026-08-21): 소스 343 중 어떤 테스트도 import 하지 않는 것 **58**, 그중 판정 로직 5종이 완전 미도달 — `proxy.ts` · `lib/route-matcher.ts` · `lib/auth.ts`(geo L3 + 탈퇴 fail-closed) · `lib/auth-server.ts` · `lib/legal-links.ts`. ★이 레포는 같은 자리에서 「geo L3 이 한 번도 발화한 적 없었다」를 이미 밟았다([LESSON-114]). 처방 = 대상 **무변경** + 테스트 파일 10개 신설, 8 lane 워크트리 병렬(`phases/fe2-*`)
