...
★**2026-08-01 soak 으로 [BL-560]·[BL-566] 이 함께 닫혔다 — 최대 리스크 슬롯이 비어 있다.** 다음 지정은 CONTROL 이 한다(전략 결정이라 여기서 추정하지 않는다). 창 = 2026-07-31 17:40→18:46 UTC · 66분 · 66틱 · 세션 `dc1e08f1`.
...
- [ ] **BL-542** [P3] 계정 포지션 표의 "잘렸다" 경고가 포지션 1건에도 켜진다 — 거짓 양성 **확정** · **n=2**(계정 2/2) · **기전 확정**(2026-08-01: `providers.py:1201-1206` 주석이 "ccxt 는 커서를 첫 항목에 도장만 찍는다" 고 적어 놓고 `:1207` 이 그 커서를 "다음 페이지 있음" 으로 읽는다). 남은 것은 판정식 교체 + 회귀 가드 2케이스
