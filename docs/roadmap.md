...
★**현재 최대 리스크는 [BL-560] 이다 — 성질이 또 옮겨갔다.** 진입 유실을 쫓던 두 스프린트가 **분모가 틀렸음**을 확인하고 끝났다(BL-536 판정 철회). 대신 **청산측에서 엔진과 거래소가 반대 방향을 드는 상태**가 **5개 세션에 걸쳐 9건** 실재한다는 것이 원장에서 나왔다. 그 방벽은 `reduce_only=True` **하나뿐**이다. 지금까지 안 보였던 이유는 `110017` 무해 갈래(30건)가 같은 라벨에서 위험 갈래(9건)를 수적으로 가렸기 때문이고, PR #511 이 그 라벨을 갈랐다 — **이제 셀 수 있다.** ★다음 회차 첫 step 은 **신규 라벨이 실주행에서 발화하는지** 확인이다(사전등록 문턱 V1~V5 = `status.md`). **원인 3후보는 전부 크기 미측정이라 아직 짓지 않는다.**
