...
6. ★**live-entry-completeness** (다음 후보 A — **BL-536 + BL-553**) — 세션이 이제 오래 살므로 **진입 유실 5채널 재측정**이 성립한다. ★계측기를 먼저 고르고 그 계측기를 검증하라 — `engine_only` 는 재생 아티팩트로 오염됐고 `live_signal_events` 는 **조건부 진입이 거치지 않는다**. 유효한 것은 `orders.idempotency_key` 분해뿐이다. 같은 soak 에서 BL-553(`applied` 실주행 확인)을 기회주의적으로 닫는다.
