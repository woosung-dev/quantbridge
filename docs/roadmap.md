...
- [x] **BL-414 ✅ Resolved (2026-08-17 night3 레인 γ)** [P3] 스트레스 테스트 이력 리스트 UI — 이력 표 + 행 선택으로 상세 교체. BE 는 `headline_metric` 한 필드만 늘었다. ★원장의 「페이지 응답」은 BE 엔드포인트가 아니라 **FE React Query 캐시**였다(레인 파일이 오독했고 코드 대조가 바로잡았다)
- [x] **BL-427 ✅ Resolved — 원장이 낡았다 (2026-08-17 night3 CONTROL 코드 대조)** [P3] 전략 목록 파라미터 열 / 수명주기 칩 — BE 필드도 **FE 렌더도** 이미 있었다(`strategy-list.tsx:449`·`:417`). 상태줄이 주장한 「FE 미렌더 사유 주석」은 strategy 도메인에 0건
- [ ] **BL-428** [P3] 트레이드 구간 미니차트 share 페이지 미지원 — `/trades/{i}/ohlcv` owner-authed 401 · 선행: token 공개 OHLCV 경로
- [x] **BL-429 ✅ Resolved (2026-08-17 night3 레인 β)** [P3] 대시보드 §03 최적화행 수익률/MDD — 백테스트 행과 같은 열·같은 의미의 숫자. ★착수 근거의 인과가 거짓이었다 — grid 는 `result.cells[]` 에 값이 **이미 있었고** 없던 것은 꺼내는 이름이었다(진짜 부재는 bayesian·genetic)
- [x] **BL-430 ✅ Resolved — 원장이 낡았다 (2026-08-17 night3 CONTROL 코드 대조)** [P3] 전략 목록 성과 정렬 — `sort.ts` 4옵션(수익률·샤프 포함) + BE `order_by` Literal 4종 **서버 정렬**이 이미 있었다. 상태줄의 세 주장이 전부 거짓이었다
...
- [x] **BL-462** [P3] Sharpe 목록 정렬 신·구 컨벤션 혼재 — 2026-08-11 gate-freshness 종결: 정렬은 ledger-truth(`1d4d7e0b`) `_sharpe_sort_criteria` 등급 정렬이 이미 분리했고(구행 = 등급 1, dev DB 실측 원값 1위 → 4위), 이 회차가 FE 고지를 사실로 교체 + read-time recompute·`Decimal("0")`→NULL 두 잔여 주장을 코드로 기각(`metrics.py:111-116` 독스트링·`grid_search.py:249`)
...
- [x] **BL-722** [P3] `assert-main-checkout.sh` 판별력 덮개 부재 — **2026-08-14 gate-pointer-axis 종결.** `assert-main-checkout-test.sh` 신설(케이스 4 + 변이 M1), 게이트 하네스 9→10종. 처방 2건이 뒤집혔다: `--selftest` 보다 별도 하네스가 싸고(호출 자리 2곳이 이름 규약 기반), 「비 git → rc≠0」은 코드와 반대였다
- [x] **BL-723** [P2] 영역 판정이 싼 게이트에만 붙어 있다 — **2026-08-14 gate-pointer-axis 종결.** 앱 코드 diff 0 인 회차에서 `BE pytest` 357초 + `e2e authed` 268초 + `design-canon` 42초가 그냥 탔고 CI 는 같은 회차에 그 잡들을 전부 skip 했다. `has_be`/`has_fe`/`has_fe∥has_be` 로 배선 + 하네스 8→9 케이스(환경 의존 단언 3개 동반 수리)
...
- [x] **BL-724** [P1] ★수수료가 gross 를 16배로 먹는다 — 고유 청산 592건에서 gross **+74.18** vs 수수료 **−1,200.00** ⇒ 순 **−1,125.82**. 소크 게이트가 PASS 해도 안 바뀌는 사실이라 [BL-003] cutover 판단 **전**에 답이 있어야 한다. 수리가 아니라 판정 항목 (2026-08-14 money-path-attribution)
...
- [x] **BL-815 ✅ Resolved** [P2] FE 화면 계층 테스트 0건 — **2026-08-21 밤샘 루프 3차 종결**. `error.tsx` 7개 · 공개 법무 페이지 4 · `QueryProvider` 정책 · 온보딩 persist 스키마 · 고지/지역 배너 · 손익바 · `OptimizerPageView` · Waitlist 승인 흐름을 8 lane 병렬로 채웠다(`phases/fe3-*` · 8/8 · 충돌 0 · 변이 8/8 red · PR #735~#743). `apps/web` **237 files/1,647 → 247 files/1,780 passed**(신규 10파일 **+133**) · 미도달 **53 → 32** · 대상 소스 전건 무변경. ★AC red 측정이 lane 1을 폐기시켰고 · 러너가 `blocked` 로 멈춰 [BL-816] 이 드러났다
- [x] **BL-816 ✅ Resolved** [P3] `/not-available` 만 `metadata` 를 export 하지 않는다 — **2026-08-21 밤샘 루프 4차 ① 사전 배치 PR 종결**. ★본문의 「`<title>` 이 비어 나간다」는 **과장이었다** — root layout 의 `template` default 로 **`"QuantBridge"`** 가 나간다(실측). 실질은 「geo-block 착지 화면이 다른 모든 페이지와 구별되지 않는다」다. ★**같은 병을 앓던 `page.tsx` 가 6개 더 있어 일곱을 함께 채웠다** (미export **8 → 1** · 랜딩만 의도적 제외) + `invite` 브랜드 2회 중복 제거. ★`fe3-public-legal-pages` 가 고정해 둔 케이스가 **예고대로 red 로 뒤집혀** 갱신했다 — [BL-814] 에 이은 두 번째 사례다
- [x] **BL-817 ✅ Resolved** [P2] `app/**` 라우트 조립층 테스트 0건 — **2026-08-21 밤샘 루프 4차 종결**. 목록 SSR 프리페치 키 동일성([BL-786]) · 동적 세그먼트 UUID 가드 · 앱 셸 title template/viewport/SSR identity · 랜딩 redirect 갈래 · 얇은 위임 7종 배선 · OG fallback · 주문수량 우선순위([BL-479]) · funding prefork-safe 를 8 lane 병렬로 채웠다(`phases/fe4-*`·`be4-*` · **8/8 · retry 0 · 충돌 0 · 변이 8/8 red · 6분** · PR #746~#753). `apps/web` **247 files/1,780 → 253 files/1,896 passed**(신규 6파일 **+116**) · `apps/api` 신규 **29** · 미도달 `web` **33 → 15** · `api` **2 → 1** · 대상 소스 전건 무변경. ★★착수 프롬프트의 BE 재료 주장이 거짓(「미도달 6」은 실측 2) · ★AC red 측정이 또 lane 하나를 폐기시켰다
- [ ] **BL-820** [P2] 밤샘 루프 6차 — FE·BE 커버리지 + 린트 부채 180건, **12 lane × step 4 · 동시 4** (저작 완료·실행 대기, 2026-08-22). ★**「FE 축은 바닥」은 AST 축의 결론이었다** — 커버리지 실측 `apps/web` **85.04%** · 미커버 **4,597줄/344파일**(`api.ts` 6종 553줄 · `hooks.ts` 6종 623줄이 통째로 빔). BE **91.51%** · 미커버 1,385줄. a11y **67건** + `noArrayIndexKey` 59 + `useTemplate` 35 + `noConsole` 15. 착수 전 AC red **12/12**. 무인 구간은 lane PR 의 stage 머지까지 — **사람이 판단하는 자리는 stage→main PR 하나**. 실행 = `tools/harness/execute.py --parallel 4 --stage stage/night6 --confirm`
- [x] **BL-818 ✅ Resolved** [P2] `apps/api` 미커버 모듈 커버리지 — **2026-08-21 밤샘 루프 5차 종결**. 8 lane 병렬 · **8/8 completed · 충돌 0 · 변이 8/8 red** · PR #755·#756~#764. `apps/api` **5,165 → 5,292 collected**(신규 8파일 · 92 함수 → **+127**) · 대상 소스 전건 무변경. ★★**재료를 AST 가 아니라 커버리지가 정했다** — 초판 8 lane 중 둘이 **실제로는 100% 커버**였다. ★★★**blocked 2건 중 하나가 진짜 결함**([BL-819] worker DI `UnboundLocalError`) · 나머지는 내 낡은 테스트 DB. ★부수 미해결 — `[tool.coverage.run]` `concurrency` 부재로 async 커버리지 저평가([BL-308]/[BL-309] 래칫 동반)
- [x] **BL-819 ✅ Resolved** [P2] `build_backtest_service_for_worker()` 가 **기본 설정(`fixture`)에서 `UnboundLocalError`** — 함수-지역 재import 가 `OHLCVRepository` 를 함수 전체에서 지역화해, else 를 안 타는 fixture 갈래의 반환부가 미바인딩 지역변수를 읽었다. 호출 경로는 `tasks/backtest.py:_execute`(**Celery 백테스트 워커**). 배포 env 는 `timescale` 이라 잠복이지만 **코드 기본값이 `fixture`** 다. ★**밤샘 루프 5차의 무인 세션이 `blocked` 로 잡았고** 코드 대조 + 실증으로 확인했다 — 형제 `optimizer`·`stress_test` 는 반환부에서 그 이름을 안 써 무사하다. 수리 = 중복 import 1줄 삭제(2026-08-21)
...
- [ ] (그룹 1 의 BL-428 참조 — BL-413 은 2026-08-15 종결 · BL-414/427/429/430 은 2026-08-17 night3 종결)
...
- [ ] **tasks 도메인 deepen** — 최대 미감사 **9,625 LOC** (`src/tasks/` **전체**, 2026-08-21 실측; `live_signal.py` 4,493 + `trading.py` 2,343 = **두 파일만으로 6,836**, money-path Celery). ★이 수치는 **두 번 낡았다** — 4,098(초판) → 6,342(2026-07-28, 두 파일 합) → 9,625. **파일 두 개가 아니라 도메인 합계**로 적는 이유가 그것이다. 재측정 = `find apps/api/src/tasks -name '*.py' -not -path '*__pycache__*' | xargs wc -l | tail -1`. Iron Law = 새 세션이 착수 전 재측정
