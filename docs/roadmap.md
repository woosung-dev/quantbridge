...
## 3) 리팩토링 부채 (**ACTIVE 166 · PARTIAL 6 · RESOLVED 0 · UNKNOWN 0 / 전체 172** — 2026-08-06 `scripts/bl-audit.sh` 실측, RESOLVED 94건은 문서 대개편에서 일괄 강등)
...
> ★**P별 ACTIVE 내역 = P0 1 · P1 6 · P2 65 · P3 83** (2026-08-06 병합 후 재측정 —
> `bash scripts/bl-audit.sh` 실측. 같은 실행의 전체 판정 = **ACTIVE 155 · PARTIAL 7 ·
> RESOLVED 91 · UNKNOWN 0 / 전체 253**). ★**위 §3 제목 줄의 수치는 2026-07-30 스냅샷이고
> 이 실측이 최신이다** — 어느 쪽도 손으로 세지 말고 스크립트를 다시 돌려라.
> ★**직전 표기(2026-08-05: P2 66 · ACTIVE 156 · RESOLVED 90)와의 차이는 P2 −1 하나뿐이고
> 귀속된다** — [BL-596] Resolved. 그전 2026-08-05 회차 몫은 P1 −1 / P2 +1([BL-591] 강등,
> 근거 = [`ADR-023 §재판정`](decisions/023-engine-state-ssot.md))이었고, 2026-07-30 스냅샷과의
> 나머지 증감은 그 사이 여러 스프린트가 쌓은 것이라 귀속하지 않았다.
> 직전에 "기계 집계가 1건만 잡혀 미측정" 이라 적었던 것은 **정규식 문제였다** —
> `**Priority:**` 는 그 시점 217 섹션 **전부**에 있었다.
> ★~~**UNKNOWN 17 건은 판정하지 않고 남겼다**~~ → **2026-08-05 실측 UNKNOWN = 0.**
> 그 17건이 어떻게 사라졌는지(상태 줄 보강인지 산식 변경인지)는 **확인하지 않았다.**
> 새로 생기면 스크립트 출력에 목록이 나온다. **추측으로 메우지 마라.**
