...
- [ ] **BL-820** [P2] 밤샘 루프 6차 — FE·BE 커버리지 + 린트 부채 180건, **12 lane × step 4 · 동시 4** (저작 완료·실행 대기, 2026-08-22). ★**「FE 축은 바닥」은 AST 축의 결론이었다** — 커버리지 실측 `apps/web` **85.04%** · 미커버 **4,597줄/344파일**(`api.ts` 6종 553줄 · `hooks.ts` 6종 623줄이 통째로 빔). BE **91.51%** · 미커버 1,385줄. a11y **67건** + `noArrayIndexKey` 59 + `useTemplate` 35 + `noConsole` 15. 착수 전 AC red **12/12** · 판정기 판별력 15/15. 무인 구간은 lane PR 의 stage 머지까지 — **사람이 판단하는 자리는 stage→main PR 하나**. 실행 = `tools/harness/night_run.py`
