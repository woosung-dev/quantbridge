...
- [ ] **BL-641** [P1] [BL-003] 의 실질 선행조건은 **MTBF 를 24h 이상으로 올리는 것**이다 — C1 은 실격 시 0 으로 리셋되므로 「누적 168h」는 「168h 연속 무실격」이고, 08-03 이후 MTBF 8.70h 에서 **P=4.115e-09** 다. 병목은 문턱이 아니라 사망률이고 자동사망 7건 중 6건이 `position_divergence` 다
- [ ] **BL-642** [P2] `soak-observe.sh` 가 아직 `localhost:8100/metrics` 를 긁어 재기동 ⑺ 을 실패시킨다 — [BL-620] 이 `soak-gate.sh` 에 대해 `.metrics` 직독으로 닫은 그 실패 계열이 이 도구엔 남아 있다. 재기동 자체는 성공하지만(세션 등재는 ⑺ 전) ⑻ 게이트 확인까지 못 간다
- [ ] **BL-643** [P2] `docs/status.md` 「다음 스프린트」 블록의 최신성을 어떤 게이트도 안 본다 — §G8 이 2026-07-27 에 같은 처방을 적고 실행처를 안 만들어(`sprint-template.md` 에 「본문과 대조」 0건) 2026-08-08 에 재발했다. 산문은 세 번째다(PR #562). 수리는 「다음 행동」의 6필드 승격 + `docs-audit.sh` 검사
...
- [ ] **BL-628** [P3] 마케팅 푸터 법적 고지가 `--warning` 을 `--warning-subtle` 위에 얹어 **5.66:1** — 캐논 5.82 미달(AA 는 통과)이고, 공개 4라우트 라이트 canon **68 > 다크 24** 미충족의 **단일 원인**이다(인증 12라우트·전체 16라우트는 충족). ★구팔레트에선 같은 자리가 **4.30:1 = AA 하드 실패**였다
- [ ] **BL-629** [P3] `--chart-axis` 는 정의만 있고 아무도 안 읽는 **데드 토큰** — `chart-tokens.ts:65` 가 `--text-muted` 를 축 색으로 읽고 `--chart-axis` 참조는 `frontend/src` 전체에 0건. 증거: 다크 `--text-muted` 는 캐논 교정으로 `#8b939c` 가 됐는데 `--chart-axis` 는 구값 `#7a828c` 에 남아 있다
- [ ] **BL-630** [P3] `.pos`/`.neg` **단독**(`.num` 없이)은 여전히 `td` 색에 진다 — `td.num.pos`(명시도 0,3,3)로 이미 수리된 자리 말고, `.pos` 단독(0,1,0)이 `td`(0,2,3)에 지는 구멍이 남았다. 지금 마크업이 늘 `.num` 과 함께 써서 미발현 — **관례가 지키고 있을 뿐 규칙이 아니다**
