...
- [ ] ⏳ **BL-715 대기 (트리거 미도래)** [P3] 원격 브랜치 잔재 23건 — 2026-08-12 branch-debris 가 원격 290건 중 267건을 지우고 남긴 것. C 14(PR 이력 없음, 미머지 80커밋) + **D 9**(이름축은 A 인데 팁 sha 가 어떤 PR head 와도 불일치 ⇒ PR 복원 버튼 밖, 미머지 72커밋). ★diff 로는 판정 불가 — 표본 8건의 1,600여 파일 차이는 고유 작업이 아니라 3개월치 시간 차이였다. 판정 = 미머지 커밋 152개를 커밋 단위로 읽어 main 반영 여부 대조
