...
15. **Bybit public WS URL 단일 상수** — demo 는 mainnet 공개 시세 공유(public 스트림에 demo variant 없음). `wss://stream.bybit.com/v5/public/linear` 공용. 구현 시 공식 docs 재확인 표기 의무(tickers delta 에 markPrice 부재 가능 → 스냅샷 병합).
