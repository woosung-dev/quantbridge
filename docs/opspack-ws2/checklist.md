...
- [x] **dogfood D5~D8** — D5 `qb:rt:ticker:BTCUSDT` ≈1s 발행 + 동시각 REST delta 0.0035% + FE KPI 실틱 / D6 손계산 3표본 정확 일치((mark−62863)×0.001, 실 open trade) / D7 lease 소멸→beat reconcile 재기동(공백 ~3분, 5분 설계 창)→"시세 지연" 배지 4상 관찰→비활성화 54s 내 no_symbols 종료·lease 해제·발행 0 / D8a 웹 콘솔 포함 3계통 0.02% 이내 / D8b 콘솔 error 0 + **폴링 5.03s 단일 폴 실측**(초기 보고 "초당 수회"는 Resource Timing 재측정으로 오판정 정정)
...
| DB/외부 오라클           | —                                         | Telegram 실수신 / ✅ D4a 직발송 True + D4b beat 실발화 `fired:1`·throttled·TTL 3597                  | ticker 2계통·미실현 손계산 / ✅ 3계통 0.02%·손계산 3표본 일치 |
| Opus MCP dogfood         | —                                         | D1~D4 / ✅ **전 항목 PASS** (증거 스크린샷 5종·네트워크·콘솔 덤프)                                   | D5~D8 / ✅ **전 항목 PASS** (배지 4상·폴링 5s 실측·콘솔 0)    |
