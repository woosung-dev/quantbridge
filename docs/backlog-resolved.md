...
**원인 / 영향:** [BL-813](#bl-813)·[BL-815](#bl-815)가 `src/lib/**` 와 화면 컴포넌트를 채우면서
...
**Est:** S
**상태:** ✅ **Resolved (2026-08-21 밤샘 루프 1차 원장 정리)** — 남아 있던 실패 모드 3(소유권 집합의 계정 축)은 `apps/api/src/trading/services/account_exclusivity.py` 의 `ownership_scope_ids` docstring 이 **결정과 근거를 명시하고 구현까지 하고 있다** — 축 = 「등재 대상 계정과 `exchange_uid` 를 공유하는 계정 행 전체」이고, 양 극단(스코프 없음 / 행 하나로 좁힘)이 각각 어떻게 틀리는지를 [BL-639] 실패 모드 3 이라는 이름으로 그대로 인용한다. ★즉 「[BL-634] 구현에 속한다」던 결정이 **그 구현과 함께 내려졌고**, 원장만 2년치 낡아 있었다(2026-08-20 하네스 3회차 실사가 지목 → 2026-08-21 코드 대조로 확정)
**트리거 판정:** 도래 → **소진** — 선행 [BL-634] 가 2026-08-09 Resolved 됐고, 그 구현이 이 항목의 잔여까지 함께 닫았다 (2026-08-21)
