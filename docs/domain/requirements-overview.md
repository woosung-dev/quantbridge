...
>
> **관련:** 제품 방향은 [`vision.md`](./vision.md), 도메인·상태의 상세 계약은 [`../domain/`](./), API 경계는 [`../interfaces/endpoints.md`](../api/endpoints.md)다. 초기 REQ 카탈로그와 Sprint 설계·측정 기록은 `archive/product/`에 보존한다.
...
| 영역        | 현재 계약                                                                                   | 상세 정본                                                                                            |
| ----------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------- |
| 전략        | Pine Script 등록·파싱·지원 범위 판정. 미지원 항목이 하나라도 있으면 부분 실행하지 않는다.   | [`domain-overview.md`](./domain-overview.md), [`supported-indicators.md`](./supported-indicators.md) |
...
| 트레이딩    | 사용자 계정 모드는 **Bybit Demo만** 허용하며, 주문 전 리스크 평가와 Kill Switch를 적용한다. | [`state-machines.md`](./state-machines.md), [`endpoints.md`](../api/endpoints.md)                    |
...
- 오래 유지할 구현 계약 → 해당 정본 문서(`architecture/`·`domain/`·`api/`·`development/`·`operations/`)
