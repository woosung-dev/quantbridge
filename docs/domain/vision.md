...
> ★★★**2026-08-23 사용자 결정 3건 — 이 문서의 여러 문장이 이것에 덮인다.**
> ⑴ **실자금(mainnet) 트레이딩 안 간다** (「아직 멀었다」) — 계정 모드는 **Bybit demo 만**이다.
> ⑵ **Beta 외부 공개 당분간 안 연다** — 실사용자 0명이 현재의 전제다. waitlist 파이프라인은 키만 넣으면 열리지만 열지 않는다.
> ⑶ **멀티 거래소(OKX·Binance) 안 한다** — **Bybit 하나**로 간다. 연결 코드는 현재 0건이고 그대로 둔다.
> 이 셋에 걸려 있던 원장 항목 **170건**을 같은 날 삭제했다 — 복원 좌표·판정 근거 = `backlog.md` 헤더의 「원장 다이어트 tombstone」.
> **셋 중 하나가 뒤집히면 그 SHA 에서 되살려라 — 다시 쓰지 마라.**
...
5. **(2026-05-04 Addendum) Auto-Loop 자동화** — Beat scheduler + Worker dispatch 로 dogfood/Beta 사용자가 매분 수동 개입 없이 평가→주문→close cycle 자동 실행 (Sprint 27 §0.5 dogfood Day 0-4 evidence: 26h+ 무중단, dispatch rate 1.0/min, 5 sessions 동시). **외부 노출 가능 핵심 신뢰 지표** — 처음 office-hours (Sprint 7c) 의 "curl/psql 감내" 가정이 dogfood 결과로 무효화, 자동화 필수성으로 승격. 상세: [`ADR-008`](../adr/008-sprint7c-scope-decision.md) "2026-05-04 Addendum"
...
본 비전을 코드로 옮길 때 반드시 지킬 규칙은 [`AGENTS.md`](../../AGENTS.md) §"QuantBridge 고유 규칙" 및 [`requirements-overview.md`](./requirements-overview.md) §8에 정리.
...
- 도메인 모델: [`domain-overview.md`](./domain-overview.md)
...
- **2026-04-17** — Phase 로드맵에 Horizon 매핑 열 추가 + 제품 로드맵([`roadmap.md`](../roadmap.md)) 링크 추가 ([ADR-010b](../adr/010b-product-roadmap.md))
