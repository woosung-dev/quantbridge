...
> → **2026-09-06 정정 — 이 문장의 세 전제가 다 사라졌다.** `backlog-resolved.md` 는
> 2026-08-23 에 **파일째 삭제**됐고(RESOLVED = 삭제), `bl-audit.sh` 는 [ADR-037] 로 **철거**됐으며,
> `roadmap.md` 는 2026-08-23 에 `PRD.md` 로 통합됐다. 지금 원장은 **두 파일**(`backlog.md` ∪
> 이 파일)이고 판정은 **사람이 상태줄을 읽는다**(`grep '^\*\*상태:\*\*'`).
>
...
> 「파일 배치」 축이 그 대응을 rc=1 로 집행한다.~~ → **2026-09-06 정정 — 집행자가 없다.**
> `bl-audit.sh` 는 [ADR-037] 로 철거됐고 그 「파일 배치」 축도 함께 사라졌다. 규칙은 산문으로
> 남아 있고 확인은 `grep -l '^### BL-NNN' docs/backlog*.md` 다. 앵커 잔여는 [BL-801] 로 등재했다.
