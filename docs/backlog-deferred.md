...
**Title:** authed 화면 증거의 **요청 수 축이 비결정**이다 — 같은 빌드 연속 2회에 ±1
**Category:** 테스트 / 인프라
**Priority:** P3
**Trigger:** ⏳ **대기 (트리거 미도래)** — 요청 수를 실제로 근거로 쓰고 싶어질 때. 지금은 번들 축만으로 [BL-662~665]·[BL-786] 류 회귀를 잡는다
**Est:** M (원인 규명이 먼저다. 고치는 것은 그다음)
**출처:** 2026-08-19 n6-authed-evidence — [BL-797] authed 확장의 결정성 실측

**원인 / 영향:** 같은 커밋·같은 프로덕션 빌드로 authed 라우트를 연속 2회 재면 `/api/v1/` 요청
수가 흔들린다 — `/backtests` **5 → 4** · `/optimizer` **5 → 6**. 같은 실행에서
`firstLoadBytes` 는 **비트 단위로 같았다**(371891 → 371891 · 414513 → 414513).
⇒ [BL-797] 원장이 적은 「번들 바이트 + 요청 수」 중 **요청 수 축만 성립하지 않는다.**

★**지금은 무해하다.** authed 라우트의 두 계수를 `null`(대조 제외)로 저장하고 spec 이
`apiRequests > 0` · `totalRequests > 0` 을 하드 단언하므로 **계수기가 죽는 것은 여전히 잡힌다.**
잃은 것은 「정확한 수」의 대조 하나뿐이고, 그것을 살리려고 임계를 두면 그 축은 아무것도 못 잡는다.

**[확인 필요] — 원인 후보:** ⑴ React Query 의 캐시 재검증이 `networkidle` + 1초 창 경계에
걸린다 ⑵ 세션/토큰 갱신 요청이 조건부로 난다 ⑶ 화면이 실제로 조건부 요청을 낸다(빈 목록이면
상세를 안 부른다 등). **셋 다 측정하지 않았다** — 창을 늘리는 것은 5초 폴링을 끌어들이므로
답이 아닐 수 있다.

**권장 접근:** ⑴ 먼저 **무엇이 흔들리는지 찍어라** — 두 실행의 요청 URL 목록을 diff 하면
한 줄로 갈린다. 수를 세지 말고 목록을 남겨라 ⑵ 원인이 폴링이면 창을 그 주기 밖으로 옮기고,
조건부 요청이면 그 라우트만 축에서 빼라 ⑶ ★**임계(±N 허용)를 두지 마라** — 이 게이트의
스크린샷 축이 `maxDiffPixels: 0` 을 지키는 것과 같은 이유다(주석에 근거가 있다).

★**같은 축에 딸린 사각 둘** (2026-08-19 vercel 지침 리뷰가 짚었다):
⑴ **「first-load JS」의 정의가 `next/dynamic` 개선을 못 잰다.** 이 게이트가 재는 것은 `networkidle`+1초
창 안에 **도착한 전부**라 `next build` 가 인쇄하는 동명 지표(초기 HTML 참조 청크)의 **초집합**이다.
무거운 컴포넌트를 지연 로딩으로 옮겨도 마운트 시 도착하면 그대로 계수되고, 청크가 쪼개진 만큼
요청 수는 오히려 +1 이 된다 — **지침이 CRITICAL 로 권하는 최적화를 「나빠졌다」로 인쇄할 수 있다.**
⑵ **authed 쪽 폰트 축이 무증거다.** 바이트 축은 woff2 를 뺐고(서브셋 수가 텍스트에 따라 변해
결정성이 깨진다 — 타당한 결정) authed 는 요청 수도 대조 제외라, `globals.css:5` 가 못박아 둔
「한글 variable 단일 파일 2.3MB preload 금지」가 퇴행해도 이 게이트에 **어느 축으로도 안 나타난다.**
공개 라우트는 `totalRequests` 가 잡는다(이 회차 실측에서 실제로 3라우트 전부 +1 을 잡았다).

**Risk:** 🟢 (지금 상태로 거짓 red 는 안 난다. 잃은 것은 판별력 일부다)

**상태:** ⏳ **대기 (트리거 미도래)** — 2026-08-19 등재. 미착수. 번들 축이 authed 회귀를 덮는 동안은 발현하지 않는다
**트리거 판정:** 미도래 — 번들 축이 실사용 회귀를 덮는 동안은 발현하지 않는다 (2026-08-19 n6-authed-evidence)
