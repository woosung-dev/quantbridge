...
2. **실측 반전 — 기구현 5건**: 계획 후보였던 screen-06 필터+CSV / screen-07 진단 / screen-10 안정성(grid) / sprint55 e2e / BL-402 는 main @ 88faccd 에 이미 구현 완료(#464 부채 마감 슬라이스). `docs/archive/sprints/c-language-port/checklist.md` §FIX 는 그 픽스 이전 기록이었음. → **문서만 보고 재구현 착수하면 안 되고, 계획 시 main 실측이 선행이어야 한다** (§7.1 baseline preflight 의 재확인).
