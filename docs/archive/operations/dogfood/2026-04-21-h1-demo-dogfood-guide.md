# H1 Demo Dogfood 운영 가이드

> **작성일:** 2026-04-21
> **목적:** QuantBridge H1→H2 게이트 dogfood를 Bybit Demo Trading으로 3~4주 운영하기 위한 실전 가이드.
> **컨텍스트:** 2026-04-21 결정 — 실자본 mainnet 대신 demo trading으로 기술 검증 우선.
> 기술 검증은 demo trading에서 90%+ 충분. 실자본 부담 없이 실거래 파이프라인 전체를 검증.
...
### 2.1 Bybit Demo Trading API 키

1. Bybit 앱/웹 로그인 → 우측 상단 프로필 → **API 관리**
2. **Demo Trading** 탭 선택 → "API 생성"
3. 권한: Read + Trade (Withdraw 비활성)
4. Demo 잔고 충전: Demo Trading 환경 진입 → "Asset" → Demo USDT 지급

> mainnet 계정과 별도 가입 불필요. CCXT `enable_demo_trading(True)` → `api-demo.bybit.com` 라우팅.

### 2.2 환경 변수

```bash
# .env.demo (커밋 금지, .gitignore 확인)
EXCHANGE_PROVIDER=bybit_futures
BYBIT_DEMO_KEY=<demo_api_key>
BYBIT_DEMO_SECRET=<demo_api_secret>

KILL_SWITCH_CAPITAL_BASE_USD=10000   # demo 가상 자본 기준
...
- 결과 있으면 → Bybit Demo 대시보드에서 수동 확인 + 원인 분석
...
4. Bybit Demo Trading "Orders" 탭에서 동일 주문 확인
...
- QB PnL vs Bybit Demo PnL 차이 < 1% (fee 포함)
...
- [ ] **Fee 정확성 검증** (PR-C 이후): QB 수수료 계산 vs Bybit Demo 수수료 내역 대조
...
2. Bybit Demo 대시보드에서 수동으로 오픈 포지션 0 정리
...
| [`ADR-006`](../../../decisions/006-sprint6-design-review-summary.md)                             | Kill Switch 설계 근거                 |
| [`2026-04-20-h2-kickoff.md`](../../../archive/superpowers/plans/2026-04-20-h2-kickoff.md)       | H2 Sprint 9~11 분해                   |
...
- **2026-04-24** — Testnet → Demo Trading 전환 반영. Bybit `enable_demo_trading(True)` (`api-demo.bybit.com`) 사용으로 변경. 환경변수 `BYBIT_TESTNET_*` → `BYBIT_DEMO_*`, 계정 생성 절차 Demo API 키 발급으로 대체.
