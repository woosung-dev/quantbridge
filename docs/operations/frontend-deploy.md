...
> **첫 도입:** 2026-08-07 fe-oracle-deploy 회차. ★**2026-09-10 [ADR-043]** — 빌드는 CI(`release.yml`)가, 서버 반영은 `deploy.sh` 가 한다. 이 문서는 구조·함정만 남긴다.

---

## 1. 이 배포가 무엇인가 (그리고 무엇이 아닌가)

**맞다** — 브라우저에서 데모 트레이딩이 도는 실계정 화면을 본다. 앞단은 **Cloudflare Access(이메일 OTP)**다.
...
**BE 스택에 아무것도 더하지 않는다** — 이 스택은 BE compose 와 서비스도 네트워크도 공유하지
...
**⑴ 빌드는 맥, 서버는 실행만.** 서버는 2 OCPU 를 다른 프로젝트와 공유하고 그 위에서 데모 트레이딩 워커가 돈다.
서버 빌드는 워커를 굶긴다(그래서 빌드는 CI 다 — `release.yml`). 서버에 Node 는 **설치돼 있지 않다** — 그래서 `next start` 가 아니라
...
> ★**2026-09-10 [ADR-043] — 아래 블록은 더 이상 사람이 치지 않는다.** `release.yml` 이 main 머지마다
> `ghcr.io/woosung-dev/quantbridge-frontend:sha-<7>` 를 올리고, 서버의 `deploy.sh <sha>` 가 `.env` 의 `QB_FRONTEND_TAG` 를
> 그 태그로 맞춘 뒤 `docker compose … -p quantbridge-fe up -d` 한다. NEXT_PUBLIC_* 3종은 repo Variables 다.
> 손으로 해야 할 때(CI 가 죽었을 때)의 원문 = `git show c488b545:docs/operations/frontend-deploy.md`.

```bash
ssh <서버> 'bash -lc "cd ~/quantbridge && tools/scripts/deploy.sh <sha>"'   # BE·FE 함께
```
...
이전 커밋에서 다시 밟는다. **FE 롤백 = `deploy.sh <이전 sha>`** — BE 와 같은 태그를 쓴다([ADR-043]).
...
ssh <서버> 'bash -lc "cd ~/quantbridge && tools/scripts/deploy.sh --status"'   # 24h 실격 0 · head == DB · /health 200
...
★**스크립트를 비로그인 ssh 셸에서 부르지 마라.** `ssh <서버> 'tools/scripts/deploy.sh --status'` 는 PATH 에
...
★**`deploy.sh` 는 이미지가 아니라 체크아웃에서 돈다.** 서버 체크아웃이 낡으면 스크립트도
...
★**체크아웃 전환은 워커에 안전하다** — 워커 코드는 이미지 안이고 `apps/api/.metrics` 만
마운트한다. `.env.local` 도 미추적이라 살아남는다. 단 **추적 파일 변경이 0건인지
먼저 확인**해라.
...
- 배포 파이프라인: [ADR-043](../adr/043-deploy-pipeline-images-replace-soak-pin.md) · [backend-deploy.md](./backend-deploy.md)
