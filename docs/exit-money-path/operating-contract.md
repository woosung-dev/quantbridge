...
| Site 3 — loss-limit 알림              | `backend/src/tasks/alert_rules.py:60` → `OrderRepository.sum_filled_realized_pnl_for_session` | **세션 스코프**                  | ✅ event-join → 세션 스코프          |
...
Order.symbol              == session.symbol          -- 정확 문자열 동등
Order.state               == filled
...
### 3.2 표기가 다른 TV 웹훅 주문은 스코프에서 빠진다 (D5)

`symbol` 은 정확 문자열 동등이다. ingress 정규화가 **어디에도 없다** — `RegisterLiveSessionRequest.symbol` 은 길이만 보고(`schemas.py:183`), TV 웹훅은 payload 원문을 그대로 싣는다(`webhook.py:89`). `normalize_symbol`(`market_data/constants.py:18`)은 존재하지만 `src/trading/`·`src/tasks/` 에서 호출 0건이다.
...
| TV 웹훅 (`webhook.py:89`)             | payload 원문          | **보장 없음**    |
...
- **추정값과 확정값이 한 합계에 섞인다.** 5곳 어디도 `realized_pnl_synced_at` 을 구분하지 않는다. ★필터링은 틀린 해법이다(체결~스윕 도착 구간이 통째로 안 보이는 fail-open). **라벨로 노출하라.**
