...
- 보호된 HTTP 경로는 Better Auth 가 발급한 JWT를 JWKS로 검증하고([ADR-034](../adr/034-auth-self-host-better-auth.md)), 소유권 확인은 service 계층이 맡는다. 검증기는 `realtime/auth.py`다.
