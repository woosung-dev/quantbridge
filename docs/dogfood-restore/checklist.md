...
### 체크리스트 — ✅ 조사 완료 (2026-07-26). **가설이 틀렸다.**

★★**"pine_v2 상태가 롤백되지 않는다" 가 아니라, 애초에 진입 주문이 나간 적이 없다.**

- [x] **재현 조건 특정** — 상태는 발주 결과를 **알 수 없다**. `upsert_state` + commit(`live_signal.py:638-649`)이 dispatch enqueue(`:657`)보다 **먼저**고, 그 사이에 Celery 경계가 2개(dispatch task → execute task) 있다. 거래소 거부는 `tasks/trading.py:399-416` 이 `orders` 테이블만 건드리고 `LiveSignalEvent`/`LiveSignalState` 를 import 조차 하지 않는다(전수 grep). **롤백 경로 0.**
- [x] **설계된 동작인가** — 그렇다. Option B(warmup replay): 매 tick `run_historical` 을 300바 위에서 통째로 재실행하므로(`event_loop.py:270-272`) **되먹일 자리 자체가 없다**. 되먹여도 다음 tick 재생이 덮어쓴다. `models.py:469-473` 이 이 테이블을 "캐시" 라고 명시.
- [x] **재동기화 경로** — `Reconciler`(WS 재연결)·`orphan_scanner`·`resync_exchange_realized_pnl` 전부 **orders 만** 본다. 포지션/시뮬 상태 재동기화는 **존재하지 않는다**. `qb_live_signal_divergence_total` 은 다른 개념(Pine 커버리지 ↔ 인터프리터 런타임)이다.
- [x] ★**진짜 원인 = [BL-478]. `run_live` 가 `fill` 이벤트를 버린다**(`event_loop.py:287-288`). 독스트링은 _"broker 가 자체 fill 알림 처리"_ 를 전제하는데 — **broker 에 그 stop 주문을 올린 적이 없다**(`live_signal.py` 에 `trigger_price`/`PendingOrder` 참조 **0건**). `strategy.entry(..., stop=)` 은 `PendingOrder` 만 파킹하고 `return None`(`strategy_state.py:598-608`) 하므로 **진입 이벤트가 아예 안 생긴다**. 반전 시 생기는 `close` 만 dispatch 되어 매번 110017. 시드 `s1_pbr` 은 진입 2개가 전부 `stop=` 이라 100% 이 경로. **시장가 진입 전략은 정상**(`:634-642` 이 `event_action="entry"` 발행).
- [x] ★**수량 1.0 = [BL-479] 별개 결함.** `run_live` 가 `run_historical` 을 **사이징 인자 없이** 호출(`event_loop.py:270-272`) → `configure_sizing` 미호출 → `compute_qty()` fallback `1.0`. `position_size_pct` 는 라이브에서 **아무 데서도 안 읽힌다** — 사이징 계산의 유일한 자리 `compat.parse_and_run_v2` 의 프로덕션 호출자는 `v2_adapter.py:96`(백테스트) 하나뿐. Pine `default_qty_type` 선언조차 라이브에선 무시된다(같은 게이트 뒤).
- [x] ★**신규 [BL-480] — 화면이 발산을 은폐한다.** 백엔드는 정확히 안다(실측: `verdict="local_only"` + `local_open_trades_snapshot: [PivRevLE long qty 1 @ 64557.51]`). 프론트에 문구도 있다(`open-positions-table.tsx:29`). 그런데 행 생성이 `positions` 배열 순회라(`hooks.ts:161`) **`local_only` = `positions` 빈 배열**인 그 순간에만 정확히 렌더 불가 → 화면은 **"열린 포지션이 없습니다"**.
- [x] **BL-466 과 뿌리가 다르다** — 그쪽은 백테스트 마진 게이트 no-op(L=1), 이쪽은 라이브 배선 부재.
