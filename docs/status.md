...
**다음 행동 = 두 브랜치를 **이 순서로** squash 머지하고, 그 뒤 `screen.ok` 를 취득해
`final-gates.sh --run eod` 를 한 번 돌린다.**
⑴ `stage/ledger-truth`(원장 정정 + Sharpe + `/metrics` + skip 래칫 + FE coverage) →
⑵ 그것이 머지된 **뒤에** `stage/zero-table-gate`(⓪ 표 정체성 게이트) — 🔴 **순서를 바꾸면
`ci.yml:57`+`:385` 때문에 이후 모든 PR CI 가 red 다.** ⑵ 는 `final-gates.sh` 에서 ⑴ 과
같은 자리를 만지므로 **rebase 충돌 1건**이 있다(래칫 배선 ↔ 문서 감사 하네스 배선 — 둘 다 살려라).
