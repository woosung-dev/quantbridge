...
> **Last Updated:** 2026-08-01 (**entry-completeness-rejudgement** — ★★★**4개 채널 중 3개가 유실 채널이 아니었고, 착수 전제(「분모 12 가 C2」)마저 오독이었다.** BL-536 Resolved · BL-522 P1→P2 ⇒ **머니-패스 P1 슬롯이 비었다**)
> **Active Sprint:** 없음. **다음 스프린트는 아래 §다음 스프린트 참조.**
> **미머지:** `stage/entry-completeness-rejudgement` — PR 준비 중.
> **Last Merged:** `stage/silent-surface-honesty` → `main@f631f1c7` (**PR #517**, 2026-08-01) · 그 앞 `stage/conditional-fill-visibility` → `main@51f8e51d` (PR #516)
...
**스코프.** 근거가 무너진 P1 두 건([BL-536]+[BL-522])의 **재판정**. herdr 함대 3벌 + CONTROL.
**마이그레이션 0 · 새 엔드포인트 0 · FE diff 0.**
상세 = [dev-log](dev-log/2026-08-01-entry-completeness-rejudgement.md).
...
**[BL-536] Resolved · [BL-522] P1 → P2.** 열린 P1 **8 → 6**, 남은 6건은 전부 테스트·인프라 부채 =
**머니-패스 P1 슬롯이 비었다.**

> ★★**「P1 이 비었다」를 「cutover 해도 된다」로 읽지 마라 — 두 개는 다른 축이다.**
> **P1 = 지금 무엇부터 할 것인가(우선순위).** **Trigger = 무엇을 하기 전에 닫혀야 하는가(차단 조건).**
> [BL-522] 는 P2 로 내렸지만 **Trigger 「실자금 cutover 전 필수」는 그대로 유지**했다 —
> 크기가 작아진 것이지 cutover 체크리스트에서 빠진 것이 아니다. 잔여는 [BL-578].
> 실자금 cutover 가능 여부는 **Trigger 가 걸린 BL 전수**로 판단하고, P1 개수로 판단하지 마라.
> (2026-08-01 codex 적대 리뷰 MINOR — 두 문구를 나란히 읽으면 오독 가능하다는 지적을 반영)

### ★★★착수 전제 자체가 거짓이었다

「유실률 `2/12` 의 분모 12 가 C2 합산」은 **오독**이다. 12 = **`filled 10 + rejected 2`** 이고
C2 가 **마침 12** 였을 뿐이다(`backlog.md` 가 `2/(10+2)` 를 `2/12` 로 압축하며 라벨을 잃었고
6행 위에 `C2 … 12` 가 있었다). ⇒ 「두 근거가 모두 C2 의존」이 거짓이고, 그 오독 때문에
**A2 「C2 제외 유실률 0」은 구조적으로 발화 불가**였다. `status.md`·`roadmap.md`·`backlog.md` 정정.

### ★★★C3 도 유실 채널이 아니다 — 5축 전건 일치

① 조건부 진입 부분체결 = **원장 전 기간 0건** · ② `qb_partial_fill_total` 4 call-site **전부
`reduce_only` 미검사** · ③ `metrics.py:637-650` 이 **이미** 「쓰지 마라」(PR #509) ·
④ **소급 재현** — 07-29 창 **청산측** 부분체결이 **정확히 7**, 진입측 **0**(±6h 확대해도 7 고정,
이동하면 5→4 = 경계 절단이지 우연 겹침 아님) · ⑤ 그 7건 전부 `filled_quantity=0.029` =
**`qty_step` 절삭 아티팩트**라 **부분체결조차 아니다**.

★**철회가 반증까지 되감았다** — 07-30 PR #509 가 C3 를 반증했는데, 같은 날 PR #511 의 「축소」
철회가 07-29 표로 되돌리며 C3=7 을 되살렸다. **직전 「유지」는 그 당시 데이터로도 틀렸다**
(정정 술어로 07-29 창도 최대 채널 C1 100% = 축소).

### ★★계측기가 0 을 낼 때 판별력을 증명했다

C4 는 로컬 `ts.ohlcv` 가 그 창에 **1행뿐**이라 **공개 Bybit kline 2호스트 교차**로 갈음했다
(2881/4321봉, **불일치 0**). 취소분 68건 전건 **0**. ★**같은 판정기를 체결분에 걸어 35/37 검출** —
그 양성 대조가 **내 「엄격 규칙」을 폐기시켰다**(체결분 1/37 밖에 못 잡아 판별력 0).

### ★★Evaluator 가 CONTROL 을 3건 잡았다

① **「분모 12 = C2」 오독**(위) · ② 내 층위1 정의가 **출시 코드와 반대**
(`confirmed_rejection_rate` 는 `unknown` 을 분자에서 **뺀다**. 내 근거 「직전 2/12 재현」은
두 정의가 같은 값을 내므로 **판별력 0**) · ③ 「C3=7 출처 미상」은 과장
(`status.md:437` 이 `qb_partial_fill_total{ws}` 라 명시 — dev-log 에만 없다).

### ★사전등록 문턱 자체의 결함 3건 (다음 회차 몫)

**A2 공허**(C2 가 분모에 없어 절대 발화 불가) · **A5 문턱 미정의**(A3 이 N=1 에서 자동 100%) ·
**A4 는 AND 인데 과거 기록 3곳이 OR**(07-29 는 두 근거가 둘 다 참이라 **판별력 0인 창에서 통과**).

### 정직하게 남긴 것

- **C1 잔여를 고치지 않았다** — 1건/2일 · 격차 최대 0.071% · **22/33 이 3~54분 뒤 자연 회복** ·
  남은 수단은 머니-패스 변경뿐. 크기를 근거로 보류하고 **[BL-578]** 등재.
- **창 P 의 그 `lost` 1건은 사실 판정 불가에 가깝다** — 거절 44초 뒤 세션이 `user_stopped` 로 꺼져
  **회복 tick 자체가 없었다.**
- **A5 는 「평가 불가」** — 문턱이 숫자로 정의된 적이 없다. A3 을 채택한 근거는
  「C3/C4/C5 의 0 은 표본이 작아 못 잰 0 이 아니라 **판별력을 증명한 계측기로 잰 0**」이다.
- **QA 별도 워커를 두지 않았다** — FE diff 0 이라 렌더 QA 대상이 없었다. QA 몫은
  Evaluator(수용기준 반증) + CONTROL(실주행)이 나눠 가졌다.
- **C5 전용 실주행 창을 열지 않았다** — 5일치 원장 + counter 수명 전체가 이미 0 을 말해
  1~2h 창이 **더 약한 증거**였기 때문이다. 계획 변경을 명시한다.
