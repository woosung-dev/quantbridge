...
→ **2026-08-19 완료 — 둘 다 RESOLVED**(PR #PRNUM). 「`/pricing` 이 화면 증거 ROUTES 밖」이라는 선행 질문도 그 자리에서 답했다: **AC 를 결정론(단위·grep 부재·tsc·build)으로 짜서** 화면 증거 팩에 의존하지 않았다.
...
**다음 행동 = 하네스 v2 도입 여부를 사용자가 판정한다** (위 표 + 게임 1건). 머지 순서는 사용자 결정대로 **zero-base(PR #687) → 기능(PR #PRNUM)**. 「도입」이면 다음 회차는 [BL-776] 카피 수정을 같은 러너로 돌리고, AC 저작 규약(`.claude/commands/harness.md` §C-5)에 **「부재 grep 은 소스와 테스트를 갈라라」** 를 더한다.
