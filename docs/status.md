...
**새 마찰 1건**([BL-855] 는 코드 대조 산물이라 마찰 카운트 밖) · 수리 3건.
...
~~**다음 행동 = [BL-855] 임포트가 `parse_status="unsupported"` 를 만들게 한다**~~
→ **2026-09-06 사용자 결정으로 한 회차 미룬다**(폐기 아님). [BL-855] 는 PRD §5⑵ 「Pine 파싱
성공률」의 **분모를 세우는 선행**이고 그대로 유효하다 — 아래 UI/UX 축을 닫은 **직후**가 그 자리다.
...
| **V**  | [BL-855] 임포트가 미지원 Pine 을 저장하고도 `parse_status="unsupported"` 를 **한 번도 안 만든다** | P2 | ★★★ | 하 | S | **건드림** | **2026-09-06 등재 · d1 실사용 루프 산물** — `grep -rn "ParseStatus.unsupported" apps/api/src` = **대입 0건**인데 FE 는 라벨·필터 탭·집계 등 **6곳**에서 그 값을 그린다. 그 필터 탭은 영원히 빈 결과다. ★**저장을 막는 것이 아니다** — 저장된 행이 스스로 말하게 하는 것이고 **FE 변경 0줄**이다. ★`analyze_coverage` 는 정규식이라 corpus 9벌 **8.6ms**(비용은 반대 근거가 아니다) ★착수 전 red 에 「파싱 성공 ∧ 미지원」 픽스처가 없으면 초록이 무증거다 |
| **W**  | [BL-856] 화면 리뷰가 **절반이다** — 동적 라우트 6종을 한 번도 안 열었고 그중에 백테스트 리포트가 있다 | P2 | ★★★ | 하 | M | 0줄 | **2026-09-06 등재 · 전 화면 스윕(PR #872)의 자기 실측** — 스윕이 21 라우트를 25 로 셌는데 실제 `page.tsx` 는 **27개**다. 빠진 6개는 전부 **id/토큰이 URL 에 필요**해서 정적 목록으로 못 만든 것들이고, 빠진 이유가 곧 **재발 조건**이다. ★재개 재료(백테스트·전략·옵티마이저 실데이터 id)가 원장 항목 안에 있다 — **id 찾는 데 시간 쓰지 마라.** ★`ui-ux-pro-max` 축은 아직 **한 번도 안 돌았다** |
