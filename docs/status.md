...
~~9.50h 연속 (02:05Z)~~ → ~~2026-08-16T13:50Z 실측 21.0063h~~ → ~~22.5008h~~ → 위로 대체.
★**여기 숫자를 인용하지 마라 — 게이트를 돌려라.** 그리고 ★**날짜에 주의해라**: 종전 표기
「2026-08-16T13:50**Z**」는 KST 날짜에 `Z` 를 붙인 것이고 실제 UTC 는 08-15T13:50Z 였다.
로컬과 서버 시계는 **정확히 일치한다**(둘 다 `2026-08-15T15:02:52Z` 실측) — 뒤처져 있지 않다.
★표본은 30분마다 `:28` 초에 들어온다 — 판독이 표본 직전이면 값이 안 움직이는 것처럼 보이지만
정지가 아니다(16:00·16:30 정확히 확인).
...
**다음 행동 = [BL-767]·[BL-768] 의 서버 잔여를 닫는다** — PR #643 머지 뒤 서버에서
`db-backup.sh --install` · `disk-guard.sh --install` · OCI 버킷 생성 · `run` 1회.
★검증 3종: ⑴ 덤프·메타 생성 ⑵ OCI 객체 존재 ⑶ **소크 게이트 C2 가 전후 불변**(백업이 창을
안 끊는다는 증거). 디스크 경보는 `QB_DISK_WARN_PCT=1` 로 강제 발화시켜 텔레그램 **도착**을
확인하고 원복한다 — 「유닛이 Finished」는 도착의 증거가 아니다([BL-737] 실측).
★버킷이 없으면 첫 실행은 **rc=3 + 알람**이 정상 동작이다(로컬 사본은 남고 원격만 실패).
그 셋이 끝나면 두 BL 이 🟡 PARTIAL → ✅ 로 간다.

★그다음 창 3까지는 **자격이 올 때마다 같은 절차**다. 창 2·3 은 pin 이 같으므로([ADR-024])
층화가 필요 없지만, **창 1(`fb7bb772`)과는 섞지 마라**.
...
| **AH** | [BL-754] rate limit 이 프록시 뒤에서 **전 사용자 공용 버킷**으로 붕괴 — `request.state.user_id` 를 아무도 세팅하지 않고 `TRUSTED_PROXIES` 가 비어 있다                                                                                                                                                                         | P2  | ★★   | 하     | ~~S~~ → **M** | **건드림**     | ★기전은 **지금 성립한다**(코드 대조 2026-08-15). ★★**2026-08-16 Est 정정** — 원장의 「auth dependency 1줄」은 성립하지 않는다. `SlowAPIMiddleware` 가 `Depends` 보다 먼저 돌아 `default_limits=100/min` 경로는 `user_id` 를 **영원히 못 본다** ⇒ 인증 전용 미들웨어 신설이다. 기존 테스트 2건이 무증거·버그고정이라 **초록이 증거가 아니다**. 공개 전환([BL-071])과 동승 |
...
| **AK** | [BL-767] 🟡 DB 백업 — 코드·하네스·**복원 실증**은 끝났고 **서버 설치 + OCI 버킷 + 업로드 실증 1회**가 남았다                                                                                                                                                                                                                   | P1  | ★★★  | 낮     | XS            | 0줄            | ★잔여는 전부 **서버 조작**이라 사용자 승인 사안이다. `--install` → `run` 1회 → ⑴ 덤프·메타 생성 ⑵ OCI 객체 존재 ⑶ ★**소크 게이트 C2 전후 불변**(백업이 창을 안 끊는다는 증거). 버킷이 없으면 첫 실행은 **rc=3 + 알람**이 정상 동작이다                                                                                                                                   |
| **AL** | [BL-768] 🟡 디스크 80% 경보 — 코드·하네스·soak-watch 회귀는 끝났고 **서버 설치 + 발화 실증**이 남았다                                                                                                                                                                                                                          | P2  | ★★   | 낮     | XS            | 0줄            | ★양성 대조가 본체다 — `QB_DISK_WARN_PCT=1` 로 강제 발화시켜 텔레그램이 **도착**하는지 보고 원복한다. 「유닛이 Finished」는 도착의 증거가 아니다([BL-737] 실측). 음성은 `=99` 로 무발화                                                                                                                                                                                   |
