...
**다음 행동 = Stage 4 — LLM 해설 층**([ADR-040] §해설 층).
`GET /strategies/{id}/brief/narrative` 를 **별 엔드포인트**로 낸다(결정론 층은 이미 즉시 뜬다).
★첫 step 은 프롬프트가 아니라 **스키마 강제 배선**이다 — 현재 `convert/service.py` 는
`tools=`/`response_schema=` **0건**이고 응답을 문자열로 수동 파싱한다(Gemini 는 ``` 펜스를 손으로 벗긴다).
브리핑은 JSON 이어야 하므로 그 배선이 선행이고, **항목마다 `pine_lines` 가 비면 렌더하지 않는다**.
