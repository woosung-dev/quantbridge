...
| **AS** | [BL-779] `docs/backlog.md` 단일 파일이 **512k 토큰** — `docs/` 텍스트의 24%이고 열면 컨텍스트가 끝난다                                                                                                                                                                                                                         | P2  | ★    | 중     | M             | 0줄            | ★`context-budget.sh` 실측 1,019,776자 / 511,772 tok. RESOLVED 117건 본문이 Open 과 **같은 파일**에 있다. ★**검사기를 먼저 고쳐라** — `docs-audit.sh:21`·`bl-audit.sh` 가 `BACKLOG` 경로를 상수로 갖는다. 옮기고 나서 전체 수(326)가 줄면 검사기가 한쪽을 못 보는 것이고 그때 게이트는 **초록으로 통과한다**                                                                                                                                                                                                                                                                                                                                                                                                               |
