...
★**내가 회차 중에 §8.6 을 밟았다** — 셸 cwd 가 워크트리라 `docs-audit` 이 **수정 안 된 트리**를 재고
초록을 냈다. 절대경로로 다시 재니 `zero_table_identity=2` 로 발화. **초록은 「통과」가 아니라
「그 트리를 안 봤다」였다.** 게이트를 부를 때 **어느 트리를 재는지** 먼저 말해라.
