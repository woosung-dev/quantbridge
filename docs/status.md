...
🔴 **순서를 바꾸면 24건이 즉시 red 다.** 그리고 `bl-trigger-sweep.sh:229` 의 `targets` 를 넓힐 때
**`--selftest` 를 전량 스윕보다 먼저** 돌려라 — 2026-08-10 에 그 판정기 초판이 5건을 근거 없이
「도래」로 올렸고 잡은 것은 스윕이 아니라 그 앞의 음성 대조였다. **기본값은 「판단 필요」다.**

★**왜 이것이 다음인가** — ⓪ 표 게이트는 머지됐지만 `(PARTIAL ∧ 도래)` 가 **데이터 공집합**이라
P0 1건 + P1 4건([BL-003]·[BL-438]·[BL-619]·[BL-641]·[BL-661])이 **아직 표에 안 올라온다.**
게이트가 있다고 G3 가 충족된 것이 아니다.
