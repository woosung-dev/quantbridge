...
> **PR #628 머지(`4b11da26`)** · Bybit demo linear perp 실주문 1건이 프로덕션 dispatcher 를 타고
> `filled` 까지 갔다 — **실거래소가 처음으로 검증됐다**. ★★★한 줄 = **「2층 자기정리 하네스는
> 지어진 뒤 10일 동안 한 번도 작동한 적이 없었다」**([LESSON-109]) — `pytest.skip` 이 REGISTRY 를
> 늘 비웠고 첫 타깃이 들어가자 청산이 **개발 DB 를 열고** 있었다. ★증인은 pytest rc 가 아니라
> **거래소**였다. ★차단자가 원장에 없었다 = **7일 묵은 고아 포지션**(stop 은 flat 하지 않는다).
> ★codex 3/3 참 — 셋 다 내 첫 수리판이 fail-open. ★`--deferred-only` 는 docs-audit 을 안 돈다.
> tombstone: 원문 = `git show 3e159691:docs/status.md` (295~370행).

## ★2026-08-14 money-path-attribution — [BL-438] 진단 교체 + 헤지 모드 기각 (강등 — 본문 = git)

> **PR #630 머지(`4d0b1a3d`)** · 한 줄 = **「미귀속 71.5% 는 결함이 아니라 잘못된 DB 를 봤다는
> 뜻이었다」** — 소크 주문은 **서버 DB** 에만 있다(`filled` 654 vs 로컬 321). ★**진짜 구멍은 옆에
> 있었다** — 라벨은 `exact` 인데 `reduce_only=true` 필터가 백필을 막아 kill-switch 가 **6.9%** 만
> 봤다. ★헤지 모드는 실거래소 프로브로 기각([ADR-032]) — T3 이 공식 문서를 반증했지만
> **T4 가 도입 근거를 없앴다**(헤지 청산도 `closed-pnl` 키가 one-way 와 동일, `positionIdx` 없음).
> tombstone: 원문 = `git show 4d0b1a3d:docs/status.md` (307~347행).

## ★2026-08-14 money-path-close — [BL-438] 종결 + [BL-724] 경제성 판정

★★★**한 줄 = 「소크가 돌리던 전략은 백테스트에서도 지고 있었다」.** [BL-724] 는 이 회차의 산출이고
「수수료가 흑자 전략을 먹었다」는 **라이브 한 축에서만** 참이었다.

| 축                 | 라이브 원장 n=596        | 백테스트 `a22faccb` n=1028 |
| ------------------ | ------------------------ | -------------------------- |
| gross(슬리피지 후) | **+82.64**               | **−34,582**                |
| 수수료             | −1,204.21                | −183,978                   |
| net                | −1,121.57 (PF **0.223**) | −218,560 (PF **0.607**)    |

수수료 실효율 **0.05500%** = 단면 taker 공시치와 소수점까지 일치(maker 체결 0건). 손익분기 요율
단면 **0.00377%** = 현재의 **1/14.6** ⇒ **비용 축으로 구제되지 않는다** — 요율이 공시치에 고정이니
남는 변수는 회전량뿐이고 **회전 2,189,464 = 계좌의 11.6배**다. **레버는 요율이 아니라 빈도**이고
이건 비용가정 버그가 아니라 전략 설계 질문이다. 같은 전략(`PbR Pivot Reversal`, 청산 551/596)의
백테스트 **4벌이 전부 PF < 1**(0.57~0.86)이고 **4벌 모두 낡은 요율**(`0.001`/`0.0005`, [BL-603] 이
08-07 에 교체) 로 돌았다 → [BL-729]. FE 미러 2곳도 낡았다(온보딩이 왕복 0.30% 하드코딩 submit)
→ [BL-730]. **판정 = 실자금 불가 · 데모 유지**(사용자 결정: 「실매매는 진짜 최종에 최종만이다」).

★**착수 전제 반증 3건** — ⑴ **Lane B 가 착수 도중 남의 세션에서 머지됐다**(#631 `dde53e68`,
18:09 KST). codex 를 중단하고 **검증자로 전환**했다: 서버 실데이터 음성 대조 `구 술어 0건 →
신 술어 490건/−1,023.87`, 표적 pytest 72 passed ⇒ 귀속률 **6.9% → 98.2%**. ⑵ **게이트는 1곳이
아니라 3곳**이었다(`_enqueue_closed_pnl_refresh:1227` · `_refresh_closed_pnl_with_session:1493` ·
리포지토리). 앞 둘은 **일부러 안 고쳤다** — 순진하게 열면 모든 entry 주문이 `transient` 4회
재시도 뒤 **운영자 알림**을 낸다. ⑶ **[BL-726] 은 허위 경보**였다(값은 pine_v2 추정치이고
`exchange_order_id` 가 46건 전부 NULL = 거래소 미도달) — 원장 처방 ⑵ 는 그 46건에 **no-op** 이다.
★부수 = #631 의 머니-패스 주석이 생산자를 **웹훅으로 잘못** 지목해 정정했다(46건은 전부 `live:`).

★**아직 소크에는 안 실렸다** — `.soak/src` pin 이 `4b11da26` 로 #631 보다 앞선다. **재-pin 이
배포**다. kill-switch 발화 우려는 **해소됐다** — 백필 후 누적이 −386.91(로컬)/−1,125.81(서버)이고
잔고가 **189,427**(`equity_baseline_usdt` 실측)이라 0.2~0.6% vs 문턱 10% = **17~50배 여유**.
일일 축도 최악의 날 −103.71 vs 문턱 −500 이다.
★단 evaluator 의 실제 입력은 그 컬럼이 아니라 `balance_provider.fetch_balance_usdt()` **라이브
조회**다(`kill_switch.py:113-131`) — 프록시로 강할 뿐 등가는 아니다.

★★**그런데 소크가 죽었다** — `e9c504f1` 이 **2026-08-14T12:26:37Z** 에
`gap_resync_position_mismatch` 로 자동 사망(05:53Z 기동 → **6h33m**). 컨테이너는 살아 있다.
그 사인은 이미 `Resolved` 인데 **그 수리가 든 pin 에서 재발**했고, 로그의 `ledger_net=0.06` 은
정확히 **반전 주문 수량**(2×0.03)이라 이 회차가 백필 축에서 본 반전 의미론과 같은 계열일 수
있다(리드) → [BL-732]. 실격 원장 17행째로 등재했다(`cause_class: undecided`).
⇒ **클럭은 이미 끊겼으므로 재-pin 비용이 0 이다.**

**다음 행동 = 재기동 전에 거래소 포지션을 먼저 확인한다** — 마지막 체결이
`buy 0.06`(09:37:54, `reduce_only=false`)이고 세션은 그 뒤 죽었다. 반전 의미대로면 **+0.03 롱이
열린 채**일 수 있고, 고아 포지션 위에 새 세션을 얹으면 [BL-024] 회차가 밟은 함정을 반복한다.
플랫이 확인되면 `soak-stack.sh down → pin <#631 이후 sha> → up` 으로 **백필 수리를 실어** 새
세션을 열고(AC-3 동시 충족), 잔존이 있으면 정리가 선행이다. 그 뒤 ⓪ 표에서 고른다.
...
| 소크      | ~~**돌고 있다** — 세션 `de3db35a` · pin `c3a39d0d` · C5 6/6 · 전 이력 실격 12건~~ → ★**2026-08-14 반증: 안 돌고 있다.** `.soak/pin-history.jsonl` 마지막 이벤트가 **`down` (2026-08-07T09:33:02Z)** 이고 그 뒤 `up` 이 없다 — **7일째 정지**. `soak-gate --no-collect` = UNKNOWN(rc=2) · C5 `stack_pinned=`**✗** · C1 **0/3**(누적 1.56h) · C2 **0.91h** · 전 이력 실격 **14건** · 표본 간격 최대 **1524.5분(25.4h)**. 지금 도는 `quantbridge-*` 컨테이너 6종은 소크가 아니라 `up-isolated` 개발 스택이다              |
...
| **P** | ~~[BL-724] ★수수료가 gross 를 **16배**로 먹는다 — 실자금 전환의 경제성 전제~~ → **2026-08-14 종결 (판정)** — 실자금 불가 · 데모 유지. 손익분기 요율 단면 0.00377%(현재의 1/14.6)        | P1  | ★★★  | 중     | M (2-3h)   | 0줄                            | 즉시 — gross **+74.18** vs 수수료 **−1,200.00**(고유 592건 실측). 소크 게이트가 PASS 해도 안 바뀐다 · **판정 항목이지 수리가 아니다**                                                                                                                                                                                                                                                                                                                                                                                              |
| **Q** | ~~[BL-727] `soak-gate.sh:706` 이 맨 `python3` 라 **맥에서 판정 불가**~~ → **2026-08-14 종결** (`dde53e68`/#631)                                                                         | P2  | ★★★  | 낮음   | XS (10분)  | 0줄                            | 이미 발화 — 3.9 vs `itertools.pairwise`. 죽고도 빈 `판정:` 을 찍는 **fail-open**. 같은 파일 `:449` 가 이미 고쳐 둔 함정                                                                                                                                                                                                                                                                                                                                                                                                            |
| **R** | ~~[BL-726] `rejected`+reduce-only **46건**에 `realized_pnl` +55.32 인데 evaluator 가 못 본다~~ → **2026-08-14 기각으로 종결** — 값은 pine_v2 추정치이고 거래소 미도달 46건. 동작 변경 0 | P2  | ★★   | 낮음   | S (1h)     | **건드림**                     | 동승 — [BL-438] 백필과 **같은 SUM**. 「채워졌다(=청산)」 ↔ 「rejected(=발주 실패)」가 모순이고 판정된 적 없다                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **S** | [BL-725] `exchange_exits` **중복 290행** — 같은 uid 에 계정 행이 둘                                                                                                                     | P2  | ★    | 낮음   | S (30분)   | 0줄                            | 미도래(동승) — 신규 적재는 08-08 에 멈춘 잔재. `exchange_accounts` 행 축 결정 대기                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **U** | ~~[BL-728] `classify_exit` 이 Bybit `CreateByLiq` 를 못 잡는다~~ → **2026-08-14 종결** (`dde53e68`/#631)                                                                                | P3  | ★    | 낮음   | XS (10분)  | **건드림**                     | 미도래(동승) — 관측 0건 잠복. 실자금 전환 후엔 잠복이 아니다                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **V** | [BL-729] 전략 채택 근거 백테스트 4벌이 **낡은 비용 가정**(0.001/0.0005)으로 돌았다 — 왕복 0.30% vs 실측 0.138%                                                                          | P2  | ★★   | 하     | S (1h)     | 0줄                            | 즉시 — 소크 전략을 다시 고를 때의 입력. `config_mapper` 가 저장값을 복원해 **재실행으로 안 고쳐진다**. Cost-Assumption 축이 이 일을 한다 (소크 정지 창에서)                                                                                                                                                                                                                                                                                                                                                                        |
| **W** | [BL-730] FE 비용 기본값 **drift 2곳** — 온보딩이 왕복 0.30% 를 하드코딩 submit                                                                                                          | P2  | ★★   | 하     | XS (15분)  | 0줄                            | ★이미 발화 — 신규 사용자의 첫 백테스트가 2.2배 비관으로 돈다. [BL-603] 이 「미러 4곳」이라 적었지만 실제 5곳이었다                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **X** | [BL-732] ★`gap_resync_position_mismatch` **재발** — 이미 Resolved 인 사인이 그 수리가 든 pin 에서 다시 났다                                                                             | P1  | ★★★  | 중     | M (3-4h)   | 0줄                            | ★이미 발화 — 소크가 **6h33m 만에** 사망(12:26:37Z, 실격 원장 17행). `ledger_net=0.06` = **반전 수량**이라 반전 의미론 계열 리드. **재발인가 별건인가** 판정 선행                                                                                                                                                                                                                                                                                                                                                                   |
| **Y** | [BL-731] `list_synced_with_exchange_exit` 의 **LIMIT 500** — 재검증 모집단 73 → 563                                                                                                     | P2  | ★★   | 하     | S (1h)     | 0줄                            | 동기화 축은 **단조 증가**라 안 풀린다. 처방 = 상한 확대 말고 「원장 합계 ≠ 저장값」으로 모집단을 좁혀라                                                                                                                                                                                                                                                                                                                                                                                                                            |
| **Z** | [BL-733] 체결 직후 refresh 2곳이 아직 `reduce_only` 게이트 — 반전 백필 최대 5분 지연                                                                                                    | P2  | ★★   | 중     | M (2-3h)   | 0줄                            | ★**필터만 지우면 더 나빠진다**(entry 마다 거짓 운영자 알림). `_reversal_bucket_at_fill` 재사용                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| **P** | ~~[BL-438] 거래소 네이티브 TP/SL·트레일링 청산 손익이 머니-패스에 전혀 계상되지 않는다~~ → **2026-08-14 종결** (`dde53e68`/#631) — 구 술어 0건 → 신 술어 490건/−1,023.87 로 음성 대조   | P1  | ★★★  | 중     | M 6-8h     | **건드림**                     | Trigger 줄 자신이 **「즉시」**다(조건절 없음). 브래킷으로 익절/손절된 거래의 손익이 Kill Switch·loss-limit·에쿼티 커브 **어디에도** 안 잡힌다. 종전에는 PARTIAL 이라는 이유만으로 표에 못 올라왔다                                                                                                                                                                                                                                                                                                                                 |
...
| **Y** | ~~[BL-720] `docs-audit` 의 지식 정본 축 결손 — ID 유일성·[ADR-029] 죽은 경로·백틱 코드 스팬~~ → **2026-08-14 종결**                                                                     | —   | —    | —      | —          | —                              | ★**등재된 처방 3개 중 ①(`legacy_paths` 확장)이 착수 전 반증됐다** — 살아 있는 문서 **147줄**이 `backend/`·`frontend/` 를 정당하게 인용하고 그중엔 [ADR-029] 의 **이동 매핑 표 자신**이 있다(부분문자열 매치라 예외 불가). 죽은 포인터는 새 포인터 축이 이유 불문 잡으므로 흡수했다. 이행 = 축 2종 + 하네스 7→**12 케이스**. ★★**스텁 11/11 초록인 채로 진짜 `lessons.md` 에 대고 돌리니 오탐 3건**(자리표시자·코드 표현식·슬래시 커맨드) — 케이스 ⑿ 가 그 셋을 입력으로 심었다                                                     |
| **Z** | ~~[BL-722] `assert-main-checkout.sh` 판별력 덮개 부재 (파괴적 타깃 가드)~~ → **2026-08-14 종결**                                                                                        | —   | —    | —      | —          | —                              | ★**처방 2건이 둘 다 뒤집혔다.** ⑴ `--selftest` 보다 **별도 하네스**가 싸다 — 호출 자리 2곳이 전부 이름 규약 기반이라 `--selftest` 는 양쪽에 특례 배선이 든다 ⑵ 「비 git → rc≠0」은 **코드와 반대**였다(`:32-37` 이 판정 불가를 의도적으로 통과 — 그대로 썼으면 하네스가 프로덕션 동작을 「고장」으로 고정했다). 하네스 **9→10종** · 변이 M1 에서 ⑴⑵ 둘 다 red                                                                                                                                                                      |
