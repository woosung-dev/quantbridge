...
| **BJ** | [BL-797] 화면 증거 게이트가 **공개 3라우트만** 잰다 — authed 화면을 바꾼 PR 은 여전히 무증거다. ★**2026-08-18 트리거 도래**: 선행 두 항목이 **둘 다 Resolved**(PR #651 `d28bf28f` — 상세는 원장 섹션). ★CI 결정론 시더와 **같은 차단자를 공유**하므로 그 뒤에 붙여라                                                                                            | P2  | M                                                                                                                 |
| **BK** | [BL-805] `quantbridge-api.service` 가 **레포에 없다** — 레포가 만들지 않는 유일한 systemd 유닛. 서버 실측으로 `ExecStart`·`WorkingDirectory`·`Environment` 는 확보돼 있다                                                                                                                                                                                       | P2  | ★★                                                                                                                | 하     | S             | ✗              | 서버를 다시 세우면 그 자리에서 밟는다. 복원할 원본이 없다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **BL** | [BL-806] 모델은 python `default=` 인데 마이그레이션이 `server_default` 를 넣은 컬럼 **6개** — `create_all` 경로의 테스트 DB 와 프로덕션 스키마가 갈린다                                                                                                                                                                                                         | P3  | ★                                                                                                                 | 하     | S             | ✓              | [BL-803] 의 `default` 축이 실측으로 6건을 확정했다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| **BM** | [BL-807] authed 캐논 **3케이스가 「행은 DB 에 있는데 화면이 빈다」** — `/backtests/:id/trades` 0행 · `/optimizer/:id` 링크 부재 · report shell 미표시                                                                                                                                                                                                           | P2  | ★★                                                                                                                | 중     | M             | ✗              | BL-802 가 18/20 을 올리고 차단자를 이 셋으로 좁혔다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **BN** | [BL-808] 스키마 동등성 축의 잔여 3구멍 — 정규화가 따옴표 리터럴 안까지 지운다 · 정지 규칙이 앞 축 baseline 으로 **뒤 축을 끈다** · CHECK 표현식 약화 미검                                                                                                                                                                                                       | P3  | ★                                                                                                                 | 하     | S             | ✓              | 적대 리뷰가 셋을 재현 시나리오와 함께 확정했다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
## ★2026-08-18 n5-ci-truth-close — authed 게이트가 「실제로 도는」 상태가 됐다 ([BL-802]/[BL-777]/[BL-803] · [BL-789]/[BL-749] 동승 종결)

**무엇을 했나** — 워크트리 슬롯 4·5 에서 β·γ 병렬 + CONTROL 이 α 직접 수행. PR **α** #677 · **β** #678 · **γ** #679 → A(`stage/n5-ci-truth-close`) → main.

⑴ ★★★**CI 의 authed spec 이 1건 → 18건이 됐다** ([BL-802] · [BL-789] 종결). 최종 run `32121054465` —
setup 3 passed + `chromium-authed` **72 passed 4.0분**(착수 시점 7 passed / 1 spec).
⑵ ★★★**원장의 진단이 통째로 반증됐다.** 「12파일이 하드코딩 UUID 40종 때문에 막혔다」인데 그 중
**11건이 무수정 통과**한다 — 그 UUID 는 DB 행이 아니라 `page.route()` mock 의 id 이고,
**이미 CI 에서 통과 중이던 spec 도 똑같이 전량 mock** 이었다. 매니페스트 사유 12건이 동일
보일러플레이트였던 것이 증거다: **측정이 아니라 grep 으로 붙은 분류**였다.
⑶ **실측 궤적** — 시더 없음 `79/13` → 시더 착지 `84/5` → 플래그+페이로드 `86/3`.
최초 13건 실패의 실제 원인은 **기능 플래그 1 · 페이로드 형상 3 · 진짜 없는 행** 순이었다.
⑷ [BL-777] BE 배포/롤백 런북 신설 — 맨 앞에 **「어느 명령이 소크 창을 끊는가」 표**(5파일에 흩어져
있던 지식). ★**`pin` 이 실질 배포 단계**이고 `up` 만으로는 새 코드가 안 뜬다. 동승으로 **문서 오류 9건 정정**.
⑸ [BL-803] 스키마 5/5 완료 — `default` 축이 **진짜 drift 6건**을 냈다(→ [BL-806]). CHECK 는 표현식이
아니라 **이름 집합** 대조다.
⑹ ★**신설** — [BL-804] BE 이미지 태그 부재, DEFERRED · [BL-805] `quantbridge-api.service` 유닛 부재 ·
[BL-806] server_default 비대칭 6건 · [BL-807] 행은 있는데 화면이 빈다 3건.

~~**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-807]** — 동승 [BL-805]·[BL-806]. ★[BL-797] 은 차단자였던 결정적 시드가 이제 있다.~~ → **2026-08-18 design-t1t2 회차(PR #682 머지)가 이 방향을 실증으로 좁혔다**: FE 25라우트 감사→폴리시 5커밋(적대 검증 13레인 + codex + vercel, 신호 4종 = `.claude/gates/design-t1t2/`)이 바꾼 화면 대부분이 authed 라 **증거 팩 밖에서 무증거**였고 검증을 손 playwright 로 때웠다(사고 기록 = screen.ok — Turbopack 캐시 유령 [BL-650] 재발 + Docker VM 디스크 고갈). ~~다음 행동 = [BL-797] authed 화면 증거 게이트 + [BL-807] authed 캐논 3케이스를 한 회차로 닫는다.~~ → **2026-08-19 [ADR-037] 하네스 제로베이스가 먼저 끼어들었다(아래 회차)** — 화면 증거 게이트 자체가 철거됐으므로 [BL-797] 은 「구 팩 복원」이 아니라 **재입힘 규칙의 두 번째 티켓**(위 authed 무증거 사고 기록이 근거)으로 슬림 재정의해야 한다. 회차 중 사용자 결정 2건(⑴ [BL-776] 가입 방향 ⑵ 마케팅 아이브로우 — 추천 = 마케팅 면 한정 제거)은 그대로 접수 대기.
...
**다음 행동 = 새 얇은 하네스로 [BL-807]+[BL-797] 회차 실주행** — [BL-807] authed 캐논 3케이스를
`/review-code`+codex 훅+CI 단일 게이트만으로 완주하고, [BL-797] 은 design-t1t2 의 authed 무증거
사고 기록을 근거로 **재입힘 규칙의 두 번째 슬림 티켓**으로 재정의해 같은 회차에서 닫는다.
뚫린 곳은 기록한다(그 기록이 다음 복귀 티켓이다). Eval 첫 실행(`ANTHROPIC_API_KEY` 로컬 필요)도
이 회차에서. 사용자 결정 2건(⑴ [BL-776] 가입 방향 ⑵ 마케팅 아이브로우 — 추천 = 마케팅 면 한정
제거)은 접수 대기 그대로.
