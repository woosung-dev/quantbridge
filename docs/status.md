...
| **BD** | ★[BL-788] `tests/conftest.py` 의 `create_all` 범위가 **import 목록으로만** 정의된다 — 빠진 모델은 조용히 안 만들어지고 head 로 stamp 된다. **표적 실행으로는 안 보인다**(단독 20 passed / 전량 2 failed)                                                                                                   |
| **BE** | [BL-789] authed e2e 가 **GitHub CI 에서 안 돈다** — authed 계열 spec 전부가 로컬 전용 게이트다 — 격리 슬롯 authed 불가와 한 뿌리다. CI 초록이 그 게이트 통과를 뜻하지 않는다                                                                                                                               |
| **BF** | [BL-790] funding 인제스션이 `exchange_name: str` 을 받는다 — CCXT 속성명 검사만 한다. native enum 전환으로 실패 지점이 INSERT 로 앞당겨졌다                                                                                                                                                                |
| **BG** | [BL-795] 「authed red」의 **두 번째 원인** — Turbopack 영속 캐시 물림. 구분식 = 실패가 `setup` 단계 + 429 **0건**이면 캐시 쪽이다                                                                                                                                                                          |
...
## ★2026-08-17 night-3lane — 야간 자율 3레인 (1회차)

> **강등 tombstone.** 회차 서술 12줄 → 이 2줄. 원문 = `git show 5423fc68:docs/status.md`(496~511행).
> PR #654·#655·#656. 살아남는 하나: ★**증거가 없던 이유가 확인 행위였다** — playwright 가 매 실행 `outputDir` 을 지운다([LESSON-117]).

~~**다음 행동 = … [BL-784] 수리**~~ → **2026-08-17 야간 2회차가 완주.** 아래 §gate-trust 참조.

## ★2026-08-17 gate-trust — 야간 자율 3레인 2회차 · 게이트 신뢰 복구 팩

**무엇을 했나** — preflight 8축 전건 일치 뒤 claude 레인 3기(슬롯 3·4·5, 전부 워크트리)를 띄워
PR **#659**(α `3358bde4`)·**#658**(β `cb2d17b3`)·**#660**(γ `dd453d75`) 머지 · 유예 원장 3/3 소멸.
주제는 하나였다 — **게이트가 red 를 냈을 때 그것이 진짜인지 사람이 매번 판단해야 하는 상태를 끝낸다.**

⑴ **[BL-784]·[BL-785]·[BL-782]·[BL-786] 전건 Resolved.** 상세는 각 BL 본문. 요지 셋 —
e2e 신원 **하나만** 면제(authed 3회 rc=0 · 429 **0건**) · `alembic check` 의 정본을 **migration-only DB** 로
확정 · 중복은 StrictMode 가 아니라 `uid` 키 churn 이었고 **프로덕션에도 그대로** 있었다.
⑵ ★★★**내가 보고한 preflight 기준선이 조건부였다** — 「BE 4759 passed」는 **더러운 DB 에서만** 참이고
fresh DB 에서는 `2 failed, 4757 passed` 다([BL-788]). ★**표적 실행으로는 안 보인다**(단독 20 passed).
⑶ ★★**적대 리뷰 8건이 전부 참이었다(phantom 0)** — 그중 셋을 CONTROL 이 직접 수리했다:
staging 에 면제 방어층이 **하나도 없었고**(α), 새 감사기가 **env 하나로 꺼졌으며**(β),
새 dedup 게이트가 **429 를 「중복」으로 읽어 가짜 red 를 냈다**(γ).

**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-788]** — 이 회차가 기준선 오염을 겪은 자리이고,
수리는 이미 들어갔으나 **재발을 막을 검사면이 없다**(`metadata` 등록 테이블 수를 재는 축).
동승 후보 = [BL-789] (authed 가 CI 에서 안 돈다 · [BL-781] 과 한 뿌리). ★고르는 것은 사용자다.
