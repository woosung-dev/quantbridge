...
~~**다음 행동 = ⓪ 표 M([BL-704])으로 착수한다 — `/metrics` 인증 상태를 부팅 로그 1줄로
찍는다.**~~ → **2026-08-11 metrics-boot-log 가 닫았다.** ⓪ 표 **9행**.

★★**N 과 M 은 같은 병이었고 처방도 같았다** — 「검사기가 보는 표면 < 실제 실패 표면」.
N 은 자기검사가 **스캔층**을 안 봤고, M 은 부팅 가드가 `app_env` **문자열**만 봤다.
둘 다 **그 층을 실제로 발화시켜** 고쳤다(임시 트리 하네스 / lifespan 왕복 테스트).
★그리고 둘 다 **검증기 자신에게서 한 겹을 더 찾았다** — N 은 「자기검사를 지우면 아무도 못
잡는다」, M 은 「로그 수집을 컨텍스트 밖에서 세면 shutdown 으로 미루는 변경이 샌다」.
**검사기를 고칠 때는 그 검사기의 검사기도 같이 의심해라.**

★★★**S([BL-559] ②)는 「제거」가 아니라 「기각」으로 닫혔다 — 처방이 반증된 첫 사례다.**
앞선 넷([BL-307]·[BL-703]·[BL-672]·[BL-704])은 원장의 **전제**가 틀렸는데, 이번엔 전제가
맞고 **처방**이 틀렸다. 「사문이니 제거하라」가 **사문인 이유**(상위 필터 2겹이 막고 있고 그
분기는 그 필터가 깨지는 날을 위한 방어선)를 안 봤다. ⇒ **「없다/제거하라」 둘 다 되물어라.**

**다음 행동 = `scripts/soak-gate.sh`(서버, `bash -lc` 필수)로 창을 확인하고 ⓪ 표 8행에서
고른다. ★P1 둘([BL-641] O · [BL-438] P)은 **각자 통짜 회차**다 — O 는 닫는 조건이
「사망률이 실제로 내려간다」라 한 회차로 안 끝나고, P 는 **귀속 설계**에 사용자 결정이
붙을 수 있다. ★~~작은 것을 묶으려면 R + S 가 같은 `live_session_admin` 근처다~~ →
**S 는 이 회차에 기각으로 닫혔다.** 남은 작은 것은 R([BL-639]) 하나이고 **결정이 선행**이라
착수 전에 물어야 한다 — 「계정 축 소유권 집합」을 어디까지로 볼지가 사용자 몫이다.**
