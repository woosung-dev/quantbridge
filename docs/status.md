...
| 1   | E2E `screen.ok` 정상화           | codex-a (슬롯 2)    | `pnpm e2e` 3 / `e2e:design-canon` 42 / `e2e:authed` 83 전건 green · `screen.ok` 실취득         | 진행                      |
| 4   | [BL-427]+[BL-430] 전략 목록      | codex-b (슬롯 3)    | 서버 정렬 4축(샤프는 `_sharpe_sort_criteria` **재사용**) + 파생 lifecycle/파라미터 · alembic 0 | 대기                      |
| 2·3 | 캐논 2벌 → [BL-414]+[BL-413]     | CONTROL → codex-a   | `preflight.py --all` FAIL 0 + `runtime-check.mjs` 4해상도 → 그 뒤에만 코드                     | **stretch** (예산 초과분) |
| 5   | 종단 데모 리허설 + 리포트        | CONTROL (메인 전용) | 전략→백테스트→스트레스→옵티마이저→코크핏 **실화면** 완주 · 콘솔 error 0                        | 대기 (머지 후)            |
