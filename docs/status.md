...
공개 라이트 5라우트 canon 이 **전부 0** 이 됐고 래칫을 0 으로 조였다. 상세 = `lessons.md` LESSON-130.
⇒ **게이트가 초록이다** — 다음 회귀가 묻힐 빨강이 더 없다.

**다음 행동 = `d1` 실사용 루프 — 사용자가 전략 ≥5벌을 임포트해 1개를 고른다.** PRD §5⑵ 를 전진시킨다.
★**왜 지금인가.** 이 축을 UI/UX 로 열어 둔 이유가 셋이었고(320px 오버플로 · 키보드로 못 읽는 표 ·
...
★**d1 의 사용자 몫은 살아 있다.** TradingView 전략 ≥5벌을 `qb.woosung.dev` 에 임포트해
백테스트 → 스트레스로 1개를 고르는 절반은 **세션이 대신할 수 없다**. 막히는 곳마다 우회하지 말고
적어 두면 「사용 마찰」 축 BL 로 등재한다. PRD §5⑵ 지표 3종은 그 루프가 돌아야 산출된다.
