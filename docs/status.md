...
~~**다음 행동 = harness A/B 회차 step 파일을 써라.**~~
→ ★**2026-08-12 A회차 · 2026-08-13 B회차 종료** ([BL-708]·[BL-709] ✅) — 전문 = [ADR-030](decisions/030-harness-pilot-verdict.md) §측정. ★★**B 회차를 죽인 것은 대조해 둔 as-is 위험 6건이 아니라 목록에 없던 7번째**(`TimeoutExpired` 미처리) — 그때 트리에 남은 것이 정확히 B 가 막으려던 상태다(**AC 0건 실행 + `completed` 커밋**).

~~**다음 행동 = harness 파일럿 판정을 사용자와 함께 낸다 — 승격(ADR-029) / 삭제(+tombstone) / 3회차 중 하나.**~~
→ ~~★**2026-08-13 사용자 판정 = 「일단 유지」**(보류). 삭제도 3회차도 하지 않고 현 상태 그대로 둔다.~~
→ ★★**2026-08-13 (같은 날 재판정) = 「걷어낸다」 — 종결.**
[ADR-030](decisions/030-harness-pilot-verdict.md) 이 정본이다 — **하네스를 (a) 조종 장치 /
(b) 증거 장치로 가른 뒤** (a) 전량(230.7 KB / 27파일)을 회수하고 (b) 전량(249.7 KB)을 남겼다.
★**근거를 갈아 끼우지 마라** — 이유는 「모델이 좋아졌으니까」가 **아니다**. 실측이 잰 것은
「이 조종 장치가 40건에서 캐치 0이고 커밋 노이즈가 5배였다」다(ADR-030 §근거 4건).
★**잔여 부채 1건** — `AGENTS.md` 의 「게이트는 마지막 커밋 뒤에」에 **범위(회차 단위)가 없어**
무인 세션이 66분을 태웠다(미착수). 재개 조건은 ADR-030 §Consequences.
...
→ **2026-08-15 ledger-thaw (PR #640) — 그 지시는 유효했고, 시계를 채우는 동안 원장 쪽을 열었다.**
넣은 것 넷: C1 자격 판정기(「지금 `up` 을 눌러도 손실 0인가」, 판독 전용) · 스윕 `--include-deferred`
(**DEFERRED 165건 중 기계 도래 0건**) · 스윕 사람 판정 축(**미판정 0건** — ★원장은 얼어붙지 않았다.
판정은 이미 있었고 기계가 안 읽었을 뿐이다) · [BL-749] 타입 축(실측 drift 0건). 신규 [BL-751].

~~**다음 행동 = C1 첫 창을 채우고, 판독이 `✓ 자격 획득` 을 낼 때 사용자 승인을 받아 2번째 창을 연다.**~~
→ **2026-08-15 surface-truth — 그 지시는 그대로 유효하고, 시계가 도는 동안 보안 P1 5건과 여정 축을 열었다.**
서버는 **무접촉**이었다(읽기 전용 `grep` 2회 · `curl` 3회뿐 — `pin/up/down`·`migrate`·유닛 조작 0건).

## ★2026-08-15 surface-truth — 배포 호스트가 `APP_ENV` 를 잊었고, 서버가 아는 것이 화면에 안 왔다

★★★**착수 전제 3건이 반증됐다.**
⑴ 「배포 호스트가 traceback 을 흘린다」 — **거짓**. 그 호스트 `.env.local` 에 `DEBUG=false` 가
이미 있어 Starlette debug 분기는 발화한 적이 없다. ⑵ 「`entry(limit=)` U7 은 6종 중 2종만
미러링」 — **낡았다**. 5종은 이미 미러링돼 있었고, 아무도 안 보던 것은 **다른 하나**
(`position_side_unsupported`)였다. ⑶ 「FE 스키마에 `read_only` 가 있으니 배선은 살아 있다」 —
**거짓**. 코크핏 어댑터가 그 값을 버리고 있었다.

**보안 P1 5건 (S1~S5)** — `/openapi.json`·`/docs` 가 **인터넷에 200** 이었다(실측). 뿌리는
`APP_ENV` 미설정이고 `_enforce_production_safety` 가 조기 반환해 production 게이트 4개가
꺼져 있었다. 코드로 3개를 닫았다(`is_production or not debug` + `debug` 기본값 True→**False**).
나머지 하나(placeholder 시크릿 fail-fast)는 [BL-753] — **사용자 결정 대기**다.
그 밖에 **거래소 API secret 평문이 422 body 에 반사**(S2 · 전역 `RequestValidationError` redaction) ·
**탈퇴가 돈을 안 멈췄다**(S3 · beat 가 계속 실주문 · 유출 웹훅 시크릿이 계속 체결) ·
**Redis `allkeys-lru` 가 큐 메시지와 분산 락을 축출**할 수 있었다(S4 · 「DB 를 나눴으니 격리된다」는
문장이 **세 곳**에 있었고 전부 거짓) · `orders.created_at` 인덱스(S5).

**여정 축 (U1~U8)** — 「서버는 아는데 화면에 도달하지 않는다」 6건 + 화면이 **반대로** 말하던 1건.
`entry(limit=)` 은 백테스트에서 지정가로 체결되게 하고 **라이브는 fail-closed 로 막았다**
(동작 변경 — 종전에는 시장가로 나가고 있었다). 엔진 경고는 `backtests.warnings` 로 원장에 실려
리포트 ⑨ 에 인쇄된다 — 그 값은 **계산되고 있었는데 소비자가 0건**이었다.

★★**변이 13종 중 하나가 진짜 초록이었다** — 계획서가 ★★로 표시한 `and limit is None` 가드에
커버리지가 **0** 이었다(엔진 기본값이 `bar_close` 라 어떤 테스트도 그 분기를 안 지났다).
그리고 **내 변이 하네스 초판이 `| tail` 때문에 pytest 가 아니라 tail 의 rc 를 읽어** 「7종 전부
초록(판별력 0)」이라는 거짓 전건을 냈다 — 출력에 `6 failed` 가 찍혀 있는데 rc=0 이었다.
⇒ **[LESSON-087] 이 3/3 에 도달**해 `apps/api/AGENTS.md` §10 「판별 절차」로 승격됐다(도달 확인 3층).
★신규 LESSON 을 만들지 않은 이유 — 같은 패턴이면 누적이지 신설이 아니다(`lessons.md:12` 자기 규약).
...
결정 — 보고 후 대기). 자격 전에 누르면 그때까지 번 시간이 **창 0회로 소멸**한다.
★**사용자 결정 2건이 대기 중이다** — ⑴ [BL-753] 배포 호스트 `APP_ENV=production` 전환(선행:
비어 있는 시크릿 채우기 · 2026-08-07 에 그 전환이 게이트를 죽인 전례가 있다) ⑵ G1 DB 호스팅(아래).
★**이 회차 수리는 배포 호스트에 재배포되어야 발효한다** — `/docs` 노출은 그때까지 열려 있다.
...
| **AH** | [BL-754] rate limit 이 프록시 뒤에서 **전 사용자 공용 버킷**으로 붕괴 — `request.state.user_id` 를 아무도 세팅하지 않고 `TRUSTED_PROXIES` 가 비어 있다                                                                                                                                                                         | P2  | ★★   | 하     | S          | **건드림**     | ★기전은 **지금 성립한다**(코드 대조 2026-08-15). 영향은 사용자 2명부터라 공개 전환([BL-071])과 동승이 합리적이다                                                                                                                                                                                                                                               |
| **AI** | [BL-757] 시크릿 스캐너 **0건** + `.gitignore` 가 `.env.prod` 계열을 안 막는데 추적되는 `.env.prod.example` 이 그 작명을 유도                                                                                                                                                                                                   | P2  | ★★★  | 낮     | S (30분)   | 0줄            | ★막는 비용이 XS 이고 **되돌릴 수 없는** 사고(히스토리에 들어간 키)를 막는다. `git check-ignore` 실측 + grep 0건                                                                                                                                                                                                                                                |
