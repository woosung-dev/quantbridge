...
> **Last Updated:** 2026-08-01 (**conditional-fill-visibility** — ★★★**기다리는 대신 조건을 만들었다.** write-back 이 **프로덕션에서 실행됐다** · BL-566 은 크기가 **1/3.2** 로 재판정)
> **Active Sprint:** 없음. **다음 스프린트는 아래 §다음 스프린트 참조.**
> **미머지:** `stage/conditional-fill-visibility` (PR 대기).
> **Last Merged:** `stage/reversal-ledger-sync` → `main@a6d891b1` (**PR #515**, 2026-07-31) · 그 앞 `docs/soak-closeout-state` → `main@262ed28e` (PR #514)

---

## 🎯 다음 스프린트 — **silent-surface-honesty** (조용히 실패하는 표면들)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.
> 시작 방법: **"다음 스프린트 진행해줘"**. `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 직전 회차가 라이브 원장 쪽 숙제(BL-560 프로덕션 검증 · BL-566 크기 재판정)를 닫았고,
그 과정의 QA 트랙이 **사용자가 누르는 것이 조용히 실패하는** 결함 묶음을 꺼냈다. 넷 다
**기전이 이미 확정돼 있어** 「먼저 재라」 단계가 필요 없다 — 이번엔 바로 고칠 수 있는 회차다.

### 후보 (전부 기전 확정 · 착수 가능)

| BL              | 무엇                                                                                     | 왜 지금                                                                                                                             |
| --------------- | ---------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| **BL-570** [P2] | 무편집 상태에서 `설정 저장` 이 **활성인데 아무 일도 안 한다**(PUT 0 · 토스트 0 · 에러 0) | 탈락 필드 `max_trigger_breach_pct` 까지 특정됨(2/2 재현). **세 회차를 살아남았다** — GET 도 「편집 후 저장」도 멀쩡해서             |
| **BL-542** [P3] | 포지션 1건인데 「거래소가 더 많다 · 첫 200건만」 경고가 **상시** 발화                    | 증거 n=1 → **n=2**, 기전 확정(주석이 스스로 「커서를 첫 항목에 도장만 찍는다」고 적어 놓고 그걸 「더 있음」으로 읽는다). 판정식 1줄 |
| **BL-571** [P3] | enum 밖 종료 사유가 화면에 원문 노출 + 콘솔 경고 40초 67건                               | **운영자(CONTROL)가 만든 오염**이다. AST 가드가 호출부만 훑어 원장 직접 기입을 못 막는다                                            |
| **BL-572** [P3] | 같은 세션이 표에서는 `PAUSED`, 옆 카드에서는 「종료된 세션」                             | 영문 리터럴 하드코딩 2곳                                                                                                            |

### ★착수 전 반드시 읽을 것

1. ★★**「먼저 재라」를 반복하지 마라.** 위 넷은 크기·기전이 **이미 확정**됐다. 직전 회차들이
   반복한 「크기 확정 → 다음 회차 수정」 2단계를 여기에 또 적용하면 순수한 낭비다.
2. ★**nullable BE 설정 필드를 추가하면 FE 폼 초기값 정규화까지 같은 PR 에서 해라**
   (`gates-and-traps.md` 에 등재). `schemas.ts` `.strict()` 만 맞추는 건 **파싱**만 맞추는 것이다.
3. ★**무편집 저장을 눌러봐야 보인다.** BL-570 이 세 회차를 살아남은 이유가 정확히 그것이다.
4. ★**BL-565 는 여전히 막혀 있다** — `bracket_attached` **0%**, `strategy.exit` 쓰는 전략 없음.
   전략을 바꾸지 않는 한 손대지 마라(고쳐도 검증 불가).
5. ★**`engine_only_awaiting_trigger` 는 프로덕션에서 실행된 적이 없다.** 기회가 오면
   (트리거 미돌파 국면) 로그를 확인해라 — 단 그걸 위해 창을 따로 잡지는 마라.

### 사전등록 판정 문턱 (착수 전 확정할 것)

| ID  | 판정        | 조건                                                                               |
| --- | ----------- | ---------------------------------------------------------------------------------- |
| Z1  | 계측기 선행 | BL-570 수정 전, **무편집 저장 클릭 → PUT 0** 을 재현해 red 로 고정                 |
| Z2  | 성공        | 무편집 클릭이 **PUT 200 이거나 버튼이 `disabled`** 둘 중 하나 (그리고 화면에 사유) |
| Z3  | 표적 변이   | 초기값 정규화를 되돌리면 Z1 회귀가 **실패**                                        |
| Z4  | 음성 대조   | 「편집 후 저장」 경로가 **그대로 200** (지금 멀쩡한 것을 깨지 않는다)              |

### 하지 않는 것

**BL-565**(구조적 측정 불가) · **BL-553 PbR 재시도** · `same_side` 시간당 발생률 재시도 ·
codex #1/#3/#5(BL 등재만 — 구조적/공유 메서드/선재 패턴).

### baseline (2026-08-01 실측 — `stage/conditional-fill-visibility`)

**BE 3735 passed / 46 skipped**(직전 3721 대비 **+14**) · **FE 1232**(205 파일, **FE diff 0줄**) ·
...
> ★**boolean 을 판정에 쓰지 마라** — `psql -At` 는 맨 컬럼이면 `t`/`f`, 캐스트하면 `true`/`false` 다.
> nullable 텍스트 컬럼(예: `deactivated_reason`)의 존재 여부로 판정해라.
> ★**소크 전에 거래소를 flat 으로 맞춰라** — 직전 소크가 남긴 포지션이 창을 통째로 날릴 수 있다.
