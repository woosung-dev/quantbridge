...
## 🎯 다음 스프린트 — engine-exchange-alignment (BL-543 먼저, 이어서 BL-536)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.
> 시작 방법: **"다음 스프린트 진행해줘"**. 에이전트는 `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 직전 회차가 BL-536 을 재측정해 **유실이 실재함(16.7%)** 을 확인했다. 그런데 같은 soak 이 **그 측정을 하려면 먼저 고쳐야 할 것**을 찾아냈다 — 세션은 **태어날 때부터** 엔진과 거래소가 갈려 있다.

### ★순서 (2026-07-29 재측정이 정한 것)

1. **BL-543 (P1)** 이 선행이다. `run_live` 는 매 평가마다 300 bar 를 재생하는데 dispatch 는 **마지막 bar 만**(`event_loop.py:410`). 재생 구간 포지션은 **주문이 된 적 없는데 엔진에는 남는다.** 결과가 셋이다 — (a) `engine_only` 계측기 무효 (b) `close_position_flat`·`110017` 유령 청산 (c) ★**>5분 공백마다 세션 사망**(`gap_resync` 조용한 정상화는 **엔진도 flat** 이어야 타는데 그럴 일이 거의 없다).
2. **BL-536 (P1)** 은 그 위에서. 판정은 **유지**로 이미 나왔다(유실률 16.7% · 최대 채널 57% 로 분산). 정렬 없이 "전환 의도 영속화" 를 지으면 **재생이 계속 제조하는 포지션 위에** 상태를 얹게 된다.
...
5. **worker 는 src 를 bind-mount 하지 않는다**(baked image). soak 전 sentinel 로 money-path 라인을 로컬과 대조하라(§7.2).

### soak

**2~3시간.** 직전 회차 실적 = 성공 평가 **160회 ≈ 2h40m** 로 채널을 갈랐다. ★**한 leg 이 죽어도 metric 차분은 이어붙일 수 있다** — leg 별 스냅샷을 남겨라. ★**평가 수가 창 길이를 독립 확인해 준다**(1분봉).

### 하지 않는 것

**BL-535**(백테스트 스팟 봉 — 별도 스프린트) · **BL-541**(세션 행 없는 포지션 — **아직 실측된 적 없다**, 관측되면 착수) · BL-542(P3) · BL-538/539/540 · BL-527/528/529 · 거래소 확장 · `tasks/` deepen.

### baseline (2026-07-29 실측, `feat/live-orphan-close` 기준)

**직접 측정:** BE **3424** / ruff **clean** / mypy **212 clean** / 마이그레이션 **0**.
**이월(미재측정):** FE **1213**(203 파일) · e2e authed **65** · canon **32** · CI전용 **4** · tsc · eslint · 커버리지 게이트 93.14% — 이번 회차 `frontend/**` 변경 **0 파일**이라 구성상 불변이지만 **돌리지는 않았다.**
