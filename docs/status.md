...
**다음 행동 = [BL-832] 의 저장소·신뢰 경계를 정한다** — 구현이 아니라 결정이 첫 step 이다. 축 둘: ⑴ **Redis**(이미 있다 · 워커/API 프로세스가 공유 · 그러나 pickle 역직렬화 신뢰 경계 판단이 선행) vs **로컬 디스크**(표면이 좁다 · 컨테이너/워크트리마다 별개라 이득도 좁다). ⑵ 캐시 키에 넣을 버전 축 (pynescript 버전 + AST 스키마 버전 — stale AST 방어). 정해지면 AC 5축(계수 1회 · digest 동일 · 1바이트 변경 미스 · 버전 변경 미스 · 예외 비캐시)은 이미 [BL-832] 에 적혀 있다.
...
| **C**  | [BL-832] 콜드 파스가 **프로세스 경계마다** 다시 든다 — AST 를 프로세스 밖으로 캐시 | P1  | ★★★ | 중 | M | **건드림** | ★**2026-08-26 [BL-829] preflight 가 이 항목을 낳았다.** 그 항목의 전제(「지배 성분 = full-context 재시도」)가 실측으로 깨졌고(SLL 로 204→0 을 만들어도 **3.7%**), 진짜 지배 성분은 `closure_` **96.8%** 였다. 콜드 비용을 줄이는 **유일하게 실측된 축**이 이것이다 — pickle 왕복 digest 보존 확인, `i3_drfx` **53.38초 → 0.0048초**. ★첫 step 은 구현이 아니라 **저장소·pickle 신뢰 경계 결정** |
