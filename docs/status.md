...
> **업데이트:** 2026-08-05 — **시계를 돌리고 눈금을 만들었다** (soak-clock-restoration)
>
> **★소크가 돌고 있다 — 그리고 게이트는 `FAIL` 이다.** 세션 **`cc19abd2`** ·
> T0 `2026-08-04T16:32:47Z` · **고정 커밋 `f5f06886`**.
> ★★★**첫 3시간에 phantom 3건(사망 1건)** — `phantom` 이 **사망 없이도 난다**.
> 게다가 분류기가 스스로 「봉경계식 vs 시간문턱식 **불일치 2건**」을 보고한다:
> 경과 **6.90초**짜리가 `phantom` 이다(관측이 봉 시작 직후면 짧은 경과도 봉 경계를 넘는다).
> ⇒ **[ADR-024] 가 명시한 우려 —「무해 갈래를 실격에 넣으면 게이트가 안 닫힌다」— 가
> 첫 창에서 실현됐다.** ★**판별식을 지금 느슨하게 고치지 않는다** — 그건 정의를 통과시키기
> 쉬운 쪽으로 바꾸는 것이다. 재설계는 **[BL-591]/[ADR-023] 축**이고 그게 다음 회차의 일이다.
>
> **★★★게이트를 세운 지 38분 만에 첫 `FAIL` 이 나왔다.** 세션 `39731d57` 이
> `2026-08-04T16:25:01Z` 에 `position_divergence` 로 죽었고 그 순간이 **`phantom`** 이었다.
> ⇒ **[BL-591] 슬라이스 A / [ADR-023] 의 명시된 재개 조건(「`phantom` 1건이라도 재발」)이
> 충족됐다** — 다음 회차의 유력 후보다. 재가동 완료(flatten → 새 세션 → 새 창).
> ★★**단 판별식 자체가 흔들린다** — **같은 발산 하나**가 엔진값·거래소값·최근 체결이
> 완전히 동일한 채 경과만 `1.53초 → 61.55초` 로 흐르자 `replay_lag` → `phantom` 이 됐다.
> 판별식이 경과 시간의 함수라 **자가치유하지 않는 발산은 반드시 `phantom` 이 된다.**
> 직전 회차의 「경과 37배 · 겹침 0」은 **그 표본의 성질이지 판별식의 성질이 아니다.**
> ★★**그 FAIL 이 게이트 자신의 결함도 드러냈다** — 죽은 직후 판정이 `UNKNOWN 진행중` 이었다
> (`window_start` 가 곧 마지막 사건 시각이라 「초과」 필터가 항상 비었다) ⇒ **기본 실행이
> 구조적으로 FAIL 을 낼 수 없었다.** 자기시험(`--since`)에서만 FAIL 이 나왔으므로 완료 기준을
> 통과한 채로 운영에서는 영영 못 볼 뻔했다. 수리 후 실측 `FAIL 실격` · exit 1.
> ★★**이제 `backend/src` 를 편집해도 소크가 안 죽는다** — 워커가 작업 트리가 아니라
> `.soak/src`(커밋 고정 스냅샷)를 mount 하고 watchfiles 가 없다. **음성 대조로 확인**:
> 같은 세션·같은 편집(md5 일치)에서 **수리 전 배너 1→2(9초 뒤) · celery PID 전부 교체** vs
> **수리 후 배너 1→1(지문 0건) · PID 불변**. ★단 **그 편집을 celery 로 검증할 수는 없다**
...
## 다음 스프린트 — **소크를 멈추지 않는다** + **[BL-024] 실주문 leg** / **[BL-580] 잔여 84곳**
