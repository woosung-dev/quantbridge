...
~~**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-807]** — 동승 [BL-805]·[BL-806]. ★[BL-797] 은 차단자였던 결정적 시드가 이제 있다.~~ → **2026-08-18 design-t1t2 회차(PR #682 머지)가 이 방향을 실증으로 좁혔다**: FE 25라우트 감사→폴리시 5커밋(적대 검증 13레인 + codex + vercel, 신호 4종 = `.claude/gates/design-t1t2/`)이 바꾼 화면 대부분이 authed 라 **증거 팩 밖에서 무증거**였고 검증을 손 playwright 로 때웠다(사고 기록 = screen.ok — Turbopack 캐시 유령 [BL-650] 재발 + Docker VM 디스크 고갈). **다음 행동 = [BL-797] authed 화면 증거 게이트 + [BL-807] authed 캐논 3케이스를 한 회차로 닫는다.** 동승 후보 = [BL-805] (서버 실측값 확보라 S) · [BL-806]. 회차 중 사용자 결정 2건을 접수해라 — ⑴ [BL-776] 가입 방향(개방 유지 vs 초대 게이트 — 랜딩 「가입 없음」 카피 모순 해소의 선행 결정. 실측: `auth.ts` 가입 훅은 geo-block 뿐 = 지금은 개방) ⑵ 마케팅 3종(랜딩·프라이싱·웨이트리스트)의 번호 아이브로우·STEP 라벨·filled meter — 프로토타입 캐논 vs design-taste-frontend §9.F 충돌(추천 = 마케팅 면 한정 제거). 결정이 서면 그다음 회차 = 전략 에디터 split view(시장 리서치 수렴 2위, TV 2025 검증 패턴)가 증거 게이트 위에서 돈다.
