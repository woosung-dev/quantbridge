...
**다음 행동 = 통합 PR #820(`stage/n11` → `main`) 판단** — CI green 을 확인하고 머지한다. 그 뒤 개발 항목은 ⓪ 표에서 사용자가 고른다.
