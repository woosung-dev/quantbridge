...
> **업데이트:** 2026-08-04 (3차 — **표적 채널이 한 겹 더 갈렸다**)
...
## 다음 스프린트 — **[BL-591] 슬라이스 A: `direction` 라벨 분해** (관측 전용 · 킬 정책 불변)
...
> 커밋을 얹거나** 새 브랜치를 만들어 **이 브랜치로 PR** 을 올려라. **`main` 머지는 검증이 다 끝난
> 뒤 한 번에** 한다. PR #539 는 **열어 둔다.**

### 무조건 하는 첫 step — ★사전등록 전향 예측 판정

```bash
QB=/Users/woosung/project/agy-project/quant-bridge
$QB/scripts/soak-observe.sh          # 인자 없이 1회 · exit 0 (★파이프에 넣지 마라)
docker logs quantbridge-worker > /tmp/wl.txt 2>&1     # ★`2>&1` 은 리다이렉트 뒤에
set -a; . $QB/backend/.env.local; set +a; cd $QB/backend
uv run python scripts/classify_direction_divergence.py --log /tmp/wl.txt
```

**전향 예측 — ★창을 함께 적는다.** 노출 시간 기준이고, 기준선은 이번 회차 관측
(`phantom` 0건/6.12h · `replay_lag` 7건/8.97h)이다.

| 예측                   | 문턱           | 근거                                                             |
| ---------------------- | -------------- | ---------------------------------------------------------------- |
| `phantom` = **0**      | **라이브 ≥6h** | 95% 상계 0.49/h ⇒ 6.1h 더 무사고면 상계가 **0.25/h** 로 내려간다 |
| `replay_lag` ≥ **4건** | **라이브 ≥6h** | 노출 기준 1.27/h ⇒ 기대 7.6건. **0건이면 내 모형이 틀린 것이다** |

★**문턱 미달이면 판정하지 말고 「미판정」이라고 적어라.** 이번 회차가 그렇게 했다(노출 1.40h).
★**`phantom` 이 1건이라도 나오면 채널은 안 닫혔다** — [ADR-023] 긴급도를 되올리고 그 건을 부검해라.
...
요지만: **새 조회가 0이다** — `order_repo` 는 이미 넘어와 있고(`:2726-2731`) `last_bar_time` 은
같은 함수 스코프 지역변수(`:2299`)다. `qb_live_position_divergence_total` 에 3라벨 추가.
★**킬 정책은 이 슬라이스에서 불변.** 인과 판정 교체(슬라이스 B)는 분해 데이터를 본 뒤다.

### 착수 전 반드시 읽을 것

- ★**[BL-591] §후속 실측(direction-channel-decomposition)** 과 [ADR-023] §재분해 — 두 갈래의
  정의·판별식·「한쪽으로만 틀린다」 근거가 거기 있다. **다시 재지 마라.**
- ★**[BL-591] D1/D2** — 현행 가드의 잠재 결함 2건. 슬라이스 B 가 둘을 동시에 없앤다.
- ★**`engine_only` 를 겨냥하지 마라 — 닫힌 채널이다**(18.8/h → 0.12/h). #515/#516 귀속도
  **다시 캐지 마라**(두 커밋 사이 라이브 세션 0건 = 데이터 부재).
- ★**[BL-592]** — `exchange_accounts` 2행 중복. 계정으로 귀속하면 **3.7배** 부풀려진다.
  이번 판별식은 `idempotency_key` 로 귀속해 **구조적으로 면역**이다.
- ★**슬라이스 1 원장 shadow 는 지금 어둡다**(`duplicate_open` 155 · `open` 44 고정) ⇒ 판별기로
  쓸 수 없다. **계측을 새로 짜기 전에 있는 계측이 살아 있는지부터 봐라.**

### 비목표

[ADR-023] **구현**(Proposed — 사용자가 보류를 택했다) · 슬라이스 B(분해 데이터 전) ·
슬라이스 2(주입, [ADR-022] 재개 조건 3개 미충족) · [BL-580] 잔여 96곳 · [BL-592] 수리 ·
[BL-578] · [BL-389]/[BL-466] · [BL-462] · mainnet.
