...
| **AG** | [BL-749] 스키마 동등성 — **🔶 2026-08-18 축 3/5 (PR #673)**: 타입+nullable+인덱스/UNIQUE, 실측 drift 전부 0. partial predicate 까지 본다(적대 리뷰 P1). ★남은 `default`·CHECK 축은 [BL-803]                                                                                                                                                                     | P2  | M    |
...
| **BE** | [BL-789] authed e2e — **🟡 2026-08-18 2단계 착지 (PR #671)**: authed 가 **CI 에서 실제로 돌았다**(run `32051650551` · `e2e_authed` 7 passed). ★남은 것은 `ci` 가 **1/20** 이라는 것뿐이고 차단자는 **시더 하나** → 착수는 [BL-802] 로 간다                                                                                                                      | P2  | M    |
...
| **BH** | [BL-802] authed spec **19/20 이 아직 CI 에서 안 돈다** — 차단자는 인프라가 아니라 **데이터 하나**다. 실측(run `32051177105`): `/dashboard`·`/backtests` 통과, `/trading` 은 「등록된 거래소 계정이 없다」. `seed_dogfood.py` 는 celery·CCXT 경유라 재사용 불가 — CI 전용 최소 시더가 본체                                                                       | P2  | M    |
| **BI** | [BL-803] 스키마 동등성의 **`default` 축과 CHECK 제약 축**이 아직 안 켜졌다 — [BL-749] 의 잔여 2/5. `default` 는 표현이 갈려(`nextval(...)`·`now()`·`::jsonb`) 정규화 근거 없이 켜면 상시 red 가 된다. 앞 3축이 모양과 정지 규칙을 이미 정해 뒀다                                                                                                                | P3  | S    |
| **BJ** | [BL-797] 화면 증거 게이트가 **공개 3라우트만** 잰다 — authed 화면을 바꾼 PR 은 여전히 무증거다. ★**2026-08-18 트리거 도래**: 선행 두 항목이 **둘 다 Resolved**(PR #651 `d28bf28f` — 상세는 원장 섹션). ★[BL-802] 와 **같은 차단자(결정적 시드)를 공유**하므로 그 뒤에 붙여라                                                                                    | P2  | M    |
...
## ★2026-08-17 야간 3회차 통합 tombstone — night-3lane · gate-trust · ultracode-2lane

> **강등 tombstone.** 세 회차 서술 34줄 → 이 4줄. 원문 = `git show aadac2bb:docs/status.md`(489~513행) · `git show acdc12c5:docs/status.md`(496~509행). PR #654~#656 · #658~#665.
> [BL-788]·[BL-795] Resolved · [BL-789] PARTIAL(2단계 = CI 인증 secret → **사용자 결정**).
> 살아남는 셋: ★**증거가 없던 이유가 확인 행위였다**(playwright 가 매 실행 `outputDir` 을 지운다 — [LESSON-117]). ★**보고된 preflight 기준선이 조건부였다**(fresh DB 에선 `2 failed` — [BL-788] 이 닫았다). ★★★**「내 주석이 코드보다 앞서 나갔다」가 8건** — 매번 **다음 검증 라운드**가 잡았고 **범위를 좁히는 것이 산출**이었다.

## ★2026-08-17 night3 — 화면 증거 팩 3레인 ([BL-797] · [BL-429] · [BL-414])

> **강등 tombstone (2026-08-18 backlog-triage · 700줄 상한).** 회차 서술 16줄 → 이 4줄.
> 원문 = `git show 55bd99f3:docs/status.md`(493~508행). [BL-429]·[BL-414] Resolved · [BL-797]~[BL-800] 신설.
> ★★★**게이트를 만든 회차가 그 게이트의 사각을 스스로 증명했다** — 같은 밤 화면을 바꾼 두 PR 이 **둘 다 authed** 라 공개 3라우트만 재는 표에 안 나왔다(3브랜치 합쳐 Δ=0 확인 · 고장 아님은 **양성 대조 2픽셀 red** 로 갈랐다) → [BL-797].
> ★★**원장이 머지된 작업을 Open 이라 적은 네 번째**([BL-430]·[BL-427] 이미 구현) — 발견 경로는 문서 대조가 아니라 [BL-710] 의 **원장 내부 자기모순**이었다. ★codex 9건 전건 참 · phantom 0(`isinstance(True,int)` 가 **다른 조합의 수익률을 best 로** 표시).

## ★2026-08-18 backlog-triage — 원장 3분할 종결([BL-779]) + P1 오분류 정정

**무엇을 했나** — 열린 이슈 214건 전량 재분류. ⑴ 발화 불가 트리거 3건 수리 ⑵ P1 오분류 3건 강등 ⑶ 「멀티 거래소 확장」 4건 묶음 보류 ⑷ **원장을 판정어로 3분할** + 그 배치의 집행처 신설.

⑴ ★★★**[BL-779] 종결 — `backlog.md` 문자 594,309 → 294,462 (50.5% 감축 · 목표 40% 초과).**
DEFERRED 180건을 `backlog-deferred.md` 로 내렸다. **판정 4종 전건 불변**(ACTIVE 9 / DEFERRED 180 / PARTIAL 25 / RESOLVED 133 · 전체 347) — 이동이지 삭제가 아니다.
⑵ ★★**1차 분할이 이미 반쯤 풀려 있었다** — 2026-08-16 이후 닫힌 RESOLVED **13건이 전부 `backlog.md` 에 다시 쌓여** 있었고 아무 게이트도 안 잡았다. 규칙이 **산문**이었기 때문이다 ⇒ `bl-audit` **「파일 배치」 축**(판정어 ↔ 사는 파일) 신설. 축 무력화 변이 M2 에서 신규 2케이스만 red · 하네스 22→**26** · `bl-trigger-sweep --selftest` 17→**18**.
⑶ ★★★**내가 세운 기각 목록 6건 중 4건이 틀렸다.** [ADR-028] 의 DEFERRED 정의(외생 조건 ∪ 동승 조건)가 정확히 그 넷을 덮는다 — 기각했으면 [BL-559] 의 「전제는 맞고 **처방**이 틀렸다」를 그대로 재현했을 것이다. 대신 진짜 결함이 드러났다: **발화할 수 없는 스프린트 좌표 3건**([BL-235] 57+ · [BL-236] 56+ · [BL-186] 38+ — 최근 60커밋과 `status.md` 에 `Sprint NN` **0건**). 2건인 줄 알았는데 셋이었다.
⑷ ★**앵커 접두사는 붙였다가 되돌렸다** — 156행에 `backlog-deferred.md#` 를 붙이자 행마다 +18자가 P2 표 패딩을 줄 길이 상한 위로 밀었다(**985 → 1,012자** 실측 · 위반 11행). 되돌리고 [BL-801] 등재. ⇒ **위치의 정본은 앵커가 아니라 `bl-audit.sh --list <판정어>` 의 4번째 칸**이다.
⑸ ★**이 파일이 경고해 둔 `bl-audit:174` 함정을 그대로 밟았다** — 새 상태줄 끝에 `~~취소선~~` 을 달아 그 줄이 **통째로 무시**되고 BL-779 가 UNKNOWN 으로 떨어졌다. 경고는 296행에 있었다.

~~**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-790]** — 동승 후보 = [BL-787]+[BL-718]~~ → ★**2026-08-18 night4-ci-truth 에서 셋 다 종결.** ★**「소크 재-pin」 경고는 옳았고, 판독이 그것을 무해로 만들었다** — 창이 이미 연속 38.40h ≥ 24h 로 자격을 얻어 「지금 눌러도 손실 0」이다(2026-08-18 02:00 `soak-gate.sh` · C1 은 2/3).

## ★2026-08-18 night4-ci-truth — CI 거짓 초록 제거 + 재현성·스키마 방어면 ([BL-789] 2단계 · [BL-787]/[BL-790] · [BL-749] · [BL-718])

**무엇을 했나** — 워크트리 슬롯 3·4·5 에서 3레인 병렬 + CONTROL(슬롯 6). PR **α** #671 · **β** #672 · **γ** #673 → A(`stage/night4-ci-truth`) → main.

⑴ ★★★**authed e2e 가 GitHub CI 에서 처음으로 실제로 돌았다** ([BL-789] 2단계). run `32051650551` 의
`e2e_authed` 잡 **7 passed (54.3s)** — `authenticate`(22.0s) · authed 백엔드 도달성 · `chromium-authed` 4건.
잡을 `ci` 요약의 `needs`·`check` 에 넣었고, **1라운드에서 `ci=failure` 로 그 배선이 실증**됐다.
⑵ ★★**감사가 project 축 → spec 축으로 내려왔다.** `--project=chromium-authed` 만 배선하면 종전 감사는 초록인데
실제로는 일부 spec 만 돈다. 이제 authed spec **20건 전량**이 `ci ∪ localOnly` 에 정확히 한 번 있어야 하고,
SSOT 는 `e2e/ci-authed-manifest.json` 하나이며 **CI 명령이 그 파일에서 인자를 뽑는다**.
⑶ ★★**게이트를 세운 회차가 그 게이트의 fail-open 을 스스로 4건 만들었다** — codex 적대 리뷰가 전건 잡았고
**넷 다 이 회차 자신의 신규 코드**였다(레포 통계 그대로). 실증 P1 둘: **인덱스 서명이 partial predicate 를 안 봐**
`uq_orders_idempotency_key`(주문 멱등성 계약 자체)의 `WHERE` 를 지워도 초록 — **음성 대조로 확정**했다(구 서명을
되살리면 같은 변이가 **1 passed**). 그리고 **authed 잡이 `frontend` 에만 발화**해 BE 인증/CORS 를 깨도 skip→초록.
⑷ [BL-787] 은 원장이 「먼저 세라」고 한 것을 셌더니 **예방**이었다(backtests **7행 전부 NULL** · stress_tests **0행**).
[BL-790] 은 애너테이션이 celery args 에 안 닿아 **대입형 런타임 정규화**를 함께 뒀다. [BL-718] CODEOWNERS 는
**강제력 0**(브랜치 보호·ruleset 부재)을 파일 머리에 못박았고 「0건」은 **양성 대조**로 갈랐다.
⑸ ★**신설** — [BL-802] = CI 결정론 시더 · [BL-803] = [BL-749] 의 잔여 2축.

**다음 행동 = [BL-802] 로 시작하는 「CI 진실 완주」 3레인** — α = [BL-802] CI 결정론 시더 · β = [BL-777] BE 배포/롤백 런북 · γ = [BL-803] 스키마 `default`·CHECK 축. α 착지 뒤 CONTROL 이 [BL-797] authed 화면 증거를 잇는다. ★[BL-802] 와 [BL-797] 은 **같은 차단자(결정적 시드)를 공유**하므로 병렬로 두지 마라. ★차단자는 인프라가 아니라 데이터 하나이고 실측이 있다(run `32051177105`: `/dashboard`·`/backtests` 통과, `/trading` 은 「등록된 거래소 계정이 없다」). `seed_dogfood.py` 는 celery·CCXT 경유라 재사용 불가.

### 반증 카드 — 이 회차에 틀린 것으로 드러난 전제

★**⑴ [BL-789] 의 「CI 인증 secret 투입 = [ADR-034] 반전 = 사용자 결정」이 거짓 전제였다.** 리터럴 secret +
일회용 CI Postgres + setup 의 sign-up 부트스트랩이면 성립하고, **기존 `e2e` 잡이 이미 그 방식을 쓰고 있었다**
(`BETTER_AUTH_SECRET: ci-e2e-only-placeholder-…`). 원장이 사용자 결정으로 올려 둔 갈래가 **원래 결정거리가 아니었다.**
★★**⑵ 「CI 는 서버가 안 뜬 것」이 두 번 다 오독이었다.** 1라운드 증상은 `waitForURL` 60초 타임아웃이었고 원인은
세 줄 위의 `Invalid origin: http://localhost:3100`(포트 3000 가정). 2라운드 증상은 「백엔드 도달 불가·전송 실패 8건」이었고
원인은 **CORS 거부**(`net::ERR_FAILED`)였다 — API 는 두 번 다 멀쩡했다. **[BL-781] 이 격리 슬롯에서 기록해 둔 같은 기전**이
CI 에서 재현된 것이고, 그래서 포트를 YAML 에 박지 않고 `_base-url.ts` 에서 **파생**시켰다(못 읽으면 죽는다).
★**⑶ [BL-787] 의 위험 가정이 반증됐다.** 원장은 「0 이 아니면 지금 잘못 돌고 있는 것」을 열어 뒀는데 실측은 0이었다 —
교정이 아니라 예방이다. **세지 않았으면 없는 사고를 고쳤다고 적을 뻔했다.**
★**⑷ 내 계수기가 주석을 셌다.** 변이 복원 확인에서 `grep -c 'ExchangeName(exchange_name)'` 이 **내가 방금 쓴 설명 주석**을
2건으로 세어 「가드가 남아 있다」는 거짓 신호를 냈다(테스트는 정직하게 red 였다). 잔존 확인은 **구문 형태**로 재야 한다.
★**⑸ 내가 status.md 의 ⓪ 표를 통째로 지웠다.** 강등할 서사 블록 안에 다음 회차의 **진입점 표**가 들어 있었다 —
`docs-audit` 이 「행 0개」로 즉시 잡았다. 게이트가 없었으면 다음 세션이 고를 것을 잃은 채 시작했다.
