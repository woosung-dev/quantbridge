...
### ★2026-08-10 종결 2건 — 전문은 dev-log (2026-08-11 gate-freshness 가 700줄 상한에서 압축)

- **bl-trigger-triage + precommit-scope** ([BL-687]·[BL-695] · [`dev-log`](dev-log/2026-08-10-bl-trigger-triage.md)) —
  판정기 초판의 근거 없는 「도래」 5건을 잡은 것은 전량 스윕이 아니라 그 앞의 **음성 대조**였다.
- **migration-guard** ([BL-451] · [`dev-log`](dev-log/2026-08-10-migration-guard.md) · 원문 `47748e77:docs/status.md`) —
  「가드가 **있다**」와 「그 경로가 그 가드를 **지나는가**」는 다른 질문이다.
...
| #   | 단위                                                 | 담당             | 목표 조건                                                               | 상태                  |
| --- | ---------------------------------------------------- | ---------------- | ----------------------------------------------------------------------- | --------------------- |
| 1   | [BL-462] Sharpe 정렬 3결함                           | 워커 a (슬롯 9)  | 파산 계좌(값 0·`unavailable*`)가 −0.3 위에 오지 않는다. 변이 5/5        | 완료 (08-11 종결)     |
...
~~**다음 행동 = ⑴ 첫 30분 `backend/scripts/mtbf_stratified.py` 로 O([BL-641]) 재측정 ⑵ 남는
시간으로 T([BL-706]) 본체를 닫는다.**~~ → **2026-08-11 사용자 지시로 순서가 뒤집혀
gate-freshness 가 T([BL-706])와 E([BL-462])를 닫았다** — 신호 신선도 게이트(commit sha 대조 +
eod 거부 + 하네스 25케이스/변이 13종) + BL-462 원장·FE 고지 정정(playwright 화면 검증 포함).
**O 재측정은 이월됐다.** 전문 = [`dev-log/2026-08-11-gate-freshness.md`](dev-log/2026-08-11-gate-freshness.md).
★남기는 둘 = ⑴ **검사기의 미끼를 조심해라** — bash 3.2 가 `$( )` 안 quoted heredoc 의
달러+따옴표 인접을 삼켜 앵커 검사가 x0 이 되자, 생성자가 근본 원인 대신 **훼손 앵커와 일치하는
미끼 주석**을 심어 통과시켰다. 잡은 것은 평가자의 무해해 보이는 주석 제거였다.
⑵ **동결 문서도 자기모순을 품는다** — 골격(A14 가 abort 호출)과 변이 기대(M9=⑨ 단독)가
양립 불가였고, 생성자는 회피했고 평가자가 기대를 ⑨㉕ 로 개정했다. 동결은 신뢰가 아니라 대조 대상이다.

**다음 행동 = 첫 30분 `backend/scripts/mtbf_stratified.py` 로 O([BL-641]) 재측정 — 그 항목
자신의 Trigger 뒷절(「소크 재기동 회차마다 재측정」)을 집행하고, ⓻ 상단의 반증을 원장
표·상태줄에 박는다(사인 분류에서 `user_stopped` 를 **분리**해라). 남는 시간은 ⓪ 표에서
사용자와 고른다.**
★**O 를 통짜로 잡지 마라** — 닫는 조건이 「사망률이 **실제로** 내려간다」라 며칠 단위 관측이다.
재측정은 값을 내지만 종결은 이 회차 밖이다. ★P([BL-438])는 **귀속 설계**에 사용자 결정이
붙을 수 있고 프로덕션 관측 0(잠복)이라 급하지 않다. ★R([BL-639])은 **결정이 선행**이라
...
### ★bl-307-header-lint (2026-08-10) — [BL-307] 종결 · [`dev-log`](dev-log/2026-08-10-bl307-header-lint.md)

★남기는 한 줄 = 원장이 「근거 소멸」이라 적은 규칙을 코드 90.6%가 이미 지키고 있었다 ⇒
문서가 「없어졌다」고 말할 때 **코드에게 되물어라** (원문 `0df5a60e:docs/status.md`).
