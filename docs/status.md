...
**현행 창: 세션 `98ff6ecc` · 고정 `22e01727`(브랜치 `stage/gap-resync-autopsy`) ·
T0 `2026-08-07T07:49:17Z` · celery ready `07:48:48Z`.** 판정 **`UNKNOWN 진행중`** ·
C3 실격 **0건** · **C5 전건 ✓**. 재기동 전 `FLAT=YES` 확인했고, 사망이 남긴 고아 포지션
(short 0.029, **14시간 방치**)은 서비스 계층으로 청산해 원장에 남겼다(`a7729ddd`).
...
C5 `darkness_computed=✗`(나머지 5개 서브조건은 ✓ — **[BL-620]**, **같은 회차에 Resolved**).
...
`측정불가` 유지). `QB_METRICS_URL` 을 명시하면 종전 HTTP 를 쓴다(원격 운영안 보존).
