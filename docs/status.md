...
> **PR #630 머지(`4d0b1a3d`)** · 한 줄 = **「미귀속 71.5% 는 결함이 아니라 잘못된 DB 를 봤다는 뜻이었다」**. 진짜 구멍은 옆(`reduce_only=true` 필터가 백필을 막아 kill-switch 가 6.9% 만 봤다) · 헤지 모드 기각([ADR-032]).
> tombstone: 원문 = `git show 4d0b1a3d:docs/status.md` (307~347행).

## ★2026-08-14 money-path-close — [BL-438] 종결 + [BL-724] 경제성 판정 (강등 — 본문 = git)

> **PR #632·#633 머지(`bbeda92f`·`fb7bb772`)** · 한 줄 = **「소크가 돌리던 전략은 백테스트에서도 지고 있었다」**(라이브 PF 0.223 / 백테스트 PF 0.607). 손익분기 요율이 실효율의 **1/14.6** 이라 비용 축으로 구제되지 않는다 — **레버는 요율이 아니라 빈도**. **판정 = 실자금 불가 · 데모 유지**(사용자 결정). 파생 = [BL-729][BL-730][BL-731][BL-732][BL-733].
> tombstone: 원문 = `git show fb7bb772:docs/status.md` (311~360행).

## ★2026-08-15 soak-survival — 소크 뿌리 확정 + 이월 4항목 종결 (강등 — 본문 = git)

> **PR #634·#635 머지(`375a405b`·`d9fb9f58`)** · 한 줄 = **「소크를 죽인 것은 `live_signal.py` 가
> 아니라 우리 자신의 테스트 하네스였다」** — `close_position` 이 계정 포지션을 **소유권을 보지 않고**
> 닫았다([BL-734], [BL-633] 재발). 증인은 거래소 원장(`04:49:56 Buy 0.029 CreateByUser link=(empty)`
> → `exchange_position=+0.001` 이 관측치와 정확히 일치)과 **커밋 타임스탬프 29초**였다.
> ★**착수 전제 4건 반증** — 로컬 6h33m 사망은 **맥 sleep**(beat 가 168회 중 **15회**만 tick,
> 그 15회가 DarkWake 횟수 → [BL-735]) · C1 을 끊은 건 서버 세션의 다른 사인 · 「마지막 체결」은
> `created_at` 이었다 · status.md 자신이 이미 「게이트를 로컬에서 돌리지 마라」를 적고 있었다.
> ★**내 추론도 2번 자기반증** — 「서버는 125h 살았다」→ 게이트 `누적 0.0000h`(**세션 생존 ≠ 귀속된
> 시간**, 어둠 98%) · 「`prune -f` 로 7.4GB」→ 실행하니 **0B**(회수분이 전부 남의 프로젝트 것).
> ★**codex 2라운드가 모두 내 검증 공백을 잡았다** — ⑴ 새 테스트만 보고 기존 [BL-651] 회귀 2건 누락
> ⑵ 재사용한 판정기의 **문서화된 위양성**을 게이트 승격 맥락에서 안 봤다(그대로 갔으면 정상 entry 가
> 운영자를 페이징). ★변이가 「지워도 31 passed」인 **무테스트 게이트**를 찾아냈다.
> 실격 원장 **미판정 4→0** · BL 10건(734~743) · BE 1789 · FE 1370 · e2e 133 · 게이트 2회 전건 통과.
> tombstone: 원문 = `git show d9fb9f58:docs/status.md` (331~405행).

## ★다음 세션 — 관측이 살아났다, 이제 시계만 채우면 된다 (2026-08-15 갱신)

**지금 상태:** 서버 소크 `115b33ff` 가 **9.50h 연속 · C3 실격 0건 · C5 6/6**이다(02:05Z 실측 —
8.70 → 9.22 → 9.50 으로 회차 내내 증가했다). **C1 첫 24h 창까지 약 14.5시간.**

★★★**이것이 항목 선택을 지배한다** — 실격 1건이면 T0 리셋이고, `apps/api/src` 수리는 재-pin 을
부른다. C1 은 pin 으로 안 죽고 C2 도 `max` 라 **벌어 둔 최장 창은 살아남는다**([ADR-024] §255 —
「끊긴다」는 **진행 중 창의 종료**를 뜻하지 「max 소멸」이 아니다). 그래도 진행 중 창 9.50h 를
버리는 것이므로 **소크 코드를 안 건드리는 항목부터 고른다.** [BL-547]·[BL-591]·[BL-742] 는 지금이 아니다.

~~**다음 행동 = [BL-737] 서버 `soak-watch.service` 를 되살린다**~~
→ **2026-08-15 종결.** 사인은 `rc=127`(유닛이 재배치 전 `scripts/` 경로를 물고 있었다)이고
사망은 08-14 가 아니라 **08-13 13:52Z 부터 41시간**이었다. 같은 뿌리에서 **[BL-744] API 좀비**를
함께 찾아 닫았다. 이제 감시자가 죽으면 `OnFailure` 알람 유닛이 텔레그램을 쏜다.

~~**다음 행동 = [BL-641] 어둠 98.2% 의 처방을 세운다 — 소크가 도는 지금이 유일한 적기다.**~~
~~C1 24h 창을 3회 채우려면 관측 밀도가 함께 올라야 하는데 그 처방이 아직 없다.~~
→ **2026-08-15 clock-fill-sweep — 그 전제가 코드로 반증됐다. 어둠은 C1/C2 를 한 시간도 깎지 않는다.**
`darkness` 가 `evaluate()` 안에서 읽히는 곳은 **정확히 2곳**이고 둘 다 시간 계산과 무관하다
(`soak_gate_predicate.py:752` 존재 여부만 C5 · `:797` 출력 전용). **`ratio` 를 비교하는 부등식은
레포 전체에 없다.** 관측도 같은 말을 했다 — 02:05Z→04:51Z 사이 C2 는 **+2.86h(경과분 100% 귀속)**
인데 어둠 분자는 **+202(경과분 100% 어둠)** 로 두 셈이 **동시에** 성립한다.
⇒ **타이머 주기 단축안은 철회**한다(30분은 C4 한계 60분에 대한 안전 여유 1회분이다).
**진짜 축** = C1/C2 크레딧은 `세션 lifetime ∩ 귀속 구간 ∩ [창시작, now] ∩ phantom 커버리지` 이고,
귀속 구간을 **여는 것은 `up`** 이다(`attribution_intervals():210`). de3db35a 의 「125.6h 살았는데
0.0000h」는 어둠이 아니라 **귀속 구간 밖**이었다 — 실측으로 그 구간이 08-07 09:33 에 닫히고
08-14 05:53 에야 열렸다. 처방은 이미 들어가 있다(BL-737/744/745 감시자 부활 + `OnFailure` 알람).

**다음 행동 = 시계를 그냥 채운다 — C1 첫 창까지 남은 것은 코드가 아니라 시간이다.**
2026-08-15 05:00Z 기준 C2 **12.3643h** · 실격 0 · C5 6/6 이고 **지금 세션은 100% 귀속**된다
(수명 = 귀속). 첫 24h 창까지 약 12시간, 3회까지 약 60시간. **서버를 건드리지 마라** —
`soak-stack.sh pin/up/down`·`migrate --confirm`·`git pull`·유닛 조작이 전부 창을 끊는다.
개발·테스트·커밋·머지는 **창과 무관하다**(§400-412 가 코드 3곳으로 확정). 다음 회차는
⓪ 표에서 고르되 **`--list ACTIVE` 가 이제 2건뿐**이다([BL-547] M · [BL-591] L+사용자 설계 선행) —
둘 다 소크 코드를 건드리므로, 창이 도는 동안은 **PARTIAL ∧ 도래** 쪽에서 골라라.

★**그 회차를 무인으로 돌릴 수 있다** — 순차 step 러너가 2026-08-15 에 돌아왔다([ADR-033]).
진입은 **`/harness`** 슬래시 커맨드이고 파이프라인상 자리는 **§4 G2-b** 다(step 3개 이상 · 순서
의존 · 사람이 못 붙는 회차에만). 착수 전 `.harness/README.md` **§2·§4 필독** — as-is 위험이
살아 있고([BL-746]) 막는 것은 코드가 아니라 규약 2개(**워크트리 전용 · `--push` 금지**)다.
...
2. **[BL-026] ✅** — **코드 0줄.** 지목된 데코레이터는 08-11 에 이미 삭제됐고 `:418` 은 묘비명 주석이다.
   `--run-mutations` 실행 = 7 passed + 1 xpassed(skip 0).
3. **[BL-735] ✅** — `_up()` **첫 줄** Darwin 거부 + 신규 하네스(10종→**11종**).
4. **[BL-725] ⏳** — 상태줄 리드인만 `⬜ Open` 이라 ACTIVE 로 세지던 것. **185건 전수 중 1건뿐**이라
   패턴이 아니었다. `bl-audit.sh` 에 **트리거 정합 축**(5면째)을 넣어 재발을 막았다.
...
| 축        | 상태 (2026-08-15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| main      | `9605ce79` (#636) · 트리 clean. **서버 체크아웃도 같다**(2026-08-15 `git pull` 완료). ★로컬 `main` 은 push 안 된 `0f4577c4` 위에 있어 갈라져 있을 수 있다 — 내용은 #636 squash 에 100% 들어갔다(트리 diff 0 확인). `git reset --hard origin/main` 으로 맞춘다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 소크      | ★★**서버(`truewords-oracle`)가 정본이다 — 로컬 맥에서 돌리지 마라**([BL-735]: beat 가 168회 중 15회만 tick 했고 그 15회가 DarkWake 횟수였다). 세션 `115b33ff` · pin `fb7bb772` · 2026-08-14T16:29Z 기동 · **C2 12.3643h · C3 실격 0 · C4 0건 · C5 6/6**(2026-08-15 **04:51Z**). ★**지금 세션은 100% 귀속**이다(수명 = 귀속) — 어둠 98.2% 는 **전 이력 누적**이고 판정에 안 들어간다([BL-641] 축 정정). DB 는 **head `20260815_0001`** 이고 인덱스 집합이 로컬과 diff 0 이다([BL-743]). 게이트는 **`bash -lc` 로 서버에서만** (비로그인 셸엔 uv PATH 가 없어 rc=127)                                                                                                                                                                                                                                             |
...
| 워크트리  | **4벌** (2026-08-15) — `feat+harnes01`(PR #637, **범위 밖 · `:3103`/`:8103` 점유 중**) + 이번 회차 슬롯 4·5·6(`bl741-alembic`·`bl735-darwin`·`bl413-drawer`). ★**슬롯 3 을 쓰지 마라** — 하네스 워크트리가 이미 잡고 있다(실측). 정리는 `git worktree remove`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --- | ---- | ------ | ---------- | -------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **B**  | ~~[BL-026] Mutation 「12 skip 일괄 활성화」 — ★**범위 재정의 선행**~~ → **✅ 2026-08-15 종결 — 코드 0줄(대상이 이미 없었다)**                                                                                                                                                                                                  | P1  | ★★   | ?      | **재산정** | 0줄            | 도래는 맞다(트리거 줄 자신이 적었다). ★★**2026-08-11 실측이 제목·Est·권장접근을 셋 다 반증** — 「12 skip」의 출처 `docs/TODO.md` 는 `fcc36bf7` 에서 **삭제**돼 대상 집합이 없고, 무조건 skip 실측은 **6건**(mutation 관련 **1건**)이며 진짜 게이트는 `conftest.py:138-148` 마커다. 「하/S(1-2h)」는 근거 소멸                                                  |
...
| **V**  | ~~[BL-729] 전략 채택 근거 백테스트 4벌이 **낡은 비용 가정**(0.001/0.0005)으로 돌았다 — 왕복 0.30% vs 실측 0.138% ~~ → **2026-08-15 종결** — 실측점 격자로 재판독: `−7.74%` vs `−22.59%` ⇒ [BL-724] 유지(부호 불변). 비용↔손실은 **비선형**(2.17배 → 2.92배)                                                                    | P2  | ★★   | 하     | S (1h)     | 0줄            | 즉시 — 소크 전략을 다시 고를 때의 입력. `config_mapper` 가 저장값을 복원해 **재실행으로 안 고쳐진다**. Cost-Assumption 축이 이 일을 한다 (소크 정지 창에서)                                                                                                                                                                                                    |
| **W**  | ~~[BL-730] FE 비용 기본값 **drift 2곳** — 온보딩이 왕복 0.30% 를 하드코딩 submit ~~ → **2026-08-15 종결** — FE 리터럴 5벌 → `cost-defaults.ts` 하나. 온보딩 payload 테스트 + 프리셋 격자 재정렬                                                                                                                                | P2  | ★★   | 하     | XS (15분)  | 0줄            | ★이미 발화 — 신규 사용자의 첫 백테스트가 2.2배 비관으로 돈다. [BL-603] 이 「미러 4곳」이라 적었지만 실제 5곳이었다                                                                                                                                                                                                                                             |
...
| **Y**  | ~~[BL-731] `list_synced_with_exchange_exit` 의 **LIMIT 500** — 재검증 모집단 73 → 563 ~~ → **2026-08-15 종결** — `IS DISTINCT FROM` 을 SQL 술어로. 모집단이 0 으로 수렴한다                                                                                                                                                    | P2  | ★★   | 하     | S (1h)     | 0줄            | 동기화 축은 **단조 증가**라 안 풀린다. 처방 = 상한 확대 말고 「원장 합계 ≠ 저장값」으로 모집단을 좁혀라                                                                                                                                                                                                                                                        |
...
| **H**  | [BL-750] `final-gates-test.sh` 케이스 ⑩ 이 **CI 에서 틀린 이유로 통과**한다                                                                                                                                                                                                                                                    | P2  | ★★   | 하     | S (1-2h)   | **0줄**        | ★**이미 발화했다** — 같은 트리에서 **로컬 9/10 · CI 10/10** 을 실측했다. CI `documentation` 잡이 `checkout@v4` 기본(depth=1)이라 `origin/main` 이 없어 `merge-base` 실패 → `has_fe=0` → 「필수 아님」. **로컬 red 가 정답**이고 `final-gates.sh:299` 가 이미 아는 실패 모드다. 게이트 하네스층의 거짓 초록이라 [BL-705] 와 같은 축                             |
...
- ~~★**`apps/web/` 안 json/yml 을 스테이징하지 마라**([BL-602])~~ → **2026-08-15 [BL-602] 종결.**
  회피 3줄도 `.prettierignore` 에서 제거했다 — 이제 그 셋도 prettier 가 정상 포맷한다.
...
(**지금 막힌다**)~~ → **2026-08-15 종결** (뿌리는 2026-08-09 `71f7101e` 로 이미 사라져 있었다).
