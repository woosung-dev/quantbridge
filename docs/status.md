...
★**브리핑 축의 잔여 2건은 원장 밖이다**(이 회차가 표면화했고 닫지 않았다) —
⑴ Optimizer 폼의 `var_name` 자유 타이핑(`features/optimizer/form-schemas.ts:44`, brief 가 이제 데이터를 갖는다)
⑵ `convert` 의 `mode="sliced"`(토큰 77~97% 절감이 FE `"full"` 하드코딩으로 도달 불가) + 정상 흐름 진입점 0개.
