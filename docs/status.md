...
## ★2026-08-14 money-path-close — [BL-438] 종결 + [BL-724] 경제성 판정 (강등 — 본문 = git)

> **PR #632·#633 머지(`bbeda92f`·`fb7bb772`)** · 한 줄 = **「소크가 돌리던 전략은 백테스트에서도
> 지고 있었다」** — 라이브 net **−1,121.57**(PF 0.223) · 백테스트 `a22faccb` net **−218,560**(PF 0.607).
> 수수료 실효율 **0.05500%** = 공시 taker 와 소수점까지 일치(maker 0건)이고 손익분기 요율은 그
> **1/14.6** ⇒ 비용 축으로 구제되지 않는다. **레버는 요율이 아니라 빈도**(회전 = 계좌의 11.6배).
> **판정 = 실자금 불가 · 데모 유지**(사용자 결정). ★착수 전제 반증 3건 + 자기 리드 1건
> (`ledger_net` 수량축)이 최대 산출이었다. 귀속률 6.9%→98.2%.
> 파생 = [BL-729][BL-730][BL-731][BL-732][BL-733].
> tombstone: 원문 = `git show fb7bb772:docs/status.md` (311~360행).

## ★2026-08-15 soak-survival — 소크 사망의 뿌리 확정 + 하네스 배타성 가드

★★★**한 줄 = 「소크를 죽인 것은 `live_signal.py` 가 아니라 우리 자신의 테스트 하네스였다」.**

**착수 전제 4건이 반증됐다 — 그것이 이 회차의 최대 산출이다.**

| 프롬프트가 전제한 것                          | 실측                                                                           |
| --------------------------------------------- | ------------------------------------------------------------------------------ |
| 뿌리는 gap-resync 로직(H1 시간/H2 창/H3 이월) | 그 표본(로컬 `e9c504f1` 6h33m)은 **맥 sleep** 이 만든 인프라 사고다            |
| 6h33m 사망이 부검 대상                        | C1 을 실제로 끊은 것은 **서버 `de3db35a`** 의 `position_divergence`(다른 사인) |
| 마지막 체결 = `buy 0.06` 09:37:54             | 그건 `created_at` 이고 **체결은 12:26:23** — 사망 14초 전                      |
| 소크는 로컬에서 돌린다                        | `status.md` 자신이 **「게이트를 로컬에서 돌리지 마라」**를 이미 적고 있었다    |

★**로컬 사망 = 맥 sleep.** `pmset -g log` 와 컨테이너 로그가 초 단위로 겹친다 — 09:11:59
Clamshell Sleep ↔ `last_evaluated_bar_time=09:11:00`, 09:28:18 Wake ↔ 09:28:46 `gap_resync_deferred`.
**beat 가 09:38~12:26 에 168회 중 15회만 tick 을 보냈고 그 15회가 DarkWake 횟수다.** DarkWake
직후엔 `socket.gaierror` 가 따라붙는다. 별건으로 06:04:11 **Redis AOF 가 디스크 풀로 쓰기 실패**
→ celery `Unrecoverable error`(로컬 Docker VM **94% / 3.1G**) → [BL-736].

★★**서버 사망 = 계정 배타성 위반([BL-633] 재발, 경로만 다르다).** 거래소 원장 실측:

```
04:44:07  소크 sell 0.058                                    → 서버 숏 −0.029
04:49:56  Buy 0.029  CreateByUser · link=(empty) · inferred  ← real_broker 하네스의 청산
04:50:27  exchange_position=+0.001                           ← 남은 잔량. 관측치와 정확히 일치
04:51:27  같은 값 2연속 → strike kill → position_divergence 사망
```

우리 소크 주문은 예외 없이 `CreateByStopOrder` + `order_link_id=<우리 UUID>` + `conf=exact` 다.
그 외부 주문 5건은 **개발·테스트 DB 어디에도 없다**(pytest `drop_all` 이 자기 기록을 지웠다).
시스템은 이미 신고했었다 — `04:53:32 exchange_exit_link_id_unverified classification=unknown`.
**범인은 `_harness.flatten_one` 의 `close_position`** — 계정 포지션을 **소유권을 보지 않고** 닫고,
verify-flat 은 `positions` 가 비었으니 **성공으로 보고**했다.

★**divergence 4건 중 3건은 별개 현상이고 무해하다** — 23:31·01:02·03:32 은 체결 **1~2초 뒤**이고
`engine`=체결 전 값 / `exchange`=체결 후 값이다. 즉 **거래소가 아니라 엔진이 늦었다.** 고립
발생이라 strike 가 유예했다. E1(엔진 산출)·E3(provider 매핑)는 불성립 — E3 는 `long` 으로 읽은
포지션을 `sell reduce_only` 로 닫아 `legs=0` 이 된 것으로 **거래소가 증인**이다.

★**수리** = `find_foreign_resting` 을 `live_session_admin._cmd_status` 인라인에서 **함수로 추출**
(새 판정기를 만들지 않았다) → `flatten_one` 이 청산 **전에** 호출, fail-closed. 회귀 3건 green ·
변이 2건이 **서로 다른 테스트를** 정확히 red 로 만들었고 도달도 확인했다 → [BL-734].
양성 대조 = 로컬에서 status 를 돌리면 서버 소크의 조건부 주문을 `FOREIGN_RESTING=1 · EXCLUSIVE=NO`
로 잡는다.

★**자기정정** — 중간에 「서버는 125h 살았으니 24h 를 못 채운다는 전제가 거짓」이라 적었는데
게이트 실측이 반증했다: `C1 0/3 · 누적 0.0000h`. **세션 행 생존 ≠ 귀속된 시간**이다
(귀속 불가 **107.02h** · 어둠 **98.6%** → [BL-641] 에 실측 추가).

★**인프라 조치** — 고아 포지션(`long 0.03`, 미실현 +8.68) 승인 청산 → 로컬 소크 down →
서버 `git pull` → pin **`fb7bb772`**(#631 백필 + #633 정정이 함께 실렸다) → up.
새 세션 **`115b33ff`** 가동, `FLAT=YES · EXCLUSIVE=YES · QUIET=YES`.

**다음 행동 = 서버 소크가 6h33m 을 넘겨 사는지 관측한다** — `115b33ff` 는 2026-08-14T16:29Z
기동이고 창은 그 시각부터다. `ssh truewords-oracle 'bash -lc "cd ~/quantbridge &&
tools/scripts/soak-gate.sh --no-collect"'` 로 **C3 실격 0** 유지를 확인해라(★`bash -lc` 필수 —
비로그인 셸엔 uv PATH 가 없어 rc=127 로 죽는다). 접어 둔 것 = A3 반전 의미론 감사 ·
Lane B([BL-730][BL-731][BL-733]) · C1([BL-729]). 그 뒤 ⓪ 표에서 고른다.

### ★환경 상태 (2026-08-15 실측으로 교체 — 다음 세션이 그대로 이어받는다)

| 축        | 상태 (2026-08-15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| main      | `fb7bb772` (#633) · 트리 clean. 서버 체크아웃도 같다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 소크      | ★★**서버(`truewords-oracle`)가 정본이다 — 로컬 맥에서 돌리지 마라**([BL-735]: beat 가 168회 중 15회만 tick 했고 그 15회가 DarkWake 횟수였다). 세션 `115b33ff` · pin `fb7bb772` · 2026-08-14T16:29Z 기동 · `FLAT`·`EXCLUSIVE`·`QUIET` 3종 YES · C5 6/6. 게이트는 **`bash -lc` 로 서버에서만** (비로그인 셸엔 uv PATH 가 없어 rc=127)                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| **AA** | [BL-735] ★**소크를 로컬 맥에서 돌리지 않는다** — AC 전원에서도 `sleep 1` 이고 Clamshell Sleep 은 못 막는다 ⇒ 로컬에서 24h 연속 창은 **구조적으로 불가능**                                                                                                                   | P1  | ★★★  | 낮     | S          | 소량            | 규칙은 문서화됐다. 잔여 = `soak-stack.sh up` 의 Darwin 거부                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **AC** | [BL-737] 서버 `soak-watch.service` **failed** — 표본 타이머는 정상인데 **알림 축만** 죽어 있다                                                                                                                                                                              | P2  | ★★   | 낮     | S (1h)     | 소량            | 2026-08-14 사망을 아무도 몰랐던 이유. 문서와 실태가 갈렸다                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| **AD** | [BL-739] `final-gates:481` 화면 검증 신호가 **FE diff 0 에도 required=1** — `vercel.ok` 와 비대칭                                                                                                                                                                           | P3  | ★    | 낮     | S (30분)   | 소량            | ★단순 `$has_fe` 전환은 **BE 가 화면을 깨는** 경우를 놓친다([BL-707] 계열)                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
