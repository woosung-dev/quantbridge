...
~~9.50h 연속 (02:05Z)~~ → **2026-08-16T13:50Z 실측 21.0063h / 24h · 실격 0 · C5 6/6 · 남은 2.9937h.**
...
## ★2026-08-16 deploy-activation — 절차가 재시작을 빼먹어서, 고쳐 둔 보안이 발효하지 않고 있었다

★★★**착수 전제 4건이 반증됐다 — 그것이 이 회차의 최대 산출이다.**
⑴ **계획된 R0 절차대로 했으면 `/docs` 는 안 닫혔다.** 배포 API 는 소크 스택이 아니라 **호스트
systemd 유닛**(`quantbridge-api.service`)이고 `WorkingDirectory` 로 **작업 트리를 직접 읽는다**.
...
서버는 이미 `DEBUG=false`. #641 이후 `is_production` 이 더 사는 곳은 deprecation 로그 1줄과 `/health` 라벨뿐.
⑶ **[BL-753] 선행 조건이 종류부터 틀렸다** — 「비어 있는 시크릿 1종」이 아니라 `WAITLIST_TOKEN_SECRET`
**부재**다(빈 값은 0건). 그리고 유닛에 구워진 반대 근거(「production 이면 게이트가 401」)는 **낡았다** —
[BL-620] 이후 게이트는 HTTP 가 아니라 `.metrics` **디렉터리 직독**이고 `QB_METRICS_URL` 은 미설정이다.
⑷ [BL-759] 의 `.env.example` 「외 4」는 **과대** — 실질 1건(`HEALTHZ_CELERY_TIMEOUT_S`).

★★**`bl-audit.sh:174` 함정 — 상태줄에 `~~취소선~~` 을 쓰면 그 줄이 통째로 무시되고 섹션이
`:107` 에서 ACTIVE 로 떨어진다.** 이 회차에 [BL-753] 이 DEFERRED → ACTIVE 로 조용히 뒤집혔고
docs-audit 의 ⓪ 표 정체성 축이 그것을 잡았다. 관용구 `~~옛 문장~~ → 새 사실` 은 **그 두 줄에서만 예외**다.

**넣은 것** — R0a 배포 발효(아래) · [BL-762] 시범 수리(라우터의 private 관통 7건 제거 ⇒
**LESSON-019 commit-spy 를 처음으로 붙일 수 있게 됐다**, 변이 2/2 red) · 계층 감사 등재
[BL-762]~[BL-765] · e2e 로케이터 감사 [BL-766] — `tr` 9건 중 헤더를 잡을 수 있는 것 5건 ·
`getByRole('row')` **0건** · 문서 정정 3건.

~~**다음 행동 = C1 첫 창을 채우고, 판독이 `✓ 자격 획득` 을 낼 때 사용자 승인을 받아 2번째 창을 연다.**~~
~~그때까지 **서버 무접촉**(`pin/up/down`·`migrate --confirm`·`git pull`·유닛 조작 전부 창을 끊는다)~~
→ ★★**2026-08-16 정정 — 그 괄호 목록이 이 파일 자신의 `:399-412` 와 모순이었고, 실증으로
`:399-412` 가 맞았다.** `git pull` 은 창을 **끊지 않는다** —
`infra/compose/docker-compose.soak.yml:35,51,68,86` 이 워커 4벌 전부 `./.soak/src:/app/src:ro`
로 **git archive 스냅샷**을 mount 하므로 작업 트리 갱신이 워커에 닿지 않는다.
**2026-08-16 실측이 그것을 증명했다** — 서버에서 `git pull`(e11b09f5→821786ac) +
`migrate --confirm`(3건) + `systemctl --user restart quantbridge-api.service` 를 돌린 전후로
C2 가 **21.0063h 그대로**, 실격 0, C5 6/6 이었다.
⇒ **창을 끊는 것은 `down`/`up`/`pin` 과 DB 실격뿐**이다. `migrate --confirm` 은 창을 끊지는
않지만 소크 DB 에 DDL 이라 **매번 명시 승인** 사안으로 남는다(⓵ 비목표 문구가 정본).

**다음 행동 = C2 가 24h 를 넘으면 판독의 `✓ 자격 획득` 을 사용자에게 보고하고, 승인을 받아
`down → pin → up` 으로 2번째 창을 연다.** ★**게이트가 ✓ 를 내도 자동으로 누르지 마라**
(2026-08-15 사용자 결정 — 보고 후 대기). 자격 전에 누르면 그때까지 번 시간이
**창 0회로 소멸**한다. 2026-08-16T13:50Z 판독 = 연속 **21.0063h / 24h · 남은 2.9937h**.

★★★**보안 노출은 2026-08-16 에 닫혔다** — ~~「이 회차 수리는 재배포되어야 발효한다 ·
`/docs` 노출은 그때까지 열려 있다」~~ → `#639`~`#641` 배포 완료.
`https://qb-api.woosung.dev/docs`·`/openapi.json`·`/redoc` **전부 404**(인터넷에서 실측).
절차 = `git pull` → `migrate`(dry-run) → `migrate --confirm` → **API 유닛 재시작** → read-back 5종.
[BL-753] `APP_ENV=production` 은 **2026-08-16 「보류」로 결정**됐다 (위 ⑵⑶).
