...
| 로컬 스택 | ★**소크 스택은 내려 있다**(2026-08-15 — 계정 배타성 회복이 목적이었다). 조회용으로 **db 하나만** 떠 있다(**5433**) — 워커가 없으므로 세션 부활도 발주도 구조적으로 불가능하다(`gates-and-traps.md` §342 가 권하는 형태). ★로컬 Docker VM **94% / 3.1G** — `docker build` 금지, `docker image prune -f` 로 7.4GB 회수 가능([BL-736])                                                                                                                                                                                    |
...
| **AB** | [BL-736] 로컬 Docker VM **94% / 3.1G** — Redis AOF 쓰기 실패가 celery 를 통째로 죽였다(06:04:11Z 실사고)                                                                                                                                                                    | P2  | ★★   | 낮     | S (1h)     | 0줄             | 안전한 회수는 `docker image prune -f` **7.4GB 뿐** — 볼륨엔 남의 DB 가 섞여 있다                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
