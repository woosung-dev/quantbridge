...
> **강등 tombstone.** 회차 서술 10줄 → 이 3줄. 원문 = `git show 5423fc68:docs/status.md`(336~345행). PR #652·#651·#650·#653.
> 살아남는 둘: ★**게이트 설계 갭 잔존** — `final-gates.sh:62-70` 브랜치 가드가 모드 분기 **앞**이라 main 에서 `--deferred-only` 가 rc=1 이다.
> ★**표적 전건 통과 + 변이 5/5 red 를 통과한 구현에 전량 회귀 5건**(낡은 mock fixture — [LESSON-116]).

> **강등 tombstone.** 08-16 마감 4블록 → 이 2줄. 원문 = `git show d28bf28f:docs/status.md`(356~375행).
> 살아남는 하나: ★**codex 적대 리뷰 P1 이 그 회차 자신의 수리가 만든 결함이었다**(한도 **바깥**의 신원
> 미들웨어가 JWKS 를 쳐 미상 kid 10회에 fetch 11회 — 음성 캐시 0).

### ★환경 상태 (2026-08-17 실측으로 교체 — 다음 세션이 그대로 이어받는다)

| 축        | 상태 (2026-08-17)                                                                                                                                                                                                          |
| --------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| main      | `dd453d75` (#660) · 트리 clean. ★**서버 체크아웃은 뒤에 있다** — `git pull` 은 소크 창을 끊으므로 승인 사항이다                                                                                                            |
| 소크      | ★★**서버(`truewords-oracle`)가 정본이다 — 로컬 맥에서 돌리지 마라**([BL-735]: beat 가 168회 중 15회만 tick 했고 그 15회가 DarkWake 횟수였다). `soak-watch.timer` 가 정본이고 `soak-gate.timer` 는 disabled([BL-737])       |
| 로컬 스택 | ★**소크 스택은 내려 있다.** 조회·테스트용으로 **db(5433) + redis(6380) 둘만** 떠 있다 — 워크트리에서 `mise run up`/`down`/`migrate`/`seed` 는 가드가 rc=1 로 거부한다                                                      |
| 포트 함정 | ★**`:3101` 은 남의 앱이다**(docker, 2026-08-17 재확인) — 2026-08-14 회차가 authed 12건을 두 번 오진한 자리다. 슬롯 포트를 쓰기 전에 `lsof` 로 확인해라                                                                     |
| CORS 짝   | ★★**포트를 맞춰도 `FRONTEND_URL` 이 어긋나면 브라우저는 API 에 못 닿는다.** FE·BE 를 **짝으로** 띄워라 — FE 만 띄우면 playwright 가 자기 webServer 를 올리고 그것은 `BETTER_AUTH_URL` 을 못 받아 403 `INVALID_ORIGIN` 이다 |
| 워크트리  | **6벌** (2026-08-17) — `wt-plan`(계획) · `wt1`·`wt2`(다른 회차) · **`wt3`·`wt4`·`wt5`(이 회차 α·β·γ — 머지 완료, 회수 대상)**. 슬롯 3·4·5 의 FE/BE 가 떠 있을 수 있다                                                      |
...
| **BE** | [BL-789] authed e2e 가 **GitHub CI 에서 안 돈다**(spec 29개 중 **20개**가 실행 0회) → **🟡 2026-08-17 PARTIAL (PR #663)** — 1단계 = 배선 감사 + `LOCAL_ONLY` 로 **거짓 초록의 재유입 차단**. ★**2단계가 본체이고 열려 있다**: CI 데이터 스택 + 인증 secret 은 [ADR-034] 「CI 인증 secret 0개」의 반전이라 **사용자 결정**이다                                   |
| **BF** | [BL-790] funding 인제스션이 `exchange_name: str` 을 받는다 — CCXT 속성명 검사만 한다. native enum 전환으로 실패 지점이 INSERT 로 앞당겨졌다                                                                                                                                                                                                                     |
...
**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-790]** — funding 인제스션이 CCXT 111종을 통과시키고 enum INSERT 에서 죽는다(기존 테스트 8건이 전부 MagicMock 이라 INSERT 가 DB 에 닿은 적이 없다). 동승 후보 = [BL-787]+[BL-718]. ★단 **[BL-790]·[BL-787] 은 소크 감시 경로**라 머지가 재-pin 을 부른다 — 창을 보고 골라라.
