...
**전문 강등 → [`dev-log/2026-08-11-bl701-c1-window-count.md`](dev-log/2026-08-11-bl701-c1-window-count.md)**
([ADR-026] §3 · 2026-08-11 gate-surface 가 700줄 상한에서 내렸다). ★남기는 한 줄 =
**얼린 캡처 하나로 된 하네스는 그 캡처가 찍힌 날짜에 묶인다** — `soak-watch` 크래시 앵커가
`C1 누적` **문자열**에 걸려 있어 이 수리가 무인 감시를 죽일 뻔했는데 하네스는 초록이었다.

~~**다음 행동 = … ⓪ 표 12행에서 고른다.**~~ → **2026-08-11 Q([BL-672])를 닫아 11행이 됐다.**

★**[BL-672] 는 한 줄도 새로 짜지 않고 닫혔다** — 닫은 것은 코드가 아니라 **원장의 거짓
문장**이고(계약 테스트·runbook §7 둘 다 이미 이행돼 있었다), [BL-307]·[BL-703] 에 이은
**네 번째** 실증이다. ★**함정** — **상태줄에 `~~취소선~~` 금지**(`bl-audit.sh:171` 이 철회
표기로 보고 상태 근거에서 제외 ⇒ UNKNOWN). **판정줄에는 써도 된다.**
...
★★**N·M 은 같은 병이고 처방도 같았다** — 「검사기가 보는 표면 < 실제 실패 표면」(N=스캔층
무검증 · M=`app_env` 문자열). 둘 다 **그 층을 실제로 발화시켜** 고쳤다(임시 트리 하네스 /
lifespan 왕복). ★둘 다 **검증기 자신에게서 한 겹을 더** 찾았다 ⇒ **검사기를 고칠 때는 그
검사기의 검사기도 같이 의심해라.**
...
| 축        | [BL-641] 원장 (2026-08-08) | 2026-08-11 실측                      |
| --------- | -------------------------- | ------------------------------------ |
| 최장 연속 | **19.42h**                 | **60.97h** (3.1배)                   |
| 24h 도달  | **전 이력 0건**            | **1건** (지금 창)                    |
| 사망      | 「사망률은 그대로다」      | 마지막 08-07T15:10Z → **3.87일 0건** |

⇒ 그 항목의 상태줄·MTBF 표가 **낡았다.** 게다가 그 표는 `user_stopped` **7건**(운영자 재기동)을 자동 사망과 함께 세고 있어 P(24h) 추정 자체가 오염돼 있다.

**다음 행동 = ⑴ 첫 30분 `backend/scripts/mtbf_stratified.py` 로 O([BL-641]) 재측정 — 그 항목
자신의 Trigger 뒷절(「소크 재기동 회차마다 재측정」)을 집행하고, 위 반증을 원장 표·상태줄에
박는다(사인 분류에서 `user_stopped` 를 **분리**해라). ⑵ 남는 시간으로 T([BL-706]) 본체를
닫는다 — S·하이고 하네스 패턴은 이 회차가 `skip-ratchet-test` 로 두 번 세웠다.**
★**O 를 통짜로 잡지 마라** — 닫는 조건이 「사망률이 **실제로** 내려간다」라 며칠 단위 관측이다.
재측정은 값을 내지만 종결은 이 회차 밖이다. ★**T 를 뒤로 미루지 마라** — 그것이 거짓인 동안은
O 회차 자신의 마감 보고 4축도 근거가 없다. ★P([BL-438])는 **귀속 설계**에 사용자 결정이
붙을 수 있고 프로덕션 관측 0(잠복)이라 급하지 않다. ★~~작은 것을 묶으려면 R + S 가 같은 `live_session_admin` 근처다~~ →
