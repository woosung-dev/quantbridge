...
→ **2026-09-10 완료(PR 2/2 · #881 위에 스택).** 실측(Playwright · 로그인): 리포트·원장·옵티마이저 헤더에
전략명 칩(`EMA Crossover Demo …` → `/strategies/:id/edit`) · 리포트 §04 「전체 원장 보기」 링크 · 편집기 「이 전략의
백테스트」 → `/backtests?strategy_id=`(목록이 거른다). 동승 셋 종결: [BL-860] 원장 번호 1-based(리포트와 같은 거래 =
같은 번호 **14/14** · 검색·차트 라벨·CSV `no` 열 동행) · [BL-861] 공유 화면에 「가정과 데이터 출처」 블록(본문 357자 →
589자 · 수수료/슬리피지/데이터 구간 전부) · [BL-862] 색 규칙 하나 「부호가 의미를 갖는 값에만」(낙폭 중립 · 샤프 부호색 ·
공유 KPI `+0.48%` 초록 / `-0.15` 빨강). [BL-859] ⑶ (리포트→옵티마이저)만 PARTIAL — 목록에 backtest 필터가 없다.
...
| **AC** | [BL-859] 결과 3화면이 **「무엇의 결과인지」를 안 싣고**, 부모→자식 링크가 **전부 없다** → **2026-09-10 ⑴⑵ 종결 · PARTIAL(⑶ 리포트→옵티마이저)** | P2 | ★★★ | 하 | S | 0줄 | 2026-09-08 실데이터 리뷰. 리포트·원장·옵티마이저의 `h1` 이 `BTC/USDT · 1h` / `거래 내역` / `그리드 탐색` 이고 **전략명 0건 · `/strategies/:id` 링크 0건**이다 — 이름은 편집기 `h1`(`EMA Crossover Demo`)에 있는데 안 쓴다. 이 계정 백테스트 17건이 같은 심볼이면 **어느 전략인지 알 수 없다**. ★리포트가 「전체 원장은 **상세 보기에서** 확인합니다」라고 적어 놓고 `/trades` 링크가 **0건**이다(원장→리포트는 있다). 같은 비대칭이 전략→백테스트·리포트→옵티마이저에도 있다 |
| **AD** | ~~[BL-861] 공유 화면이 **가정·데이터 출처를 통째로 뺐다** — 랜딩이 원칙 04로 약속한 그것~~ → **2026-09-10 종결(공유 화면에 가정 블록)** | P2 | ★★★ | 하 | S | 0줄 | `/share/backtests/:token` 본문 **357자** · `가정\|수수료\|슬리피지\|데이터 출처` 매치 **0건**. 로그인 리포트는 그 섹션을 갖는다. ⇒ **맥락이 0인 외부인에게만** 약속이 안 지켜진다. 로그인 화면 원칙 04 원문 = 「가정을 모르면 수익률도 읽을 수 없기 때문입니다」. ★부수 — 리포트 `+0.48%`(초록)가 공유에선 `0.48%`(검정)로 **부호와 색을 동시에 잃는다** |
| **AE** | ~~[BL-860] 리포트와 원장이 **같은 거래에 다른 번호**를 단다 (0-based vs 1-based)~~ → **2026-09-10 종결(원장 1-based · CSV `no` 열)** | P2 | ★★ | 하 | S | 0줄 | 같은 거래(롱 · 진입 `2026-08-23 14:00` · `77,172.60` · 「보유 중」)가 리포트 **#77**, 원장 **#76**. **컬럼명은 양쪽 다 「번호」**다. 「거래 #62 를 봐라」가 화면마다 다른 거래를 가리킨다. ★CSV 내보내기 번호는 **미대조** — 함께 봐라 |
| **AF** | ~~[BL-862] 손익 **색 규칙이 화면마다 다르고**, 샤프는 어디서도 색이 없다~~ → **2026-09-10 종결(부호 의미 값에만 색)** | P3 | ★★ | 하 | S | 0줄 | 최대 낙폭은 **항상 음수**라 빨강이 정보를 0 나르는데, 샤프 `-0.15`(이 실행의 가장 중요한 나쁜 신호)는 **본문색**이다. 공유 화면은 KPI 4개가 **전부 검정**. ⇒ 화면이 「초록 = 좋음」을 가르쳐 놓고 진짜 나쁜 신호에만 색을 안 준다. ★처방은 한 줄 — 「**부호가 의미를 갖는 값에만** 색을 싣는다」 |
