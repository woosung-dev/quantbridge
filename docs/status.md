...
**다음 행동 = Stage 5 — 자연어 → 전략 생성 + 드리프트 탐지기**([ADR-041]).
`POST /api/v1/strategies/generate`. LLM 이 **Pine + Python 둘 다** 내고 **Pine 이 정본**이며,
`analyze_coverage` all-or-nothing 이 저장 여부를 판정한다(사용자 결정 2026-08-27).
★**드리프트 탐지기가 이 회차의 핵심이다** — 통과한 Pine 을 [ADR-042] 렌더러로 Python 화해
LLM 이 쓴 Python 과 대조한다. 위험을 **제거하지 못하고 가시화**한다는 것을 화면 문구가 말해야 한다.
★같이 처리 = `convert` 를 정상 흐름에 노출(현재 진입점이 422 에러 카드 1곳뿐) ·
`mode="sliced"` 배선 복구(토큰 77~97% 절감이 FE 하드코딩으로 도달 불가).
