...
**★서버는 `0c9ccc6` 에 고정돼 있고 main 은 그보다 앞서 있다 — 서두르지 마라.** 병렬 회차의
PR #556([BL-620])이 게이트 기본 취득 경로를 `/metrics` HTTP → `backend/.metrics` **직독**으로
바꿨다. 두 가지 이유로 **지금 재pin 할 필요가 없다**:
⑴ 그 변경은 `QB_METRICS_URL` 을 **명시하면 종전대로 HTTP** 를 쓴다(원격/터널 운영안 보존).
   서버는 그걸 설정하고 돌므로 동작 동일하다.
⑵ BL-620 의 근본 원인은 「소크 스택에 API 가 없어 `:8100` 리스너가 0」이었는데, 이 배포는
   **호스트 uvicorn 을 systemd 로 실제 띄워서** 그 원인을 이미 없앴다(`qb_live_*` 37개 노출 확인).
★그리고 `0c9ccc6` 은 **이미 main 의 조상**이다(merge commit `acff6913`). 재pin 은 **연속 창(C2)을
끊는다** — 연속 24h 를 딴 뒤에 하는 게 맞다.
