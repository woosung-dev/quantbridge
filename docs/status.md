...
**다음 행동 = `scripts/soak-gate.sh`(서버, `bash -lc` 필수)로 창을 확인하고, ⓪ 표
13행에서 사용자와 함께 고른다. 새로 올라온 O~S 중 Q([BL-672])는 종결 판정만 남은 XS 다.**
...
>
> ★★★**2026-08-11 [BL-703] — 오른쪽 항이 그때까지 데이터가 없어 비어 있었다.** PARTIAL 24건이
> 판정줄 의무 밖이라 「도래」로 판별될 수가 없었고, 그래서 이 표는 **ACTIVE 와 같았다.**
> 지금은 24/24 가 판정을 갖고 그중 **5건이 도래**(O~S)다. `docs-audit` 이 양쪽을 다 강제한다.
...
| **Q** | [BL-672] `flatten` 계약 테스트 + runbook §7 — **잔여가 이미 0 이다**                                              | P3  | ★★★  | 하     | XS         | **0줄**                        | ★**원장이 「⑵ runbook §7 갱신 미이행」이라 적은 것을 문서가 이미 이행하고 있었다** — `bybit-mainnet-runbook.md:363-372` 이 2026-08-10 정정으로 `no_open_position` 의 새 의미와 **rc 0/1/3/4 분기**를 적는다. 종결 판정만 남았다([BL-307] 과 같은 형태의 세 번째 실증)                                                                                                                                                                                                                                                              |
...
C1 누적 **56.4197h / 168h**(33.6%) · C2 최장 연속 **41.1057h / 24h ✓** · C3 실격 **0** ·
