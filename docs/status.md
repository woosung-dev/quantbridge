...
PRD **§5**⑵ 를 **다음 루프가 제품을 재게** 하려는 것이다. 2026-09-06 스윕은 **20/26** 라우트만 덮었고
빠진 6개에 **`/backtests/[id]` 리포트**가 들어 있다 — d1 실사용 루프와 §5⑵ 지표가 **반드시 지나는
화면인데 아직 한 번도 열지 않았다.** 그 상태로 [BL-855] 분모를 세우면, 다음 루프에서 나오는 마찰이
「제품 결함」인지 「안 본 화면의 UI 결함」인지 또 못 가른다. 순서를 이렇게 두는 이유가 그것이다.

★**이 축의 범위(2026-09-06 사용자 확정 = A안)** — 셋을 닫으면 이 축은 끝이다.
⑴ [BL-856] 미열람 6화면을 **실데이터로** 열고 `ui-ux-pro-max` 렌즈로 리뷰(재개 재료 = 그 항목의 id 표)
