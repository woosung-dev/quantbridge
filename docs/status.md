...
적을 낱말이 없어서 열린 항목 전부가 ACTIVE 로 떨어졌고, 그 낱말을 만들자 9 가 됐다.
최대 산출은 그 수가 아니라 — 내 판정기의 첫 판이 5건을 근거 없이 「도래」로 올렸고
그걸 잡은 것이 전량 스윕이 아니라 그 앞에 세운 음성 대조였다는 것이다.**

### ★bl-trigger-triage (2026-08-10) — 어휘 `DEFERRED` 신설 · ACTIVE **159 → 9** · 신규 [BL-694]·[BL-695]

| 축            | 착수 시                          | 종료 시                                                       |
| ------------- | -------------------------------- | ------------------------------------------------------------- |
| main          | `7db0d426`                       | `stage/bl-trigger-triage`                                     |
| `bl-audit`    | ACTIVE **159** / 전체 244 · RC=0 | ACTIVE **9** / DEFERRED **152** / 전체 246 · RC=0 · UNKNOWN 0 |
...
`bash scripts/bl-audit.sh --list ACTIVE` 9건 = ⓪ 표에서 사용자와 함께 고른다.**
★**골라 주지 않는다** — ⓪ 표 제목이 「고르는 것은 사용자다」이다. 다만 성격이 갈린다:
**A**([BL-687])·**B**([BL-026])는 `backend/src` **0줄**이고 XS~S 라 창과 무관하게 지금 된다 ·
...
★**후보가 9건뿐인 것은 원장이 줄어서가 아니다** — 152건은 `--list DEFERRED` 로 대기 중이고
...
| ----- | ----------------------------------------------------------------- | --- | ---- | ------ | ----- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A** | [BL-687] pre-commit backend 훅이 스테이징된 py **첫 하나만** 검사 | P2  | ★★★  | 하     | XS    | 0줄           | **매 커밋이 이 결함을 탄다.** 2026-08-10 close-ownership-axis 가 실측 재현. 루트 `package.json` 의 `bash -c '…${0#backend/}'` 가 `$@` 를 버린다 — 고치는 것은 한 줄 |
...
| **H** | [BL-695] `**트리거 판정:**` 줄에 소유자가 없다                    | P3  | ★★★  | 하     | XS    | 0줄           | **이 회차 자신의 구멍이다.** 159/159 를 채웠지만 그걸 지키는 게이트가 0 이다 ⇒ 다음 등재부터 조용히 낡는다. 지금 원장이 전량 정합이라 **도입 즉시 초록**(비용 0)    |
