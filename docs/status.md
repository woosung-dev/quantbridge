...
**다음 행동 = n10 2 lane 병렬 주행** — `python3 tools/harness/execute.py --parallel 2 --stage stage/n10 --confirm`.
lane 1 = `n10-metric-guard`(9 step · [BL-520] 머니-패스 metric 스윕) · lane 2 = `n10-coord-anchors`(4 step · 좌표 앵커 전환).
★**저작 PR 이 머지된 뒤** 시작해라 — `phases/index.json` 사전 등록이 전제다.
★**착수 전 CONTROL 1건** — `du -sm apps/web/.next` 를 **지우기 전에** 채취해 [BL-650] 에 4번째 측정점으로 남겨라(소멸성 표본).
