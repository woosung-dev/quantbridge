...
> ★★**2026-08-15 — 이 셋 중 둘이 실제로 깨졌다. 사용자가 문구를 정해야 한다.**
> ⑴ **거래소 쓰기** — 고아 포지션(`long 0.03`)을 `reduce_only` 시장가로 청산했다.
> **사용자 승인을 받고** 했고 근거는 [BL-024] 회차의 차단자 전례다. ⇒ 「승인 후 허용」으로
> 읽는 것이 실태에 맞다.
> ⑵ **alembic 마이그레이션** — codex 가 인덱스 부재를 지적해 `20260815_0001` 을 만들었다
> (로컬 적용 · CI `fresh DB alembic` 통과). **승인을 받지 않았다.**
> 그리고 그 결과 **서버 DB 와 격차가 생겼다** → [BL-743].
> ⇒ 「금지」인지 「승인 후 허용」인지가 지금 모호하다. **[BL-743] 착수 전에 이 문구부터 정해라.**
...
## ★다음 세션 — 소크 시계가 도는 중이다 (2026-08-15 정의)

**지금 상태:** 서버 소크 `115b33ff` 가 **8.70h 연속 · C3 실격 0건**이다. 이 회차의 닫는 조건
(6h33m 초과)은 충족됐고, **C1 첫 24h 창까지 약 15.3시간** 남았다. `귀속 창`이 0개 → **1개**가 됐다.

★★★**이것이 항목 선택을 지배한다** — 실격 1건이면 T0 리셋이고, `apps/api/src` 수리는 **재-pin →
C2 리셋**을 부른다(지금 8.70h 를 버린다). C1 은 pin 으로 안 죽지만 C2 는 죽는다([ADR-024] 리셋 표).
⇒ **소크 코드를 안 건드리는 항목부터 고른다.** [BL-547]·[BL-591]·[BL-742] 는 그래서 지금이 아니다.

**다음 행동 = [BL-737] 서버 `soak-watch.service` 를 되살린다** — C1 시계가 도는 지금
**실격이 나도 아무도 모른다**(2026-08-14 사망을 다음 세션 preflight 에서야 발견한 것이 그 결과다).
표본 타이머는 정상이고 **알림 축만** 죽어 있다. 서버 systemd 유닛만 건드리므로 **소크 무영향**이다.
`ssh truewords-oracle 'bash -lc "systemctl --user status dev.quantbridge.soak-watch.service"'` 로
사인을 먼저 읽어라. ★문서는 watch 가 게이트 타이머를 **대체**한다는데 실태는 둘 다 필요하다 —
어느 쪽이 정본인지 함께 정해라. 그 뒤 ⓪ 표에서 고른다.

**그 다음 후보 (전부 소크 무영향 · 권장 순서)**

1. **[BL-743] P1 🔴** — 서버 DB 에 migration 이 **도달하지 않는다**(로컬 `20260815_0001` vs 서버
   `20260801_0001`). 지금은 성능뿐이나 **스키마 migration 이 오면 소크가 조용히 죽는다.**
   ★착수 전에 위 ⓵ 의 **비목표 문구부터 정해라** — 「alembic 금지」인지 「승인 후 허용」인지가
   2026-08-15 회차 때문에 모호해졌다.
2. **[BL-641] P1** — C1 문턱 해석 · MTBF 재측정. **소크가 도는 지금이 적기**다.
   ★같이 볼 것 = **어둠 98.2%**(표본 30분 간격의 결과). C1 을 채우려면 세션 생존만으로 부족하고
   **관측 밀도**가 함께 올라야 한다 — 그 처방이 아직 없다.
3. **[BL-741] P2** — `create_all` ↔ alembic 충돌. 다음 migration 마다 재발하고 CI 는 안 잡는다.
4. **[BL-739] P3** — 게이트 화면 신호 비대칭. S(30분)이고 매 회차 걸린다.
...
| **AF** | [BL-743] ★**서버 DB 에 migration 이 도달하지 않는다** — `pin` 은 `apps/api/src` 만 뜨고 `alembic/` 은 그 밖이다                                                                                                                                                             | P1  | ★★★  | 중     | S (1-2h)   | 소량           | 실측 로컬 `20260815_0001` vs 서버 `20260801_0001`. ★지금은 성능뿐이나 **스키마 migration 이 오면 소크가 조용히 죽는다**                                                                                                                                                                                                                                        |
