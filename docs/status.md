...
~~9.50h 연속 (02:05Z)~~ → ~~2026-08-16T13:50Z 실측 21.0063h~~ → **2026-08-15T15:30Z 실측
22.5008h / 24h · 실격 0 · C5 6/6 · 남은 1.4992h ⇒ 16:30Z 표본에서 자격.**
...
(2026-08-15 사용자 결정 — 보고 후 대기). 자격 전에 누르면 그때까지 번 시간이 **창 0회로 소멸**한다.
2026-08-15T15:30Z 판독 = 연속 **22.5008h / 24h · 남은 1.4992h** ⇒ **16:30Z 표본에서 자격**이다
(표본은 30분마다 `:28` 초에 들어오고 창 끝은 그보다 ~29초 이르다 — 판독이 표본 직전이면 값이
안 움직이는 것처럼 보인다. 그것은 정지가 아니다).
★그 `pin` 은 워커를 `fb7bb772`(#633) → 현재 main 으로 **4개 PR 점프**시킨다. 창 2는 창 1과 **다른
코드**를 재므로 [BL-641] MTBF 층화에서 같은 모집단으로 묶으면 안 된다.
