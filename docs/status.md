...
> **업데이트:** 2026-08-03
> **★[BL-590] Resolved — 소크를 105분에 끊은 거절의 뿌리를 닫았다.** 브랜치
> `stage/breach-rejection-recovery` (`main` @ `18d1fdaa`) · 마이그레이션 **0** · FE **0**.
> ★**가드가 뚫린 게 아니다.** 계획기의 돌파 가드는 **발주 시각에 옳았고**(카운터 차분이 연역
> 증명 — `conditional_placed +1` · 전환 계열 전부 불변), 거래소가 **2.1초 뒤** 자기 시각으로
> `110093` 을 냈다. 없었던 것은 **거절 뒤의 복구**다. 거절을 「돌파 확정 증거」로 읽고
> PR #493 이 이미 정의한 시장가 전환을 그 자리에서 집행한다.
> 상세 = [`dev-log/2026-08-03-breach-rejection-recovery.md`](dev-log/2026-08-03-breach-rejection-recovery.md).
>
> **★앞 진입점이 낡아 있었다** — 「소크가 돌고 있다」던 세션 `a201a47b` 는 이미 죽어 있었다
> (`15:54:34Z`, T0+**104.9분**). ★**이 클래스는 `110093` 단독이 아니다** — 거울 코드
> **`110092`**(long/RISE)가 있고 원장 4건 중 **2건이 그쪽**이다.
>
...
그게 일일 대조 전량이다. 분기는 셋뿐이다:

- **생존 + 이상 없음** → ★★**아무것도 고치지 마라.** 시계를 세우는 게 더 비싸다. 아래 B 로 간다.
- **`UNKNOWN` + exit 3** → 조회가 실패한 것이다. **「이상 없음」으로 읽지 마라.**
- **죽었다** → `death_reason` 은 원인 조사의 **시작점**으로만 써라. 두 회차 연속으로 그 이름이
  가리킨 곳과 **실제 뿌리가 달랐다**([BL-589] 계획기 · [BL-590] 거절 뒤 복구 부재).

### 판정 축 — 「N분 무사고」를 증거로 쓰지 마라

★★★**이번 수리의 판별력은 소크에 거의 없다.** 돌파 거절은 **0.21건/h** 라 105분 기대값이
**≈0.34건**이다 — 창의 약 **31%** 에서만 발화한다. 통과해도 수리와 무관할 확률이 69% 다.
**이 수리를 지지하는 것은 오프라인 변이 8/8 과 프로덕션 유도 주입이다.**
프로덕션 자연 발화 확인은 `recovery_placed` 가 **다음에 증가할 때** 그 시점의 원장에
`condmkt` 주문이 짝으로 있는지로 한다.

### A. 창 유지 — `backend/src` 편집이 **구조적 금지**다

★★★워커가 `watchfiles` 로 물고 있어 다단계 편집의 **중간 상태**를 적재하면 세션이 죽는다.
문서·`scripts/`·`tests/` 는 안전하다. ★**변이 테스트도 금지다** — 변이는 `backend/src` 를
고치므로 **워커에 고장을 주입하는 것**과 같다(2026-08-03 실측: 변이 중 워커가 그 코드를 적재).
...
밖**이다. 권장 = 스칼라부터 늘리고 리스트형은 **필드별 digest** 로 접는 2단계.

★**착수 전에 「이걸 늘리면 무엇이 red 가 되나」를 먼저 적어라.**

### ★뿌리가 등재됐다 — [BL-591] (증상 BL 을 또 열기 전에 여기를 읽어라)

**증상을 30건 고쳤는데 병이 그대로다.** 엔진↔거래소 정렬 축 BL **32건 중 30건 Resolved**,
그런데 자동 종료 **15회**이고 `direction` 가드 머지(#497) 이후 **12/12 전부 2시간 미만**이다.

뿌리 = **엔진 포지션의 SSOT 가 없다.** `run_live` 가 매 tick 봉을 재생해 포지션을 **도출**하고
(`live_signal.py:2360`), 정상 운행 중엔 현실로 **보정되지 않는다** — **엔진에 「쓸」 자리가 없다.**
유일한 원장→엔진 주입(`_ledger_gap_seed`, BL-544)은 **긴 공백에서만** 돌고 프로덕션 성공 seed
**0건**이다. 단 **주입점 자체는 옳은 자리에 있다**(`event_loop.py:192`, 마지막 bar Pine 실행 직전).

★**다음 행동은 코드가 아니라 설계 결정이다** — [BL-591] §설계 축의 4개 질문에 답해야 착수 가능.
권고안은 **C(원장 SSOT · `ledger_seed_legs` 상시 주입)**. **B(거래소 SSOT)는 기각** — 코드 주석이
이미 「거래소에서 seed 하면 대조가 동어반복이 되어 가드가 사라진다」고 적고 있다.

### 비목표

[BL-580] 잔여 96곳 · [BL-578] 취소 사유 영속화 — `backend/src` 라 창 이후.
[BL-389] · [BL-466] 파산 모델 — **사용자 설계 결정 선행** · [BL-462] · mainnet.
★**[BL-591] 이 열려 있는 동안 새 발산 증상 BL 은 거기에 링크**해라 — 30건째 증상 수리를 반복하지 마라.

### 사전등록

- **V1** 매 관측에서 `soak-observe.sh` exit 0. `UNKNOWN` 이 나오면 그것부터 닫는다.
- **V2** [BL-586] 착수 시, 늘린 필드마다 **인위 변조에 red** 임을 보여라.
