...
아니라 3곳**이었다(`trading.py:1227`·`:1493` · 리포지토리). 앞 둘은 **일부러 안 고쳤다** —
열면 **정상 선물 entry** 가 `transient` 4회 재시도 뒤 **운영자 알림**을 낸다. ⑶ **[BL-726] 은 허위 경보**였다(값은 체결 전 추정치이고
`exchange_order_id` 가 46건 전부 NULL = **주문 ID 미발급·미체결**) — 원장 처방 ⑵ 는 **no-op** 이다.
★부수 = #631 주석 정정. 단 `live:` 접두는 생산자 증명이 못 된다 — 정본은
`LiveSignalEvent.order_id` 조인(2026-08-15 2차 리뷰).
...
| **Z** | [BL-733] 체결 직후 refresh 2곳이 아직 `reduce_only` 게이트 — 반전 백필 최대 5분 지연                                                                                                    | P2  | ★★   | 중     | M (2-3h)   | 0줄                            | ★**필터만 지우면 더 나빠진다**(정상 선물 entry 가 거짓 운영자 알림). `_reversal_bucket_at_fill` 재사용                                                                                                                                                                                                                                                                                                                                                                                                                             |
