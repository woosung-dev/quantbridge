...
C1 은 「세션이 살아 있었다」가 아니라 **「귀속된 시간」**으로 센다. 2026-08-15 실측이 그 둘을
갈랐다(125.6h 살았는데 누적 0.0000h). 지금도 어둠 98.2%(9179/9347) — 표본이 30분 간격이라
그 사이가 전부 어둠이다. **C1 24h 창을 3회 채우려면 관측 밀도가 함께 올라야 하는데 그 처방이 아직 없다.**
★먼저 `soak_gate_predicate.py` 의 **credit 계산(phantom 커버리지 구간)을 코드로 확정**하고,
⑴ 타이머 주기 단축 ⑵ 표본 하나가 덮는 폭 확대 를 **셈으로** 비교해라 — 주기를 줄이면
게이트 실행 비용과 C4 해상도가 따라 움직인다(`soak-watch.sh:159` 주석이 그 축을 금지하고 있다).
`mtbf_stratified.py` 가 Trigger 뒷절(회차마다 재측정)을 집행 가능하게 해 뒀다.

**그 다음 후보 (전부 소크 무영향)**

1. **[BL-741] P2** — `create_all` ↔ alembic 충돌. **이 회차가 시간상 접었다.** 축은
   `tests/conftest.py:333-334` 의 `drop_all` 이 `alembic_version` 을 **안 지우는** 것이다
   (metadata 밖이라). CI(fresh DB)는 안 잡고 로컬에서 pytest 를 돌린 개발자만 걸린다.
2. **[BL-026] P1** — Mutation skip 일괄 활성화. ★**범위 재정의가 선행**이다(원장 참조).
3. **[BL-736] P2** — 로컬 디스크 94%. **사용자 판단 선행**(회수분이 전부 남의 프로젝트 이미지).
4. ★**순차 step 러너가 되돌아왔다** ([ADR-033] · [ADR-030] 은 Superseded **아님**, 뒤집힌 것은 재개 조건 ⑴ 하나).
   무인으로 돌릴 회차를 고를 때 **`.harness/README.md` §2·§4 를 먼저 읽어라** — as-is 위험 7종이 살아 있고
   ([BL-746]) 막는 것은 코드가 아니라 규약 2개(워크트리 전용 · `--push` 금지)다. 첫 회차는 **저위험 BL 1건**.

★**[BL-736]**(로컬 디스크 94%)은 **사용자 판단이 선행**이다 — 안전한 자동 회수 경로가 없다
(회수 가능분 5.5GB 가 전부 남의 프로젝트 이미지). 무엇을 버릴지는 그 프로젝트 소유자가 정한다.
...
| 축        | 상태 (2026-08-15)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| main      | `d9fb9f58` (#635) · 트리 clean. **서버 체크아웃도 같다** — 단 `tools/scripts/` 3벌이 이 회차 실증분으로 앞서 있다(PR 머지 후 `git pull` 로 정리된다)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| 소크      | ★★**서버(`truewords-oracle`)가 정본이다 — 로컬 맥에서 돌리지 마라**([BL-735]: beat 가 168회 중 15회만 tick 했고 그 15회가 DarkWake 횟수였다). 세션 `115b33ff` · pin `fb7bb772` · 2026-08-14T16:29Z 기동 · **C2 9.50h · C3 실격 0 · C5 6/6**(2026-08-15 02:05Z). DB 는 **head `20260815_0001`** 이고 인덱스 집합이 로컬과 diff 0 이다([BL-743]). 게이트는 **`bash -lc` 로 서버에서만** (비로그인 셸엔 uv PATH 가 없어 rc=127)                                                                                                                                                                                                                                                                                                                                                                                    |
