...
> **Last Updated:** 2026-07-28 (**live-close-completeness** — 엔진과 거래소가 **서로 다른 상품**을 보고 있었다)
> **Active Sprint:** 없음 — `feat/live-close-completeness` PR 대기. **다음 스프린트는 아래 §다음 스프린트 참조.**
> **Last Merged:** `feat/live-outcome-parity` → `main@9a80e3c2` (PR #496)

---

## 🎯 다음 스프린트 — live-entry-completeness (BL-536 + BL-537)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.
> 시작 방법: **"다음 스프린트 진행해줘"**. 에이전트는 `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 직전 회차가 계기를 맞췄다(엔진 봉 스팟→perp). 이제 **계기가 맞은 상태에서** 진입 유실의 실제 크기를 재고, 세션이 꺼진 뒤 남는 **무방비 포지션**을 닫을 수 있게 만든다.

**본체 = BL-536**(BL-522 재측정 후 설계) + **BL-537**(활성 세션 없이 고아 포지션 청산).

### 왜 지금인가

- **BL-536** — BL-522 의 유실 채널 5종과 실측(`deferred_market_inflight` 시간당 14회)은 **계기가 어긋난 상태**에서 나온 값이다. 유령 진입이 사라지면 각 채널 크기가 달라진다. **먼저 재측정하지 않으면 사라질 문제에 새 상태 저장소를 만든다** — BL-522 자신이 경고한 함정이다.
- **BL-537** — 직전 회차 soak 준비 중 **실제로 밟았다.** 거래소에 `short 0.03 @ 63470.6, SL 없음, 미실현 −7.24 USDT, 2x` 가 **9시간 방치**돼 있었고, `close_service.close_position` 이 `session_id` 를 요구해 **앱에서 닫을 방법이 없었다**. provider 원시 호출로 내려가야 했다(Order 행 없음 · kill-switch 미평가). 레버리지가 걸린 무방비 노출이 무기한 지속된다.

### ★설계 시 반드시 짚을 것

1. **재측정이 먼저다.** `qb_live_position_divergence_total{category=engine_only}` 가 계기 정렬 후에도 유의미하게 남는지부터 본다. 남지 않으면 BL-536 은 축소되거나 소멸한다.
2. ★**계기를 맞춰도 이미 갈린 포지션은 수렴하지 않는다.** `run_live` 는 거래소 포지션을 **입력으로 받지 않는다**(순수 OHLCV 재생). 직전 회차 실측 — 거래소를 flat 으로 만들어도 엔진은 `short 0.0299` 를 계속 들고 있었다.
3. **BL-537 은 `OrderService` 를 거쳐야 한다** — 원장·kill-switch 를 우회하는 청산 경로를 정식화하지 마라.
4. 진단 절차는 [`reference/live-close-diagnostics.md`](reference/live-close-diagnostics.md). **창을 안 건 비율은 읽지 마라**(§0).

### 하지 않는 것

BL-535(백테스트 계기 — 별도 스프린트, 심볼 컨벤션 파급 결정 필요) · BL-527/528/529(P2) · 거래소 확장 · `tasks/` deepen.

### baseline (2026-07-28 실측, `feat/live-close-completeness` 기준)

**직접 측정(2026-07-29 CI 재현, `fix/bl530-review-followups` HEAD 기준):** BE **3415** / FE **1213**(203 파일) / ruff·mypy(212) **0** / `alembic upgrade head` fresh ✓ / **커버리지 게이트 93.14%**(기준 90) / 마이그레이션 **0**.
**이월(미재측정):** e2e authed **65** · canon **32** · CI전용 **4** · tsc · eslint — 이번 회차 `frontend/**` 변경 **0 파일**이라 결과가 구성상 불변이지만 **돌리지는 않았다.**

> ★측정한 것과 이월한 것을 섞어 적지 마라. 직전 회차가 이 줄에 e2e·tsc·eslint 를 measured 처럼 실었고 사후 리뷰가 잡았다.
> ★★**그리고 이 줄 자체가 다시 틀렸었다** — "직접 측정" 이라 써놓고 **main 기준 3411**(직전 PR 값)을 실었다. 이 브랜치 실측은 **3415** 다. **어느 커밋에서 잰 값인지까지 적지 않으면 "측정" 이라는 라벨이 스스로를 보증하지 못한다.** 이 저장소는 "핸드오프 게이트 숫자가 틀렸다" 를 이미 여러 번 겪었고, 이번엔 그 경고를 적은 바로 그 줄에서 재발했다.
