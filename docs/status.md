...
## 🎯 다음 스프린트 — **close-mismatch-soak** (신규 라벨이 실제로 발화하는지부터 본다)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.
> 시작 방법: **"다음 스프린트 진행해줘"**. `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 직전 회차가 **보이게** 만들었다. 이번 회차는 그것이 **실주행에서 발화하는지**를 재고,
그 숫자 위에서만 원인을 겨눈다. ★**직전 회차는 soak 을 돌리지 않았다** — 기존 원장만으로 판정했다.

### ★왜 지금인가 (실측 숫자, 기존 원장 2026-07-26~07-30)

| 갈래                                   |                         n | 뜻                              |
| -------------------------------------- | ------------------------: | ------------------------------- |
| `110017` + `reduce-only ... same side` |                     **9** | ★★엔진과 거래소가 **반대 방향** |
| `110017` + `current position is zero`  |                        30 | 유령 포지션 (무해)              |
| 위험 갈래가 발생한 **세션 수**         |                     **5** | 일회성이 아니다                 |
| 그 9건의 화면 표시                     | **9/9 `dispatched` 초록** | 주문은 `rejected` 였다          |

★**무해 갈래가 3배 많아 위험 갈래를 같은 라벨 안에 묻고 있었다.** 그래서 counter 는 계속
"유령 포지션 문제" 만 가리켰다. 직전 회차가 라벨을 갈랐고 화면도 고쳤다 — **이제 셀 수 있다.**

### ★착수 전 반드시 읽을 것

1. ★★**첫 step 은 계측기 생존 확인이다.** 신규 라벨 3종
   (`reduce_only_same_side` · `reduce_only_position_zero` · `deferred_market_inflight_noop`)이
   `/metrics` 에 **샘플과 함께** 뜨는지 본다. 이름만(HELP/TYPE) 뜨면 **백엔드·워커를 재기동**해라
   (배선 이전에 뜬 프로세스는 단일 프로세스 모드다).
2. ★★**신규 라벨은 이번 배포에서 태어났다.** 기존 라벨과 **절대값 비교 금지, 차분만.**
   before 스냅샷에 그 series 가 없으면 `after - 0` 은 차분이 아니라 절대값이다.
3. ★**C2 를 유실률 분자로 다시 쓰지 마라.** 직전 회차가 그것이 **청산 tick 수**임을 확정했다.
   `_noop` 라벨이 "미룰 진입이 정말 있었는가" 를 처음으로 알려준다.
4. ★**BL-553 은 공백을 30분+ 로.** 그리고 **공백 시작 시점에 조건부 진입이 장전돼 있어야** 한다
   (`state IN ('pending','submitted') AND trigger_price IS NOT NULL`). flat·미장전으로 쉬면
   `applied` 는 구조적으로 불가능하고 그 0 은 아무것도 뜻하지 않는다.

### 사전등록 판정 문턱 (★soak 결과를 보기 **전**에 확정했다)

| ID  | 판정                                                             | 조건                                                            |
| --- | ---------------------------------------------------------------- | --------------------------------------------------------------- |
| V1  | **계측기 생존** (전제 — 이게 실패하면 다른 숫자를 해석하지 마라) | 신규 라벨 3종이 `/metrics` 에 샘플과 함께 존재                  |
| V2  | **소멸**                                                         | `same_side` 차분 **0** 이고 관측 창 ≥ 2h                        |
| V3  | **실재 · 원인 착수**                                             | `same_side` 차분 **≥ 1**                                        |
| V4  | 표본 하한                                                        | 청산 시도 **< 10** 이면 **어떤 비율도 인용하지 않는다**         |
| V5  | 무효                                                             | 세션이 `position_divergence`·`run_live_error` 로 죽으면 창 폐기 |

★**V3 이면 그때 원인을 겨눈다** — 후보는 재가격 경주 / 청산 거절 후 엔진 미정렬 / 재생 아티팩트.
**어느 것도 지금 크기를 모른다. 그래서 이번엔 짓지 않는다.**

### 하지 않는 것

**defer 를 좁히는 코드**(대가가 0으로 측정됐다) · **defer 귀속 계측기**(분모가 청산 횟수라 사문이 된다) ·
