...
PRD **§5**⑵ 를 **다음 루프가 제품을 재게** 하려는 것이다. 2026-09-06 스윕은 21/27 라우트만 덮었고
...
| **X**  | [BL-853] 컴포넌트 어휘가 **둘로 갈려 있다** — 버튼 48/38px · 반경 6·10/12px · **라이트에서 다크 그림자** | P2 | ★★ | 중 | M | 0줄 | **눈에 보이는 결함 중 최대.** `--shadow` 가 `:root` 에만 있고 `.dark` 에서 재정의되지 않아 **라이트 카드가 `rgba(0,0,0,.55/.8)`** 를 쓴다(같은 페이지 shadcn 카드의 약 10배). ★**KITPORT 를 직접 못 고친다**(`globals.css:966~1876` 은 `_kit.html` 과 주석까지 대조) — 1879줄 아래 언레이어드 오버라이드가 길이다. **그림자 라이트값 하나부터**가 가장 싸다 |
