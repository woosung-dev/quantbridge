...
★**착수 전 표본 1건을 열어라.** 이 계열로 **두 회차 연속** 상속 사실이 낡아 있었다 —
2026-08-11 은 20개 중 2건, 2026-08-12 는 **6건**이었다. 표를 읽지 말고 도구를 돌려라.
...
| **V** | [BL-708] `design-canon-calibration` 대비 측정이 **회차마다 다른 파일**에서 실패한다                               | P2  | ★★   | 하     | S          | 0줄                            | ★**2026-08-12 도래 — 재현 3회가 이미 있다.** 같은 커밋에서 실패 집합이 `{screen-10}` → `{screen-08, screen-15}` → `{}` 로 **서로 겹치지 않는다**(`5.41:1` vs `5.82 필요`, 계산 폰트 10.08px). 거짓 red 는 회차를 태우고 거짓 green 은 **「하드 실패 0」을 무증거로 만든다**                                                                                                                                                                                                                                                        |
