...
## ★2026-08-14 money-path-attribution — [BL-438] 진단 교체 + 헤지 모드 기각

★★★**한 줄 = 「미귀속 71.5% 는 결함이 아니라 내가 잘못된 DB 를 봤다는 뜻이었다」.**
착수 전제가 **두 번** 뒤집혔고 그것이 이 회차의 최대 산출이다.

**⑴ 원장 882행은 부풀어 있었다** — 고유 청산 **592건** + 중복 290행([BL-725] 등재).
**⑵ 「미귀속 631행」은 로컬 DB 아티팩트였다** — 소크 주문은 **서버 DB** 에만 있다
(`filled` 654 vs 로컬 321). 서버 기준 `exact` **94.8%** · `none` **2.4%** 다.
`soak-disqualifications.jsonl:1` 이 실격 원장에 적어 둔 **「어느 DB 를 봤는가」 함정이
`exchange_exits` 에도 그대로 적용된다.**

**★진짜 구멍은 옆에 있었다** — 라벨은 `exact` 561건인데 손익이 흐르지 않는다.

| 축                                                     | 값                            |
| ------------------------------------------------------ | ----------------------------- |
| kill-switch 가 보는 것 (`filled` × `realized_pnl` SUM) | **−78.02 USDT (6.9%)**        |
| 거래소 실현손익 전량 (고유 592건)                      | **−1,125.81 USDT**            |
| `reduce_only` 필터가 막은 것                           | **490건 / −1,023.87 (90.9%)** |

`list_unsynced_reduce_only`(`order_repository.py:758-772`)가 `reduce_only=true` 만 고르는데
소크는 **반전 주문**(`sell 0.058 = 2×0.029`)을 쓰고 반전에는 그 플래그를 걸 수 없다.
★**`reduce_only` 는 「내가 요청한 안전장치」이지 「이 주문이 청산했는가」의 답이 아니다** —
Bybit·OKX 어느 계약에서도 그 등가가 성립하지 않는다(문서 대조 + 실측). **이것은 [BL-438]
의 잔여가 아니라 [BL-003] P0 의 미발견 전제다.**

★**동시에 더 큰 것이 나왔다 → [BL-724]** — 수수료 전 gross **+74.18** vs 수수료 **−1,200.00**.
**전략은 흑자인데 수수료가 16배로 먹었다.** 소크 게이트가 PASS 해도 이 사실은 안 바뀐다.

★**헤지 모드는 실거래소 프로브로 기각했다 → [ADR-032]**. T1·T2 통과(데모 전환 O · long/short
동시보유 O)이고 **T3 이 공식 문서를 반증**했지만(`reduceOnly` 없이 청산됨 — 사용자 직관이 맞았다),
**T4 가 도입 근거를 없앴다** — 헤지에서 만든 청산도 `closed-pnl` 키가 one-way 와 **동일한 19종**
이고 `positionIdx` 가 없다 ⇒ 귀속 이득 0. 대가는 163곳/12파일(`pine_v2` 4개 포함)이다.

★**부수** — 실격 원장 3건 등재(서버 세션 `de3db35a` 가 **08-14T04:51:27Z 에 사망**해 있었다.
「7일째 정지」는 로컬 pin-history 만 본 판정이었다. 로컬 재기동은 그 62분 뒤라 **이중 호스트
동시 활성은 없었다**) · 프로브가 만든 ETH 청산 2건이 원장에 `unknown/none` 으로 들어온다(정상).

**다음 행동 = `money-path-close` 회차를 연다** — Lane B(codex, 워크트리 슬롯 3)가 [BL-438]
백필 조인 수리 + [BL-726]·[BL-727]·[BL-728], Lane A(CONTROL, `docs/**` 독점)가 **[BL-724]
경제성 판정** + 서버 `git pull`(체크아웃이 `c3a39d0d` 라 실격 원장 13행을 못 본다).
★**착수 실측은 반드시 서버 DB 로** — 로컬로 재면 답이 달라진다.
...
| **P** | [BL-724] ★수수료가 gross 를 **16배**로 먹는다 — 실자금 전환의 경제성 전제                                             | P1  | ★★★  | 중     | M (2-3h)   | 0줄                            | 즉시 — gross **+74.18** vs 수수료 **−1,200.00**(고유 592건 실측). 소크 게이트가 PASS 해도 안 바뀐다 · **판정 항목이지 수리가 아니다**                                                                                                                                                                                                                                                                                                                                                                                              |
| **Q** | [BL-727] `soak-gate.sh:706` 이 맨 `python3` 라 **맥에서 판정 불가**                                                   | P2  | ★★★  | 낮음   | XS (10분)  | 0줄                            | 이미 발화 — 3.9 vs `itertools.pairwise`. 죽고도 빈 `판정:` 을 찍는 **fail-open**. 같은 파일 `:449` 가 이미 고쳐 둔 함정                                                                                                                                                                                                                                                                                                                                                                                                            |
| **R** | [BL-726] `rejected`+reduce-only **46건**에 `realized_pnl` +55.32 인데 evaluator 가 못 본다                            | P2  | ★★   | 낮음   | S (1h)     | **건드림**                     | 동승 — [BL-438] 백필과 **같은 SUM**. 「채워졌다(=청산)」 ↔ 「rejected(=발주 실패)」가 모순이고 판정된 적 없다                                                                                                                                                                                                                                                                                                                                                                                                                      |
| **S** | [BL-725] `exchange_exits` **중복 290행** — 같은 uid 에 계정 행이 둘                                                   | P2  | ★    | 낮음   | S (30분)   | 0줄                            | 미도래(동승) — 신규 적재는 08-08 에 멈춘 잔재. `exchange_accounts` 행 축 결정 대기                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| **U** | [BL-728] `classify_exit` 이 Bybit `CreateByLiq` 를 못 잡는다                                                          | P3  | ★    | 낮음   | XS (10분)  | **건드림**                     | 미도래(동승) — 관측 0건 잠복. 실자금 전환 후엔 잠복이 아니다                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
