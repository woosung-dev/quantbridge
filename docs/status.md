...
★★**사용자 결정 — Cloudflare Access 를 유지한다.** 「왜 제거하나」라는 반문에서 재측정한 결과가 판정을
뒤집었다: **가입에 초대 토큰이 필요 없고**([BL-776] — `/sign-up(.*)` 은 공개 라우트), 서버의
`WAITLIST_ADMIN_EMAILS`·`RESEND_API_KEY` 가 **둘 다 미설정**이라 승인도 발송도 안 된다 ⇒ Access 를 걷으면
얻는 것은 0 이고 **개방 가입만 열린다.** [BL-070] 의 Access 축은 [BL-776] 뒤로 간다. WAF geo 규칙(L1)은
무관하게 배치 가능 — [`security/geo-block-setup.md`](reference/operations/security/geo-block-setup.md).
...
## ★2026-08-17 night-3lane · gate-trust — 야간 자율 3레인 1·2회차

> **강등 tombstone.** 두 회차 서술 20줄 → 이 3줄. 원문 = `git show aadac2bb:docs/status.md`(489~513행). PR #654~#656 · #658~#661.
> 살아남는 둘: ★**증거가 없던 이유가 확인 행위였다**(playwright 가 매 실행 `outputDir` 을 지운다 — [LESSON-117]). ★**보고된 preflight 기준선이 조건부였다** — 「BE 4759 passed」는 더러운 DB 에서만 참이고 fresh DB 에선 `2 failed` 였다([BL-788] — 이 회차가 닫았다).

~~**다음 행동 = … 가장 강한 후보는 [BL-788]**~~ → **2026-08-17 ultracode 2레인이 완주.** 아래 참조.

## ★2026-08-17 ultracode-2lane — [BL-788] · [BL-789]/[BL-795] · 검사면의 정직성

**무엇을 했나** — ACTIVE 12건을 코드 대조로 스코핑해 **소크 감시 경로 밖 2레인**만 골라(C1 창 보존)
워크트리 슬롯 3·4 에서 병렬 실행. PR **#662**(`stage/bl788-metadata-scope`)·**#663**(`stage/bl789-795-e2e-truth`).

⑴ **[BL-788] Resolved**(`create_all` 범위에 4다리 검사면 — 기대치는 `src/**` AST census, 하드코딩 0) ·
**[BL-795] Resolved** · **[BL-789] PARTIAL**(1단계 = 거짓 초록 재유입 차단. 2단계 = CI 인증 secret → **사용자 결정**).
⑵ ★★★**이 회차의 주제는 「내 주석이 코드보다 앞서 나갔다」였고 그것이 8건 나왔다.** 전부 같은 모양 —
**「그렇게 적혀 있다」를 「그것이 그렇게 동작한다」로 읽었다.** 매번 **다음 검증 라운드**가 잡았다
(3렌즈 27건 → codex 6건 → 독립 검증자 10건). 두 검사면 다 **첫 판은 착취에 전부 초록**이었다.
⑶ ★★**「모순 문장이 남아 있지 않다」를 두 번 틀렸다** — 잔존 확인을 한 파일에서만 돌린 탓이다.
잔존 확인은 처음부터 `git grep -- docs/` **전체**여야 한다.
⑷ ★**범위를 좁히는 것이 산출이었다** — 레인 B 는 셸 제어흐름 모델링(적대 전용)을 **걷어내** 순 −7줄.
파서를 키울수록 새 결함이 나왔다. 검사면이 잡을 것은 **사고**이지 적대적 저자가 아니다([BL-796] 도 같은 판정).

**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-790]** — funding 인제스션이 CCXT 111종을 통과시키고
enum INSERT 에서 죽는다(기존 테스트 8건이 전부 MagicMock 이라 INSERT 가 DB 에 닿은 적이 없다).
동승 후보 = [BL-787]+[BL-718]. ★단 **[BL-790]·[BL-787] 은 소크 감시 경로**라 머지가 재-pin 을 부른다 — 창을 보고 골라라.
