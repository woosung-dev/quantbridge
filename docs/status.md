...
> ★★**2026-08-16 layout-alignment 가 남긴 미검증 하나** — `worktree-feat+harnes-ac` 브랜치의
> [ADR-035] FE 이동(234파일)은 **`e2e:authed` 86건을 못 돌린 채**다. 워크트리 env 가 격리
> 스택(5433/6380)을 가리키는데 그 기동은 메인 체크아웃 몫이기 때문이다([ADR-029] 워크트리 규약).
> 정적 3종·vitest 1414/1414·design-canon 44/44·smoke 4/4 는 통과했다.
> **머지 전에 메인에서 `make up-isolated` 후 `pnpm e2e:authed` 를 돌려라.** BE pytest 도 같다.
