...
> **PR #628 머지(`4b11da26`)** · Bybit demo 실주문 1건이 프로덕션 dispatcher 를 타고 `filled` 까지
> 갔다 — **실거래소가 처음으로 검증됐다**. ★★★한 줄 = **「2층 자기정리 하네스는 지어진 뒤 10일간
> 한 번도 작동한 적이 없었다」**([LESSON-109]) — 청산이 **개발 DB 를 열고** 있었다. ★증인은
> pytest rc 가 아니라 **거래소**였다. ★차단자 = **7일 묵은 고아 포지션**. ★codex 3/3 참.
...
> 뜻이었다」**(소크 주문은 **서버 DB** 에만 있다). ★진짜 구멍은 옆 — `reduce_only=true` 필터가
> 백필을 막아 kill-switch 가 **6.9%** 만 봤다. ★헤지 모드 기각([ADR-032]) — T4 가 도입 근거를
> 없앴다(헤지 청산도 `closed-pnl` 키가 one-way 와 동일, `positionIdx` 없음).
...
그 사인은 이미 `Resolved` 인데 **그 수리가 든 pin 에서 재발**했다 → [BL-732]. 실격 원장 17행째
등재(`cause_class: undecided`). ⇒ **클럭은 이미 끊겼으므로 재-pin 비용이 0 이다.**

★★**초판 리드(「`ledger_net=0.06` = 반전 수량이니 수량축」)는 codex 적대 리뷰로 반증됐다** —
생산자 `_ledger_gap_seed`(`live_signal.py:434`)가 **부호 있는 합산**을 하고, 같은 독스트링이
진짜 한계를 적고 있다(「공백 창은 **공백 이전 포지션**을 모른다」). ⇒ 축은 수량이 아니라
**창**([BL-547] 영역). 가설 = ⑴ 시간축([BL-622] 재발) ⑵ 창 축. **수량축은 죽었다.**
