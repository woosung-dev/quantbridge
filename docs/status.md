...
### ★사전등록 예측 (다음 회차 첫 커맨드로 판정해라)

```bash
curl -s localhost:8100/metrics | grep qb_live_position_divergence
```

**예측** — `engine_only` 는 **314 에서 그대로일 것이다.** 근거: `.soak/snap-*.txt` **32벌 전량**
(08-03 09:53Z → 08-04 04:26Z = **18.5시간**) 차분에서 `engine_only`·`exchange_only`·`size`·
`engine_only_unexplained`·`probe_failed` 가 **전부 0** 이었고 움직인 것은 `direction_transient`
**하나뿐**(15→24, ≈0.5/h)이다. **`direction_transient` 는 계속 오를 것이다.**

★**틀리면**(= `engine_only` 가 올랐으면) 그게 이 회차 최고의 수확이다 — 라이브 부검이 가능해지고
[ADR-023] §선행 측정을 **과거 재생 대신 실시간 관측**으로 할 수 있다. ★**맞으면** 그 채널은
휴면이므로 분해는 **과거 원장·워커 로그 재생**으로만 가능하다.

### 본 작업 — `engine_only` 314 의 **원인** 을 갈라라 (읽기 전용이라 소크와 공존한다)

★★★**먼저 알아야 할 것 — 314 는 과거 누적이고 지금은 휴면 중이다.** 위 예측 근거 참조.
「`engine_only` 314 vs `exchange_only` 21 = 15 대 1」은 **누적 총계의 비율이지 현재 발생률이
아니다.** ⇒ **첫 질문은 「314 가 언제 쌓였나」**다 — 시각 분포를 뽑아 [BL-589]/[BL-590] 수리 시각
전후로 갈라라. **[가정]** 두 수리가 그 채널을 닫았을 수 있다(둘 다 「엔진이 유령을 든다」의 생성
경로를 겨냥했다). **[확인 필요].**
