...
★**Lane B·C1 도 같은 회차에서 끝냈다** (PR #634 머지 후 `feat/backfill-resync-coverage`) —
소크 관측은 시계일 뿐이라 개발과 병행했다(이 파일의 「창과 무관하게」 규칙 그대로다).

- **[BL-731]** 재검증 모집단을 「원장 합계 ≠ 저장값」으로 좁혔다. **상한도 정렬도 안 바꿨다** —
  정정된 행이 다음 tick 에 술어에서 빠지므로 모집단이 0 으로 수렴한다. 수리 전 red 선확인.
- **[BL-733]** `_reversal_bucket_at_fill` 판정을 **재사용**해 반전이 증명된 leg 만 refresh 를
  예약한다(`unmeasured_*` 는 스윕에 맡긴다 — 늦는 쪽이 틀리는 쪽보다 낫다).
  ★**실행측 게이트(`not_reduce_only`)에는 테스트가 0건이었다** — 통째로 지워도 31 passed 였다.
  완화하는 쪽만 재고 막는 쪽을 안 재면 다음 사람이 필터를 지운다. 그 그물도 함께 신설.
- **[BL-730]** FE 리터럴 **5벌**을 `features/backtest/cost-defaults.ts` 하나로 모았다.
  이미 맞던 3벌까지 모은 이유 = 안 그러면 다음 조정에 **같은 3/5 문제**가 난다.
  stress 프리셋 격자는 기본값이 **격자 밖**이었다(최저점 0.0005/0.0001) → 1x/2x/4x 로 재정렬.
- **[BL-729]** 실측점 포함 격자로 재판독 — 실측 `−7.74%` vs 저장값 `−22.59%`.
  **[BL-724] 판정 유지**(비용 축으로 구제되지 않는다). ★부수 = 왕복 비용 **2.17배**에 손실
  **2.92배**로 **비선형**이라 선형 외삽은 틀린다. 파생 = **[BL-740]** — 9 cell 전부 `sharpe=0` 인데 `is_degenerate=False` 다.

**다음 행동 = ⑴ 소크 창을 1회 판독하고 ⑵ 하네스를 실전 회차에 처음 쓴다.**
(~~옛 「소크가 6h33m 을 넘겨 사는지 관측한다」~~ → 2026-08-15 harness-readopt 로 교체 — 소크는 ⑴ 로 살아 있다.)

⑴ `115b33ff` = 2026-08-14T16:29Z 기동, 창은 그 시각부터. `ssh truewords-oracle 'bash -lc "cd ~/quantbridge && tools/scripts/soak-gate.sh --no-collect"'` → **C3 실격 0** 유지 확인(★`bash -lc` 필수).
⑵ **순차 step 러너가 되돌아왔다**([ADR-033]; [ADR-030] 은 Superseded **아님** — 뒤집힌 것은 재개 조건 ⑴ 하나).
첫 회차는 **저위험 BL 1건**. ★착수 전 `.harness/README.md` **§2·§4 필독** — as-is 위험 7종이 살아 있고([BL-742])
막는 것은 코드가 아니라 규약 2개(워크트리 전용 · `--push` 금지)다. 위험 7 도래 시 **고치지 말고 뗀다**.
남은 이월 = **A3 반전 의미론 감사**([ADR-032] 163곳/12파일) 하나뿐이다. 그 뒤 ⓪ 표에서 고른다.
...
| **AE** | [BL-741] `conftest` 의 `create_all` 스키마 위에서 **새 migration 이 `DuplicateTable` 로 죽는다**                                                                                                                                                                            | P2  | ★★   | 낮     | S (1-2h)   | 소량           | ★CI(fresh DB)는 안 걸리고 **로컬에서 pytest 를 돌린 개발자만** 걸린다 — 증상이 「내 migration 이 깨졌다」로 보인다                                                                                                                                                                                                                                             |
