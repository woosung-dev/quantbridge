...
**다음 행동 = harness 파일럿 판정을 사용자와 함께 낸다 — 승격(ADR-029) / 삭제(+tombstone) / 3회차 중 하나.** `harness/README.md` §5 의 2회차 합산 표가 판정 근거다. ★2회차 모두 불일치 0 이지만 그건 **결정적 AC 만 준 결과**라 원래 질문(자기채점이 뚫리는가)은 **아직 안 물어졌다** — 3회차를 고른다면 조건은 ⑴ 실행기를 claude 로 되돌려 축 분리 ⑵ 확률적·비결정적 AC 를 일부러 섞기다(step 파일은 실행기와 무관해 저작 비용 0). soak-gate 판독은 유지.
