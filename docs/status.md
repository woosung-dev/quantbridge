...
→ **2026-08-17 착수·완주.** 아래 §sprint-parallel-lanes 참조.

---

## ★2026-08-17 sprint-parallel-lanes — 3레인 병렬(herdr) · [BL-779]/[BL-780]/[BL-781]/[BL-773]

**무엇을 했나** — ACTIVE 8건 중 4건을 3레인으로 나눠 동시에 돌렸다. 자리 = 메인 체크아웃(슬롯 0) +
워크트리 슬롯 1·2, 워커 = A codex / B·C claude, 조정·판정·변이·종결은 CONTROL.
PR **#652**(B, `51ab405d`) · **#651**(C, `d28bf28f`) 머지 · **#650**(A) 진행.
★**이월 부채 1건 흡수** — #649 의 유예 원장이 미청산이었는데 `final-gates.sh:62-70` 브랜치 가드가
**모드 분기 앞**에 있어 main 에서는 `--deferred-only` 자체가 rc=1 이다(청산 경로 부재). 이 회차 3레인이
그 9종의 실질 축을 다 돌렸으므로 커버로 보고 로컬 원장을 지웠다. 게이트 설계 갭은 남아 있다.

★**착수 preflight 는 전건 일치했다**(HEAD·ACTIVE 8·700줄·좌표 5/5·원장 6/6·BE 4749) — **반증 0건**은 드문 일이다.
회고 전문은 [`lessons.md`](lessons.md) · 한 줄 요약은 `dev-log/INDEX.md`. 다음 세션이 쓸 넷만 남긴다:

⑴ ★★★**표적 검증을 전건 통과한 구현에 전량 회귀 5건이 있었다.** 표적 37 passed + 변이 **5/5 red**
를 통과했는데 전량 BE pytest 는 5 red — 원인은 프로덕션 결함이 아니라 **낡은 mock fixture** 라
표적 테스트로는 구조적으로 안 보인다. **2단 게이트가 아니었으면 그대로 나갔다.**
⑵ ★★★**AC 문턱을 실측 없이 적은 것이 둘** — AC-5 「40% 감축」은 도달 불가(RESOLVED 실제 비중
**21.2%**, 레인 B·CONTROL 독립 일치) · AC-6 은 공허참(신규 service mutation 메서드 **0개**).
⇒ **대상 크기를 재기 전에 숫자를 적으면 AC 가 검증이 아니라 장식이 된다.**
⑶ ★★**원장 처방에 없던 변이가 구멍을 찾았다** — `set_current_version` 을 no-op 해도 **1154 passed**
(포인터 커버리지 0 → 제출마다 버전 행 폭증). 레인이 닫은 뒤 CONTROL 이 **재삽입해 red 확인**.
⑷ ★★**적대 리뷰는 claude 가 codex 산출을 봤다**(자기 승인 고리 차단) — P1 0 · P2 2 · P3 6 에 **반증 5건**을 함께 냈고 상위 3건 코드 대조로 **phantom 0건**.

**다음 행동 = ⓪ 표에서 고른다. 가장 강한 후보는 [BL-783] — P1 · Stress Test 재현성이다** —
[BL-773] 이 닫은 것과 **같은 결함의 네 번째 소비자**이고, 처방은 이미 검증된 optimizer 축의 복제다.
★**원장이 「3경로」라 적었는데 `CONTEXT.md` 는 이미 Stress Test 를 소비자로 명시하고 있었다** —
처방이 헌법보다 좁았다. ★고르는 것은 사용자다.

> **강등 tombstone (2026-08-17 sprint-parallel-lanes · 700줄 상한).** 08-16 layout-alignment ·
> tool-version-ssot · beta-cutover 마감 4블록(20줄)을 이 다섯 줄로 압축했다.
> 원문 = `git show d28bf28f:docs/status.md` (356~375행). 발견 색인 = `dev-log/INDEX.md`. 살아남을 셋:
> ⑴ **[BL-781] 은 layout-alignment 가 발견만 하고 우회로 돌았다** — 증명은 2026-08-17 [BL-780/781] 회차 몫이었다.
> ⑵ **[ADR-036] 의 「러너는 Make 유지」는 낡았다** — 같은 날 `mise.toml` 이 러너까지 흡수했다(Makefile 삭제).
> ⑶ ★**codex 적대 리뷰 P1 이 그 회차 자신의 수리가 만든 결함이었다** — 한도 **바깥**의 신원 미들웨어가
> JWKS 를 쳐 미상 kid 10회에 fetch **11회**(음성 캐시 0). 수리 후 11→0.
...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --- | ---- | ------ | ------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **S**  | ~~[BL-725] `exchange_exits` **중복 290행** — 같은 uid 에 계정 행이 둘~~ → **⏳ 2026-08-15 DEFERRED 재분류 — 트리거 미도래**                                                                                                                                                                                                    | P2  | ★    | 낮음   | S (30분)      | 0줄            | 미도래(동승) — 신규 적재는 08-08 에 멈춘 잔재. `exchange_accounts` 행 축 결정 대기                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| **AW** | [BL-783] **Stress Test 도 실행 시점의 mutable Pine 을 다시 읽는다** — PR #650 이 닫은 것과 같은 결함이 네 번째 소비자에 남았다                                                                                                                                                                                                 | P1  | ★★★  | 중     | M             | **건드림**     | ★2026-08-17 적대 리뷰 + CONTROL 코드 대조 — `stress_test/service.py:326` 이 현재 Strategy 를 읽고 `:360·:380·:427` 이 그걸 엔진에 넘긴다(`strategy_version` 참조 **0건**). `StressTest.backtest_id` 가 부모를 참조하므로 구조가 optimizer 와 같다. ★★**원장이 「3경로」라 적었는데 `CONTEXT.md` 는 이미 Stress Test 를 소비자로 명시하고 있었다 — 처방이 헌법보다 좁았다.** 처방은 PR #650 의 optimizer 축 복제                                                                                                                                                                                                                                                                                                           |
| **AX** | [BL-784] `e2e authed` 레그가 **게이트 실행에서만 비결정적으로 red** — 기전 미확정                                                                                                                                                                                                                                              | P2  | ★★   | 낮     | S             | 0줄            | ★★**가설 2건이 실측으로 반증됐다** — ⑴ `force:true` 하이드레이션 기전(force 그대로 둔 채 통과) ⑵ pytest 가 시드를 지운다(데이터 의존 4지점 전부 통과). 남은 후보는 부하 지연이지만 **실패 시점 trace 가 없어 못 좁힌다.** ★**증상이 2026-08-10 프로덕션 사고(브라우저가 submit 미발화)와 같은 모양**이라 코드 쪽으로 오인하기 쉽다 — 이 회차가 실제로 오판했다. ★★**왜 안 보였나 — 그 레그가 안 돌았다**(`has_fe ∪ has_be`, #651·#652 는 둘 다 0 이라 skip). ★첫 처방은 수리가 아니라 **`trace: 'on'` 으로 증거를 만드는 것**                                                                                                                                                                                             |
| **AY** | [BL-785] 게이트가 [ADR-036] 의 버전 SSOT 를 **우회**한다 — `final-gates.sh` 가 PATH 의 `pnpm`·`uv` 를 부른다                                                                                                                                                                                                                   | P2  | ★★★  | 낮     | S             | 0줄            | ★2026-08-17 실측 — 같은 워크트리·같은 명령에서 **PATH pnpm 8.15.9 rc=1 · `mise exec` pnpm 9.12.0 rc=0**(lockfile 은 `lockfileVersion 9.0`). ★**증상이 「내 PR 이 lockfile 을 깼다」로 보인다** — 실제로는 lockfile diff 0 인 브랜치도 red 다. 이 회차가 그 오인 경로를 밟았다. ★[ADR-036] 이 모은 핀을 **소비자 중 게이트만 안 따라왔다.** ★첫 처방은 pnpm 1곳 수리가 아니라 **`pnpm`·`uv`·`node` 직접 호출 전수**                                                                                                                                                                                                                                                                                                        |
