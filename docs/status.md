...
**사용자 결정 3건:** ⑴ 축 = **Beta 잠금 해제** ⑵ **밤샘 병렬 루프 중단**(FE 85%·BE 91.5%, 커버리지 축 포화) ⑶ **로그인 방식
불가침** — 기존 결정(「Cloudflare Access 유지」)과 일치하므로 **[BL-776](개방 가입)은 트리거 미도래가 맞다.**

★★**조사가 착수안을 두 번 반증했다** — ⑴ 「초대 게이트를 짓는다」는 사용자가 이미 내린 결정을 되돌리는 일 ⑵ 「waitlist 테스트를
짓는다」는 **대상이 없다**(BE 8파일 + FE `/invite/[token]` 이 이미 있다, [LESSON-111] 계열). ★★★**진짜 공백은 하나** —
