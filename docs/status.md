...
## 다음 스프린트 — **[ADR-023] 슬라이스 설계** (표적 = `direction`, `engine_only` 는 닫혔다)
...
### ★끝난 것 — `engine_only` 314 분해 (2026-08-04 후속 회차에서 **완료**)

**채널은 닫혔다. 157배 감소.** 라이브 **노출 시간** 기준(이 발산은 세션이 돌 때만 발화한다):
...
기대 **157건** 대비 관측 **1건**. `4bf679af`(163분) 단일 세션으로도 반증(기대 51 vs 0).
★**라벨 이동이 아니라 현상 정지다** — 분류기가 `None` 을 안 돌려주므로 계속됐다면
`engine_only_awaiting_trigger` 가 자랐어야 하는데 **그 series 가 `/metrics` 에 없다.**
★닫은 것은 **`#515`(BL-560 write-back 수리)가 유력** — `#516` 은 계측이라 인과 불가.
★★**귀속은 「관측 판정 불가」로 닫았다** — 두 커밋 사이에 **라이브 세션이 0건**이라 가를 데이터가
아예 없다. **다시 캐지 마라.** 결론(표적 = `direction`)은 귀속과 무관하다.
전문은 [ADR-023 §선행 측정](decisions/023-engine-state-ssot.md).

### ★사전등록 예측 (다음 회차 첫 커맨드로 판정해라 — ★**창을 함께 적었다**)

```bash
curl -s localhost:8100/metrics | grep qb_live_position_divergence
```

**예측** — `engine_only`·`engine_only_awaiting_trigger` 는 **오르지 않는다.** `direction_transient`
는 **오른다.**

★**판별력을 위한 창을 명시한다** — 창 B 발생률이 0.12/h 이므로 `engine_only` 는 **라이브 5시간
이상** 누적돼야 「안 올랐다」가 의미를 갖고(기대 0.6건), `direction_transient` 는 ≈0.5/h 이므로
**라이브 2시간 이상**이 필요하다(기대 1건). **그보다 짧은 창에서 「안 올랐다」는 판정이 아니다.**
`scripts/soak-observe.sh` 의 세션 `soak_hours` 로 노출 시간을 먼저 재라.

> ★**직전 회차의 반성** — 예측에 **창을 안 적어** 26.5분 만에 판정을 시도했고, 그 창에서는
> 두 가설 모두 0 을 예측하므로 **판별력이 없었다.** 이 레포가 이미 적어둔 규칙(「판정 지표가 그
> 창에서 발화 가능한지 먼저 계산해라」)을 **내 예측에는 적용하지 않았다.**

### 본 작업 — 표적을 `direction` 으로 옮겨 [ADR-023] 슬라이스를 짜라

★**`engine_only` 를 겨냥하지 마라 — 닫힌 채널이다.** 살아 있는 것은 `direction` 계열
하나(≈0.5/h)이고 **`position_divergence` 사망 2건이 모두 그 경로**다.

물어야 할 것: **반전 tick 에서 거래소가 못 따라오는 구간**에 무슨 일이 일어나는가.
재료 = 사망 2건(`04097fdc` T0+65분 · `a201a47b` T0+105분)의 원장 + 워커 로그
(`live_signal_position_divergence category=direction` **11줄**이 로그 창에 남아 있다) +
`direction_transient` 24건. ★사망 2건은 이미 [BL-589]/[BL-590] 에서 부검했으니 **그 두 회고를
먼저 읽어라** — 같은 것을 다시 파지 마라.
