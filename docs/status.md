...
**전문 강등 → [`dev-log/2026-08-11-bl703-partial-verdicts.md`](dev-log/2026-08-11-bl703-partial-verdicts.md)**
([ADR-026] §3 · 2026-08-11 gate-surface 가 700줄 상한에서 내렸다).
★남기는 둘 = ⑴ **착수 근거가 반증된 것이 그 회차의 최대 산출**이었다 — 「P0 1 + P1 4 가
올라온다」 중 셋이 미도래였고([BL-003]·[BL-619]·[BL-661]) 대신 원장이 몰랐던
[BL-639]·[BL-672] 가 올라왔다. **부분 해결 라벨은 도래 여부와 무관하다.**
⑵ **함정 2종** — 판정줄의 「도래」를 **볼드로 쓰면** `\s*도래` 가 **0매치**라 요구되지도
금지되지도 않은 채 샌다 · 변이 복원에 **`git checkout` 금지**(커밋 안 된 편집을 같이 날린다.
**스냅샷 되쓰기 + sha256** 만 써라).
...
고른다. ★첫 후보 추천은 **T([BL-706])** — S·하 인데 **모든 회차의 「끝났다」 판정 4축**이
여기 걸려 있고, 고치기 전까지 이후 회차의 마감 보고가 계속 부분적으로 거짓이다. ★P1 둘([BL-641] O · [BL-438] P)은 **각자 통짜 회차**다 — O 는 닫는 조건이
...
| **T** | [BL-706] `final-gates` 신호 4종이 신선도를 안 봐 **남의 회차 파일로 초록**이 난다                                 | P1  | ★★★  | 하     | S          | 0줄                            | ★**2026-08-11 등재 — 이 회차가 자기 마감에서 밟았다.** `check_signal()`(`final-gates.sh:309`)이 `[ -s "$f" ]` 뿐이고, `docs/status.md:112` 가 **모든 회차에 같은 `--run eod` 를 시켜** 앞 회차 신호를 물려받게 만든다. 실측 — codex·화면검증·G9 를 **안 했는데 4/4 PASS**(mtime 02:53~11:39, 전부 남의 회차). ★증거는 파일 안에 있었다(`g9.ok` 첫 줄이 회차명을 적는다) — 검사기가 크기만 봤다                                                                                                                                     |
