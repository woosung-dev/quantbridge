...
**다음 행동 = `d1` 실사용 1차 루프 — 앱을 실제로 써서 전략 하나를 고른다.** PRD §5⑵ 를 전진시킨다.

★**사람이 하는 부분과 세션이 하는 부분이 갈린다.**

| 누가 | 무엇 |
| --- | --- |
| **사용자** | TradingView 에서 전략 후보 **≥5벌**을 가져와 `qb.woosung.dev` 에 임포트 → 백테스트 → 스트레스로 걸러 **1개**를 고른다. 막히는 곳마다 **우회하지 말고 적는다** |
| **세션** | ⑴ 착수 전 데이터 갭 선제 확인(OHLCV 37일) ⑵ 마찰을 `[BL-*]` **「사용 마찰」 축**으로 등재 ⑶ 루프 후 §5⑵ 지표 3종 산출 ⑷ 소크 세션 교체 판단 |

★**이 루프의 산출은 「고친 결함」이 아니라 「막힌 지점 목록」이다.** 마찰이 0 건 나오면 그것도 산출이다
— 그 경우 다음 질문은 「그럼 왜 안 쓰나」로 바뀐다. **이번엔 고치는 회차가 아니라 세는 회차다.**
