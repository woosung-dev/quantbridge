...
**K([BL-841] 가드 fail-open · ★★★ · 하)** · **L([BL-740] stress_test sharpe convention · 영속 스키마 동반)** ·
**Q([BL-845] Playwright 31개 중 1개 · 문서가 커버리지를 30배 과대로 읽게 한다)** 다.
★**T([BL-847])는 사용자 결정이 선행**한다(degraded 플래그 승격 vs 실제 timestamp 배선).
