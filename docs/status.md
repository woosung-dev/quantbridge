...
> **업데이트:** 2026-08-05 — **판별식을 확정하고, 라이브 재생의 눈을 실제로 재봤다**
> (live-replay-visibility)
>
...
## 다음 스프린트 — **[BL-595] 조건부 진입 stop 의 소유권** (= [BL-003] 의 실질 차단자)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.

### ★2026-08-05 live-replay-visibility 가 이 항목에 더한 것 (먼저 읽어라)

**⑴ 「첫 단계 = 라이브 재생을 태우는 회귀 테스트」는 폐기됐다.** 그 회귀망은 이미 있다 —
`test_run_live_*.py` **7파일 89테스트**가 `run_live` 를 직접 ~90회 호출하고, 이 항목이 바꿀
두 지점이 못 박혀 있다. **첫 단계는 테스트 신설이 아니라 「그 테스트들을 의식적으로 뒤집는
설계」다.**

**⑵ ★★★설계 제약이 하나 생겼다 — 공유 머니-패스를 고치면 백테스트 골든이 red 다.**
형 A 를 최소 모형으로 주입하니 **12건이 죽었고 그중 하나가 Trust Layer 골든**
(`test_p3_execution_metrics_match_golden[s1_pbr]`)이다. 조건부 체결이
`strategy_state.check_pending_fills` — **백테스트와 라이브가 공유**하는 자리이기 때문이다.
⇒ **대칭 수리는 `check_pending_fills` 를 바꾸는 것이 아니라 라이브 전용 경로에서만
갈라져야 한다.** 뒤집어야 할 12건 목록 =
[dev-log](dev-log/2026-08-05-live-replay-visibility.md) §3.3.

**⑶ 「MTBF 1~2.3h」를 인용하지 마라** — n=2 의 산물이고, 현재 세션이 이미 5시간 넘게 살아 있다.

### ★★★한 줄 — 고칠 것이 무엇인지 이제 이름이 있다

**엔진의 시뮬 stop 과 거래소의 resting stop 은 같은 주문이 아니다**([BL-595]).
수준도 수명도 달라서 **양쪽 모두 혼자 발화**하고, 그때마다 세션이 죽는다.
현행 코드에서 관측된 사망 2건이 **서로 반대 방향**이다:

| 세션       | entry_bar    | 엔진 체결가  | 거래소 트리거                       | 방향              |
| ---------- | ------------ | ------------ | ----------------------------------- | ----------------- |
| `cc19abd2` | 299(마지막)  | **64285.81** | **64285.80**(18:49:14 생성, 미체결) | 엔진이 앞섬       |
| `39731d57` | 281(19봉 전) | 64032.69     | 64071.9 → **체결됨**                | **거래소가 앞섬** |

⇒ **한쪽만 닫으면 절반만 막는다.** 대칭 수리 = 라이브에서 조건부 진입 체결을 **거래소 원장에만**
맡긴다. `run_live` 에 `position_epoch`/`ledger_seed_legs` 자리가 이미 뚫려 있어 배관은 있다.

★**첫 단계는 수리가 아니라 회귀 테스트다** — Trust Layer 23테스트가 `run_live` 를 **0회**
호출하므로([ADR-023] 실측) 라이브가 갈라져도 CI 가 초록이다. **라이브 재생을 실제로 태우는
테스트가 없으면 이 수리는 검증 없이 머지된다.**

### ★소크 취급 — 지금은 `FAIL` 이고 누적이 0 이다

`scripts/soak-gate.sh` = **FAIL** · C3 3건 · 누적 **0h** · **전 이력 실격 10건**(회복식 교체로
`16:24:01 phantom` 이 추가됐다). 열려 있는 귀속 구간이 마지막 실격보다
**앞서** 시작해 애초에 안 세어진다(`soak_gate_predicate.py:405`).
⇒ **지금 `down`/`pin`/`up` 을 해도 잃는 계상 시간이 0** 이다. 그러나 새 창을 여는 것은
「인지했고 **고쳤다**」는 행위이므로 **[BL-595] 수리 커밋 뒤에** 해라.

```bash
scripts/soak-gate.sh                 # ★첫 명령
scripts/soak-stack.sh status         # 고정 커밋 · 활성 세션 · main 조상 여부
docker logs quantbridge-worker 2>&1 | (cd backend && uv run python \
  scripts/classify_direction_divergence.py)          # 발산 재판정 (재무장 도장식)
```

★**판별식을 또 바꾸면 옛 아카이브를 옮겨라** — `PREDICATE_VERSION` 을 올리고
`.soak/superseded-<판>/` 로 이동([ADR-024] §아카이브 판). 안 그러면 **취소한 라벨이 게이트에
영원히 남아 개선이 반영되지 않는다**(실측: 교체 후에도 7건으로 보였다).

### ★[BL-591] 슬라이스 B 는 **판별력 0** 으로 판정됐다 — 착수하지 마라

킬 정책을 「phantom 즉시 킬 / replay_lag 절대 안 킬」로 바꿔도 **살아났을 세션이 0개**다
(사망 4/4 가 phantom · 무해 12/12 가 생존). 폐기는 아니다(D1/D2 는 도달 가능) — **보류**다.

### 그 다음 후보 (순서 없음)

**[BL-024] 실주문 leg** (자격증명·로컬 스케줄 배치 완료) · **[BL-580] 잔여 84곳** ·
**[BL-594]** Redis AOF 재기동 내성. 상세는 아래 절들에 그대로 둔다.
