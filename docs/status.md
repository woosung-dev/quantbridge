...
| **AW** | [BL-783] **Stress Test 도 실행 시점의 mutable Pine 을 다시 읽는다** — [BL-773] 이 닫은 것과 같은 결함이 네 번째 소비자에 남았다                                                                                                                                                                                                | P1  | ★★★  | 중     | M             | **건드림**     | ★2026-08-17 적대 리뷰 + CONTROL 코드 대조 — `stress_test/service.py:326` 이 현재 Strategy 를 읽고 `:360·:380·:427` 이 그걸 엔진에 넘긴다(`strategy_version` 참조 **0건**). `StressTest.backtest_id` 가 부모를 참조하므로 구조가 optimizer 와 같다. ★★**원장이 「3경로」라 적었는데 `CONTEXT.md` 는 이미 Stress Test 를 소비자로 명시하고 있었다 — 처방이 헌법보다 좁았다.** 처방은 [BL-773] optimizer 축 복제                                                                                                                                                                                                                                                                                                             |
