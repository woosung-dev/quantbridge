...
⑵ [BL-853] 어휘 이원화 — **라이트에서 다크 그림자**(`--shadow` 가 `.dark` 에서 재정의 안 됨)부터
⑶ [BL-852] 스크롤 표 28곳 이전 + `/onboarding` 진입 링크 0건
...
| **X**  | [BL-853] 컴포넌트 어휘가 **둘로 갈려 있다** — 버튼 48/38px · 반경 6·10/12px · **라이트에서 다크 그림자** | P2 | ★★ | 중 | M | 0줄 | **눈에 보이는 결함 중 최대.** `--shadow` 가 `:root` 에만 있고 `.dark` 에서 재정의되지 않아 **라이트 카드가 `rgba(0,0,0,.55/.8)`** 를 쓴다(같은 페이지 shadcn 카드의 약 10배). ★**KITPORT 는 `globals.css` 978~1888**(`_kit.html` 과 주석까지 대조) — 오버라이드는 **1888줄 아래**다. ★★**「그림자 라이트값부터가 싸다」는 함정**(2026-09-06 반증): `--shadow` 는 176행으로 KITPORT 밖이지만 `design-canon-tokens.test.ts:69` `NON_COLOR_TOKENS` 가 캐논 `:root` 와 **값 동일**을 단언한다. 프로토타입도 함께 옮기든가 `.dark` 재정의로 가든가 **먼저 정해라** |
| **Y**  | [BL-852] 스크롤 표 **28곳**이 아직 키보드로 도달 불가 (34곳 중 6곳만 수리) | P2 | ★★ | 하 | M | 0줄 | 2026-09-06 스윕이 **실제로 걸린 6곳**만 `TableScrollRegion` 으로 옮겼다. 나머지 28곳은 그 라우트·폭이 안 걸렸을 뿐 같은 결함이다(데이터가 늘면 발화). ★기계적이지만 **라벨은 사람이 정한다** — 탭이 멈췄을 때 읽히는 이름이라 「표」 같은 총칭은 고친 값이 없다 |
