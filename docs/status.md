...
> **업데이트:** 2026-08-06 — **backtest-reality-gap 회차: 백테스트↔라이브 원장 첫 대조 — 격차에 처음 숫자가 붙었다**
>
> **★한 줄 — 병목은 비용도 체결가도 아니라 「진입 집합」이다.** 같은 전략·같은 창
> (08-05T00:00Z~08-06T11:00Z)에서 백테스트 **210 진입** vs 라이브 **84 진입**, ±3봉 느슨
> 매칭 **34쌍(40%)**. 매칭쌍의 진입가 잔차는 중앙 **0.014%**(겹치면 거의 같은 값) — 그런데
> **백테스트가 예측 못 한 라이브 진입 46건의 net −92.81 = 창 전체 손실(−149.85)의 62%**.
> 신규 **BL-604**(P1, 동인 분리 실험 = 300봉 롤링 재생 재실행). 4단 워터폴(매칭 34, 감도):
> 기대 gross −8.84 → 가격 격차 +28.35 → 비용 −69.95 → 확정 net −50.45.
>
> **★비용 축은 원장에서 닫혔다** — 라이브 실효 = **taker 0.055%/leg 단일 성분**(84 event 중
> 77 이 8자리 일치, 펀딩 가설은 2×2 표로 반증), 왕복 **0.1101%**. 백테스트 기본 가정(왕복
> 0.30%)은 그 **2.7배** — 신규 **[BL-603]**. 이 창의 라이브 Σgross +28.27 vs Σ비용 −172.83:
> **수수료가 전략 손익의 6.1배**다.
>
> **★사전등록 4관측 판정: ①②④ 적중 · ③ 미판정.** 스팟/perp 봉-매칭 144쌍 **전건 스팟이
> 위**, 중앙 **+29.95 USDT**(예측 [25,42] 적중 — 1h 실측이 1m 재현) ⇒ **[BL-535] Resolved**.
> ③(비용 설명 비율)은 판정 축(strict) 이 **구조적 공집합**(라이브 key 의 bar_epoch=장전봉 vs
> 엔진 진입=교차봉 · 반전 수량 2배)이라 미판정 — ★eval2 가 ③의 판별력 자체를 반증했다
> (FAIL 영역이 물리적으로 원격 — **사전등록 지표의 판별력을 등록 전에 검증하라**가 교훈).
>
> **★측정이 스스로 교정된 궤적이 이 회차의 방법론 산출물이다.** codex 정적 2회 + Opus
> 실행형 2회 + qa 화면 + CONTROL 재측정의 6층이 각자 다른 결함을 잡았다: exits 덤프
> **정확 2배 중복**(86 event=172행, BL-605) · 귀속 off-by-one(FIFO 가 직결 링크와 59/86
> 불일치 — **가격 격차 부호가 +19.36→−0.36→+28.35 로 두 번 뒤집혔고**, 최종값은 행별
> 판별자 `entry_price_agreement` **80/81 exact** 로 검증) · **합계 닫힘은 귀속의 옳음을
> 증명하지 않는다**(틀린 귀속도 파티션이면 닫힌다 — 실증).
>
> **★qa 실측(BL-526 표면):** 활성·종료 세션 모두 도달 · NaN/콘솔 에러 0 · 세션 축 빈 것은
> 데이터의 사실(예측 적중). 발견 3건 등재 — 경고 스코프 맹목 [BL-606] · Decimal 오버플로
> 55노드 중 17 [BL-607] · e2e 0건 [BL-608]. 부수 BL-609 — herdr claude 워커 모델 상속
> (사용자가 중간 발견, Opus 재기동으로 교정).
>
...
> ---
>
> **★이전 회차 (ci-diet): PR #548/#549/#550 3연속 머지 · CI 23~25분 → 14.8분 · 저장소 public 전환**
>
> **★목표 12분은 미달이고, 그 이유가 이 회차의 가장 값나가는 결과다.** 뿌리는 코퍼스
> 「첫 접촉」 파싱 비용이다 — `test_ast_classifier[i3_drfx]` 가 **단독 42.66s** 인데
> **전체 스위트 안에서는 4.58s** 다. **프로세스 전역** 비용이라 쪼개면 샤드마다 중복되고,
> CI 3샤드 합 **1796s** vs 단일 **1278s** 의 **+519s 전부**가 이 중복이다. 재분배로도 못
> 내려간다(샤드 a 에 `i3_drfx` 소비 파일이 9개 더 있다) ⇒ **14분 아래는 첫-접촉 비용 자체를
> 고쳐야 한다**, 신규 [BL-598].
>
> **★내가 틀린 것 넷** — ⑴ 「고정 오버헤드 F≈305s」 모델(샤드 b 로그가 반증 — 70 테스트에
> 615.42s 인데 top-10 만 596s ⇒ 오버헤드 ~20s) ⑵ 로컬 `--durations` 로 잡은 샤드 경계가
> **2.2배** 빗나갔다(추정 385s → 실측 **847s**. 그 목록은 「누가 먼저 돌았나」의 함수다)
> ⑶ 「sprint32 는 tier46 에 포함」 — **4 중 1만** 겹쳤다 ⑷ 그 겹치는 1개마저 **다른 testid**
> 였다(`unsupported-card` vs `friendly-message` = `FormErrorInline` `:127` vs `:145`).
>
> **★★final-gates §5 전제는 죽은 게 아니라 휴면이었다.** 「CI 가 러너 미할당으로 `steps=0` 에
> 죽는다」를 **「이제 거짓이다」로 단정한 지 3시간 22분 뒤 재발**했다 — run `31079229987`
> (`07:25:38Z`, PR #550)에서 `backend_coverage` 와 `ci` 가 **둘 다 steps=0 · failure**.
> ⇒ 그 주석을 **휴면**으로 완화했다. **기본 skip 판단 자체는 불변**(중복 근거 = 무계측
> 298.97s vs 계측 529.16s, 배율 **1.770** — 러너 사정과 독립).
>
> **★저장소를 public 으로 전환했다.** 목적은 위 Actions 차단 해소이고 **해소 확인됨**
> (전환 후 main run `31085796101` 이 10잡 전건 success). 사전 보안감사 = 시크릿 유출 **0건**,
> 커밋 author 의 **개인 이메일 2개** 노출은 **수용**. ★부수 효과 — 「branch protection 은 이
> 레포에서 못 쓴다(private free — API 403)」는 codex 처분의 **전제가 바뀌었다**. 그 항목은
> 다시 열린다(`ci-cd.md` 정정, 켤지는 다음 회차 판단).
>
> **★사고 2건 — 소크 옆에서 일어났고 창은 살아남았다.** ⑴ **Docker 디스크 100%**
> (`05:10:27Z` short write → `05:14:04Z` 까지 `No space left on device` **7회**) 로 Postgres 가
> 죽고 **소크 워커가 11회 재시작**했다. ⑵ **redis AOF 189,403바이트 손상** — 그대로 재기동했으면
> [BL-594] 의 그 상황이다. **`BGREWRITEAOF` 로 무중단 치유**(`05:28:08Z` 에 `appendonly.aof.6`
> 신설 + 손상된 `.5.incr` 제거, redis **RestartCount 0** 불변) ⇒ **소크 창 보존**. 밤샘 회차가
> 스크래치 컨테이너에서 확인한 치유 경로의 **프로덕션 첫 실증**이다(단 자동 발동이 아니라 수동).
> ★**게이트는 이 사고를 못 봤다** — C4 표본 공백 **0건** · C5 전건 ✓ 로 창을 연속 처리한다.
> 설계대로인지(세션이 안 죽었다) 맹점인지는 **[확인 필요]**.
>
> **★★★[BL-602] 를 먼저 읽어라 — 지금 `frontend/` 안의 json 을 커밋하면 pre-commit 이 죽는다.**
...
> ---
>
> **★이전 회차 (night-watch — [ADR-025] Accepted · [BL-594]/[BL-596] Resolved · [BL-591] P2 강등)**
>
