...
T0+17분 실측 — 생존 · 평가 +16 · 주문 4건. ★**시장가 전환 경로가 실제로 발화해 체결됐다**
(`condmkt` 14:10:55, `market_converted` 10→11). 단 **대기 주문이 없던 기존 경로**다
(`breach_with_resting` 11 그대로) — 내 수리가 여는 갈래는 아직 안 밟았다.
★**`direction` 발산이 한 번 났는데 세션은 살았다** — 거래소가 `14:20:20` 에 뒤집힌 14초 뒤라
엔진이 아직 이전 봉 종가에 있던 **자기해소 skew**. 가드가 1회차를 유예했고 다음 평가에서 해소
(`direction_transient` 17→18, hard `direction` 증가 **0**). **사망 사례와 갈리는 지점이 정확히 이것이다.**
