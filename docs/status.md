...
> `tools/scripts/ledger-vitals.sh`(pre-commit 배선)가 검사한다 — ~~⓪ 표 행 **≥3**~~ → **≥1**(2026-08-25) ·
...
- ★**환경 짝 먼저** — `mise run fe-isolated` 는 **BE 를 같은 슬롯으로** 부른다. ★**포트는 슬롯에 따라 달라진다**(2026-09-06 정정): `3100 + QB_SLOT` / `8100 + QB_SLOT` 이라 **슬롯 1 인 워크트리는 3101/8101** 이다. 종전의 `:3100`/`:8100` 고정 표기는 슬롯 0(메인) 기준이고, 이 파일 아래쪽 재개 절차와 어긋나 있었다. `cat .worktree-slot` 으로 먼저 확인해라. `mise run be` 는 `:8000`
...
| 5   | [BL-838] worker 경보 무음 소실               | CONTROL | compose 4서비스 alert env 대칭 + 파생 스코프 테스트                             | **완료** — 아래 ⓻ |
| 5b  | [BL-846] 한 줄 두 호출의 `ta.*` 슬롯 공유    | CONTROL | 격리 키에 `col_offset` · 한 줄 vs 두 줄 동치 단언 · 변이 1/1 red                | **완료** — 아래 ⓻ |
| 6   | [BL-837] WS supervisor 침묵 (P1)             | CONTROL | done-callback 으로 예외 표면화 + `stop_event` set 해 lease 해제                 | **완료** — 아래 ⓻ |
...
★★**2026-09-06 정정 — ① 은 「세션 첫 명령」이 아니다.** 제로컨텍스트 검증(서브에이전트)이
이 블록을 그대로 따라가 **첫 수를 틀렸다.** 이 파일 안의 네 문장이 서로를 반증한다:
⑴ 아래 ①은 「세션 첫 명령·순서 고정」이라 말하는데 ⑵ ⓹ 표는 그 타이머가 **2026-08-11 에 이미
unload 완료**라 하고 ⑶ ⓹ 말미는 `launchctl` 을 **사전 승인 대상**으로 묶었으며 ⑷ ⓻ 착수 전 실측은
소크 게이트 **PASS** + **「여는 문이 없다」**([BL-003] 삭제로 소비자가 사라졌다)고 적는다.
⇒ **①은 소크를 실제로 건드리는 회차에만 해당한다.** 그런 회차가 아니면 **건너뛴다** —
그리고 건드릴 때도 `launchctl` 은 **사람 승인이 먼저다.** FE/UI 회차의 첫 수는 ⓻ 진입점 블록의
...
mise run ledger-vitals                                               # 원장 사활 4축 · rc=0 (pre-commit 배선)
...
★**단, 「커버리지 4쌍 → 30건」이라고 적으면 거짓이 된다** — `_harmful_scan_candidates()` 는 **여전히 4쌍**이다(in-scope ∪ 4쌍 = ∅ ∪ 4쌍). 바뀐 것은 **집합이 아니라 유도 규칙**(손으로 고른 목록 → 기계 유도)이고, `test_harmful_scan_covers_every_in_scope_census_entry` 는 in-scope 가 0인 지금 **항진명제**다. ~~판별력을 지탱하는 것은 그 옆의 `_lower_bound_is_still_covered`(4쌍)와 「결과 보고 try ≥100」 대조다.~~ → **2026-09-06 정정 — `_lower_bound_is_still_covered` 도 같은 항진명제였다.** `_harmful_scan_candidates()` 의 정의가 `in_scope | MANUAL` 이므로 두 단언은 각각 `(A|B) >= A` 와 `(A|B) >= B` 이고 **in-scope 가 0이든 아니든 집합대수로 참**이다 — 즉 이 줄은 항진명제 하나를 지목하면서 **그 대체품으로 또 다른 항진명제를 골랐다.** 실제 판별력은 「결과 보고 try ≥100」 대조 **하나**가 진다(실증: 변이 1건이 red 4건을 냈고 그중 그 테스트가 포함, 항진명제 2건은 초록 유지). 두 항진명제와 지배당한 중복 1건은 삭제했다(PR #875).
...
**2026-08-30 종결로 섹션 삭제** — 좌표 = `4b270510`) · **[BL-834]**(`convert` 가 스키마 강제·provider 선택 밖 + `sliced` 도달 불가).
둘 다 ACTIVE·도래라 ⓪ 표 후보가 **3 → 5** 가 됐다.

---

## ⓾ 2026-08-30 n14 3 lane — **종결**

> ★**강등 tombstone.** 이 자리에 있던 n14 **저작 계획 90줄**(착수 baseline 표 · lane 구성 표 ·
> 확정된 결정 3건 · 착수 전 실측 2건 · 하네스 함정 · 비목표 목록)을 지웠다. 회차가 끝났으므로
> 계획은 git 이 갖는다 — 원문 = `git show 4b270510:docs/status.md`. 재사용되는 함정은 이미
> `AGENTS.md` §5·§7 과 `lessons.md` 에 있다.
> ★아래 블록인용 5벌은 **2026-08-27 [ADR-040]~[ADR-042] 축의 회고**라 이 회차 소관이 아니다.
> 같은 규칙으로 강등할 후보지만 이 회차는 건드리지 않았다.

~~**다음 행동 = n14 3 lane 완주를 기다렸다가 통합 PR #851 을 판단한다.**~~
→ **2026-08-30 완주·머지.** lane **3/3 완주**(blocked·error 0건) · lane PR #848·#849·#850 →
통합 **#851**(`4b270510`). 주행 30분(00:41 → 01:11). 판정은 러너가 AC 를 재실행해 냈고,
**최종 검출자는 사람의 diff 대조였다**(아래 발견 2건이 그것이다).

**산출 4종** — ⑴ `ci.yml` backend 잡에 계약 drift·mypy **차단 게이트 2종**(`ruff` 다음 · `pytest` 앞 ·
`continue-on-error` 없음) ⇒ [BL-827] ⑴⑵ 종결 · ⑵ PoC 7좌표 삭제(**−2,600줄**) + 계약 파일
**+848줄** 재생성으로 엔드포인트 4종 복귀 · ⑶ `mode:"sliced"` ⇒ **LLM 왕복 0회 경로가 처음으로
도달 가능**해졌고 위저드가 정상 흐름 진입점을 얻었다 ⇒ [BL-834] ⑵⑶ 종결 · ⑷ Optimizer `var_name`
드롭다운(스윕 불가는 숨기지 않고 비활성+사유) ⇒ **[BL-833] RESOLVED — 섹션 삭제**(좌표 = 이 SHA).

★**게이트가 실제로 도는 것을 CI 가 증인으로 세웠다** — #851 의 backend 잡이 두 스텝을 포함해 green,
머지 후 main 재측정도 `openapi-check` · `mypy src` 둘 다 rc=0.

★★★**저작이 잡은 것 2건 — AC 자신이 거짓말하고 있었다.**
⑴ 잔존 검사가 `grep -r` 이라 **gitignore 된 `tsconfig.tsbuildinfo`** 를 물었다 — 삭제를 옳게 해도
**영원히 red** 였을 AC 다(`git ls-files` 순회로 교체). ⑵ vitest 에 **신규 + 기존 스펙을 한 커맨드**로
주면 존재하는 쪽만 필터로 잡아 **작업 전에 초록**이었다(스펙당 커맨드 하나로 분리).
⇒ **착수 전 red 확인은 AC 를 지키는 절차가 아니라 AC 를 검사하는 절차다.**

★★**사람 diff 대조가 잡은 것 2건** — [BL-835](변환 진입점이 `supported` 분기에만 있어 미지원 builtin 을
...
→ **2026-08-31 [BL-838] · [BL-846] 둘 다 종결.** 열린 결함 16 → 14건.

★★**원장이 적은 반경이 절반이었다 — 텔레그램은 `backend-worker` 만이 아니라 4/4 전부 무음이었다.**
`TELEGRAM_BOT_TOKEN`/`TELEGRAM_CHAT_ID` 는 `.env.example:82-83` 과 `config.py:318,326` 엔 있는데
**compose 에는 0건**이었다. `trading/alerting.py:31` 이 `AlertChannel.telegram|both` 를 fan-out 하므로
컨테이너에선 그 절반이 **항상** 죽어 있었다. 원장이 요구한 것은 「slack 대칭」이 아니라
**「alert env 대칭」**이었고, 그 문장을 글자 그대로 읽은 것이 이 발견의 전부다.
★**대칭이 반대로도 어긋나 있었다** — `backend-beat` 는 순수 스케줄러(`command: celery … beat`)라
task 를 실행하지 않는데 webhook 을 갖고, 실행하는 worker 가 못 갖고 있었다.

★**대칭 테스트의 스코프를 compose 에서 파생시켰다** — `command` 에 `celery` 가 들어간 서비스만
고르고, **파생 집합이 비면 즉시 실패**시킨다. 하드코딩 목록이었으면 [BL-843] 이 잡은 것과 같은
결함(「목록형 스코프는 파일이 사라져도 조용히 통과한다」)을 새로 만드는 것이었다.

★★★**[BL-846] 은 「줄바꿈이 백테스트 숫자를 바꾼다」였다.** 프로브로 확정 —
`pynescript.ast.Call._attributes` 는 `lineno/col_offset/end_*` 뿐이고 **`node_id` 가 없다.**
`lineno` 는 1 이상이라 항상 truthy 이므로 `node_id or lineno or id(call_node)` 는 `id()` 폴백에
**영원히 도달하지 않았다.** `z = f(close) + f(open)` 의 두 Call 은 둘 다 `lineno=5` 이고
`col_offset` 만 4 vs 15 로 갈린다. 실측 격차 = 한 줄 202.72 vs 두 줄 201.76.
★**단언을 `col_offset` 이 아니라 의미로 세웠다** — 「한 줄에 두든 두 줄에 두든 `z` 가 같아야 한다」.
구현 세부에 결합하지 않아 다음 사람이 키 구성을 바꿔도 계약이 산다. 변이(`:{col_offset}` 제거)
**1/1 red** · 기존 8건 green(그중 두 줄짜리 기존 테스트가 그대로 음성 대조다).
★★**기존 `test_different_prefixes_produce_different_ids` 가 이 결함을 못 본 이유** —
그것은 `StdlibDispatcher` 를 **직접** 부르는 단위 테스트라 **인터프리터가 실제로 서로 다른
prefix 를 만드는지**를 안 잰다. 결함은 정확히 그 사각에 있었다.

★**⓪ 표가 2행이었다** — PR #858 이 등재한 13건이 진입점 표에 **하나도 없었다**(원장 ACTIVE 는 14건).
`ledger-vitals` ② 는 **행 수 ≥1** 만 봐서 구조적으로 못 잡는다. 2 → **13행**으로 재동기화했다.

~~**다음 행동 = [BL-837] WS supervisor done-callback — 예외 표면화 + `stop_event` set 해 lease 해제**~~
→ **2026-08-31 종결.** 열린 결함 14 → 14건(BL-837 닫고 [BL-848] 등재 — 아래).

★**착수 전 소크 게이트 = PASS(rc=0)이고 창이 22.24h/24h 로 돌고 있다.** 그래서 이 회차는
**파일 수정까지만** 했다 — 컨테이너 재기동은 남은 1.76h 를 창 0회로 소멸시킨다.
배선은 다음 배포 회차에 동승한다.

★**설계에서 층을 갈랐다** — 스트림 클래스는 **관측 + `stop_event` set** 만 하고(`Settings` 를 안 쥐고
알림 책임도 안 진다), **경보는 task 층**이 낸다. `BybitAuthError` 가 이미 그 자리에서 알림을 내고 있어
선례가 있었다.

★★**원장이 안 적은 두 번째 구멍을 같이 막았다 — 첫 연결 중 crash.** 원장은 「첫 연결 후」만 적었는데,
첫 연결 중에 supervisor 가 죽으면 `__aenter__` 가 **60초를 기다린 뒤 `TimeoutError`** 를 냈고 진짜 원인은
어디에도 안 나타났다. 게다가 그 다음 줄의 `RuntimeError("stop_event set before first connect")` 는
**거짓말**이다 — stop 을 누른 사람은 없고 supervisor 가 죽은 것이다. 이제 원래 예외를 그대로 올린다.

★★**「정상 완료」로 보고하던 세 번째 거짓말도 막았다.** done-callback 이 `stop_event` 를 set 하면
`_stream_main` 의 대기가 풀리는데, 거기서 구분하지 않으면 **task 결과가 `status: "completed"`** 다.
변이 C 가 그것을 그대로 보여줬다(`{'status': 'completed'} != {'status': 'supervisor_failed'}`).

★**변이 3층 — 셋 다 정확히 겨냥한 테스트만 죽였다.**
A(콜백의 `stop_event.set()` 제거) → crash 2건 red · B(`__aenter__` 재raise 제거) → 첫연결 1건 red ·
C(task 층 분기 제거) → task 1건 red. **음성 대조 2건(정상 종료 · 외부 stop)은 세 변이 모두에서 green** —
콜백이 `CancelledError` 를 crash 로 오보하지 않는다는 증거다.

★**광역 실행이 페이크 2개를 잡았다** — `_StoppingPrivateStream`(tasks) · `_Stream`(position_fanout)이
`supervisor_error` 를 안 가져 `AttributeError` 였다. **프로덕션 코드에 `getattr(..., None)` 을 쓰지 않았다** —
그러면 진짜 인터페이스 불일치가 영원히 숨는다. 페이크가 실물을 따라가는 것이 맞다.

★**인접 코드에서 같은 종류를 하나 더 찾아 등재했다([BL-848])** — `asyncio.wait` 의 **done 집합을 `_` 로
버려** heartbeat/receive 의 예외를 아무도 안 꺼낸다. 다만 그쪽은 **자가 치유된다**(재연결이 돈다) —
잃는 것이 침묵이 아니라 원인이라 P3 다. ★`bybit_public_stream.py` 는 `create_task` 가 **0건**이라 무관하다.

★**구조 규칙 1줄을 `apps/api/AGENTS.md` §9 에 올렸다** — 「수명이 요청보다 긴 `create_task` 에는
`add_done_callback` 이 붙는다」. 이 P1 을 애초에 막았을 규칙이고, 바로 위 `track_pending_alert` 줄과 같은 층이다.

~~**다음 행동 = 개발 항목을 ⓪ 표에서 고른다** — P1 은 이제 **0건**이다. 살아 있는 13행 중 추천 순은
**K([BL-841])** · **L([BL-740])** · **Q([BL-845])** 다.~~ → **2026-08-31 사용자 결정으로 진입점의 성격이 바뀌었다** —
결함은 **후보 풀로 내려갔다**. 아래 ⓬ 참조.
...
## ⓬ 2026-08-31 방향 재설정 — **진입점이 결함에서 실사용으로 바뀌었다**

**무엇을 물었나** — 「원래 그리려던 그림 대비 어디까지 왔나」. 답을 내려고 삭제된 원문을 git 에서
복원해 현재와 대조했다: `89ff1d4e:docs/00_project/vision.md` · `89ff1d4e:QUANTBRIDGE_PRD.md` ·
`4c65bc0e:docs/roadmap.md` · 화면 캐논 `docs/design/prototypes/README.md`.

### 대조 결과 — 만드는 축은 끝까지 갔고 쓰는 축이 0 이다

| 원래 핵심가치 (2026-04-14) | 지금 |
| --- | --- |
| ① 쉬운 전략 임포트 | ✅ **초과 달성** — 원안에 없던 브리핑([ADR-040])·Python 뷰([ADR-042])·AI 생성([ADR-041])까지 |
| ② 고급 검증 (MC · WF · 슬리피지) | ✅ 기능 완성 — **단 실사용 0건**(아래 착수 전 실측) |
| ③ 실시간 검증 = 백테스트↔실제 **괴리 측정** | 🟡 **절반** — 주문 단위 체결 parity 는 깊다(`trading/outcome_parity.py` + `outcome-parity-panel.tsx`) · **성과 수준 대조는 없다** |
| ④ 원클릭 라이브 전환 | ⛔ 결정 ⑴ 로 닫힘 |

**화면** — 2세대 캐논 20종이 라우트 19개로 **전부 소진**(캐논만 있고 라우트 없는 것 0개).
미설계 잔여는 공통 설정 4종(프로필·빌링·알림센터·도움말)뿐이고 빌링은 결정 ⑵ 로 값이 0 이다.

### ★진단 — 나침반 셋을 같은 날 다 뗐고 빈 자리에 결함이 앉았다

결정 ⑴⑵⑶(2026-08-23)은 각각 합리적이었지만 **합치면 외부 신호가 0** 이다 —
돈(⑴) · 사용자(⑵) · 확장 압력(⑶). 방향을 줄 주체가 사라지자 **결함이 그 자리에 앉았다.**
결함은 로드맵의 형식을 전부 갖췄다(항상 참 · 항상 있음 · 닫으면 명확 · P1/P2/P3 우선순위).
**다만 어디로도 가지 않는다.**
...
| 축 | 실측 |
| --- | --- |
| `lessons.md` 132건의 주제 | **전부** 「내가 나를 어떻게 속였나」(검사기·측정·도구). 제품·사용자 지식 **0건** |
| ADR 42건의 성격 | 「어떻게 일하나」(문서·하네스·도구 거버넌스) **10** vs 「무엇을 만드나」(제품 범위) **4** = **2.5 : 1** |
| 최근 10일 300커밋 | 테스트 **+216,427줄** vs 소스 **+115,150줄** = **1.88 : 1** — 생산력의 ⅔ 가 이미 만든 것을 확인하는 데 갔다 |

★**AI 코딩이 이 병을 만든 것이 아니라 잠복기를 없앴다.** 사람 팀은 결함이 마르면 「그래서 다음에
뭘 만들지」를 **강제로** 묻게 된다. 여기서는 BL 번호가 **848 번**까지 갔고 **834 개를 태웠는데도**
열린 것이 14 건이다 — **공급이 안 마르므로 재설정 트리거가 영원히 오지 않는다.**

★★**거버넌스가 절반만 있었다.** ~~`ledger-vitals` 3축~~(다음행동 ≤1 · ⓪ 표 ≥1행 · RESOLVED 역류 0)은
**전부 양(量)의 게이트**이고 **방향의 게이트는 하나도 없다.** 이 파일이 한 층 아래에서 이미 같은
문장을 적어 뒀다 — 「상한만 있고 하한이 없다」(⓻ 절). **같은 병이 한 층 위에 있었고 14 회차가 통과했다.**

### 사용자 결정 — 「만든 사람이 진짜 사용자가 된다」

나침반 셋 중 **가장 싼 하나만 다시 단다.** 결정 ⑴⑵⑶ 은 하나도 건드리지 않는다.
지표 정본 = [`PRD.md`](./PRD.md) §5⑵.

### ★착수 전 실측 (2026-08-31 · 서버 원장 직독)

| 무엇 | 값 | 뜻 |
| --- | --- | --- |
| 최근 30일 백테스트 실행 | **0건** (마지막 2026-07-29) | ② 축이 33일간 안 돌았다 |
...
★★**소크의 목적을 바꿀 최적 시점이 지금이다.** 게이트가 PASS 라 「안 죽나」는 16일간 증명됐고
그 증명이 여는 문은 이미 없다. **다음 질문은 「쓸만한가」다.**

~~**다음 행동 = `d1` 실사용 1차 루프 — 앱을 실제로 써서 전략 하나를 고른다.**~~
→ **2026-09-06 UI/UX 축으로 교체**(사용자 결정). d1 은 「세는 회차」로 못박혀 있었으나
사용자가 **수리까지 진행**하도록 정했다. d1 의 **사용자 몫**(TradingView 전략 ≥5벌 임포트 →
백테스트 → 스트레스 → 1개 선택)은 세션이 대신할 수 없으므로 **그대로 남는다** — 아래 참조.

~~**다음 행동 = 화면 전수 스윕이 남긴 것을 닫는다 — [BL-851] 부터.**~~
→ **2026-09-06 [BL-851] 종결.** `pnpm e2e:design-canon` **45 passed / 0 failed**(종전 44/1).
공개 라이트 5라우트 canon 이 **전부 0** 이 됐고 래칫을 0 으로 조였다. 상세 = `lessons.md` LESSON-133. ★번호는 **2026-09-06 병합에서 130 → 133 으로 옮겼다** — 그 자리를 2026-08-25 카드(「ANTLR DFA 는 프로세스당 1회」)가 이미 쓰고 있었다(ID 재사용 금지, `AGENTS.md` §6).
⇒ **게이트가 초록이다** — 다음 회귀가 묻힐 빨강이 더 없다.

~~**다음 행동 = `d1` 실사용 루프 — 사용자가 전략 ≥5벌을 임포트해 1개를 고른다.**~~
→ **2026-09-06 같은 날 완주**(PR #874) — 아래 참조. 이 줄이 적힌 뒤 몇 시간 만에 닫혔다.
★**왜 지금이었나.** 이 축을 UI/UX 로 열어 둔 이유가 셋이었고(320px 오버플로 · 키보드로 못 읽는 표 ·
main 에서 빨간 게이트) **셋 다 닫혔다.** 남은 [BL-852]·[BL-853]·[BL-854] 는 전부 **하드 실패 0 ·
게이트 초록**인 개선 항목이라 §5⑵ 를 막지 않는다 — 실사용 마찰이 그 우선순위를 정하게 두는 편이 낫다.

★**이번 회차가 한 것 (2026-09-06 · PR 대기).** 25 라우트 × 5폭(320/375/768/1024/1440) × 2테마 =
42 레코드를 Playwright 로 실측하고, 잡힌 하드 결함을 고쳤다.

| 무엇 | 착수 전 | 수리 후 |
| --- | --- | --- |
| 320px 가로 오버플로 | authed **6 라우트 × 2테마** (+1~7px) | **0** |
| axe serious (`scrollable-region-focusable`) | **10건 / 4 라우트** | **0** |
| 모달 위에서 앱 셸 클릭 가능 | 가능(드로어 열고 상단바 hit-test = `button.hamburger`) | 불가 |
| 사이드바 계정 신원 | 하드코딩 문자열 `"계정"` | 실제 이름 + 이메일 |
| 포커스 링 누수 | **0** (원래 건강했다) | 0 |

★**축 하나는 이번에 처음 측정됐다** — a11y. 레포에 axe 의존성이 **0건**이었다.
★**남긴 것 = [BL-852](./backlog.md#bl-852)(스크롤 표 28곳 미이전) ·
[BL-853](./backlog.md#bl-853)(어휘 이원화 — 라이트에서 다크 그림자) ·
[BL-854](./backlog.md#bl-854)(z-index 스케일 문서 정합).
~~[BL-851]~~ 은 같은 날 종결했다 — 라이트 `--text-muted` 를 `#555c65` 로 옮겨 canon 을 0 으로.

~~★**d1 의 사용자 몫은 살아 있다** … 절반은 **세션이 대신할 수 없다**.~~
→ ★★**2026-09-06 반증됐다**(PR #874). 세션이 그 절반을 **대행**했다 — 온보딩 위저드가 샘플 전략을
자동 등록하므로 「TradingView 후보 ≥5벌」은 **루프의 전제가 아니었고**, 그 전제가 진입점을 6일간
멈춰 세우고 있었다. 남은 것은 「파싱 성공률의 분모」뿐이고 그것만 실제 후보 N벌을 요구한다.

★**프로덕션은 Playwright 로 못 본다**(2026-09-06 실측) — `qb.woosung.dev` 는 랜딩·`/pricing`
포함 **전 경로가 Cloudflare Access 302** 이고 레포에 Access service token 은 **0건**이다.
이번 실측은 전부 로컬 워크트리 슬롯(FE 3101 · BE 8101)에서 했다.
→ **2026-09-06 완주.** 세션이 신규 사용자를 대행해 여정을 **수리 전 3회 · 수리 후 3회** 주행했다
(계측기 = `apps/web/e2e/friction-run.mjs`, 산출 = `runs/d1-friction/`).
**임포트 → 첫 백테스트 결과 = 33.58s → 5.69s (−83.1%, median of 3 · 33.42~33.89 → 4.97~5.74)** ·
제출→화면 **30.09s → 2.36s (−92.2%)** · 신규 가입부터 첫 결과까지 **16.3s**(Journey B 1회) ·
**새 마찰 1건**(임포트 `unsupported` 배선은 코드 대조 산물이라 마찰 카운트 밖 — 같은 날 수리됨) · 수리 3건.
★**지배 성분은 코드 읽기가 지목한 것이 아니었다** — 착수 전 후보는 「온보딩 창이 격자에 안 붙어
캐시가 매번 미스」였는데, 성분 분해가 그것을 밀어냈다: 서버는 **0.82~1.19초**에 끝냈고 화면이
**30초**를 기다렸다(`progress` 요청이 주행당 **2회**뿐). 폴링이 계산의 **25배**였다.

~~★**사람이 하는 부분과 세션이 하는 부분이 갈린다.**~~ → **2026-09-06 세션이 대행했다** —
사람 몫이던 「TradingView 후보 ≥5벌」은 이 루프의 전제가 아니었다(그래서 6일간 착수되지 않았다).
무대도 `qb.woosung.dev` → **로컬**로 옮겼다: 서버 워커는 `.soak/src` **고정본**을 돌아 수리가
측정에 안 닿는다(`docker inspect` 실측). ~~**이번엔 고치는 회차가 아니라 세는 회차다.**~~
→ **2026-09-06 사용자 결정으로 「수집 + 상위 수리 + PR」이 됐다** — 「고쳤다」가 아니라
**「시간이 줄었다」**로 끝낸다는 조건이 붙었고, 위 수가 그 답이다.

~~**다음 행동 = 임포트가 `parse_status="unsupported"` 를 만들게 한다**~~
→ **2026-09-06 완주** — `_status_with_coverage()` 헬퍼 1개를 `create()`·`update()` 두 입구에 배선했다.
**FE 0줄** 변경으로 라벨·목록 필터 탭·집계 등 6곳이 살아났고, 계약 drift 0(enum·필드 불변).
헌법도 같이 고쳤다(`CONTEXT.md` — 「`unsupported` 는 enum 예약·**미사용**」이 4개월간 참이었다).
★**변이 2건 중 하나가 처음에 판별력 0 이었다** — 「파싱 실패를 덮지 않는다」 가드를 제거해도 13건이
전부 초록이었다(기존 `_BAD` 에는 미지원 함수가 없어 그 경로를 **안 지난다**). 「파싱 실패 ∧ 미지원」
입력을 실측으로 찾아 넣고서야 그 가드가 재졌다.

~~**다음 행동 = [BL-842] `TimescaleProvider` 가 조용히 좁힌 창을 요청 기간으로 라벨하는 것을 멈춘다**~~
→ ★**2026-09-06 병합에서 한 회차 미룬다**(폐기 아님 — 아래 UI/UX 축을 닫은 **직후**가 그 자리다).
그 항목의 근거는 그대로 유효하다: PRD **§5** 핵심가치가 「결과와 가정이 얼마나 정직하게 보이는가」이고
갭을 못 메운 창을 요청 기간으로 라벨하는 것은 그 **정면 위반**이다. ★**두 진입점이 같은 날 각각
세워졌다** — 이 브랜치는 UI/UX 축을, main 은 [BL-842] 를 세웠고 병합에서 만났다. **사용자가 A안
(UI/UX 를 끝까지)을 확정**했으므로 그것을 살리고 [BL-842] 를 다음 자리에 둔다.

~~**다음 행동 = 화면 리뷰의 나머지 절반을 닫는다 — [BL-856] 미열람 6화면부터.**~~
→ **2026-09-08 완료.** 5/6 을 실데이터로 열었고(`/invite/:token` 은 픽스처 0행) `ui-ux-pro-max` 렌즈로
봤다. 발견 5건을 [BL-859]~[BL-863] 으로 등재했고, 재발을 막는 산출물을 **커밋했다** —
`apps/web/scripts/app-routes.mjs`(라우트를 `page.tsx` 글롭에서 만든다) +
`app-routes-coverage.test.ts`(분류 래칫 · **vitest 라 CI 가 문다** — 변이로 red 확인).
★**가장 큰 발견** — 리포트가 「전체 원장은 **상세 보기에서** 확인합니다」라고 적어 놓고 그 화면으로 가는
링크가 0건이고, 같은 비대칭이 **부모→자식 전 방향**에 있다([BL-859]). 그리고 로그인 없이 남에게 보이는
유일한 화면이 **가정·데이터 출처를 통째로 뺐다**([BL-861]) — 랜딩이 원칙 04로 약속한 바로 그것이다.

~~**다음 행동 = [BL-853] 라이트 테마에서 다크용 그림자를 쓰는 것을 멈춘다.**~~
→ **2026-09-10 완료 — A안 셋을 다 닫았다.** 갈래는 **⑶ 라이트 오버라이드**(BL 이 적은 두 갈래 밖) —
`:root` 는 byte 그대로 두고 KITPORT 아래 `html:not(.dark)` 에서 `--shadow`/`--shadow-hi` 만 덮었다.
실측(Playwright · 로그인 6라우트): 라이트 카드 box-shadow α **.55/.8 → .06/.18**, 다크 **불변**.
회귀 = `design-canon-light-shadow.test.ts`(라이트 알파 ≤0.3 ∧ `:root` 캐논값 존치). [BL-852] 는 살아 있는 표
래퍼 **16곳 전부** `TableScrollRegion` 으로 옮겨 **종결**(잔여 12곳은 전부 스켈레톤·`aria-hidden` 이라
포커스 대상이 아니다 — 옮기면 오히려 결함) · `/onboarding` 진입 링크는 PR #874 가 이미 넣었다(`strategy-list.tsx`).
[BL-853] 의 버튼 48/38 · 반경 6·10/12 두 축은 **PARTIAL** 로 남긴다.

**다음 행동 = [BL-859] 결과 3화면에 「무엇의 결과인지」(전략명)와 부모→자식 링크를 싣는다.**
PRD **§5** 「결과와 가정이 얼마나 정직하게 보이는가」 — 리포트가 「상세 보기에서 확인합니다」라고 적고 그 링크가
0건이고, 같은 심볼 백테스트 17건 중 어느 전략인지 화면이 말하지 않는다. 같은 회차에 [BL-860](번호 0/1-based) ·
[BL-861](공유 화면 가정 누락) · [BL-862](손익 색 규칙) 을 **동승**시킨다 — 넷 다 결과 화면 3종을 같이 만진다.
PRD **§5** 가 「결과와 가정이 얼마나 정직하게 보이는가」를 핵심가치로 두는데, 그 결과를 담는 카드가
라이트에서 `rgba(0,0,0,.55)`+`rgba(0,0,0,.8)` 를 써 같은 페이지 shadcn 카드(알파 `.06`)와 **약 10배**
차이 난다. 화면 전체에 걸리는 시각 결함 중 가장 큰 단일 축이다.
★**착수 전에 갈래를 먼저 정해라**(그 항목이 함정을 적어 뒀다) — `--shadow` 는 `globals.css:176` 로
KITPORT **밖**이지만 `design-canon-tokens.test.ts:69` `NON_COLOR_TOKENS` 가 캐논 `:root` 와 앱 `:root` 의
**값 동일**을 단언한다. ⇒ ⑴ 프로토타입도 함께 옮기거나 ⑵ 라이트 `:root` 는 두고 `.dark` 재정의를
추가하거나 — **둘 중 하나를 고르고 시작한다.**

★**이 축의 범위(2026-09-06 사용자 확정 = A안)** — 남은 둘을 닫으면 이 축은 끝이다.
⑴ ~~[BL-856] 미열람 6화면 리뷰~~ → **2026-09-08 완료**(위 참조)
⑵ ~~[BL-853] 어휘 이원화 — **라이트에서 다크 그림자**~~ → **2026-09-10 그림자 축 종결**(위 참조)
⑶ ~~[BL-852] 스크롤 표 28곳 이전 + `/onboarding` 진입 링크 0건~~ → **2026-09-10 종결**(16곳 이전 · 링크는 #874)
★**[BL-859]~[BL-863] 은 이 축이 아니라 그 다음이다** — A안은 셋으로 확정됐고, 새 발견으로 범위를
늘리면 이 축이 또 안 닫힌다. 등재해 두고 A안을 먼저 끝낸다.

★**재개 절차**(서버는 세션마다 죽는다) — 슬롯 **1**(`.worktree-slot`). `mise run be-isolated` 와
`mise run fe-isolated` 를 **반드시 짝으로**(한쪽만 띄우면 `403 INVALID_ORIGIN`, [BL-781]).
FE **3101** · BE **8101**. ★**CSS 를 고쳤는데 화면이 안 바뀌면 Turbopack 캐시다** — 서버 재시작으로도
안 갈리므로 `.next` 를 치우고 다시 띄워라(2026-09-06 두 번 밟았다). e2e 는 기존 서버를 재사용하므로
**스테일 서버가 실측을 오염시킨다.**
...
| **W**  | [BL-856] 화면 리뷰가 **절반이다** — 동적 라우트 6종을 한 번도 안 열었고 그중에 백테스트 리포트가 있다 | P2 | ★★★ | 하 | M | 0줄 | **2026-09-06 등재 · 전 화면 스윕(PR #872)의 자기 실측** — 스윕이 **20 라우트**를 25 로 셌는데 실제 `page.tsx` 는 **26개**다. 빠진 6개는 전부 **id/토큰이 URL 에 필요**해서 정적 목록으로 못 만든 것들이고, 빠진 이유가 곧 **재발 조건**이다. ★재개 재료 = 백테스트 `a4971a38…`(**로그인 계정 소유** · 스트레스 2건 · share_token 살아 있음) 하나로 **6종 중 4종**이 열린다. ★`/invite/[token]` 은 **픽스처 0행**이라 못 연다. ★`ui-ux-pro-max` 축은 아직 **한 번도 안 돌았다** |
| **X**  | [BL-853] 컴포넌트 어휘가 **둘로 갈려 있다** — 버튼 48/38px · 반경 6·10/12px · ~~**라이트에서 다크 그림자**~~(2026-09-10 종결 · PARTIAL) | P2 | ★★ | 중 | M | 0줄 | **눈에 보이는 결함 중 최대.** `--shadow` 가 `:root` 에만 있고 `.dark` 에서 재정의되지 않아 **라이트 카드가 `rgba(0,0,0,.55/.8)`** 를 쓴다(같은 페이지 shadcn 카드의 약 10배). ★**KITPORT 는 `globals.css` 978~1888**(`_kit.html` 과 주석까지 대조) — 오버라이드는 **1888줄 아래**다. ★★**「그림자 라이트값부터가 싸다」는 함정**(2026-09-06 반증): `--shadow` 는 176행으로 KITPORT 밖이지만 `design-canon-tokens.test.ts:69` `NON_COLOR_TOKENS` 가 캐논 `:root` 와 **값 동일**을 단언한다. 프로토타입도 함께 옮기든가 `.dark` 재정의로 가든가 **먼저 정해라** |
| **Y**  | ~~[BL-852] 스크롤 표 **28곳**이 아직 키보드로 도달 불가 (34곳 중 6곳만 수리)~~ → **2026-09-10 종결**(살아 있는 16곳 전부 이전 · 잔여 12곳은 스켈레톤) | P2 | ★★ | 하 | M | 0줄 | 2026-09-06 스윕이 **실제로 걸린 6곳**만 `TableScrollRegion` 으로 옮겼다. 나머지 28곳은 그 라우트·폭이 안 걸렸을 뿐 같은 결함이다(데이터가 늘면 발화). ★기계적이지만 **라벨은 사람이 정한다** — 탭이 멈췄을 때 읽히는 이름이라 「표」 같은 총칭은 고친 값이 없다 |
| **Z**  | [BL-854] `DESIGN.md` §4.4 z-index 스케일이 **실제 셸과 어긋난다** | P3 | ★ | 하 | S | 0줄 | 문서는 `--z-modal: 60` · `--z-nav: 100` 인데 KITPORT `.topbar` 는 **110** · `.sidebar` 는 **120** 이다. 2026-09-06 은 증상만 껐다(`[data-base-ui-portal] { z-index: 200 }`) — **200 이라는 수가 문서 어디에도 없는 것**이 남은 빚이다. KITPORT 는 못 고치므로 **문서를 코드에 맞춰라** |
| **AA** | [BL-857] 라이트 캐논 게이트가 **텍스트×표면 몇 짝만** 본다 — 남은 미달 2건 + 잉크 계층 간격 무측정 | P3 | ★ | 중 | M | 0줄 | [BL-851] 수리의 적대 검토 산물. `--destructive-subtle`(#fae9e8)이 새 잉크로도 **5.7602** 로 미달인데 PAIRS 밖이고, **텍스트 대 텍스트**를 재는 검사가 **0건**이다(수리로 secondary↔muted 가 1.2084→1.1539, 다크는 1.3725). ★**둘은 반대 방향으로 당기므로 함께 정해야 한다** |
| **AB** | [BL-858] `status.md` 가 **자기 크기를 1/2.9 로 적고** 자기 상한(700줄)의 **1.6배**이며, 철거된 `bl-audit.sh` 를 **여러 번** 부른다(취소선 없는 인용 포함) | P3 | ★ | 하 | S | 0줄 | 2026-09-06 제로컨텍스트 재개 검증이 덤으로 잡음. `:346` 다이어트 표가 **49,845 B** 라 적는데 실제는 **약 3배**다. ★**수는 항목에 박지 않았다** — 초판이 박은 값이 이미 낡아 있었다. `wc -l`·`wc -c`·`grep -c` 로 재라. ★셋 다 원인이 같다 — `ledger-vitals` 는 **크기도 인용 대상의 생존도 안 본다**. ★비용은 **착수 비용**이다(`AGENTS.md` 가 이 파일을 필독 3종으로 지정한다) |
| **AC** | [BL-859] 결과 3화면이 **「무엇의 결과인지」를 안 싣고**, 부모→자식 링크가 **전부 없다** | P2 | ★★★ | 하 | S | 0줄 | 2026-09-08 실데이터 리뷰. 리포트·원장·옵티마이저의 `h1` 이 `BTC/USDT · 1h` / `거래 내역` / `그리드 탐색` 이고 **전략명 0건 · `/strategies/:id` 링크 0건**이다 — 이름은 편집기 `h1`(`EMA Crossover Demo`)에 있는데 안 쓴다. 이 계정 백테스트 17건이 같은 심볼이면 **어느 전략인지 알 수 없다**. ★리포트가 「전체 원장은 **상세 보기에서** 확인합니다」라고 적어 놓고 `/trades` 링크가 **0건**이다(원장→리포트는 있다). 같은 비대칭이 전략→백테스트·리포트→옵티마이저에도 있다 |
| **AD** | [BL-861] 공유 화면이 **가정·데이터 출처를 통째로 뺐다** — 랜딩이 원칙 04로 약속한 그것 | P2 | ★★★ | 하 | S | 0줄 | `/share/backtests/:token` 본문 **357자** · `가정\|수수료\|슬리피지\|데이터 출처` 매치 **0건**. 로그인 리포트는 그 섹션을 갖는다. ⇒ **맥락이 0인 외부인에게만** 약속이 안 지켜진다. 로그인 화면 원칙 04 원문 = 「가정을 모르면 수익률도 읽을 수 없기 때문입니다」. ★부수 — 리포트 `+0.48%`(초록)가 공유에선 `0.48%`(검정)로 **부호와 색을 동시에 잃는다** |
| **AE** | [BL-860] 리포트와 원장이 **같은 거래에 다른 번호**를 단다 (0-based vs 1-based) | P2 | ★★ | 하 | S | 0줄 | 같은 거래(롱 · 진입 `2026-08-23 14:00` · `77,172.60` · 「보유 중」)가 리포트 **#77**, 원장 **#76**. **컬럼명은 양쪽 다 「번호」**다. 「거래 #62 를 봐라」가 화면마다 다른 거래를 가리킨다. ★CSV 내보내기 번호는 **미대조** — 함께 봐라 |
| **AF** | [BL-862] 손익 **색 규칙이 화면마다 다르고**, 샤프는 어디서도 색이 없다 | P3 | ★★ | 하 | S | 0줄 | 최대 낙폭은 **항상 음수**라 빨강이 정보를 0 나르는데, 샤프 `-0.15`(이 실행의 가장 중요한 나쁜 신호)는 **본문색**이다. 공유 화면은 KPI 4개가 **전부 검정**. ⇒ 화면이 「초록 = 좋음」을 가르쳐 놓고 진짜 나쁜 신호에만 색을 안 준다. ★처방은 한 줄 — 「**부호가 의미를 갖는 값에만** 색을 싣는다」 |
| **AG** | [BL-863] 화면당 **주 행동이 안 보인다** — 리포트는 94% 지점, 편집기는 활성 primary **0개** | P3 | ★ | 하 | S | 0줄 | 리포트의 「이 결과로 무엇을 할까요?」가 `y=6154` / `scrollHeight` **6549**. 폴드의 유일한 코퍼 채움은 **「재실행」**(같은 계산 반복)이고 채움 버튼이 한 화면에 **3개**다. 편집기는 `.btn-primary` 셋이 **전부 disabled** 이고 유일한 전진 행동 「백테스트 실행」은 **modifier 없는 `.btn`**. 옵티마이저는 KPI 4칸 중 **2칸이 같은 값**(목표 함수가 샤프라 정의상 동일) |
