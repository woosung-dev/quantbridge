...
**직접 측정(2026-07-29 CI 재현):** BE **3411** / FE **1213**(203 파일) / ruff·mypy(212) **0** / `alembic upgrade head` fresh ✓ / **커버리지 게이트 93.14%**(기준 90) / 마이그레이션 **0**.
**이월(미재측정):** e2e authed **65** · canon **32** · CI전용 **4** · tsc · eslint — 이번 회차 `frontend/**` 변경 **0 파일**이라 결과가 구성상 불변이지만 **돌리지는 않았다.**

> ★측정한 것과 이월한 것을 섞어 적지 마라. 직전 회차가 이 줄에 e2e·tsc·eslint 를 measured 처럼 실었고 사후 리뷰가 잡았다. 이 저장소는 "핸드오프 게이트 숫자가 틀렸다" 를 이미 여러 번 겪었다.
