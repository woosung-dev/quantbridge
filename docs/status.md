...
**다음 행동 = ⓪ 표에서 다음 항목을 고른다** — CONTROL 단독 잔여가 **0** 이 됐다(러너 수리·FE 가드레일 다이어트·항진명제 제거 전부 종결). 열린 결함은 **12건**(`grep -c "^### BL-" docs/backlog.md` 실측)이고 선택은 사용자 몫이다([ADR-026] — 개발 항목은 ⓪ 표에서 고른다).
