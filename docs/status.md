...
| **P**  | [BL-844] FE 실시간이 4401 재시도에서 **같은 캐시 토큰**을 다시 보내 영구 정지 | P2 | ★★ | 하 | S | 0줄 | **2026-08-30 등재** — `lib/ws-client.ts:156` 이 캐시를 안 비우고 재연결한다. 동승 = `lib/auth-client.ts:70` 의 `clearAuthTokenCache()` 가 **in-flight fetch 를 못 막아** 로그아웃 직후 JWT 가 다시 캐시된다(세대 카운터) |
