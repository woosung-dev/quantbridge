...
### ★★★한 줄 — 죽은 원인을 고치고 **다시 켰다**. 게이트도 이제 PASS 가 가능하다

**현행 창: 세션 `39484a2c` · 고정 `0c9ccc68` · 소크는 이제 맥이 아니라 오라클 A1(도쿄)에서 돈다.**
**★실측 2026-08-07 11:38Z:** 판정 **`UNKNOWN 진행중`** · C1 누적 **2.04h / 168h** ·
C2 최장 연속 **1.97h / 24h** · C3 실격 **0건** · C4 ✓ · **C5 전건 ✓**(직독).
★**첫 명령은 언제나 `scripts/soak-gate.sh` 다** — 위 숫자는 그 시점의 값이고 곧 낡는다.
★★**서버 게이트는 반드시 `ssh <서버> 'bash -lc "…"'` 로 불러라.** 비로그인 셸은 PATH 에 `uv` 가
없어 phantom 분류기가 실패하고 **그 구간이 커버리지에서 잘려나간다**(2026-08-07 실측 8분 손실).
★**re-pin 의 대가는 C1 이 아니라 C2 다** — 누적(C1)은 이전 창을 그대로 보유하고,
리셋되는 것은 **최장 연속(C2)**뿐이다. 어느 창이든 24h **연속**을 새로 쌓아야 하므로
재고정은 늦을수록 비싸다.
★워커 로그는 이제 컨테이너 밖에 쌓인다 — `.soak/logs/worker-follow.log`([BL-619] 재관측 장치).
