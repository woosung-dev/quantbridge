...
- **bl-trigger-triage + precommit-scope** ([BL-687]·[BL-695] · [`dev-log`](dev-log/2026-08-10-bl-trigger-triage.md)) — 근거 없는 「도래」 5건을 잡은 것은 전량 스윕이 아니라 그 앞의 **음성 대조**였다.
- **migration-guard** ([BL-451] · [`dev-log`](dev-log/2026-08-10-migration-guard.md)) — 「가드가 **있다**」와 「그 경로가 그 가드를 **지나는가**」는 다른 질문이다.
...
★**2026-08-12 surface-demo-pack 로 교체됨.** 앞 회차(ledger-truth) 5단위는 전건 종결 —
전문 = [`dev-log/2026-08-11-ledger-truth.md`](dev-log/2026-08-11-ledger-truth.md) · [`…-gate-freshness.md`](dev-log/2026-08-11-gate-freshness.md).

| #   | 단위                             | 담당                | 목표 조건                                                                                 | 상태                      |
| --- | -------------------------------- | ------------------- | ----------------------------------------------------------------------------------------- | ------------------------- |
| P   | [BL-641] MTBF 재측정 + 원장 정정 | CONTROL             | self-check 2/2 · **`사망 + 절단 == n`** · 상태줄은 🟡 유지(통짜 종결 금지)                | **완료 (08-12)**          |
| 1   | E2E `screen.ok` 정상화           | codex-a (슬롯 9)    | e2e **3** / canon **42** / authed **84/84** · CI e2e pass                                 | **완료 (PR #607 머지)**   |
| 4   | [BL-427]+[BL-430] 전략 목록      | codex-b (슬롯 10)   | 신규 **17 passed** · 변이 **4/4 red** · 음성 대조 27 · 실화면 파라미터 2·2·7 = AST 오라클 | **완료 (PR #606 머지)**   |
| 2·3 | 캐논 2벌 → [BL-414]+[BL-413]     | CONTROL → codex-a   | 미착수 — 캐논 신설이 선행이고 측정 소요 합계가 예산을 넘었다                              | **보류 (사유 = dev-log)** |
| 5   | 종단 데모 리허설 + 리포트        | CONTROL (메인 전용) | 6 라우트 실화면 200 · 콘솔 error **109 → 0** · 리포트 HTML 착지                           | **완료**                  |

★**단위 2·3 은 예산 밖이다** — 측정 합계 9.7h vs 예산 5~6.5h. 캐논 2벌만 착지하면 그 자체가
[BL-413]·[BL-414] 의 차단 해제 산출물이고, 두 BL 은 `DEFERRED → ACTIVE` 로 올려 다음 회차 0번이 된다.
★**캐논은 CONTROL 몫이다** — `docs/**` 이고, `_KIT.md:29` 가 기존 `screen-*.html`·`variant-*.html`
수정을 **금지**하므로 `cp _kit.html screen-NN-*.html` 만이 열린 길이다(편집 이력 0건으로 확인).
...
PR CI 가 red** 다.~~ → **2026-08-11 에 그 단위(⓪ 표 정체성 게이트)는 머지됐다.**
★**이 회차(2026-08-12)의 순서 제약은 다르다** — 단위 5(데모)는 celery 를 타므로 **워크트리에서
못 한다**. 단위 1·4 가 **머지된 뒤 메인 체크아웃**에서만 돌린다.
...
[`dev-log/2026-08-12-surface-demo-pack.md`](dev-log/2026-08-12-surface-demo-pack.md) 에 있다.
...
사용자와 고른다.**~~ → **2026-08-12 surface-demo-pack 이 O 를 재측정했다**(위 표 3열).
★**괄호 안 지시는 이미 이행돼 있던 것을 지시했다** — `user_stopped` 분리는 2026-08-08 부터
코드에 있었다. 재측정이 낸 진짜 산출은 ⑴ 새 셈(24h 도달 **1/40** · 최장 **65.28h** · MTBF
**24.17h**, CI 는 전 쌍 겹침) ⑵ 표시 결함 1건(`절단` 열) ⑶ 그 「오염」 주장 자체의 반증이다.
...
~~★**`screen.ok` 가 이 회차의 유일한 미취득 게이트다.** … `E2E_CLERK_USER_EMAIL`·`_PASSWORD` 가
**비어 있고** authed `global.setup` 이 로그인 경로에서 **404** 로 죽는다(선재 공백). ⇒ 다음 회차의
첫 갈래는 **자격증명을 채울지, `screen.ok` 계약을 바꿀지**이고 그건 **사용자 결정**이다.~~
→ ★★★**2026-08-12 에 그 갈래 자체가 사라졌다 — 두 전제가 모두 거짓이었다.** 자격증명 5개는
**전부 채워져 있었고**(길이 62/50/22/12/62), `/sign-in` 은 **200** 이다. 404 는 **남의 앱**에서
난다 — `PLAYWRIGHT_BASE_URL` 미설정 시 `:3000` 폴백 + `reuseExistingServer` 가 그 포트의 아무 앱이나
재사용한다(실측: `:3003` = Nexus Admin, 그 `/sign-in` = 404). 해석기를 `frontend/e2e/_base-url.ts`
**1벌**로 모으고 정체성 프로브를 세워 authed **84/84** 를 얻었다. **사용자 결정은 필요하지 않았다.**
...
→ ★**2026-08-12 A회차 종료** ([BL-708] ✅ · `feat-bl708` 미푸시) — 자기신고 4/4 vs CONTROL 의 AC 16건 전건 재실행 = **불일치 0 · 재시도 0 · 저작 3분 3초 · 벽시계 46분 · $17.83 · 커밋 9**. 전문·해석·부수발견(푸시 전 `git branch -m feat/bl708` · 베이스가 main 이 아니라 `b6a0528b`) = [`harness/README.md`](../harness/README.md) §4 「A회차 실측」.
**다음 행동 = B회차 [BL-709] 를 열어라** — `index.json` step 에 `ac` 필드를 넣고 `execute.py` 가 exit code 로 판정하는 판본. A 와 **같은 조건**(가드레일 슬롯 동일 · 착수 전 동결 · `--push` 금지). ★A 의 「불일치 0」은 자기채점이 안 뚫린다는 뜻이 **아니다** — 16건 중 12건이 결정적 AC 라 그렇게 설계한 결과다. soak-gate 판독은 유지.
...
> **강등 tombstone (2026-08-12 surface-demo-pack, 700줄 상한).** 여기 있던 두 블록
> (`★bl-307-header-lint` · `★분기 = 창이 살아 있나·죽었나`, 25줄)을 지웠다 — **삭제가 아니라
> 중복 제거**다. 승격은 이미 끝나 있었다: 교훈은 [`lessons.md`](lessons.md) 의 ANTLR 파서 DFA
> 항(「parser DFA 가 데워져 있으면 나머지가 추가 비용을 안 진다」)과 header-lint 항(코드 508개 중
> **460개(90.6%)가 이미 준수**)에, 전문은
> [`dev-log/2026-08-08-zero-touch-bundle.md`](dev-log/2026-08-08-zero-touch-bundle.md) ·
> [`dev-log/2026-08-10-bl307-header-lint.md`](dev-log/2026-08-10-bl307-header-lint.md) 에 있다.
> 「down 은 리셋이 아니다」의 코드 근거는 아래 **§pin 의 비용**이 더 정확하게 들고 있다.
> 원문 = `git show 79cea10d:docs/status.md`.

### ★환경 상태 (2026-08-08 23:30Z 정리 — 다음 세션이 그대로 이어받는다)

★**2026-08-12 실측으로 교체** (종전 08-08 표는 main·서버 체크아웃 두 줄이 낡아 있었다).

| 축        | 상태 (2026-08-12)                                                                                                                                               |
| --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| main      | `79cea10d` (PR #604 머지) · 트리 clean                                                                                                                          |
| 소크      | **돌고 있다** — 세션 `de3db35a` · C1 **24h 창 1/3회** · C2 **65.50h** · 실격 0 · C5 6/6                                                                         |
| 로컬 스택 | `up-isolated` 떠 있다(db **5433** · redis **6380** · worker/beat/ws-stream). FE dev **:3100** = `<title>QuantBridge</title>` ✓ · BE **:8000** `env=development` |
...
| **U** | [BL-707] authed e2e 실패 메시지가 「API 도달 불가」를 「데이터 없음」으로 **오지목**한다                              | P2  | ★★★  | 하     | S          | 0줄                            | ★**2026-08-12 도래 — 조건절이 없다.** 이 회차가 그 오진에 실제로 걸렸다: 12건이 `make seed` 를 지시했는데 `make seed` 는 **전건 「이미 존재」**였고 진짜 원인은 BE 가 `:8100` 에 없었던 것(콘솔 `ERR_CONNECTION_REFUSED` **109건**). 고친 뒤 authed **84/84**. **「데이터가 없다」와 「데이터를 못 가져온다」는 화면에서 똑같이 비어 보인다**                                                                                                                                                                                      |
