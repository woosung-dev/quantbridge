...
> **PR #628 머지(`4b11da26`)** · Bybit demo linear perp 에 실제 시장가 1건이 프로덕션 dispatcher
> 배선을 타고 `filled` 까지 갔다 — **실거래소가 처음으로 검증됐다**. ★★★한 줄 =
> **「2층 자기정리 하네스는 지어진 뒤 10일 동안 한 번도 작동한 적이 없었다」**([LESSON-109]) —
> `pytest.skip` 이 REGISTRY 를 늘 비웠고 첫 타깃이 들어가자 청산이 **개발 DB 를 열고** 있었다.
> ★판별력의 증인은 pytest rc 가 아니라 **거래소**였다(수리 전 long 0.001 잔존 → 후 0건).
> ★차단자가 원장에 없었다 = **7일 묵은 고아 포지션**(stop 은 flat 하지 않는다). ★codex 적대 리뷰
> 3/3 참이었고 **셋 다 내 첫 수리판이 fail-open** 이었다. ★`--deferred-only` 는 docs-audit 을 안 돈다.
> 소크 재기동 완료(05:53Z, main `4b11da26`) — C2 리셋 · C1 유지.
> tombstone: 원문 = `git show 3e159691:docs/status.md` (295~370행).

## ★2026-08-14 money-path-attribution — [BL-438] 진단 교체 + 헤지 모드 기각 (강등 — 본문 = git)

> **PR #630 머지(`4d0b1a3d`)** · 한 줄 = **「미귀속 71.5% 는 결함이 아니라 내가 잘못된 DB 를 봤다는
> 뜻이었다」** — 소크 주문은 **서버 DB** 에만 있다(`filled` 654 vs 로컬 321). 서버 기준 `exact`
> 94.8%. ★원장 882행 = 고유 청산 592 + 중복 290([BL-725]). ★**진짜 구멍은 옆에 있었다** —
> 라벨은 `exact` 인데 `list_unsynced_reduce_only` 의 `reduce_only=true` 가 백필을 막아 kill-switch 가
> **6.9%(−78.02 / −1,125.81)** 만 봤다. ★헤지 모드는 실거래소 프로브로 기각([ADR-032]) — T3 이
> 공식 문서를 반증했지만(`reduceOnly` 없이 청산됨) **T4 가 도입 근거를 없앴다**(헤지 청산도
> `closed-pnl` 키가 one-way 와 동일한 19종, `positionIdx` 없음 ⇒ 귀속 이득 0).
...
단면 **0.00377%** = 현재의 **1/14.6** ⇒ **비용 축으로 구제되지 않는다.** 같은 전략(`PbR Pivot
Reversal`, 청산 551/596)의 백테스트 **4벌이 전부 PF < 1**(0.57~0.86)이고, **4벌 모두 낡은 요율**
(`0.001`/`0.0005`, [BL-603] 이 08-07 에 교체한 값)로 돌았다 → [BL-729]. FE 미러도 2곳 낡았다
(온보딩이 왕복 0.30% 를 하드코딩 submit) → [BL-730]. **판정 = 실자금 불가 · 데모 유지**
(사용자 결정: 「실매매는 진짜 최종에 최종만이다. 데모 기준으로 돌려라」).
...
배포**이고, 그때 `PbR Pivot Reversal` 누적이 −386.91(로컬)이 되므로
`KILL_SWITCH_CUMULATIVE_LOSS_PERCENT=10.0` × 잔고와 대조해 **발화 여부를 먼저 재라**.

**다음 행동 = 재-pin 전 데모 잔고를 읽고 `386.91 / 잔고` 를 10% 문턱과 대조한다** — 10% 미만이면
`soak-stack.sh down → pin → up` 으로 백필 수리를 소크에 태우고(클럭 2h 리셋), 이상이면 사용자
결정(문턱 조정 vs 소크 정지 수용)을 받는다. 그 뒤 ⓪ 표에서 고른다.
