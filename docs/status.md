...
**다음 행동 = 러너 `phases/index.json` 동시 갱신 수리** — lane 별 파일로 쪼개거나 러너가 머지 전 rebase 하게 하는 둘 중 하나. `tools/harness/execute.py` 의 `_top_index_file`(:101) · `_update_top_index` 가 자리다. **3회차 연속 손으로 푼 값이 수리 비용을 넘었다.** 동승 2건(둘 다 XS) — metric 가드 **2벌 통합**(`test_metric_safety_guard.py` ∪ `test_metric_guard_census.py`) · n8 신설 가드의 **항진명제 3개 제거**(`actual >= frozenset()`). (~~그 뒤 CONTROL 단독 잔여 = `apps/web/AGENTS.md` 373→200줄~~ → **2026-08-24 완료**, 아래.)
~~★그 뒤 CONTROL 단독 잔여 = `apps/web/AGENTS.md` 373→200줄~~ → **2026-08-24 완료 — 373→199줄**(21,587→17,092B · −20.8%). 잔여 = 러너 `phases/index.json` 수리(위 살아 있는 항목) · n8 신설 가드의 항진명제 3개 제거.
...
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ----------------------------------------------------------------------------------------------------------------- | ------ | --------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **A**  | [BL-520] 머니-패스 metric mutation **잔여 22건** — `record_metric_safely` 미적용                                                                                                                                                                                                                                                                                                                                            | P2  | ★★★                                                                                                               | 하     | S~M             | **건드림**     | ★**2026-08-24 n9 — `live_signal.py` 15건은 닫았고 `src` 전량 AST 실측으로 잔여 22건의 좌표가 확정됐다**(`tasks/trading.py` 5 · `conditional_entry_janitor.py` 5 · `_ws_circuit_breaker.py` 4 …). Trigger「실자금 cutover 전」은 결정 ⑴ 로 **발화 불가**지만 데모에서 지금 발화 가능하다 — 판정줄 자신이 그렇게 적었다 |
| **B**  | [BL-453] StrEnum `.value` 크래시 — **사용 축**(선언 축은 n9 가 닫았다)                                                                                                                                                                                                                                                                                                                                                        | P2  | ★★                                                                                                                | 중     | M               | **건드림**     | ★**2026-08-24 n9 — 사용처 가드는 이름만으로 못 세운다**(census 12건 전부 위양성). 닫으려면 **타입**을 봐야 하므로 **mypy 게이트가 선행 조건**이다(현 CI 는 `ruff` 만 잰다). ⇒ 실질은 「mypy 도입」 회차다 |
