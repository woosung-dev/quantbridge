...
> **강등 tombstone (2026-08-13 bl719-close · 2026-08-13, 700줄 상한).** O([BL-641]) 재측정 `다음 행동`
> 이력 2블록(22줄)과 `screen.ok` 미취득 블록(8줄)을 압축했다. 원문 = `git show c3a39d0d:docs/status.md`
> · `git show 11d9bdde:docs/status.md`. 요지는 `dev-log/INDEX.md` 해당 줄.
...
★★★**2026-08-15T16:33Z — 첫 24h 창이 확정됐다. C1 = 1/3회.**
`✓ 자격 획득 — 연속 24.0007h ≥ 24h · 실격 0` 을 받고 사용자 승인 아래 R0b 를 실행했다:
`git pull`(이미 최신) → `down` → `pin b5e24fbf` → `up`. **창 2 시작 = 2026-08-15T16:35:32Z**
(celery ready 기준 · 대기 65초). 재판독 = **C1 1/3회 · C2 24.0007h 유지 · C3 0 · C4 0 · C5 6/6**
— `pin` 이 C2 를 죽이지 않는다는 [ADR-024] §255 가 **실증됐다**(24.0007h 가 그대로 남았다).

★★**창 1과 창 2는 다른 코드를 잰다.** pin 이 워커를 `fb7bb772`(#633) → `b5e24fbf`(#642)로
**4개 PR 점프**시켰다(217 파일). ⇒ [BL-641] MTBF 층화에서 **같은 모집단으로 묶지 마라.**
창 1 = fb7bb772 · 창 2 = b5e24fbf 로 층을 나눠 세야 한다.

★**여기 숫자를 인용하지 마라 — 게이트를 돌려라.** ★**날짜 표기 주의**: 종전 「2026-08-16T13:50**Z**」
같은 표기는 KST 날짜에 `Z` 를 붙인 것이다(실제 UTC 는 08-15T13:50Z). 로컬·서버 시계는 **일치한다**.
...
## ★2026-08-17 auth-selfhost — **본문 강등(2026-08-16)**

★**회고 78줄을 내렸다** — `docs/status.md` 700줄 상한([ADR-026] §3, `docs-audit.sh` 강제).
무엇을: 이 회차 회고 전문(반증 4건 · 함정 3건 · 변이 결과). 어디로: git history.
원문 = `git show 9e71aa96:docs/status.md` (324~401행). 한 줄 요약 = [`dev-log/INDEX.md`](dev-log/INDEX.md).
결정 근거는 [ADR-034](decisions/034-auth-self-host-better-auth.md), 반증은 [LESSON-114] 가 정본이다.

★**아래 「환경 상태」는 남긴다** — 다음 세션이 그대로 이어받는 실측이라 회고가 아니다.

★★**강등할 때 이 파일의 살아 있는 `다음 행동 =` 을 같이 지웠다**(2026-08-16, 곧 복구).
`docs-audit.sh` 는 **≤1 만** 재므로 **0 도 통과시킨다** — 즉 「다음 세션의 유일한 진입점」이
...
> ★★**2026-08-16 layout-alignment — ~~미검증 하나~~ → 전량 검증 완료.** PR #648([ADR-035] FE
> 234파일 이동)은 `e2e:authed` 를 **89 passed / 1 failed** 로 돌렸다. 유일한 실패는 [BL-775]이고
> 그 경로는 이 회차에서 diff 0 이다 — 이번에 **원인을 하이드레이션 경쟁으로 확정**했다.
> BE pytest 는 CI 3샤드 + 로컬 361초 양쪽 통과. 정적 3종·vitest 1414/1414·design-canon 44/44·
...
read-back 은 프로덕션에서 **화면까지** 갔다 — 로그인 → `/strategies` 에 전략 3 · 백테스트 7 · 주문 1.

★**codex 적대 리뷰 P1 이 내 이번 회차 수리가 만든 결함이었다.** rate limit 신원 미들웨어를
한도 **바깥**에 세운 것이 per-user 격리의 조건인데, 그 자리에서 JWKS 를 치면 **한도의 보호를
못 받는 외부 I/O** 가 된다. 실측: `PyJWKClient` 는 미상 `kid` 에 음성 캐시가 없어 같은 가짜 kid
10회에 fetch **11회** — `Authorization: Bearer <아무거나>` 만으로 1:1 증폭이다. 수리 후 **11→0**.
...
| **AT** | [BL-780] `final-gates-test.sh` 케이스 ⑩ 의 **음성 대조가 브랜치 내용에 의존**한다 — FE 를 건드리는 회차에서 상시 red                                                                                                                                                                                                           | P3  | ★    | 낮     | S             | 0줄            | ★게이트는 옳고 **하네스가 틀렸다** — 양성 대조는 `PROBE_SRC` 로 합성인데 음성 대조만 실행 시점 diff 에 맡겨져 있다. ★**CI 에서는 안 드러난다**(`origin/main` ref 부재로 `CHANGED=""` → 우연히 성립). 로컬에서만 red 라 로컬 게이트 신뢰만 깎는다. ★「필수 아님」이 한 번도 안 나오는 상태로 두면 판별력 0 이다                                                                                                                                                                                                                                                                                                                                                                                                            |
| **AU** | [BL-781] 격리 슬롯에서 **authed e2e 가 구조적으로 불가능** — `Makefile` 이 `BETTER_AUTH_URL` 을 슬롯 포트로 안 맞춘다                                                                                                                                                                                                          | P2  | ★★★  | 낮     | S             | 0줄            | ★`grep BETTER_AUTH Makefile` = **0건**. 앱은 `:3100` 인데 값은 `:3000` → 브라우저 로그인 전건 **403 INVALID_ORIGIN** → authed 88건 미실행. ★**두 겹으로 가려져 있었다** — curl 은 Origin 이 없어 200 이고, CI 는 authed project 를 안 돈다. ★[ADR-034] 가 이 레인을 깨뜨렸는데 재검증한 회차가 없었다. 수리는 Makefile 2줄                                                                                                                                                                                                                                                                                                                                                                                                |
