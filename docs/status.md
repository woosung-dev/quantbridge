...
**다음 행동 = `/harness parallel 3` 으로 아래 3 lane 을 저작하고 주행한다.**
...
| ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --- | ----------------------------------------------------------------------------------------------------------------- | ------ | --------------- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **B**  | [BL-453] StrEnum `.value` 크래시 — **사용 축**(선언 축은 n9 가 닫았다)                                                                                                                                                                                                                                                                                                                                                        | P2  | ★★                                                                                                                | 중     | M               | **건드림**     | ★★**2026-08-24 재조사 — 이 행의 두 절이 다 반증됐다.** ⑴ 「사용처 가드는 못 세운다」가 거짓 — `apps/api/tests/trading/test_no_strenum_value_access.py`(n7 PR #797, 298줄)가 **이미 있고 green** 이다(AST 부재 단언 + 자체 판별력 검사 동봉). ⑵ 「mypy 가 선행」도 거짓 — mypy 는 `.value` 축을 **못 잡으므로** AC 로 쓰면 항진명제다. ★곁다리 실측: `uv run mypy src` 는 지금 **에러 2건**뿐이라 mypy CI 편입은 별개로 값이 있다(단 살아 있는 ignore 488건을 차단 게이트로 올리는 위험 동반 — 별도 회차). ⇒ ~~**이 행은 재기술이 필요하다**~~ → ★★★**2026-08-30 재기술 완료 — 종결 후보다.** 섹션 본문이 「사용처 가드는 기각됐다 — 다시 만들지 마라」고 적고 있었는데 **그 가드가 이미 있고 green 이다**(`tests/trading/test_no_strenum_value_access.py` 18KB · 2026-08-30 전량 5,568 passed 에 포함). 위양성 12건 문제는 **스코프**(`src/trading`+`src/tasks`)와 **allowlist**(3튜플 · 사유 의무 · hit 대응 의무)로 이미 풀려 있고, 대상 필드를 이름이 아니라 `models.py` AST 로 파생하며 `_MIN_SCANNED_FILES=70` 양성 대조까지 있다. 선언 축(n9)·사용 축(n7) 둘 다 닫혔다. ⇒ **잔여는 (c) `sa.Enum` 복귀 하나**이고 그것을 걷어낼 이유가 지금 없다. **사용자 1줄 결정 = 종결(삭제) 하면 이 행이 내려간다** |
