...
> **Last Updated:** 2026-07-29 (**live-close-completeness** 종결 + 사후 3중 검증 반영)
...
## 🎯 다음 스프린트 — live-orphan-close (BL-537 먼저, 이어서 BL-536)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다.
> 시작 방법: **"다음 스프린트 진행해줘"**. 에이전트는 `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 직전 회차가 계기를 맞추고(엔진 봉 스팟→perp) 방향 불일치 가드를 넣었다. 그런데 **그 가드가 발화할 때마다 앱이 닫을 수 없는 무방비 포지션이 남는다.** 그것부터 닫고, 그 다음에 진입 유실을 재측정한다.

### ★순서를 바꿨다 (2026-07-29 사용자 결정)

직전 판이 `BL-536 + BL-537` 묶음이었는데 **BL-537 을 앞으로** 뺀다. 근거는 직전 회차가 만든 사실이다.

1. **가드가 고아를 늘린다.** fail-closed 가 발화하면 세션이 죽으면서 포지션이 남고, `close_service.close_position` 이 `session_id` 를 요구해 **앱에서 보이지도 닫히지도 않는다.** 한 세션에서 **두 번** 밟았고 하나는 `short 0.03, SL 없음, 미실현 −7.24 USDT, 9시간 방치` 였다(Risk 🔴).
2. **BL-537 은 S 다.** 작고 경계가 뚜렷하다.
3. ★**BL-537 은 BL-536 의 선행 조건이다.** BL-536 은 재측정 soak 이 필수인데, 그 soak 을 **안전하게** 돌리려면 고아를 앱에서 치울 수 있어야 한다. 직전 회차는 provider 원시 호출로 내려가야 했다(`OrderService` 우회 = Order 행 없음 · kill-switch 미평가).

### 본체

**BL-537 (P1)** 활성 세션 없이 고아 포지션 청산 — 계정·심볼 단위 청산 경로를 세션과 독립적으로 둔다.
**BL-536 (P1)** BL-522 진입 완결성 — 계기 정렬 후 유실 채널 5종을 **재측정**하고 그 크기 위에서 설계한다.

### ★설계 시 반드시 짚을 것

1. **BL-537 은 반드시 `OrderService` 를 거친다** — 원장·kill-switch 를 우회하는 청산 경로를 정식화하지 마라. 직전 회차의 provider 원시 호출은 **비상 조치였지 설계가 아니다.**
2. **재측정이 설계보다 먼저다.** `qb_live_position_divergence_total{category=engine_only}` 가 계기 정렬 후에도 유의미하게 남는지부터 본다. 남지 않으면 BL-536 은 축소되거나 소멸한다.
3. ★**계기를 맞춰도 이미 갈린 포지션은 수렴하지 않는다.** `run_live` 는 거래소 포지션을 **입력으로 받지 않는다**(순수 OHLCV 재생). 실측 — 거래소를 flat 으로 만들어도 엔진은 `short 0.0299` 를 계속 들고 있었다.
4. 진단 절차는 [`reference/live-close-diagnostics.md`](reference/live-close-diagnostics.md). **창을 안 건 비율은 읽지 마라**(§0).
5. ★**테스트를 고칠 때마다 그 테스트를 깨는 변이를 먼저 만들어라.** 직전 회차가 같은 실수를 **세 번** 했다 — 변환 함수 자기 호출 · 메타데이터 dict 만 읽기 · 간접 증거로 만족. 전부 "프로덕션 라인을 실행하지 않는 단언" 이었고 셋 다 사후 검증에서야 잡혔다.

### soak (2026-07-29 사용자 결정)

**2~3시간.** 직전 회차는 56분에 close 7건이라 판정하기엔 얇았고(50.0%→57.1% 를 "잡음" 으로 처리해야 했다), 같은 길이로 다시 돌리면 **같은 결론을 두 번 쓰게 된다.** 2~3시간이면 close 20건 전후로 채널별 크기를 가를 수 있다. 표본 30 은 목표가 아니다.

### 하지 않는 것

**BL-535**(백테스트 스팟 봉 — 별도 스프린트, 심볼 컨벤션이 전략·UI·기존 백테스트에 파급) · BL-538/539/540(P2/P3) · BL-527/528/529(P2) · 거래소 확장 · `tasks/` deepen.

### baseline (2026-07-29 CI 재현 실측, `main@004c374f` 기준)

**직접 측정:** BE **3415** / FE **1213**(203 파일) / ruff·mypy(212) **0** / `alembic upgrade head` fresh ✓ / **커버리지 게이트 93.14%**(기준 90) / 마이그레이션 **0**.
**이월(미재측정):** e2e authed **65** · canon **32** · CI전용 **4** · tsc · eslint — 직전 회차 `frontend/**` 변경 **0 파일**이라 구성상 불변이지만 **돌리지는 않았다.**

> ★**어느 커밋에서 잰 값인지까지 적어라.** 직전 회차가 "직접 측정" 이라 써놓고 이전 PR 값을 실었고 사후 리뷰가 잡았다. "측정" 이라는 라벨은 스스로를 보증하지 못한다.
> ★**CI 는 러너 전역 미할당이다**(잡이 `steps=0` 으로 죽고 main 도 동일). 판단 근거는 로컬 재현뿐 — 레시피는 [`reference/live-close-diagnostics.md`](reference/live-close-diagnostics.md) 와 dev-log 참조. `| tail` 로 파이프하면 **exit code 가 가려진다.**
