...
> **Last Updated:** 2026-07-28 (**live-entry-parity** — 라이브가 버리던 진입 절반을 되찾고 그것을 같은 자로 증명)
> **Active Sprint:** 없음 — `#493` 머지 완료. **다음 스프린트는 아래 §다음 스프린트 참조.**
> **Last Merged:** `feat/live-entry-parity` → `main@274dc645` (PR #493)

---

## 🎯 다음 스프린트 — live-outcome-parity (BL-526)

> ★**이것이 다음 세션의 유일한 진입점이다.** 별도 킥오프 파일을 만들지 않는다 — 레포 밖 파일은 검증되지 않아 실제로 틀린 소리를 한 이력이 있다(직전 킥오프가 없는 섹션을 "있다" 고 적었다).
> 시작 방법: **"다음 스프린트 진행해줘"**. 에이전트는 `AGENTS.md` 규약대로 `CONTEXT.md` + `AGENTS.md` + 본 파일을 읽고 시작한다.

**한 줄.** 11스프린트에 걸쳐 **라이브가 백테스트대로 주문하게** 만들었고 직전 회차에 닫혔다(조건부 진입 거절 43.3% → 0%). 그런데 **라이브가 백테스트대로 버는지는 어디에서도 물을 수 없다.** 그 자를 만든다.

**본체 = BL-526** (상세 8필드는 `backlog.md`). 새 엔진 코드 없이 **read-time 파생**(#471 perf-surface 선례).

**굴리는 법.** `docs/guides/generator-evaluator-pipeline.md` — **§4 G0~G8 단계** + **§7 재사용 레시피**(codex 스펙 파일 · 변이 하네스 · soak 세션 시작 · raw HMAC 오라클 · metric 판독). 게이트와 함정은 `docs/reference/gates-and-traps.md`(**§3.5 컨텍스트 예산** 포함). 구현 diff 는 **codex 가 쓴다**, 평가자는 판정만 한다. ★**G1(구현 전 수용 기준·변이 동결)과 G6(최종 적대 리뷰)를 건너뛰지 마라** — 후자는 8세션 연속 P1 을 적발했다.

### 왜 지금인가 (2026-07-28 soak 실측)

| 항목                          | 값                                                |
| ----------------------------- | ------------------------------------------------- |
| 실현손익                      | **−5.74 USDT** / 62분 (5 청산)                    |
| 체결당 수수료                 | ≈ **1.01 USDT** (0.029 BTC ≈ 1,840 USDT notional) |
| **라운드트립 수수료율**       | **≈ notional 의 0.11%**                           |
| 라이브↔백테스트 대조 컴포넌트 | **0건** (프론트 grep)                             |

**1m PbR 이 0.11% 문턱을 넘는 엣지가 있는지 아무도 모른다.** 그걸 모른 채 배선을 더 하면 "지는 전략을 더 정확하게 실행" 하게 된다.

★**단 5 청산 표본으로는 아무것도 결론지을 수 없다.** 이번 산출은 "PbR 이 이긴다/진다" 가 **아니라 그 판단을 낼 수 있는 자**다.

### ★설계 시 반드시 짚을 것 (전부 실측 근거 있음)

1. **세션 손익의 SSOT 는 `live_signal_states.total_realized_pnl` 이 아니다** — `run_live` 가 창 안 청산만 합산해 매 tick 덮어쓰므로 단조가 아니다. SSOT 는 append-only `live_signal_events`.
2. **시뮬 PnL 과 거래소 PnL 은 부호까지 다를 수 있다**(같은 청산이 gross `+1.0988` vs net `-1.0977`). **같은 누적기에 넣지 마라.**
3. **기간 정렬** — 라이브 세션은 임의 시각 시작, 백테스트는 bar 경계. 같은 창이 아니면 대조가 거짓이 된다.
4. **표본 크기를 화면에 같이 보여줘라** — 이 저장소는 이미 **−2179.68% 실행에 샤프 +0.029** 를 띄운 적이 있다.
5. 백테스트 슬리피지 모델은 **taker 고정 0.05%**, 수수료 0.1%(`backtest/engine/types.py`). 라이브 실효값과 직접 비교 가능한 형태로 놓아야 의미가 있다.

### blocking 결정 후보 — 나오면 `AskUserQuestion` 으로 물어라 (선택지 2개↑ 면 별점 표기)

1. **무엇을 "기대치" 로 삼는가** — 같은 기간 백테스트 재실행 vs 저장된 최신 리포트.
2. **표본이 작을 때 무엇을 보여주는가** — 감추는가, 경고와 함께 보여주는가.
3. **수수료·슬리피지를 어느 층에서 분리하는가** — 안 나누면 배선 결함과 전략 결함을 구분할 수 없다.

### 하지 않는 것

BL-522(진입 완결성, 별건 P1 — **첫 step 은 기계가 아니라 측정**이어야 한다) · BL-523/524/525 · 거래소 확장 · `tasks/` deepen · 옵티마이저.

### 미조사 실마리

`qb_live_conditional_reconcile_errors_total{stage="positions"}` = **3** (62분 soak). 원인 미확인.

### baseline (2026-07-28 실측)

BE **3341** / FE **1191**(201 파일) / e2e authed **65** · canon **32** / cov **93.14%** / ruff·mypy(209)·tsc·eslint 0 / 마이그레이션 0.
