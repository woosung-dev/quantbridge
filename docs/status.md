...
PRD **§5** 의 핵심가치는 「결과와 가정이 얼마나 정직하게 보이는가」이고 이것은 그 정면 위반이다.
d1 루프가 실측한 대로 캐시 상한(2026-08-25)을 넘는 창이 흔하고, 갭을 못 메우면 **사용자가 요청한
기간과 실제로 계산된 구간이 다른 채로** 리포트가 나온다. §5⑴ 의 「임포트→첫 결과 시간」을 이제
재고 있으므로, 다음 질문은 **그 결과가 정직한가**다.
