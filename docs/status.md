...
**다음 행동 = 화면 전수 스윕이 남긴 것을 닫는다 — [BL-851](./backlog.md#bl-851) 부터.**
★**왜 PRD §5 보다 먼저인가.** §5⑵ 는 「앱을 실제로 써서」 나오는 지표이고, 이번 스윕은 그 앱이
**320px 에서 문서를 넘기고 키보드로는 표를 못 읽으며 모달 위에서 헤더가 눌리는** 상태였음을
실측했다. 남은 [BL-851] 은 **게이트가 main 에서 이미 빨간 것**이라, 닫지 않으면 다음 회귀가
그 빨강에 묻혀 §5 루프의 마찰이 「제품 결함」인지 「측정 실패」인지 못 가른다.
...
★**남긴 것 = [BL-851](./backlog.md#bl-851)(`/pricing` 라이트 래칫이 **main 에서 이미 빨강**) ·
[BL-852](./backlog.md#bl-852)(스크롤 표 28곳 미이전) · [BL-853](./backlog.md#bl-853)(어휘 이원화 —
라이트에서 다크 그림자) · [BL-854](./backlog.md#bl-854)(z-index 스케일 문서 정합).
