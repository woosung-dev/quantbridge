...
| **AX** | [BL-784] e2e 의 `click({ force: true })` 가 **하이드레이션 전에 발화**한다 — 게이트 부하에서만 red 이고 단독 실행은 항상 green                                                                                                                                                                                                 | P2  | ★★   | 낮     | S             | 0줄            | ★2026-08-17 게이트 안에서 **2회 연속** 재현(단독 --repeat-each=3 은 3/3 green). `force` 는 actionability 검사를 건너뛰어 React onClick 이 붙기 전에 떨어진다. ★**레포가 이미 알고 있었다** — `dogfood-flow.spec.ts:152` 주석이 「force 로 우회」라 적는데 실은 force 가 원인이다. ★★**왜 안 보였나 — 그 레그가 안 돌았다**(`e2e authed` 술어 = `has_fe ∪ has_be`, #651·#652 는 둘 다 0 이라 skip). 잔존 2곳                                                                                                                                                                                                                                                                                                               |
