...
| **Z** | [BL-722] `assert-main-checkout.sh` 판별력 덮개 부재 (파괴적 타깃 가드)                                                | P3  | ★    | 하     | XS         | 0줄                            | ★**2026-08-14 도래** — §8.7(「판정기에만 하네스」) 성문화 중 `tools/scripts/` 19종 스윕으로 확정. 공유 컨테이너·앱 DB·Redis 를 지키는 가드인데 하네스·`--selftest`·다른 층 테스트가 전부 없다. ★비대칭이라 P3 — 과하게 죽으면 즉시 보이고 위험한 방향은 워크트리를 「메인」으로 통과시키는 쪽뿐이다                                                                                                                                                                                                                                |
