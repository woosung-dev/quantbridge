...
| **P**  | ~~[BL-844] FE 실시간이 4401 재시도에서 **같은 캐시 토큰**을 다시 보내 영구 정지~~ → **2026-09-06 종결**(`fix/global-401-policy`) | P2 | ★★ | 하 | S | 0줄 | 4401 재시도 **전에** `onAuthFailure`→`clearAuthTokenCache`(ws-client.ts) · 동승분(세대 카운터)은 이미 코드에 있었다 · 같은 PR 이 `apiFetch` 전역 401 정책(재발급 1회 재시도 · 세션 없음 → /sign-in 1회 · single-flight)을 넣었다 — 변이 3/3 red |
