...
| **AB** | [BL-858] `status.md` 가 **자기 크기를 1/2.9 로 적고** 자기 상한(700줄)의 **1.6배**이며, 철거된 `bl-audit.sh` 를 **8번** 부른다 | P3 | ★ | 하 | S | 0줄 | 2026-09-06 제로컨텍스트 재개 검증이 덤으로 잡음. 실측 **1,132줄 / 146,414 B** 인데 `:346` 다이어트 표는 **49,845 B** 라 적는다. ★셋 다 원인이 같다 — `ledger-vitals` 는 **크기도 인용 대상의 생존도 안 본다**. ★비용은 **착수 비용**이다(`AGENTS.md` 가 이 파일을 필독 3종으로 지정한다) |
