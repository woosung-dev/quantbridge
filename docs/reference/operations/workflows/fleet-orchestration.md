...
> 화면을 띄우는 방법은 [`worktree-parallel.md`](../worktree-parallel.md) §2.3.
...
★**워커는 celery 를 타는 검증을 못 한다**(`reference/operations/worktree-parallel.md` §3). 이게 분배를 가른다.
...
다음을 순서대로 읽고 그대로 수행해라:
  1) <wt>/docs/reference/operations/workflows/fleet-orchestration.md — §1 역할 · §3 라우팅 · §7 워커 규칙
