...
> (`docs/reference/operations/ci-cd.md:276-281`).
...
4. user systemd timer/service가 SSH 종료 뒤에도 살아야 하면 `loginctl enable-linger` 상태를 확인한다
   (`docs/reference/operations/frontend-deploy.md:163-165`).
...
(`docs/reference/operations/frontend-deploy.md:158-161`). 백업 자격증명·DB 대상의 판별은 `.env`가
...
(`docs/reference/operations/frontend-deploy.md:112-131`).
...
#       (docs/reference/operations/frontend-deploy.md:163-168).
...
(`docs/reference/operations/frontend-deploy.md:140-143`).
...
FE 배포의 `--project-directory` 누락 경고(`docs/reference/operations/frontend-deploy.md:95-98`)와
`ci-cd.md`의 경고(`docs/reference/operations/ci-cd.md:283-284`)는 이 wrapper 경로에는 해당하지 않는다.
...
- 소크 게이트·유닛 함정: [gates-and-traps.md](./gates-and-traps.md) · [ADR-024](../../decisions/024-soak-stability-gate.md)
- DB 보관 책임: [ADR-033](../../decisions/033-db-hosting-self-host-timescaledb.md) · 인증 배포 보완: [better-auth-setup.md](./better-auth-setup.md)
- FE와 분리된 배포: [frontend-deploy.md](./frontend-deploy.md) · 기존 CD 표기: [ci-cd.md](./ci-cd.md)
