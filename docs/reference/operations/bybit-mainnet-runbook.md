...
★**spot 으로 한다.** perp 최소 주문 0.001 BTC 는 명목 ~$100 이고 1x 증거금이 자본을 넘는다.
$50 로 linear 를 시도하면 잔고 부족으로 실패한다 — 그건 경로 검증이 아니라 잔고 검증이다.
...
| #   | 미확인 항목                                                            | 왜 안 쟀나                 | 닫는 법                                  |
| --- | ---------------------------------------------------------------------- | -------------------------- | ---------------------------------------- |
| 1   | **BTC/USDT perp `min_qty` = 0.001** — §6 의 $3,276 이 여기 걸려 있다   | 거래소 접촉이 승인 대상    | Bybit 콘솔 또는 공개 `fetch_markets`     |
| 2   | **spot `min_qty` / 최소 명목** — §5 의 `--quantity 0.0001` 기본값 근거 | 같음                       | 같음                                     |
| 3   | **Bybit 정책 화면** — IP whitelist·출금권한 UI 의 실제 위치와 이름     | 계정이 없다                | 콘솔을 열어 화면 이름을 이 문서에 적어라 |
| 4   | **mainnet 수수료** — 데모 실측은 taker 0.055% 단일이다([BL-603])       | mainnet 계정 등급을 모른다 | 콘솔 Fee tier                            |
| 5   | **`app_env=production` ↔ soak-gate 충돌**의 실제 해                    | 이번 회차 범위 밖          | §3 마지막 문단                           |
