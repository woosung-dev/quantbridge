...
- ★★**샤드 경계를 로컬 durations 로 정하면 틀린다.** CI/로컬 비율이 **균일하지 않다** — CPU 바운드
  pine_v2·backtest 는 **5.0~5.3배**인데 `test_metric_guard_census` 는 **2.40배**고 스위트 평균은 4.27배다.
  `tests/strategy` 는 로컬 56.3% 지만 **CI 기준으로는 ~65%** 다.
