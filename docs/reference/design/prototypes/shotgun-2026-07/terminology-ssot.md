...
1. 프로젝트 구조 규약이 도메인 자산을 `features/[domain]/` 아래 두라고 정한다(`.claude/rules/frontend.md` §4 FSD Lite). 단일 전역 모듈은 이 규약의 예외를 새로 만든다.
...
3. **`strategy.backtest_count` 열의 정의.** ✅ **해소 (2026-07-23 functional-parity)** — **완료(COMPLETED) 기준으로 확정**하고 서버 필드를 신설했다(read-time GROUP BY, `StrategyListItem.backtest_count`). 화면 열이 재렌더되며 헤더 title = `STRATEGY_BACKTEST_COUNT_HINT`("완료된 백테스트 수…") 로 정의를 인쇄한다. 프로토타입 자기 감사(strat_4e18=0, 실패·대기 미포함)와 정합. (원문: 화면이 열 이름을 `백테스트` 로만 인쇄해 완료/전체 기준을 확정할 수 없었음.)
