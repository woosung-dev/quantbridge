...
- ⚠ stale-RUNNING reclaim 미구현 (Backtest 만 보유) → worker hard-kill 시 RUNNING 고착 가능. open: CF3/ASYNC-2 (Phase C, 감사 보고서 §6).
...
- ⚠ Optimization 과 동일 — stale-RUNNING reclaim + submit idempotency 미구현 (CF3/ASYNC-4, Phase C).
...
워커 crash로 stale된 RUNNING/CANCELLING 자동 회수(startup hook + beat task)는 **현재 Backtest 만 구현**(`tasks/backtest.py` reclaim_stale_running). Optimizer/StressTest 는 reclaim 미구현 — open: CF3/ASYNC-2 (Phase C).
