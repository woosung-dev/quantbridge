...
실무 상한은 12 이 아니라 **2~3벌**이다 — 한 화면에 그 이상 띄우면 읽히지 않고, `.next` 캐시가
워크트리마다 순증한다(§7). `scripts/herdr-fleet.sh` 가 3벌 + CONTROL 1 로 상한을 강제한다.
...
그래서 `Makefile` 의 `guard-main-only` 가 슬롯 ≠ 0 이면 아래 타깃을 `exit 1` 로 거부한다.
