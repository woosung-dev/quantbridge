...
>
> **활성 GCP 프로젝트 (2026-05-19 추가):**
>
> - `<PROJECT_ID>` placeholder = **`jetaime-dev`** (project number `780943117571`, billing connected)
> - 계정: `jetaime.jang@gmail.com` / region `asia-northeast3` (Seoul) / zone `asia-northeast3-a`
> - gcloud configuration: `quantbridge` (기존 `default` = `jangwooseng97@gmail.com` / `woosung-dev` 는 Kairos/TrueWords 운영 유지 — 보존)
> - 활성화된 API: `cloudresourcemanager`, `iam`, `compute`, `run`, `cloudbuild`, `artifactregistry`, `secretmanager`, `sqladmin`, `redis`, `vpcaccess`, `servicenetworking` (모두 enable 완료)
> - 향후 staging/sandbox 분리 시 `<PROJECT_ID>` placeholder 그대로 두고 env 로 주입 (예: `gcloud config configurations create quantbridge-staging`)
...
| `CLERK_SECRET_KEY`                            | **Secret Manager**   | `--update-secrets=CLERK_SECRET_KEY=clerk-secret-key:latest`                                                                                                                                | sk*live*… (prod tier)                  |
