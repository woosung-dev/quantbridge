...
| §5 No Closing Colons (Korean)       | ⚠️ 미검증 | dev-log 92 file 콜론 종결 검증 skip → Track C1 권고                                    |
| §6 Korean Header Comments           | ⚠️ 미검증 | 신규 file 1줄 주석 검증 skip → Track C2 권고                                           |
...
### C1. CLAUDE.md §5 No Closing Colons (Korean) 검증

````bash
grep -rn ':\s*$' docs/dev-log/ docs/guides/ docs/dogfood/ \
  | grep -v '```' | grep -v '\.md:.*\(http\|file:\|http\)' | head -50
````

검출 결과 LESSON 등재 후보. BL 등재 권고.

### C2. CLAUDE.md §6 Korean Header Comment 검증

```bash
find backend/src frontend/src -type f \( -name "*.py" -o -name "*.ts" -o -name "*.tsx" \) \
  -newer .git/HEAD \
  | xargs -I{} sh -c 'head -3 "{}" | grep -q "[가-힣]" || echo "{}"'
```

ESLint custom rule + ruff custom rule 후보. BL 등재 권고.
...
- **Sprint 61 분기 결정 시:** Track C1/C2 수동 검증 (또는 BL 등재 후 별도 sprint)
