...
이로써 dogfood 계획의 "수정 후에는 carry 만으로 하락하지 않는다" 는 예측도 틀렸다. carry 의 효과는 영구 손실을 보유 기간과 지표 warmup만큼의 **일시 함몰**로 바꾸는 데까지다. 상태 행 총계는 `Σ live_signal_events.realized_pnl` 원장 SSOT로 바꿔 창과 무관하게 단조로 만들었지만, 사이징 자본의 D2 일시 함몰은 BL-489로 남겼다.

## ★프로덕션 before / after 3종

★★**먼저 정정한다.** 아래 "화면" 은 정확히는 **`live_signal_states` 상태 행**이다. 최종 리뷰가 잡았다 — `router.py:474-483`(2026-07-01 dogfood)이 `GET /live-sessions/{id}/state` 응답에서 `total_realized_pnl`·`total_closed_trades`·`equity_curve` 를 **체결(`state=filled`) 주문으로 재계산해 덮어쓴다.** 그래서 사용자가 보는 화면은 원래부터 창 드리프트를 타지 않았다. 이번 상태 행 수정의 실익은 **내부 정합**과 **`equity_curve` 무한 증식 차단**(분당 1 포인트 → 청산당 1, cf. PR #478 의 무한 spread 500)이고, **사용자에게 보이는 효과는 `initial_capital` carry = 주문 수량**이다.

| 대상                   | 수정 전                                                | 수정 후                                                                                                                                                           |
| ---------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 상태 행 총계 원장 SSOT | 16:54:57Z 화면 3건·`4.78803856`, 원장 4건·`5.88683554` | 17:10:50Z 화면 4건·`5.88683554` = 원장 4건·`5.88683554`. 17:23Z에도 5건·`5.01015117` 로 2회 연속 일치했다.                                                        |
...
3. 상태 행 총계를 **"화면 총계"** 라고 불렀다. `router.py` 가 그 값을 체결 주문으로 덮어쓰므로 화면은 그것을 렌더하지 않는다. dogfood 로 확인한 "화면 == 원장" 은 **DB 상태 행**의 일치였고, 라벨이 틀렸다. 최종 리뷰가 잡아 위 절과 `status.md` 를 정정했다.
