...
## BL-480 수정 — 화면이 아는 것을 숨기지 않게

B 조사에서 찾은 세 결함 중 **가장 작고(S) 가장 급한 것**부터 닫았다. BL-478(진입 미발주)이 살아 있는 동안 이게 **유일한 발산 표면**이기 때문이다.

`combineLiveSessionPositions` 에 `divergences` 를 추가하고, `positions` 가 비었을 때 **숨기면 안 되는 판정만** 골라 세션 단위로 건져 올린다.
...
| `exchange_only`                                    | 해당 없음  | 정의상 거래소 포지션이 있어야 도달              |

★**가드를 만들 때 "무엇을 안 잡는지" 도 같이 단정했다.** `match` 와 평가-전 `unknown` 이 조용히 남는 것을 테스트로 고정 — 안 그러면 세션 시작 직후마다 경고가 떠서 진짜 발산이 노이즈에 묻힌다. 판별력은 "잡는다" 와 "안 잡는다" 양쪽에서 나온다.

**실화면 확인** (라이브 세션이 마침 그 상태라 천연 픽스처였다):

> BTC/USDT · PbR Pivot Reversal · **전략에만 열린 거래가 있습니다.** 전략 보고: **PivRevLE 롱 1** 거래소 보고 포지션은 0건입니다.

[`screenshots/2026-07-26-bl480-divergence-surfaced.png`](screenshots/2026-07-26-bl480-divergence-surfaced.png)

RED 7건(훅 5 · 컴포넌트 2) 선확인. FE **1144**(+8) · tsc · eslint 0.

**근본 원인은 그대로다.** 이건 화면이 거짓말을 멈춘 것이지 라이브 진입이 고쳐진 게 아니다 — BL-478 은 열려 있다.
