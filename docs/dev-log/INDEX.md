...
- **2026-08-05 soak-clock-restoration** — 소크를 **커밋에 고정**해 편집과 분리(음성 대조: 배너 1→2 vs 1→1). 「1주 안정」을 기계 판정으로([ADR-024]) — **세운 지 38분 만에 첫 FAIL**(`phantom` 재발 ⇒ [ADR-023] 재개 조건 충족). ★★★**같은 발산이 경과만으로 `replay_lag`→`phantom` 으로 이름을 바꿨다** — [dev-log](2026-08-05-soak-clock-restoration.md)
