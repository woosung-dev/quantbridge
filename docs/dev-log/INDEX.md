...
- **2026-08-05 live-replay-visibility** — 판별식 = **직접 회복 검사**(원장 안 봄). **집합 포함**까지 확인, **FAIL 유지 · 실격 9→10**. ★★★**전제 반증** — `run_live` 는 이미 **89테스트가 ~90회** 호출. 변이 **12/12 KILLED ⇒ 신규 0**(내 변이 2건은 **등가 변이**). ★[BL-595] 형 A 를 **Trust Layer 골든이 잡았다** — [dev-log](2026-08-05-live-replay-visibility.md)
