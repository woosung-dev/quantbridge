...
- **2026-08-08 bl003-unblock** — ★★★사망 근인은 [ADR-025] 반례가 아니라 **이중 호스트 오염**(같은 demo 계정에 서버·맥 동시): 소유권 **7/27** · 정본 항등식 **4/4**, 반사실 최대 **1/4**. ★직전 회차의 「무관하다」 반증 — 코드 결합만 보고 계정 결합을 안 물었다. ★★**CONTROL 자기 판정식 2개가 적대 검증에 반증**(판별력 0 · 항진명제). ★게이트 크래시 fail-closed. MTBF 2벌 P(168h)=**4.1e-09** — dev-log
