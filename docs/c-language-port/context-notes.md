...
## S2/S5 이식 — 콘텐츠부터 (2026-07-20)

사용자가 "프로토타입이랑 전혀 다르다"고 지적. **정정한 사실** — 그건 정상이고 의도된 상태다. S0 은 UI 를 한 줄도 안 바꿨다(검사 장치만). 그리고 **S1a(토큰)도 사실상 무변화** — 앱 다크 팔레트가 이미 코퍼(`--primary #f08c2e`)/카본(`--bg #0b0d0f`)이라 22종 중 17 일치, 불일치 5는 전부 미세값(muted 글씨 그림자·틴트 알파). 화면이 "전혀 다른" 진짜 이유는 **색이 아니라 레이아웃·간격·타이포·컴포넌트 구조** = S2(공용 CSS) + S5~S8(화면). 사용자 선택 = **S2→S5 콘텐츠(셸은 나중)**.

### S2 — 순수 이식은 불가능했다

`_kit.html` 공용 CSS 가 **캐논 토큰 이름**(`--copper`·`--ink-3`·`--r`·`--bull`…)을 쓰는데 앱은 다른 이름(`--primary`·`--text-muted`…)을 쓴다(`_kit.html:27` 주석도 "Mirrors globals.css .dark" 로 값 동일·이름 다름을 밝힘). 그래서 이식 CSS 가 작동하려면 캐논 이름이 앱에 있어야 하고 그게 S1a 의 핵심. **리네임(blast-radius 큼) 대신 `:root` 별칭**(`--ink-3: var(--text-muted)`)으로 도입 — var() 지연해석이라 한 곳 선언으로 라이트/다크 자동. context-notes 가 이미 "두 체계 공존 = 의도된 과도기"로 승인한 방식이다.

**충돌 3종 검사 후 이식** — bare 클래스 사용 0(앱은 유틸리티만, kit 은 시맨틱), 캐논 이름 참조 0(`var(--bull*)`은 전부 `--bullish`/`--bearish` 오탐이었음), bare 엘리먼트 셀렉터 0. → 소비자 0 → 시각 변화 0(design-canon-public baseline 그대로 확인).

### S5 — 정직성이 열을 줄였다

프로토타입 screen-03 표는 수익률/MDD/샤프/거래수/전략명을 보이지만 **`BacktestSummary` 스키마엔 없다**(id·strategy_id·symbol·timeframe·period·status·created_at·completed_at 뿐). 캐논 §4.9 "데이터 모델에 없는 값 = 가짜 데이터" 에 따라 **그 열을 그리지 않는다.** C 비주얼(카드·칩·mono 표·코퍼)은 입히되 실데이터 열만. "pine_v2" 칩은 내부명 노출 금지 가드(BL-265)에 걸려 "바 단위 이벤트 루프" 로. 이게 이 프로젝트의 정직성 이식 방식이다 — 목업을 베끼는 게 아니라 **C 언어를 실데이터에 입힌다.**

### S1a #1 을 S5 가 앞당겼다

C 콘텐츠가 딤 텍스트(`--ink-3`→`--text-muted`)를 다수 써서 **`--text-muted` #7a828c 가 /backtests 에서 AA 위반 22건**으로 드러났다(4.35:1 < 4.5). 22개를 안고 낼 수 없어 그 자리에서 `#8b939c` 로 교정(이미 토큰 감사·랜딩에서 검증된 값). **한 줄이 /backtests 22 + 랜딩 2 를 동시 해소**하고 두 allowlist 를 0으로, design-canon-tokens 를 5→4 로 내렸다. 실측이 토큰의 필요성을 증명한 사례.

### 테스트 계약

옛 `backtest-list-progress-cell.test.tsx`(RunningProgressBar/queued-pulse/KPI-pulse — C 가 전부 상태 칩으로 교체)를 삭제하고 `backtest-list-status-chip.test.tsx`(상태별 칩 6 case)로 대체. `backtest-list-filter.test.tsx` 의 notice testid·chip disabled 계약은 유지(재작성 시 맞춤).

### 남은 것

**셸(사이드바)이 아직 예전이라 화면 전체는 프로토타입과 완전 일치하지 않는다.** 콘텐츠 영역만 C. 전체 일치는 S3(셸) 필요 — 셸은 공유라 한 번 고치면 전 라우트가 C 사이드바가 된다. S5 잔여(error.tsx·4폭 design-canon·design-taste)는 셸 후 재점검 권장.

### 문서 토폴로지 수술 — 이동 대신 라벨링

사용자가 "레포 문서가 어떤 곳은 A, 어떤 곳은 B라 AI가 헷갈린다" 고 지적했다. 조사해 보니 **캐논 내용은 거의 결백**했다. `_KIT.md:126` 이 우선순위(화면 > `_KIT` §4 > `terminology-ssot`)를 이미 못박았고 값마다 `file:line` 이 붙어 있다. 진짜 병은 **길찾기(토폴로지)** 였다.

**진짜 원인은 세대 라벨 부재다.** 레포엔 1세대(`DESIGN.md` Precision Instrument = 현재 앱이 실제로 따르는 것)와 2세대(`_KIT.md` C 디자인 언어 = 이식 목표)가 공존하는데 **둘 다 "확정" 도장만 있고 어느 쪽을 따라야 하는지 표기가 없다.** `docs/README.md` 는 아직 "Stage 2 프로토타입 12개 화면 ✅ 확정" 이라 C 17벌의 존재조차 안 알렸다. → `docs/design-docs-map.md` 신설로 해소.

**원안 2건을 실측이 뒤집었다.**

- **"구 프로토타입을 `_archive/` 로 이동" → 철회.** `00~11-*.html` 은 죽은 문서가 아니라 **살아있는 코드가 행 번호까지 인용**하는 참조다(`option-card-radio.tsx:2` → `05-onboarding.html:304-385`, `kill-switch-modal.tsx:3` → `INTERACTION_SPEC.md` §03 등 7건). 옮기면 인용이 전부 깨져 **불일치를 줄이려다 늘린다.** 제자리 유지 + 폐기표기 배너 + 지도 §4 에 인용 목록 명시로 대체.
- **"플랜을 레포로 편입" → 철회.** `~/.claude/plans/` 참조는 이 레포의 **확립된 관례**다(sprint 문서 20건 이상 동일 패턴). 이 하나만 옮기면 나머지와 어긋난다. 관례를 따른다.

**`DESIGN.md` 는 폐기 아님.** 이식이 3/11 슬라이스라 앱 대부분이 아직 1세대다. `DESIGN.md` 는 stale 이 아니라 **현행**이고, 필요한 건 폐기표기가 아니라 세대 라벨이다.

**`abbr` 은 유일한 진짜 캐논 모순이었다.** `terminology-ssot.md` B5/B7 헤더가 `abbr="샤프"`·`abbr="수익률"` 을 지시하는데 `_KIT.md:532` 는 전자를 "17벌 0건, 폐기", `:534` 는 후자를 "수익률은 축약하지 않는다" 로 금지한다. 헤더 2줄만 어긋났고 본문(`:102`·`:1106`)은 오히려 정확했다 — **국소 결함**. 더 중요한 건 같은 파일의 **S4 복사용 TS 모듈**이 그 폐기 규칙을 인코딩하고 있었다는 점이다(`OBJECTIVE_METRIC_ABBR` 이 `total_return: "수익률"` 을 담고 주석이 "abbr 속성에만 넣는다" 로 지시). 그대로 뒀으면 **S4 에서 폐기 규칙이 코드가 됐다.** 타입을 `Partial<Record<…>>` 로 바꾸고 `total_return` 키를 제거했다.
...
- **2026-07-20** — **D0 문서 토폴로지 수술.** 세대 라벨 부재가 "A vs B" 혼동의 진짜 원인임을 규명 → `docs/design-docs-map.md` 신설 + `docs/README.md` 배선 + superseded 배너 3건 + `abbr` 캐논 모순 교정(TS 모듈 포함). 원안 2건(구 프로토타입 이동 · 플랜 레포 편입) 은 실측으로 철회. 코드 변경 0. 상세 위 "문서 토폴로지 수술" 절.
- **2026-07-20** — **S2 + S5(콘텐츠) + S1a #1.** 사용자 재프레임(화면이 보고 싶다) → S1a 는 사실상 무변화(앱이 이미 코퍼/카본, 22종 중 17 일치)임을 실측으로 밝히고 **S2→S5 콘텐츠 이식**으로 방향 전환. 상세 아래 "S2/S5 이식" 절. 2 커밋(`f0715dc` S2, `57c3ec9` S5). 다음 = S3(셸) 또는 S1a 잔여.
