...
★**[ADR-025] 자체는 아직 `Proposed` 다** — 아래는 그 ADR 의 상태가 아니라
**[ADR-025](025-conditional-fill-ownership.md) 의 머지된 구현(PR #544, 머지 커밋 `c6201490`)이
세운 사실**이다. 이 레포에서 「확정」은 Accepted 를 뜻하므로 그 말을 쓰지 않는다:
...
> (`docs/status.md` §발산 축 = 동결 · [ADR-024](024-soak-stability-gate.md) §codex 적대 리뷰)
