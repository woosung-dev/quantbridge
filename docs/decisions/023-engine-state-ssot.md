...
> ★★★**단 이 둘은 「누적 총계」다 — 현재 발생률이 아니다.** 최근 **18.5시간** 실측에서
> `engine_only` 는 **0회** 발화했고 움직인 것은 `direction_transient` **하나뿐**이다(+9).
> **반드시 §선행 측정의 첫 소절을 먼저 읽어라** — 이 문장을 「지금 병이 그쪽에서 난다」로 읽으면
> 다음 사람이 휴면 중인 counter 를 겨냥해 설계한다.
