...
| `LOG_LEVEL`  | [기본값 OK]                   | root logger 레벨 (BL-561). celery worker / uvicorn 공통. ★**레벨의 SSOT** — compose 의 `celery --loglevel` 은 root 레벨을 정하지 않는다. |

---

## 3. 인증 — Better Auth (자체 호스팅, [ADR-034](../adr/034-auth-self-host-better-auth.md))
...
상세 셋업: [`better-auth-setup.md`](../operations/better-auth-setup.md).
...
| staging    | ⏳ 미정    | 배포 전략 결정 후 ([`PRD.md`](../PRD.md) §0 결정 3건 참조)     |
