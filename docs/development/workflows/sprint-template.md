...
- status.md 갱신 (다음 스프린트 6필드 실행 계약: 목표·입력·작업 단위·AC·차단·증거. 완료 서사는 dev-log로 이관)
...
- [ ] **승격** → 정본 층(`docs/architecture|domain|api|development|operations/`) — 스프린트가 끝나도 계속 참이고, 누군가 다시 읽어야 하는 계약·정본
...
ADR(`docs/adr/`)은 예외다. 폐기해도 **삭제하지 않고 `Superseded` 상태로 남긴다**
