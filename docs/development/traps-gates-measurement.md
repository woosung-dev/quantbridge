...
- ★★**라이브 세션 병행 e2e 는 라이브 상태와 결합한다** — e2e authed 는 라이브 세션이 도는 개발 DB 를 그대로
  검사하므로, 그 세션이 **포지션을 들고 있으면** `/trading` 에 포지션 표가 추가 렌더되고
...
- ★**트리거 도래 판정은 ~~`tools/scripts/bl-trigger-sweep.sh`~~ 사람이 한다**(2026-09-06 정정 —
  그 스크립트는 [ADR-037] 로 철거됐다. 지금은 각 섹션의 `**트리거 판정:**` 줄을 직접 읽는다).
  **아래 교훈은 도구가 아니라 판정 자체에 대한 것이라 그대로 유효하다** — 판별력을 먼저 재라.
  ~~`--selftest` 를 전량 스윕보다 먼저 돌려라~~ —
...
- ★**라이브 세션 종료가 자동 flat 이 아니다** (2026-08-02). 세션 `DELETE` 가 **204** 를 줘도 거래소에 **포지션과 resting 조건부 주문이 남는다.** 실측: DELETE 2건 성공 뒤 포지션 0.03 + resting 1건 잔존. **주문 취소 → 포지션 청산**을 따로 하고 **raw HMAC 으로 재조회**해 `FLAT=YES` 를 확인해라. 착수 시점 flat 확인도 의무다 — 직전 세션의 **고아 포지션**(활성 세션 0인데 포지션 존재)이 남아 있으면 발화 원인이 오염된다.
