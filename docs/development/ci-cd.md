...
> → **2026-08-16 일치시켰다** ([ADR-036](../adr/036-tool-version-ssot-mise.md)). CI·로컬·프로덕션
...
> ~~CI Python 버전은 3.12, 로컬 권장은 3.11+.~~ → **2026-08-16** ([ADR-036](../adr/036-tool-version-ssot-mise.md)):
...
> → **2026-09-10 [ADR-043]** — `release.yml` 이 main 머지마다 이미지를 GHCR 에 올리고, 서버 `deploy.sh` 가 반영한다(deploy 잡은 서버 전환 뒤).

| 대상           | 절차                                                                                                                                   | 정본                                                      |
| -------------- | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| FE (오라클 A1) | `release.yml` → GHCR `sha-<7>` → 서버 `deploy.sh <sha>` 가 pull + `-p quantbridge-fe up -d`                                            | [`frontend-deploy.md`](../operations/frontend-deploy.md)  |
| BE             | 같은 `deploy.sh <sha>` — 24h 무실격 · DDL 대조 · pull · 워커 롤링 · 호스트 API 재시작. rc 2 = 멈춤(사람이 `--migrate`)               | [backend-deploy.md](../operations/backend-deploy.md)      |
| DB 백업        | `tools/scripts/db-backup.sh` + systemd timer                                                                                           | [ADR-033](../adr/033-db-hosting-self-host-timescaledb.md) |
| migration      | Docker entrypoint 가 `alembic upgrade head`(api 롤 — 서버 미사용). 서버 DB 는 `deploy.sh --migrate <sha>`(사람 · 매번 승인)              | `status.md` 비목표 항목                                   |
...
★**단 이 경고는 compose 를 손으로 칠 때만이다** — `deploy.sh` 의 COMPOSE 배열은 이미 넣고 있다.
BE 문서에 그대로 옮기면 거짓 경고가 된다(2026-08-18).

향후 workflow 화가 필요해지면 그때의 후보: staging deploy on push to `main` ·
production deploy on tag `v*.*.*`.
★**staging 은 없고 production 은 이미 돈다** — 오라클 A1 한 대에 BE compose·FE·호스트 uvicorn 이
함께 올라가 있다. 「미정」이라 적힌 표를 실태로 읽지 마라(2026-08-18).

프로덕션 배포는 **이미 돌고 있다**. 추가 공개 범위 결정은 [`PRD.md`](../PRD.md) §0 **결정 ⑵**(Beta 외부 공개 안 연다)가 정본이다.
