...
루트 `package.json`은 husky/lint-staged 훅 전용. apps/web/backend 도구는 각자 독립.
...
| 패턴                            | 검사                                             |
| ------------------------------- | ------------------------------------------------ |
| `apps/web/**/*.{ts,tsx,js,jsx}` | `pnpm lint --fix --file <path>`                  |
| `apps/api/**/*.py`              | `ruff check --fix <path>` + `ruff format <path>` |
...
- `cd apps/web && pnpm lint --cache=false` 강제 재실행
...
- 해소: `rm -rf apps/api/.ruff_cache apps/web/.eslintcache` 후 재실행
