...
| [BL-233](#bl-233)                                  | Genetic 본격 구현 (`GeneticHyperparamsField` + `optimizer/engine/genetic.py` + `service.submit_genetic`) — Sprint 55 Bayesian 패턴 mirror          | Sprint 56 본격                                                                       | M (12-16h → estimate)  | Sprint 55 prereq 활용 = schema_version=2 + alembic enum 확장 패턴 + service.run() kind 분기 + FE z.discriminatedUnion 모두 활성 → scope ≈ Sprint 55 의 0.8×. `population_size` + `n_generations` + `mutation_rate` 5 v2-only 필드 이미 schemas 안 reservation 완료. ADR-013 §2.1 grammar lock 따라 진입.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
