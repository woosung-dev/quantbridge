...
| [BL-234](#bl-234)                                  | `BayesianHyperparamsField.prior="normal"` 자체 sampler 활성 + CategoricalField `encoding="one_hot"` Bayesian 활성                                  | Sprint 56+                                                                           | S (4-6h → estimate)     | Sprint 55 = `prior=normal` engine NotImplementedError + categorical ordinal only. ADR-013 §7.2 명시. 자체 sampler = `np.random.normal(loc=(min+max)/2, scale=(max-min)/4)` + clip wrapper. one_hot encoding 도 Genetic 진입 시 묶음 검토.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| [BL-237](#bl-237)                                  | Optimizer dedicated Celery queue + soft_time_limit relaxation (Bayesian 50 evaluation × 5s = 250s 보호)                                            | Sprint 56+                                                                           | M (5-8h → estimate)     | Sprint 55 = default queue + `_MAX_BAYESIAN_EVALUATIONS=50` 강제 상한 (worker block 보호). dedicated `optimizer-heavy` queue + `soft_time_limit=600` 별도 worker pool prefork-safe (backend.md §11.5 라이브 검증 의무) + max_evaluations cap 100+ relax.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
