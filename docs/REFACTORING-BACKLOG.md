...
| [BL-241](#bl-241)                                  | Pine built-in TA 함수 확장 묶음 — `ta.wma` + `ta.hma` + `ta.bb` + `ta.cross` + `ta.mom` + `ta.obv` + `ta.dmi` + `fixnan`                            | Sprint 58+                                                                           | S (4-6h → estimate)     | dogfood 2026-05-11 발견. DrFX Diamond Algo indicator 테스트 시 37개 미지원 중 TA 함수 다수 포함. `ta.wma` = HMA 전제조건. `ta.hma` = `ta.wma(2*ta.wma(src,n/2)-ta.wma(src,n), sqrt(n))`. `ta.bb` = Bollinger Bands. `ta.cross` = crossover or crossunder. `ta.mom` = 모멘텀. `ta.obv` = On Balance Volume. `ta.dmi` = Directional Movement Index. `fixnan` = 최근 non-na 값으로 대체. 8개 함수 구현 시 real-world public indicator 대부분 커버 가능. sprint-template.md §TA-ext 참조.                                                                                                                                                                                                                                                                                                                 |
| [BL-242](#bl-242)                                  | `strategy.equity` 내장 변수 지원 + display 전용 함수 (`label.*` / `table.*` / `barstate.isrealtime`) warn-only 처리                                 | Sprint 58+                                                                           | S (2-4h → estimate)     | dogfood 2026-05-11 발견. RSI Divergence strategy (v4) 실행 시 `strategy.equity` 미지원 오류. `strategy.equity` = 엔진 내부 누적 equity 노출 (XS). display 전용 함수는 실제 백테스트 신호에 영향 없으나 미지원으로 실행 차단 → warn-only + skip 처리로 변경 (DrFX 37개 중 display 함수 다수 포함). 두 변경 묶음 처리.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| [BL-243](#bl-243)                                  | 거래 목록 / 백테스트 결과 UTC 타임존 라벨 표시                                                                                                      | Sprint 58+                                                                           | XS (30min → estimate)   | dogfood 2026-05-11 발견. 거래 목록 진입·청산 시간이 UTC로 표시되나 타임존 라벨 없음. TradingView는 거래소 시간(KST +9h) 기준 표시 → 사용자 혼동. 거래 목록 헤더에 `UTC` 라벨 추가. 변환 기능은 scope 제외 (표시 일관성만).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
