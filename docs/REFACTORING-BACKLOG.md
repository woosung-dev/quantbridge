...
| [BL-220](#bl-220)                                  | 진짜 Param Stability — pine_v2 input override mechanism (EMA period × stop loss % 같은 strategy parameter sweep)                                  | **P2 (Sprint 50 등재, 2026-05-10)**                               | M (5-8h)      | Sprint 50 Cost Assumption Sensitivity MVP 와는 별도 — 후자 = backtest assumption (PnL 단계 fee/slippage), 전자 = strategy parameter (EMA period 등). pine_v2 parser input declaration AST scan + `BacktestConfig.input_overrides: dict[str, Decimal\|int]` 신규 필드 + Interpreter override apply. Sprint 51 prereq spike + 본격. dogfood Day 7 (2026-05-16) 결과 따라 우선순위 결정.                                                                                                                                                                                                                          |
