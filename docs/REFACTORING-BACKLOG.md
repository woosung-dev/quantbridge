...
| [BL-238](#bl-238)                                  | lint-staged backend `ruff check --fix` exit 0 silent skip 차단 — `--exit-non-zero-on-fix` + 별도 `ruff check` no-fix 추가                         | **✅ Resolved (Sprint 56 chore PR, 2026-05-11)**                                     | XS (30min → 실측 ~10min) | `package.json` lint-staged `backend/**/*.py` command 에 (1) `ruff check --fix --exit-non-zero-on-fix` + (2) 별도 `ruff check` (no-fix) 2차 호출 + (3) `ruff format` 3-step 추가. 이제 fixable 항목 자동 수정 후 commit 차단 (사용자 재커밋 의무) + unfixable (RUF002 × 곱셈 기호 등) commit 차단. Sprint 52/55 silent skip 패턴 재발 차단. **LESSON-068 후보 = 3/3 정식 승격 path** (Sprint 57+ 재발 시 정식 lesson 등재).                                                                                                                                                                                                                                                                                                                                                                            |
| [BL-239](#bl-239)                                  | pre-push hook 에 `uv run mypy src/` 추가 (CI parity, Sprint 55 hotfix #2 root cause)                                                              | **✅ Resolved (Sprint 56 chore PR, 2026-05-11)**                                     | XS (15min → 실측 ~5min)  | `.husky/pre-push` backend 분기에 `uv run mypy src/` 추가 (기존 ruff + pytest 다음). Sprint 55 hotfix `39ecc01` 패턴 (mypy 5 errors CI 만이 첫 검출) 재발 차단. CI parity gap 해소.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| [BL-240](#bl-240)                                  | pre-push hook 에서 `TEST_*` 변수 자동 source `backend/.env.local` (LESSON-061 영구 fix)                                                           | **✅ Resolved (Sprint 56 chore PR, 2026-05-11)**                                     | S (1-2h → 실측 ~15min)   | `.husky/pre-push` backend 분기 시작 시 mktemp + grep `^TEST_[A-Z_]+=` 추출 → `set -a + . source + set +a` 패턴 (husky=sh shell 호환, process substitution X). 운영 secret (DATABASE_URL / SECRET_KEY 등 non-TEST prefix) 차단 보장. hook 종료 시 shell context 소멸 = scope 격리. LESSON-061 영구 fix (Sprint 51~56 11+ 발생 count 누적 종료).                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
