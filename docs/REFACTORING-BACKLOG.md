...
| [BL-200](#bl-200)                                  | pine_v2 STDLIB triple SSOT 단일화 (`stdlib_registry.py` 신설, interpreter/stdlib/coverage 3 중복 → 1 single source)                               | Sprint 47 deepening sprint                           | M (4-6h)      | /deepen-modules pilot 발견 (2026-05-09) — `STDLIB_NAMES` interpreter.py:55-77 (19개) / stdlib.py:598-660 dispatch (19개) / coverage.py:26-46 `_TA_FUNCTIONS` (17개, na/nz 누락) 3중 SSOT. 한 곳만 갱신 시 silent failure (coverage false-pass). 신규 ta.\* 추가 시 3 파일 동시 수정 의무. ★★★★★ — dogfood Phase 2 진입 전 권장                                                                                                                                                                                                                                                                                               |
| [BL-201](#bl-201)                                  | pine_v2 Track S/A/M Strategy pattern (`track_runner.py` 신설, compat/ast_classifier/virtual_strategy/interpreter 4 분산 → 1 unified interface)    | Sprint 47 deepening sprint                           | M-L (9-12h)   | /deepen-modules pilot 발견 (2026-05-09) — Track 분기가 compat.py:109-138 dispatcher + ast_classifier.py:117-155 판정 + virtual_strategy.py:134 entry gate + interpreter.py:1057 entry gate 4 곳 분산. Track 추가 시 4 파일 동시 수정 + 새 Track 정의 일관성 mechanism 부재. `TrackS/TrackA/TrackM` 클래스 = `run(ast,...) → V2RunResult` 단일 인터페이스로 통합. ★★★★☆ — pine_v2 SSOT 핵심 risk 🔴 (252+ test 안전망 보장)                                                                                                                                                                                                   |
| [BL-202](#bl-202)                                  | trading Provider Registry/Factory (`providers/registry.py` 신설, 5 provider class + dependencies singleton + service ad-hoc 분기 → 1 dispatch)    | Sprint 47 deepening sprint                           | M (6-9h)      | /deepen-modules trading audit 발견 (2026-05-09) — providers.py 5 class (Fixture/BybitDemo/BybitFutures/OkxDemo/BybitLive) + dependencies.py:51 singleton 1개 (BybitFutures만) + service.py:146/515 ad-hoc `if exchange == bybit and mode == demo` 분기. (exchange, mode) tuple 자동 dispatch 부재 → 새 provider 추가 시 service.py 분기 add 의무 + silent failure (등록 누락 시). 목표: `dict[(ExchangeName, ExchangeMode), ExchangeProvider]` registry. ★★★★★ — BL-003 mainnet runbook 진입 전 권장. Risk 🔴 (CCXT 호출 전수 검증), test 안전망 48 file 강함                                                                |
| [BL-203](#bl-203)                                  | trading service.py god file 분할 (5 class → 5 파일: orders/accounts/webhook_secrets/live_sessions/protocols)                                      | Sprint 47 deepening sprint                           | S-M (4-5h)    | /deepen-modules trading audit 발견 (2026-05-09) — service.py 562 LOC 에 ExchangeAccountService + WebhookSecretService + OrderService + LiveSignalSessionService + 2 Protocol = 5+ 책임 한 파일. co-change 13회/3개월 (1위). **test 는 이미 10 파일 책임별 분리** (test 가 source 보다 좋은 design 제시). ★★★★☆. Risk 🟡 (단순 파일 분할, import path mass migration). 단독 가치 + B-pair 와 묶기 가능                                                                                                                                                                                                                        |
| [BL-204](#bl-204)                                  | trading repository.py god file 분할 (3 class → 4 파일: orders/exchange_accounts/kill_switch_events/webhook_secrets)                               | Sprint 47 deepening sprint (BL-203 묶음)             | S (2-3h)      | /deepen-modules trading audit 발견 (2026-05-09) — repository.py 714 LOC 에 ExchangeAccountRepository + OrderRepository (14+ method) + KillSwitchEventRepository 한 파일. test 는 이미 4 파일 분리. ★★★☆☆. Risk 🟢 (가장 단순, import 만 변경). 단독 가치 작음, BL-203 와 묶어서 ROI ↑                                                                                                                                                                                                                                                                                                                                        |
| [BL-205](#bl-205)                                  | trading OrderStatus Literal triple SSOT 단일화 (providers.py 3곳 + models.py + repository → models.OrderStatus enum 단일)                         | Sprint 47 deepening sprint (또는 stand-alone hotfix) | XS-S (1-2h)   | /deepen-modules trading audit 발견 (2026-05-09) — `Literal["filled", "submitted", "rejected"]` (providers.py:73, **cancelled 누락 confirmed**) vs `Literal["filled", "submitted", "rejected", "cancelled"]` (providers.py:89, 116, 145) vs `OrderStatus` enum (models.py:44-46) = 3 SSOT. providers.py:73 silent failure 1건 confirmed. ★★★☆☆. Risk 🟢. BL-202 와 함께 또는 stand-alone XS hotfix                                                                                                                                                                                                                            |
| [BL-206](#bl-206)                                  | frontend cross-page primitive cleanup (Skeleton variant 확장 + EmptyState fork 정합 — 5+ hardcode `animate-pulse` 제거 + StrategyEmptyState 정합) | Sprint 47 deepening sprint                           | S-M (3-4h)    | /deepen-modules frontend audit 발견 (2026-05-09) — `components/skeleton.tsx` 가 있으나 hardcode `animate-pulse rounded bg-[color:var(--bg-alt)]` 5+ 위치 (strategies/loading.tsx 5줄 + backtest-detail-view:238 + backtest-list:289 + error-illustration:35 + error-recovery-box:325) = 디자인 system drift, dark mode/theme 변경 시 5+ 파일 동시 수정. 추가로 `components/empty-state.tsx` + `strategies/_components/strategy-empty-state.tsx` 로컬 fork. 목표: `<Skeleton variant="row\|chip\|card" />` 확장 + EmptyState prop 확장 후 fork 제거. ★★★★☆ (A) + ★★★☆☆ (B). Risk 🟢 (1:1 치환), test 안전망 frontend 603 PASS |
