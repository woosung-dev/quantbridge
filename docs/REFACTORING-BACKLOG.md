...
| [BL-222](#bl-222)                                  | parent backtest config 손실 (Sprint 50 cost_assumption + Sprint 51 param_stability 함께) — `_execute_cost_assumption_sensitivity` + `_execute_param_stability` 가 `backtest_config` 미전달 → 모든 cell BacktestConfig() default 사용 | **P1 (Sprint 51 codex Slice 4 + G.4 review 발견, 2026-05-11)**                       | M (3-5h)              | 부모 backtest 의 `initial_capital`, `fees/slippage`, `trading_sessions`, BL-188 v3 sizing config 가 cell 마다 default 사용. Surface Trust ADR-019 + BL-188 v3 가정 불일치. backtest 모델 `bt.config` JSONB + `bt.initial_capital` 복원 + `run_*_sensitivity(..., backtest_config=BacktestConfig(...))` 전달. Sprint 50+51 함께 fix 후속 sprint 일괄.                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| [BL-223](#bl-223)                                  | Sprint 51 FE Param Stability form + BacktestDetailView wire-up + AssumptionsCard 통합                                                                                                                                                | **P2 (Sprint 51 close-out 등재, 2026-05-11)**                                        | M (4-6h)              | Slice 5 Sprint 51 = heatmap + Zod schemas 만 작성 (scope creep 회피). 사용자 form (9-cell preset EMA period × stop loss %), `postParamStability` mutation, `useCreateParamStability` hook, StressTestPanel 안 분기, BacktestDetailView 안 `param_stability_result` heatmap render branch 후속 sprint.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| [BL-224](#bl-224)                                  | FE schemas.ts `param_grid` superRefine 제약 누락 (Sprint 50 cost_assumption + Sprint 51 param_stability 함께)                                                                                                                        | **P2 (Sprint 51 codex Slice 5 review 발견, 2026-05-11)**                             | XS (1-2h)             | FE `CostAssumptionParamsSchema` / `ParamStabilityParamsSchema` 가 BE 의 grid validator (2 key + non-empty + ≤9 cell + Decimal string) 제약 검증 누락 → server 422 만으로 fail. FE 즉시 reject UX 위해 superRefine 추가. Sprint 50+51 일관 fix 후속.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| [BL-225](#bl-225)                                  | `run_param_stability` InputDecl.input_type 별 grid value validation 누락                                                                                                                                                             | **P2 (Sprint 51 codex Slice 3 + G.4 review 발견, 2026-05-11)**                       | XS (1-2h)             | `run_param_stability` 의 var_name cross-check 가 `InputDecl.input_type` 무시 → input.bool/string 도 통과 + input.int 에 `Decimal("20.5")` 통과 후 `int()` cast 시 20 잘림 → heatmap 표시 mismatch. `extract_content().inputs` 의 input_type 별 grid value validation 추가 (int = 정수 Decimal 만, float = numeric 모두, bool/string = MVP unsupported reject).                                                                                                                                                                                                                                                                                                                                                                                                                                        |
