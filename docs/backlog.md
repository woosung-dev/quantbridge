...
| 무엇 | 파일이 말하는 것 | 실제 |
| --- | --- | --- |
| `status.md` 크기 | `:346` 다이어트 표 = **49,845 B (16.6k tok)** | **146,414 B** — **2.94배** |
| 줄 수 상한 | 「700줄 상한」을 **12곳**에서 인용 | **1,132줄** — 1.6배 |
| `bl-audit.sh` | 함정 3건 + 「`--list DEFERRED` **151건**」을 살아 있는 지시로 인용(**8곳**) | ADR-037 로 **스크립트 없음** · 다이어트 후 DEFERRED **23건** |
...
★**측정 가능한 산출은 ⑵ 하나뿐이다** — 스윕 스크립트의 라우트 목록을 `page.tsx` 글롭 + id 주입으로
바꾸는 것. 그것이 재발을 막고, 나머지는 사람 판단이라 수로 셀 수 없다.
