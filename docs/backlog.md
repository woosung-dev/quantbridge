...
| [BL-580](#bl-580)    | 계측 가드 잔여 **141곳** — 승격 규칙 사전등록                                                                                                                                | `qb_metrics_mutation_failed_total` 창 차분이 0 을 벗어날 때                      | M            | 2026-08-02 metric-guard-parity                         |
...
| [BL-582](#bl-582)    | divergence counter **13 series 중 7종 도달 불가**, 프로덕션 확인 3/6                                                                                                         | 엔진이 exit level 을 공급하거나 `degraded_input` 자연 발화 시                    | S            | 2026-08-02 metric-guard-parity                         |
...
**Est:** M
**상태:** 🟢 **열려 있다 — 승격 규칙 사전등록 완료.** 2026-08-02 metric-guard-parity 에서 [BL-579](#bl-579) 분리.
**출처:** 2026-08-02 metric-guard-parity (18곳 수리 후 잔여)

가드 밖 mutation **141곳**(규칙 R1, `test_metric_guard_census.py` 가 정본이고 천장으로 고정).

★**이번에 의도적으로 뺀 것과 이유** — 「안 고쳤다」가 아니라 「이 근거로 뺐다」이다.

| 대상                                                  | 이유                                                                                                                                                                  |
| ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `trading.py:908` · `:931` · `:1093` · `router.py:376` | raw `qb_active_orders.dec()` 이지만 **거절·취소 확정 뒤**라 실패로 계상하는 `except` 가 없다. 던지면 task FAILED + gauge drift 이고 **후속 훅 유실이 없다**           |
...
| 축           | 실측 (2026-08-02)                        |
| ------------ | ---------------------------------------- |
| 파일 수      | **10277** (전날 9423 → 회차 중에도 증가) |
| 용량         | **635MB** (여유 125G)                    |
| distinct PID | **1968**                                 |
| 최초 파일    | **2026-07-28**                           |

증가 드라이버는 **PID churn**(dev `uvicorn --reload` · watchfiles)이지 컨테이너 churn 이 아니다
— 컨테이너 hostname 은 role 당 1개로 고정이다.
...
**Est:** S
**상태:** 🟢 **열려 있다 — 도달 불가로 확정, 수리 없음.** 2026-08-02 metric-guard-parity.
**출처:** 2026-08-02 [BL-576](#bl-576) 잔여 검증 중 확정

`qb_live_conditional_divergence_total` 의 **13 series 중 7종이 구조적으로 도달 불가**다.

| series                                                            | 왜                                                                                                                                                                             |
| ----------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `guard_drop`/`bracket_trailing_only` · `bracket_tp_size_mismatch` | `PendingOrderSnapshot.take_profit/stop_loss/trailing_stop` 이 **항상 `None`** — `place_exit` 가 `open_trades` 만 겨냥하는데 stop-entry 는 체결 전까지 `pending_orders` 에 있다 |
| `other` 5종                                                       | 호출부가 전부 허용 reason 리터럴 (닫힌 집합 가드용)                                                                                                                            |

★**도달 가능 6종 중 프로덕션 확인 = 3**(`stand_down/shared_account_symbol` · `market_converted` ·
**`exchange_divergence`**(2026-08-02 유도로 신규 확인)). 남은 3종:
...
트래픽을 쏘거나 MITM 프록시가 필요하다. 전자는 하지 않는다.**
