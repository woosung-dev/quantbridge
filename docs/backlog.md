...
| [BL-651](#bl-651) | 중복 계정 행이 배타성 판정식의 **개수**를 2배로 부풀린다 — 실측 `RESTING_CONDITIONAL=2` 인데 실제 조건부 주문은 **1건**(같은 `link=dd58ef44` 가 두 계정으로 계상), 포지션도 2행. `EXCLUSIVE`(존재 판정)는 배수에 불변이라 지금 깨지는 것은 없고, **개수를 문턱으로 쓰는 순간** 틀린다. BL-605 처방(스윕 루프)은 **이 자리를 안 고친다** — 여기는 거래소 조회 루프다                                                                                                                                                                                                                                                                                                                                                                                                   | BL-634 가드가 resting 개수를 문턱으로 쓰기 전                                                                   | S            | 2026-08-08 soak-attribution-close                            |
...
| 축               | 실측                                                                             | 판정                                               |
| ---------------- | -------------------------------------------------------------------------------- | -------------------------------------------------- |
| import 비용      | `classify_script` import **0.25s**, import 직후 DFA 상태 **0**                   | **(a) 기각** — cold 합계의 0.4%                    |
| 코퍼스 cold/warm | 9벌 cold 합계 **69.93s** vs 같은 프로세스 warm **5.76s**                         | 첫-접촉 프리미엄 **91.8%**                         |
| `i3_drfx` 단독   | cold **41.37s** / warm **3.74s**                                                 | warm 이 전체 스위트 안 4.58s 와 일치               |
| **인과 대조**    | 같은 프로세스·같은 입력에서 DFA 캐시**만** 비우니 3.63s → **49.61s**, 다시 3.56s | 원인이 DFA 상태임을 **인과로 확정** (13.7배)       |
| cold 램프        | 조각 크기 8.6배로 키워도 조각당 cold 는 **5.56~10.48s 로 평평**, DFA 648→5803    | cold 의 축은 크기가 아니라 **처음 보는 문법 결정** |
| warm 램프        | log-log 기울기 1.25 이나 꼬리 절반은 크기 1.6배 → 시간 1.1배(sublinear)          | **(b) 기각** — warm 5.76s 로 42.66s 를 못 만든다   |
| 샤딩 중복        | 프로세스 9개로 쪼개면 합계 **122.82s** vs 단일 프로세스 **69.93s**               | 중복분 **+52.89s** 를 실측으로 확인                |

기전: `PinescriptParser.decisionsToDFA` 와 `sharedContextCache` 가 generated 파서의 **클래스
속성**이라 프로세스 전역이고, 파서 인스턴스는 생성 시점에 그것을 읽는다. cProfile 누적시간
99% 가 `adaptivePredict` → `execATN` → `closure_`(2,396만 호출)다. ⇒ 「프로세스 전역이라
샤딩하면 중복된다」는 위 진단은 **맞았다**. 틀린 것은 원인 후보 2개(import·크기)뿐이다.

**② 처리 방향 — 테스트 쪽에서 닫힌다 (`backend/src` 0줄).** 파싱 시간이 크기에 초선형이
아니므로 **파서에는 고칠 표적이 없다**. 표적은 「테스트가 파싱을 한다」는 사실 자체다 ⇒
`classify_script()` 결과를 **소스 해시 키로 디스크 캐시**해 코퍼스 테스트가 파싱을 아예 안
하게 만든다(baseline 재생성 경로는 캐시 우회 플래그로 남긴다). ③ ②가 되면 샤드마다
중복되던 비용이 사라져 샤드 재분배로 추가 이득이 열린다.
...
**Est:** S
**상태:** ⬜ **Open** — 2026-08-08 에 **뿌리와 처방이 확정**됐고 코드 0줄 회차라 미수리다. 집행 시점 = 다음 사망 직후(`docs/status.md` §다음 스프린트)
