...
| [BL-591](#bl-591)    | ★**뿌리** — 엔진 포지션의 SSOT 가 없다. `run_live` 시뮬이 매 tick 봉을 재생해 포지션을 **도출**하고 정상 운행 중엔 현실로 **보정되지 않는다**. 이 축에서 증상 BL 을 **30건 Resolved** 했는데 소크는 가드 머지 후 **12/12 전부 2시간 미만** 사망                                                 | 발산 증상 BL 을 또 하나 열기 전에 · 소크가 또 죽었을 때                                                           | L            | 2026-08-03 breach-rejection-recovery                   |
...
**Est:** M
**상태:** 🟢 **열려 있다 — 96곳.** 2026-08-03 metric-guard-residual-sweep 가 발주 outbox **12곳을 판정**
(수리 8 · 보류 4, 104→96). 그 앞 회차가 25곳(129→104).
그 앞 회차가 12곳(141→129). 2026-08-02 metric-guard-parity 에서 [BL-579](#bl-579) 분리.
**출처:** 2026-08-02 metric-guard-parity (18곳 수리 후 잔여)

가드 밖 mutation **96곳**(규칙 R1, `test_metric_guard_census.py` 가 정본이고 천장으로 고정).
...
**우선순위:** P1 (P0 [BL-003] 의 실질 게이트)
**카테고리:** Trading / 라이브 신호 (엔진 포지션 SSOT)
**Trigger:** ★**이미 발화했다.** 자동 종료 **15회**. `direction` 발산 가드 머지(#497,
`2026-07-28T23:09Z`) 이후 **12회 전부 2시간을 못 넘겼다**(105 · 99 · 91 · 84 · 65 · 61 · 57 ·
21 · 18 · 15 · 3 · 2.6분).
**Est:** L — **설계 결정 선행**(사용자). 구현 착수 전 이 절의 §설계 축을 확정해야 한다.
**상태:** ⬜ **Open — 설계 미확정**
**출처:** 2026-08-03 breach-rejection-recovery (증상 반복의 뿌리 재판정)

**증상을 30건 고쳤는데 병이 그대로다.**

엔진↔거래소 정렬 축에 걸린 BL 은 **32건**이고 그중 **30건이 Resolved** 다(BL-014/361/362/374/
378/390/393/441/442/480/484/497/498/500/511/515/522/530/535/538/539/540/543/545/547/553/566/
576/589/590). 그런데 소크는 여전히 2시간을 못 넘긴다. **기전은 매번 달랐고 뿌리는 하나다.**
...
★**C 를 권고한다.** 이유 셋: ① 주입 배관(`ledger_seed_legs`)과 순수 함수(`_ledger_gap_seed`)가
**이미 있다** — 발화 조건만 넓히면 된다 ② 원장 ≠ 거래소이므로 **가드가 동어반복이 되지 않는다**
(원장↔거래소 대조는 여전히 독립) ③ `engine_only` **314건**의 대부분이 「우리가 낸 주문의 체결을
엔진이 모른다」는 형태라 원장 주입이 정면으로 겨냥한다.

**C 를 고르면 먼저 답해야 할 것 (설계 질문, 구현 아님):**

1. 원장이 거래소를 못 따라잡는 구간(리컨사일러 지연)에서 **어느 쪽을 믿는가.**
2. **원장 밖 청산**(수동 개입 · `ClosePositionService` HTTP 경로)을 어떻게 원장에 남기는가.
   ★오늘만 2건 생겼다(죽은 세션 정리 · 유도 정리). 이 구멍이 있으면 C 는 틀린 값을 주입한다.
3. 상시 주입이 **백테스트 재현성**을 깨지 않는가(`run_live` 는 백테스트와 같은 엔진이다).
4. 주입 후에도 남는 발산은 **여전히 fail-closed 로 죽일 것인가**(안전망 유지 여부).

**Risk:** 🔴 머니-패스 핵심. 잘못 주입하면 엔진이 없는 포지션을 믿고 반대 주문을 낸다.
**착수 전 `/deepen-modules` 또는 설계 리뷰 1회를 권고**한다(신규 도메인 아님, 큰 모듈 재설계).
