...
| ID                | 제목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | Trigger                                                                                                         | Est          | 출처                                                         |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------ |
| [BL-720](#bl-720) | ⬜ **`docs-audit` 지식 정본 축 결손 3종** — ① LESSON **ID 유일성 검사 없음**(같은 번호 2장이 초록으로 통과했다) ② [ADR-029] 의 `backend/`→`apps/api/` 가 `legacy_paths` 에 **없다** ③ 백틱 코드 스팬은 `link_re` 밖이라 규칙 파일 포인터가 안 걸린다. 2+3 이 겹쳐 `lessons.md` 영구 승격 표 **7행이 존재하지 않는 파일**을 가리키고 있었다(본문 포함 10곳) — 승격 표는 정본으로 가는 **유일한 포인터**라 그것이 죽으면 지식이 안 찾아진다                                                                                                                                                                                                                                                                                                                                                              | 도래 — 결손 3종이 실측 확정                                                                                     | S            | 2026-08-14 gate-surface-close                                |
...
| ID                | 제목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Trigger                                                                                                           | Est       | 출처                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------ |
| [BL-722](#bl-722) | ⬜ **`assert-main-checkout.sh` 판별력 덮개 부재** — 「여기는 메인 체크아웃인가」를 판정하고 아니면 죽는 **가드**이고 그 뒤에 `make up/migrate/seed`(공유 컨테이너·앱 DB·Redis)가 있다. §8.7 기준 판정기인데 하네스·`--selftest`·다른 층 테스트가 전부 없다. ★비대칭이라 P3 — 과하게 죽으면 즉시 보이고, 위험한 방향은 워크트리를 「메인」으로 통과시키는 쪽뿐이다(사고 기록 없음)                                                                                                                                                                                                                                                                                                     | 도래 — 대상 확정                                                                                                  | XS        | 2026-08-14 gate-2stage                                 |
...
**Est:** S
**상태:** ⬜ Open — 처방 미착수. 2026-08-14 gate-surface-close 가 [BL-716] 이행 중 실측으로 등재했다. 셋 다 **게이트가 초록인 채로** 살아 있던 결함이다.
...
1. `legacy_paths` 에 `backend/`→`apps/api/` · `frontend/`→`apps/web/` 추가. ★단 **과거 서사**
   (「그때 `backend/src` 가 0줄이었다」)는 정당한 인용이므로 `archive/`·`dev-log/` 는 이미 `frozen`
   이고, 그 밖에서도 오탐이 나오면 예외가 아니라 **문장을 고치는 쪽**이 맞는지 먼저 재라.
2. 코드 스팬 경로 검사 축 신설 — `` `<경로>` `` 중 레포 루트 기준 실재하지 않는 것을 잡는다.
   오탐 위험이 크므로 **대상을 `lessons.md` 승격 표처럼 「포인터가 정본인 자리」로 좁혀서** 시작해라.
3. `lessons.md` ID 유일성 + 오름차순 검사. ★**음성 대조 의무** — 같은 번호 2장을 심은 입력에서
   red 가 나는지 먼저 확인해라(빈 입력 초록 = §8.6).
...
**Est:** XS
**상태:** ⬜ Open — 처방 미착수. 2026-08-14 gate-2stage 가 §8.7(「판정기에만 하네스」)을 성문화하며 `tools/scripts/` 19종을 스윕해 등재했다.
...
**처방:** 임시 트리 fixture 로 ⑴ 메인 체크아웃 → rc=0 ⑵ 워크트리 → rc≠0 ⑶ 비 git 디렉터리 → rc≠0
세 케이스 + 판정문을 뒤집는 변이 1종. §8.7 의 세 방법 중 **내장 `--selftest` 가 가장 싸 보인다**
(파일이 작고 `bl-trigger-sweep` 선례가 있다) — 착수 시 재판단해라.
