...
| [BL-742](#bl-742) | harness 러너 **as-is 위험 7종**이 살아 있다 — [ADR-030] 이 걷어낸 러너를 2026-08-15 사용자 판정으로 재도입하며 재개 조건 ⑵⑶ 을 이행하지 않았다. ★위험 7(`TimeoutExpired` 미처리)은 B회차를 이미 죽였고 그때 트리에 남은 것이 **AC 0건 실행 + `completed` 커밋**이다. 막고 있는 것은 코드가 아니라 규약 2개(워크트리 전용 · `--push` 금지). 도래하면 **고치지 말고 뗀다**                                                                                                                                                                                                                                                                                                                                                                                                                               | 미도래 — 재도입 후 관측 0건                                                                                     | S (1h)       | 2026-08-15 harness-readopt                                   |
...
| [BL-602](#bl-602) | ✅ **Resolved** (2026-08-15). ★**착수 없이 이미 해소돼 있었다** — 뿌리(「루트 `node_modules` 에 `prettier-plugin-tailwindcss` 가 없다」)가 2026-08-09 `71f7101e` 의 루트 devDependencies 추가로 6일 전에 사라졌는데 아무도 재측정하지 않아 「대기」로 남아 있었다. 회피 3줄 제거 + 정규화. ★부수 발견 — 회피를 걷자 prettier 가 `NEXT_PUBLIC_*` 를 `NEXT*PUBLIC*\*` 로 **망가뜨렸다**(내용 손상). ignore 에 가려 6일간 안 보였던 결함이라 백틱으로 근본 수리                                                                                                                                                                                                                          | ~~동승 조건~~ → 2026-08-15 해소 확인                                                                              | S         | 2026-08-06 e2e-consolidation                           |
...
**상태:** ⬜ Open — 미착수. 신규 적재는 멈춤 (2026-08-14 money-path-attribution). ★**2026-08-15 재확인 — 「코드 0줄 판정 종결」 후보로 열었으나 종결할 것이 없다.** 처방은 이미 「집계 시 `DISTINCT ON (row_hash)`」로 적혀 있고, `exchange_exit_repository.py:48` 이 「여기서는 no-op 이라 넣지 않는다」까지 명시한다. 트리거도 미도래(동승)다 ⇒ **지금 상태가 정답이라 그대로 둔다** (2026-08-15 soak-survival)
...
**상태:** 🟡 **부분 해결 — 규칙은 문서화됐다(2026-08-15 `docs/status.md` 「환경 상태」). 기계 강제(`soak-stack.sh up` 의 Darwin 거부)는 미착수**
...
**Title:** harness 러너의 **as-is 위험 7종** — 회수 조건 도래 시 러너를 뗀다
**Category:** 개발 워크플로 / harness
**Priority:** P2
**Trigger:** ★**위험 7(`TimeoutExpired` 미처리)이 회차를 죽이면 그 자리에서 도래한다.** 파일럿 B회차에서 이미 1회 발생했고, 재도입판은 그것을 **안 고친 채** 간다
**Est:** S (1h — 러너 철거는 파일 3벌 삭제 + ADR 개정)
**출처:** 2026-08-15 harness-readopt ([ADR-033](decisions/033-harness-readopt-codex.md) 회수 조건)

**원인 / 영향:** [ADR-030](decisions/030-harness-pilot-verdict.md) 이 2026-08-13 에 걷어낸 러너를
2026-08-15 사용자 판정으로 **as-is 재도입**했다. 재개 조건 ⑴(실행기를 `claude` 로)은 뒤집었고
⑵(비결정적 AC 혼입)·⑶(위험 7·9 선수리)은 **이행하지 않았다**. 따라서 아래가 살아 있다:

| #     | 위험                                                                       | 실측                                                            |
| ----- | -------------------------------------------------------------------------- | --------------------------------------------------------------- |
| 7     | `subprocess.run(timeout=1800)` 이 `TimeoutExpired` 를 안 잡는다            | ★B회차를 죽였다. 남은 트리 = **AC 0건 실행 + `completed` 커밋** |
| 9     | non-zero exit code 가 완료를 못 막는다                                     | fail-open                                                       |
| 1·2·3 | clean-tree 검사 0건 · 실패 경로에서도 커밋 · `_finalize` 가 커밋 실패 무시 | 전건 P1                                                         |
| 4     | 권한 bypass 를 쓰면서 워크트리를 강제하지 않는다                           | 코드가 메인 실행을 안 막는다                                    |
| 11    | AC 가 세션이 변조했을 수도 있는 대상을 실행한다                            | 이번 판엔 `ac` 필드 자체가 없다                                 |

★**7번의 최악 갈래는 재실행이다.** 러너가 traceback 으로 죽어도 세션이 이미 써 둔 `completed` 는
파일에 남는다 ⇒ 다시 돌리면 그 step 은 `pending` 이 아니라 **건너뛴 채 phase 가 닫힌다.**

★**막고 있는 것은 코드가 아니라 규약 2개다** — ⑴ 워크트리 전용 ⑵ `--push` 금지.
A/B 파일럿이 원격 유출을 면한 것이 ⑵ 하나 덕이었다.

**권장 접근:** 트리거가 도래하면 **고치지 말고 뗀다.** 같은 사고의 2회 재현은 「as-is 로 둘 수 있다」의
