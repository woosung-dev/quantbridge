...
| [BL-814](#bl-814) | 🔵 **`_HINTS[name]` 이 `Object.prototype` 을 상속해 상속 키 5종이 fallback 을 못 받는다**. `toString`·`__proto__`·`constructor`·`valueOf`·`hasOwnProperty` 는 `{name}` 만 나오고 **`hint`·`category` 가 없다**(실측 2026-08-21) — 소비자 `form-error-inline.tsx` 가 `hint.hint` 를 렌더하고 `hint.category` 로 아이콘을 고르므로 **화면에 `undefined`**. 주석의 「mapping 미존재 시 generic fallback」이 그 5종에 대해 거짓이다. 처방 = `hasOwnProperty.call` 가드 또는 `Map`(1~2줄). ★지금 동작은 PR #732 가 고정해 뒀다 — 고치면 그 케이스가 red 로 뒤집힌다(그것이 종결 신호)                                                                                                                                                                                                                          | 도래 — 2026-08-21 실측                                                                                          | XS(수리)+S(BE 경로 확인) | 2026-08-21 밤샘 루프 2차                                     |
...
**Category:** 버그 / 프런트엔드
**상태:** 🔵 **ACTIVE** — 지금 단독 착수 가능 (수리는 1~2줄, 판정만 필요하다)
**Priority:** P3
