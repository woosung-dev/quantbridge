...
| [BL-583](#bl-583)    | ★**BE 스위트가 실행 순서에 따라 red/green 이 바뀐다** — 기존 테스트로 재현 확정, 뿌리 미규명. green 이 운의 산물일 수 있다                                                   | 게이트가 이유 없이 red 이거나 같은 커밋이 두 번 다른 결과를 낼 때                                               | M            | 2026-08-03 metric-guard-residual                       |
...
**Est:** M
**상태:** 🟢 **열려 있다 — 재현 확정, 뿌리 미규명.** 2026-08-03 metric-guard-residual 부수 발견.
**출처:** 2026-08-03 metric-guard-residual (고장 주입 테스트 작성 중 노출)

★**BE 스위트가 실행 순서에 따라 red/green 이 바뀐다.** `pytest-randomly` 가 매 실행 순서를
섞으므로 **이 red 는 우연히 나타났다 사라진다.**

★**새 테스트의 문제가 아니다** — 손대지 않은 **기존** 테스트로 대조 확정했다:

```bash
cd backend && set -a; . ./.env.local; set +a
uv run pytest tests/trading/test_cancel_order_task.py -p no:randomly                 # 2 passed
uv run pytest tests/tasks/test_live_conditional_divergence_labels.py \
              tests/trading/test_cancel_order_task.py -p no:randomly                 # 2 failed
```

**증상.** `_async_cancel_order` 가 `{"skipped": "not_found"}` 를 낸다 — fixture 가 방금 만들고
...
green 이었는데 부분 조합은 red 였다 — **green 이 운의 산물일 수 있다.**
