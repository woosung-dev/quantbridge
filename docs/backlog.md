...
| ID                | 제목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Trigger                                                                                                         | Est             | 출처                                                         |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------- | ------------------------------------------------------------ |
| [BL-820](#bl-820) | 🟢 **밤샘 루프 6차 — FE·BE 커버리지 + 린트 부채 180건 (12 lane × step 4 · 동시 4).** 저작 완료·실행 대기. ★**「FE 축은 바닥」은 AST 축의 결론이었다** — 커버리지로 재니 `apps/web` **85.04%** · 미커버 **4,597줄/344파일** 이고 `api.ts`·`hooks.ts` 12파일이 통째로 비었다. a11y **67건** + 린트 부채 113건 동승. 착수 전 AC red **12/12** · 판정기 판별력 15/15. 동승 종결 = coverage `concurrency`(TOTAL 90%→**91.51%**) | 도래 — 2026-08-22 커버리지·린트 실측 | L (12 lane 병렬) | 2026-08-22 night-loop-6 |
...
| [BL-818](#bl-818) | ✅ **BE 미커버 모듈 커버리지 — Resolved (2026-08-21 밤샘 루프 5차)**. 8 lane 병렬 · **8/8 completed · 충돌 0 · 변이 8/8 red** · PR #755(① 사전 배치) + #756~#762 + #764, 결함 수리 #763. `apps/api` **5,165 → 5,292 collected**(신규 8파일 · 92 함수 → **+127 케이스**) · 대상 소스 전건 무변경. ★★**재료를 AST 가 아니라 커버리지가 정했다** — 초판 8 lane 중 **둘이 실제로는 100% 커버**였고 셋은 89~94% 였다. ★★★**blocked 2건 중 하나가 진짜 제품 결함**([BL-819]) — 나머지 하나는 내 낡은 테스트 DB. ★부수 — `[tool.coverage.run]` 에 `concurrency` 부재로 async 커버리지가 낮게 나온다([BL-308]/[BL-309] 래칫 동반 영향, 미해결). 본문은 `backlog-resolved.md` 소관                                                                                                                                 | 도래 — 2026-08-21 커버리지 실측                                                                                 | M (8 lane 병렬) | 2026-08-21 밤샘 루프 5차                                     |
| [BL-819](#bl-819) | ✅ **`build_backtest_service_for_worker()` 가 기본 설정(`fixture`)에서 `UnboundLocalError` — Resolved (2026-08-21 밤샘 루프 5차)**. 함수-지역 재import 가 `OHLCVRepository` 를 **함수 전체에서 지역화**해, else 를 안 타는 fixture 갈래의 반환부가 미바인딩 지역변수를 읽었다. ★**무인 세션이 `blocked` 로 잡았고 코드 대조·실증으로 확인**했다. 배포는 `timescale` 이라 잠복이지만 **코드 기본값이 `fixture`** 다. 수리 = 중복 import 1줄 삭제. 본문은 `backlog-resolved.md` 소관                                                                                                                                                                                                                                                                                                                        | 도래 — 2026-08-21 실측                                                                                          | XS (1줄)        | 2026-08-21 밤샘 루프 5차                                     |
...
실행 = `python3 tools/harness/night_run.py --stage stage/night6 --jobs 4 --confirm` (기본 dry-run).
