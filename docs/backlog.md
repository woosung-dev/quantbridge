...
**Est:** M (설계 선행 필요)
**상태:** ⬜ **Open — 트리거 도래로 승격** (2026-08-17 레인 γ 재판정). carry 는 여전히 bar_time < window_start 단일 절단(3445)이고 2-pass 재실행 흔적이 없다 — epoch 재계산(3562)은 다른 사고다. (2026-08-09 status-triage-mass 확인)
**트리거 판정:** ★**도래** — 트리거 「[BL-488] 해소 후」의 [BL-488] 이 Resolved 이고 그 산출물 `emit_from_bar_time` 이 `tasks/live_signal.py:3457,3462,3577` 에 실재한다. ★★**단, 이 승격의 마지막 한 걸음은 근거가 아니라 해석이다** — 종전 이 줄은 「동승 조건」 문구였는데, 그것은 도래 여부가 아니라 **우선순위 주장**이고 DEFERRED 84건에 일괄로 찍혀 있었다(레인 γ 실측). 트리거 **문장**을 정본으로 본 것이 판정 근거이며 사람이 뒤집을 수 있다. (2026-08-17 deferred-resweep)
