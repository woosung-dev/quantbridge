...
| [BL-643](#bl-643) | `docs/status.md` 「다음 스프린트」 블록의 최신성을 어떤 게이트도 안 본다 — 끝난 일을 지시하는 「다음 행동」 2곳이 살아 있는 동안 bl-audit·docs-audit 이 둘 다 exit 0 이었다. ★단순 문자열 술어는 오탐한다(규칙을 설명하는 문장이 걸린다) ⇒ 「다음 행동」을 §G8 6필드의 7번째로 승격해 「정확히 1개」를 계약화하는 것이 선행                                                                                                                                                                                                                                                                                                                                                                                                                                           | 다음 회차 종결 시                                                                                               | S            | 2026-08-08 bl003-unblock                            |
...
「다음 행동」은 §G8 의 **6필드 실행 계약에 없는 임시 어구**라 셀 수가 없다. 그것을 **7번째 필드로 승격**해
「블록 안에 정확히 1개」를 계약으로 못 박으면, `scripts/docs-audit.sh` 에 `docs/status.md` 대상
`count == 1` 검사를 붙일 수 있다. ★부르는 자리는 **새로 만들 필요가 없다** —
`scripts/final-gates.sh:162` 와 CI 가 `docs-audit.sh` 를 이미 부른다(LESSON-078 조건 충족).
