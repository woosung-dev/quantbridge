...
| [BL-591](#bl-591)    | ★**뿌리** — 엔진 포지션의 SSOT 가 없다. `run_live` 시뮬이 매 tick 봉을 재생해 포지션을 **도출**하고 정상 운행 중엔 현실로 **보정되지 않는다**. 슬라이스 1(계측) 머지(PR #539). ★★★**슬라이스 2 미착수 확정** — 사전등록 V1 발동(④ = 0: 사망 2건의 상류에 `exchange_only` 0건 · 최악 상계 ≤1/21). ★★★**유도 함수 재설계 필요** — `trade_id` 는 trade 가 아니라 Pine 진입 규칙 이름이고(`PivRevSE` 56체결/19세션) 반전은 `:close:` 키를 안 만든다 ⇒ 판정 불가 **27.6%**(전량 `duplicate_open`) · **net 은 맞고 legs 는 틀리다**(오라클 11건: 오답 0 · 적중 4 전부 `legs=2` 인데 거래소는 단일 포지션) | 발산 증상 BL 을 또 하나 열기 전에 · 소크가 또 죽었을 때                                                           | L            | 2026-08-03 breach-rejection-recovery                   |
...
[BL-591] 계측을 오래 돌리려면 그 전에 처분이 필요하다. 2026-08-04 회차는 **기록만** 했다.
...
**상태:** 🟢 **Open — 슬라이스 1(계측) 머지됨(PR #539). 슬라이스 2 는 사전등록 V1 발동으로 미착수 확정 (2026-08-04)**
...
**빗나감 0 · 적중 4 · 판정불가 7**. ★적중 4건이 **전부 `legs=2`** 인데 거래소는 단일 포지션이다
