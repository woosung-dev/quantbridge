...
**Est:** M (6-8h)
**상태:** 🔴 열려 있음 — OKX 는 여전히 REST 전용 — private WS 스트림 파일이 없고 websocket_task.py:277 이 미구현을 주석으로 명시한다. (2026-08-09 status-triage-mass 확인)
**출처:** TODO.md L710
...
**Est:** M (5-6h)
**상태:** 🔴 열려 있음 — M4 의 xfail(strict=False) 가 그대로 남아 있고 KIND-B/C·NaN-tolerance 재설계 흔적은 docs 외 코드에 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 첫 bar 커버리지 공백을 다루는 코드·테스트가 없다 — 평가 tick 지연 보정도, 재발행 보장도 미구현. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS
**상태:** 🔴 열려 있음 — 계획기는 여전히 qty_step 절삭만 하고 limits.amount.min 조회는 레포 어디에도 없다(있는 건 limits.cost.min 가드뿐). (2026-08-09 status-triage-mass 확인)
**출처:** 2026-07-27 live-conditional-entry
...
**Est:** S
**상태:** 🔴 열려 있음 — 계획기는 여전히 trade_id 순 정렬(620-621)이고 엔진은 open 거리순(strategy_state.py:1057) — 정렬 통일도 독스트링 명시도 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — cancel 루프(2456) 전량 후 place 루프(2501) 구조가 그대로고, amend/edit_order 는 레포 전체에 백로그 문장 외 구현이 0건이다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (3-5h)
**상태:** 🔴 열려 있음 — 사용자 결정 대기: PDF 관련 코드·의존성 0건이고, Trigger 자체가 외부 사용자 요청 + client/server 방식 선택이라 사용자 결정이 선행이다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — Bayesian 시각화는 여전히 1D best_so_far inline SVG 뿐 — N차원 surface/parallel-coord 컴포넌트가 optimizer 디렉터리에 없다(2D heatmap 은 grid_search 전용). (2026-08-09 status-triage-mass 확인)
...
**Trigger:** Sprint 56+
**Est:** S (3-5h, estimate)
**상태:** 🔴 열려 있음 — 3엔진 화이트리스트와 \_common.metric_value_for_objective switch 모두 sharpe/total_return/max_drawdown 3종 그대로 — 확장 미착수 (2026-08-09 status-triage-mass 확인)
...
**Est:** M (4-6h)
**상태:** 🔴 열려 있음 — GA·Bayesian 둘 다 비숫자 라벨을 여전히 명시 거부하고(BL-364 주석 포함), 테스트가 그 거부를 고정하고 있다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — 공유 factory(create_order_service_for_dispatch)는 레포에 없고, OrderService+킬스위치 인라인 조립이 dispatch 2곳·recovery 1곳·HTTP DI 로 4중 중복이다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — `_merge_exit_params` 가 여전히 키명 인자를 받고 3 call site 가 "orderLinkId"/"triggerBy"/"clOrdId" 문자열을 그대로 넘긴다. `build_ccxt_params_for_order` 는 레포에 없다. (2026-08-09 status-triage-mass 확인)
**출처:** `2026-06-26-trading-deepen-2.md`
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — 권장 helper(\_execute_create_order_with_ccxt) 가 레포에 없고 3 provider 의 try/except/finally+receipt 블록이 그대로 중복돼 있다. (2026-08-09 status-triage-mass 확인)
**출처:** `2026-06-26-trading-deepen-2.md`
...
**상태:** 🔴 열려 있음 — 9항목 중 tick정규화·ccxt assert·docstring·hedge가드·alert정제·회귀테스트·dead param은 구현됨, 하드코딩 BybitFuturesProvider() registry 우회(trading.py:1369-1371)만 잔존 (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — oco_group_id 컬럼·전달만 존재하고 코드 주석이 여전히 'Wave 2 deferred' — sibling-cancel 오케스트레이션 코드는 레포에 없고 Trigger(BL-365 standalone 발주)도 미도래 (2026-08-09 status-triage-mass 확인)
...
**Est:** M (4-6h)
**상태:** 🔴 열려 있음 — \_eval_subscript 가 여전히 \_var_series 만 보고 \_scope_stack 을 안 봄 — 추적도 unsupported reject 도 미구현. (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — INFORMATION/UNKNOWN 이 여전히 무경고 continue 이고, v2_adapter 는 state.warnings 만 전파해 VirtualRunResult.warnings 유실 그대로. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — VirtualRunResult 에 warnings 만 있고 var_series 필드·반환이 여전히 없어 추출기 getattr 이 빈 dict 를 digest 한다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — BE 는 sizing_source 를 config JSONB 에 저장하지만 BacktestConfigOut 에 없고, FE schemas.ts·AssumptionsCard 어디에도 sizing 표면화가 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-3h)
**상태:** 🔴 열려 있음 — stdlib.py:308 이 여전히 `source is not None and not _is_na(source)` 로 na occurrence 를 skip 하고, na 기록을 강제하는 테스트도 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS (1-2h)
**상태:** 🔴 열려 있음 — PineVersion enum은 여전히 v4/v5뿐이고 \_detect_version이 v6를 v5로 반환하며, DB enum에도 v6 값이 없다. (2026-08-09 status-triage-mass 확인)
**출처:** 2026-06-30 QA F3
...
**Trigger:** pine_v2 coverage 후속
**Est:** XS (1-2h)
**상태:** 🔴 열려 있음 — interpreter/coverage 양쪽 \_V4_ALIASES 에 abs/max/min 만 있고 floor·ceil·round·sqrt bare 별칭이 여전히 없다 (2026-08-09 status-triage-mass 확인)
**출처:** 2026-06-30 QA F4
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — SizingCanonical 타입 VO 미도입 — dict[str,Any] seam 그대로. 단 config 조립은 .get 이 아니라 직접 인덱싱이라 drift 는 KeyError(무음 아님). (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — entry stop/\_num 은 여전히 \_is_na 만 보고 isfinite 가드가 없고, \_coerce_length 에도 maxsize 상한이 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (4-8h)
**상태:** 🔴 열려 있음 — 거래소 fill-time 소싱 컬럼·경로가 없고 코드 주석 자체가 잔여 4건을 미해결로 기록 중이다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 처방 (a) 문서화만 완료; mintick 은 여전히 0.01 하드코딩이고 CCXT precision 소싱·틱 해석 opt-in config 는 코드에 전무. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (설계 선행 필요)
**상태:** 🔴 열려 있음 — carry 는 여전히 bar_time < window_start 단일 절단(3445)이고 2-pass 재실행 흔적이 없다 — epoch 재계산(3562)은 다른 사고다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M-L (구조 변경)
**상태:** 🔴 열려 있음 — 엔진 kwargs 에 margin_mode 가 여전히 없고(주석으로 명시) leverage_model 은 MMR 0.5% isolated 단일 모델 그대로다 (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — gauge 는 여전히 multiprocess_mode="sum" + inc 1곳/dec 13곳 구조이고, DB 개수를 .set() 하는 스냅샷 경로가 코드·테스트 어디에도 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — register() 계정 게이트는 여전히 exchange/mode 만 검사하고 read_only 는 청산·표시 경로에만 있다; 세션 폼도 읽기 전용 배지/비활성화 없음. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — stand-down 술어가 여전히 DB 행 id 축(list_active_by_account) — uid 기반 dedupe/list_by_exchange_uid 는 있으나 여기서 안 쓴다 (2026-08-09 status-triage-mass 확인)
...
**Trigger:** 프로덕션 배포 시
**Est:** S
**상태:** 🔴 열려 있음 — API 컨테이너 서비스도, production 미설정 경고 로그도 아직 없다 — 폴백은 여전히 무증상이다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-3h)
**상태:** 🔴 열려 있음 — live_blocked_leverage 분기와 liveLeverage===1 게이트가 그대로 있고 leverage 는 상수 1 로 초기화 — 미러 배선 미착수. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (4-6h)
**상태:** 🔴 열려 있음 — metrics.py 는 실재하나 \_v2*\* finance 헬퍼 12개가 여전히 v2_adapter.py(1239줄) L935-1167 에 남아 있어 이동 미완. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS-S (1-3h)
**상태:** 🔴 열려 있음 — 라우팅 삼항식이 v2_adapter :354/:838 에 여전히 char-identical 복제 — 헬퍼 위임 없음(줄번호만 265/568→354/838로 이동). (2026-08-09 status-triage-mass 확인)
...
**Est:** S (3-5h)
**상태:** 🔴 열려 있음 — lint 룰·auto-fix·pre-commit 훅 어느 것도 레포에 없고 위반은 181→197줄로 오히려 늘었다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — lint 룰(ESLint/ruff/pre-commit) 전무하고 main.py·config.py·layout.tsx 모두 여전히 헤더 없음 — 다만 근거였던 전역 §6 는 소멸 (2026-08-09 status-triage-mass 확인)
...
**Est:** XS-S (1-2h)
**상태:** 🔴 열려 있음 — \_async_dispatch_event(:4217~4472, 약 255 LOC)이 그대로 있고 mark_failed+commit+metric 반복이 9회, 추출 헬퍼는 부재. (2026-08-09 status-triage-mass 확인)
**출처:** `2026-06-26-trading-deepen-2.md`
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — OrderSubmit/Order/OrderRequest 3곳 exit-field 평행 정의가 그대로 살아 있고 ExitFields mixin 은 레포 전역 0건 — Trigger 도 미도래. (2026-08-09 status-triage-mass 확인)
**출처:** `2026-06-26-trading-deepen-2.md`
...
**Est:** M (4-6h)
**상태:** 🔴 열려 있음 — BE 집계 엔드포인트가 없다 — router 는 여전히 페이지형 /trades 뿐이고 FE 는 2000-cap 페이지 루프를 그대로 돈다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (6-8h, spike)
**상태:** 🔴 열려 있음 — 여전히 lightweight-charts ^4.2.0 이고, 차트는 createChart 2회 호출로 독립 인스턴스를 쌓는다 — v5 네이티브 pane 미도입. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-3h)
**상태:** 🔴 열려 있음 — 원장 CSV 는 cumulative_pnl·runup_abs·drawdown_abs·fee_paid·slippage_paid 를 내는데 서브페이지 상세는 손익·수익률·수수료 3종뿐 — 비정합 존속 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — SAR 구현·단위테스트는 있으나 전부 성질 검사뿐 — TV 손유도 오라클 값 대조는 코드·문서 어디에도 없다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 사용자 결정 대기: optimizer만 enabled: userId != null 유지, 타 도메인은 여전히 무가드 발사 — (a)통일 vs (b)제거는 사용자 정책 결정이 선행 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 3원화 대상 3계열(파일·의존성)이 전부 그대로 있고 선행 BL-395(lwc v5) 도 미완이라 Trigger 자체가 미도래. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — ta.alma/ta.dmi 는 TA_FUNCTIONS 에 없고 coverage 가 여전히 미지원 안내만 한다 — time 은 식별자만, 호출형 stub 없음 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-3h — 전부 표시 전용)
**상태:** 🔴 열려 있음 — 6건 중 1~5 (색명·radius·글래스·레이블·mono)는 v3 재작성으로 소멸했으나 6번 --destructive-light 중복 alias(=subtle 동일값, 사용처 6곳)와 영문 aria-label 다수가 남았다 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 사용자 결정 대기: (a) ta.ema 시딩은 여전히 SMA seed 그대로라 실제 TV 관측 없이는 대조 불가(순환검증), (b)는 nan 반환 그대로지만 관측 무영향. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 정렬은 여전히 metrics->>'sharpe_ratio' 캐스팅 서버 정렬이고 equity_curve 는 defer 중 — FE 고지만 있고 처방(재계산·분리)은 미구현. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — sharpe_convention 마커는 backtest 계열에만 존재하고 optimizer/stress_test JSONB 는 여전히 순수 sharpe 값만 저장한다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — mutationKey 가 여전히 ["close-position", sessionId, symbol] 이고 두 표 테스트도 같은 sessionId 를 주입한다 — 포지션 정체성 축 전환 흔적 없음 (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — collapseRows 가 아직 view 컴포넌트 안 지역 함수 그대로이고, features/trading 어디에도 분리된 순수 함수가 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — role별 dead-pid 접기 janitor(merge accumulate=False)가 코드·테스트 어디에도 없고, 있는 것은 콜드 스타트 wipe 뿐이다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 3종 처방 중 divergence 계측만 구현(BL-536), janitor 실적·완전체결 카운터는 여전히 부재. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — alerts.yml 은 여전히 alert 2개뿐이고 placed−cancelled recording rule 은 레포 어디에도 없다(record: 0건). (2026-08-09 status-triage-mass 확인)
**출처:** 2026-07-28 live-observability 적대 검증
...
**Est:** S
**상태:** 🔴 열려 있음 — 4건 중 1건만 BL-448 로 소멸했고, 관측 계약 문서화도 multiproc /metrics HTTP 테스트도 레포에 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS
**상태:** 🔴 열려 있음 — limit=10_000 과 limit=50 두 .set() 이 그대로 공존하고 gauge 에 라벨도 없다 — 처방 미적용. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h — 전부 국소)
**상태:** 🔴 열려 있음 — 8건 중 draft.ts version 스키마 1건만 구현, 배럴 2·webhook version·js 최적화 3·fitContent 설계 모두 코드에 그대로 남아 있다 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — read 응답은 여전히 result: dict[str,Any] 이고 \_to_response 가 raw jsonb 를 그대로 흘린다 — OptimizationResultOut 유니온 자체가 레포에 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — FE 에 getOrder 배선도 상세 드로어도 없고 프로토타입 screen-11 에도 상세 affordance 가 없어 Trigger 자체가 미도래. (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (3-5h)
**상태:** 🔴 열려 있음 — byBacktest 캐시는 여전히 useLatestStressTest 의 단일 Summary 이고 이력 리스트 화면·페이지 응답 재정의 모두 없다. (2026-08-09 status-triage-mass 확인)
**출처:** 2026-07-23 functional-parity 스프린트 defer 판정
...
**Est:** S (2-3h)
**상태:** 🔴 열려 있음 — 공용 FieldError 승격 미실시(optimizer 로컬 정의 + raw .field-error 사본 다수), resolver 는 여전히 path.join(".") 평탄 키이고 재검증 테스트도 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-4h)
**상태:** 🔴 열려 있음 — pre-auth 글로벌 상한·rate-limit 코드가 전무하고 auth→realtime 역참조도 dependencies.py:13 에 그대로 남아 있다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S-M (2-6h)
**상태:** 🔴 열려 있음 — public ticker 가 여전히 private 과 같은 ws_stream 큐(concurrency=3 고정) — 분리·계정수 산정·starvation 회귀 테스트 전무. lease 갱신 유닛만 존재. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — StrategyListItem 에 파라미터 요약·lifecycle 필드가 여전히 없고 FE 주석도 미렌더 사유를 그대로 유지 중이다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — owner-authed /trades/{i}/ohlcv 그대로이고 token 공개 OHLCV 경로도, share 페이지 trade 표도 없다 — 처방 미구현. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — §03 최적화 행의 수익률/MDD 칸이 여전히 EMPTY_CELL + "결과는 최적화 상세에서 확인" 고정이라 역산·objective 표기 미구현. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — SORT_OPTIONS 는 여전히 recent/name 둘뿐이고 정렬은 클라 로컬, BE strategy 목록에 성과 정렬 축(sort 파라미터) 자체가 없다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 청산 후 잔여 조건부 주문 스윕 코드가 close_service 어디에도 없다; 계정 배타성만 갖춰졌고 post-fill flat 확인·세션 귀속 취소는 미구현. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2-3h)
**상태:** 🔴 열려 있음 — cancelled 승자 backfill 예약도, SUM 의 realized_pnl_synced_at 기준 확대도 미구현. limit 청산 매핑은 있으나 미배선이라 Trigger 미도래. (2026-08-09 status-triage-mass 확인)
...
**Est:** M (4-6h)
**상태:** 🔴 열려 있음 — 부분체결 계측(qb_partial_fill_total)만 있고 warmup-replay 수량 보정도 세션 fail-closed 비활성화도 없다 — 조건부 진입 tick 판정 불가 처리가 전부. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 분자는 여전히 strategy_id+filled 전 기간 합(시간창 없음), 분모는 trigger 시점 실잔고 — 스냅샷/기간창 처방 흔적 없음 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (2h)
**상태:** 🔴 열려 있음 — sanitize 미적용 — WS 는 여전히 str(...,""), reconciler 는 str(exch.get("id",...)), transition_to_filled 는 str 무조건 write, 계정 스코프도 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S (1h, 마이그레이션 1)
**상태:** 🔴 열려 있음 — webhook_payload 는 아직 plain JSONB 이고 'null' 정규화 마이그레이션도 없다 — none_as_null 은 ExchangeExit.raw 에만 적용됐다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S (1h)
**상태:** 🔴 열려 있음 — get_daily_summary(date) 는 여전히 date 인자 하나뿐이고 user/account 조인이 없어 전 테넌트 글로벌 합계다. (2026-08-09 status-triage-mass 확인)
**출처:** 2026-07-25 exit-attribution grounding 실측
...
**Trigger:** 즉시 (부분 완화 완료)
**Est:** S (2h)
**상태:** 🔴 열려 있음 — 잔여 4항목 중 ③(문서 승격)만 됐고 ①conftest 가드 없음 ②백업 레시피 부재(Makefile pg_dump 0건) ④alembic CLI 무가드. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 일회성 catch-up 경로가 레포에 없고, ASC 좀비 정렬(order_repository.py:769)과 meta 커서 tie(-1)도 그대로다 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — close_service 는 여전히 Order 만 만들고 이벤트를 안 남기며, 테스트가 'manual_close 는 세션 역인덱스에 안 잡힌다'를 그대로 고정하고 있다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 창은 여전히 filled_at 반열림 그대로이고(:225/:233/:238) 권장 선행조건인 filled_at−created_at 간극 실측 기록이 레포에 없다. (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — 세션 행을 파이썬에서 읽어 SessionScope 를 만든 뒤 별도 SELECT 로 주문을 조회하는 구조가 두 소비처에 그대로다 — 단일 조인 없음. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — config 기본값은 여전히 CWD 상대이고 fixture.py:30 이 심볼 슬래시를 그대로 경로에 넣는다 — 두 결함 모두 미수정. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 잔여 spec 에만 시끄러운 실패가 들어갔고 p1 spec 은 여전히 데이터 없으면 test.skip — 행수·체결수 전제 단정은 어디에도 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — on_conflict_do_nothing 멱등 적재가 그대로고, classification_version 컬럼도 재분류 마이그레이션도 레포에 전혀 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 목록 title 이 여전히 legacy/unavailable 일 때만 붙고, monthly/daily 는 undefined 라 각주가 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — compute_position_size 는 레포 전체에서 backlog 문서에만 존재하고 quantity 도 여전히 Field(gt=0) 필수 — 수량 산출 미구현. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — webhook 라우터가 여전히 동기로 OrderService.execute 를 호출하고 가드의 CCXT 3회(mark/min_notional/balance)가 그대로 인라인이다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** ✅ Resolved — BL-605 수리로 closed-pnl 스윕이 exchange_uid 단위 dedupe 를 하므로 같은 서브계정 이중 적재(phantom unknown)가 소멸했다. (2026-08-09 status-triage-mass 코드 대조)
**출처:** 2026-07-26 BL-474 dogfood 실측
...
**Est:** M
**상태:** 🔴 열려 있음 — entry 의 limit/trail/qty_percent 는 여전히 unsupported 로 버려지고, PendingOrder 에 limit_price 필드가 없다(있는 건 exit leg 뿐). (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — run_live 는 아직 run_historical 만 호출하고, trading 라우터/서비스 어디에도 Track 판정·422 가드가 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — pnl_by_trade 는 여전히 t.id 단일 키 dict 이고 거짓 전제 주석도 그대로이며, catch-up 정상 경로도 유지된다 (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — created 창은 BL-536 진입 원장에만 열렸고, /state·손실한도·parity 3소비처는 여전히 terminal 창 기본값 + grace 없음 (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — \_to_scope 36필드 수동 평탄화·private \_session_scope_where import·linked/confirmed 술어 중복이 모두 그대로 남아 있다. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS
**상태:** 🔴 열려 있음 — \_sum_decimals 사본이 여전히 2벌이고 리포지토리 호출부 4곳(92·159·169·174)은 localcontext(PARITY_DECIMAL_CONTEXT) 밖 그대로다. (2026-08-09 status-triage-mass 확인)
...
**Est:** XS
**상태:** 🔴 열려 있음 — 오라클 총계를 관측 1건에 몰고 26건을 0으로 채우는 구조·테스트 이름 모두 그대로다(:47, :54-66). (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — 차단 사유만 있고 원장 귀속 폴백은 미구현(position_service 는 세션 없으면 즉시 no_owning_session), Trigger 관측도 아직 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 메시지가 여전히 stage 분기 없는 단일 f-string 이고 '전략 수정 후 재활성화 필요' 하드코딩, remedy 원소 부재. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — get_state 는 1회로 줄었으나 deactivate 의식 중복(헬퍼로 쪼갰지만 본문 동일·테스트가 7건 동결)·provider+creds 5곳·category 맨 str 이 그대로다 (2026-08-09 status-triage-mass 확인)
...
**상태:** 🔴 열려 있음 — seed 경로가 여전히 float(fill.filled_quantity/price) 로 강등하고, Trade.qty 도 float이며 dust 상한을 고정하는 테스트가 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — OpenPositionsTable 은 여전히 is_active 필터된 activeSessions 만 받는다 — 비활성 세션 per-session 대조 UI 미구현. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — 코크핏 선택은 여전히 useState(:82)이고 trading 트리 어디에도 useSearchParams/searchParams 사용이 없다 (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — seed 는 여전히 gap tick 1회에만 계산되고 `_qb_ledger_seed_since` watermark 는 레포에 0건 — 처방 미착수 (2026-08-09 status-triage-mass 확인)
...
**Est:** XS (검증만 — 코드 변경 없음)
**상태:** 🔴 열려 있음 — 코드 변경 없는 실주행 관측 항목 — 계측은 그대로 있고 관측된 outcome 은 no_basis 뿐, applied>0 근거 0건. (2026-08-09 status-triage-mass 확인)
...
**Est:** S
**상태:** 🔴 열려 있음 — Gauge 그대로이고 inc 1곳(order_service:457) vs dec 17곳 비대칭 유지 — created/terminal Counter도 terminal 단일 훅도 없다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🔴 열려 있음 — 구조화 코드 컬럼이 없고(models.py 에 error_message 문자열뿐), WS·submission 경로는 여전히 평문이라 retCode 정규식 파싱에 의존한다. (2026-08-09 status-triage-mass 확인)
...
**Est:** M
**상태:** 🟡 **부분 해결 — 측정과 등재는 끝났다. 사망률은 그대로다** (2026-08-08
soak-exclusivity-and-observability 회차). ⑴ 층화 + **95% 신뢰구간**을 [ADR-024] 에 등재했고
