...
| [BL-633](#bl-633) | ★★**[ADR-025] Accepted 이후 `phantom` 이 재발했고 그대로 세션을 죽였다** — 2026-08-07 세션 `39484a2c` 가 `15:09:49`·`15:10:49` phantom 2건 뒤 `15:10:49.561534` 에 `position_divergence` 자동 사망(phantom→사망 **2/2**). 게이트 FAIL, C1 5.31h·C2 5.24h **전부 리셋**. ADR-025 는 노출 **12.28h phantom 0건**으로 Accepted 됐는데 이 세션은 **~5.7h 에 2건**이다. ★단 「phantom」 판별식 자체가 과거에 흔들린 전례가 있어 **재판정이 선행**이다 | ★**즉시** — 다음 소크 창을 열기 전에 | M | 2026-08-07 backtest-fidelity |
...
**Priority:** P1
**카테고리:** Trading / 라이브 신호 (phantom 재발 · [ADR-025] 반례 후보)
**Trigger:** ★**즉시** — 다음 소크 창을 열기 전에 읽어라
**Est:** M
**상태:** ⬜ **Open**

**[ADR-025] Accepted 이후 `phantom` 이 재발했고 그대로 세션을 죽였다.**

2026-08-07 backtest-fidelity 회차 **중에** 소크 세션 `39484a2c` 가 죽었다(회차와 무관 — 서버 HEAD
`0c75aaa3` · 고정 커밋 `0c9ccc68` 불변, 그 회차는 서버에 아무것도 배포하지 않았다).
...
분류기 `2026-08-05-recovery-ratchet`, `classifier_ok: true`. 같은 세션 `10:18:49` 1건은 무해 `replay_lag`.
게이트 **FAIL 실격** · C1 **5.31h → 0.0000h** · C2 **5.24h → 0.0000h**.

**왜 P1 인가.** [ADR-025] 는 노출 **12.28h 에서 phantom 0건**(p≈0.020 기각 성립)을 4관측량 중 ①로
삼아 **Accepted** 됐다. 이 세션은 노출 **~5.7h 에 2건**이고 **둘 다 60초 안에 사망으로 이어졌다**
(phantom→사망 **2/2**). 그 판정의 근거가 흔들린다.

★★**단, 결론으로 쓰기 전에 판별식을 먼저 재판정해라.** 「phantom」은 `classify_direction_divergence.py`
가 붙이는 라벨이고, **그 판별식의 성질 자체가 과거 회차에서 흔들린 전례가 있다**
(2026-08-05: phantom→사망 2/7, 분류기 자기 불일치 4건, 경과가 연속 분포). 이번 2건은
**같은 분(minute) 안에서 사망과 붙어 있어** 그 계열과 서명이 다르다 — 그 차이가 실재하는지가 첫 질문이다.

**다음 창에서 물을 것:**

1. 두 phantom 의 원문(주문 id · 봉 · 엔진/원장 상태)을 떠서 [BL-595] 형 A 인지 확인해라.
2. [ADR-025] 가 닫았다고 한 경로를 실제로 밟았는지 — `qb_live_conditional_fill_ownership_total`
   차분과 대조해라(같은 창의 원자료는 `.claude/fleet/backtest-fidelity/artifacts/` 에 동결돼 있다:
   T0 `13:40:18Z` 71/21/10 → T1 `15:17:20Z` 89/24/13).
3. ★그 창의 A-1 판정은 **`undecidable`(D=24 < 30)** 이었다 — **이 사망을 그 표본으로 설명하지 마라.**

**Risk:** 🔴 [BL-003] — P0 — 의 직접 차단자다. 24h 연속을 채우려면 MTBF 가 24h 를 넘어야 하는데
이 세션은 **5.7h** 를 살았다.
