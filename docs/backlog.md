...
★★**2026-08-25 4번째 측정점 = 1,715MB — 그리고 「빌드 직후에 재라」는 처방이 반증됐다.** 메인에서 `pnpm build` 를 돌린 뒤 재니 1,711 → **1,715MB**(**+4MB**)뿐이다. 구성을 가르면 `.next/dev` **1,618MB**(`dev/cache` **1,462MB**) 대 **프로덕션 빌드 산출 전체 85.7MB** — 이 표본의 **94.4% 가 dev 서버 turbopack 캐시**다. ⇒ **재야 할 축은 `.next/dev/cache`, 그것을 자라게 하는 것은 `pnpm build` 가 아니라 `mise run fe` 가동 시간이다.** 선행 측정점(1.99GB 사망 · 593MB 무해)도 같은 `du -sm .next` 라 축은 처음부터 일관됐다 — 틀린 것은 늘리는 방법에 대한 처방뿐이었다.
★**2026-08-25 n11 preflight 3번째 재채취도 1,711MB — 그리고 같은 값인 이유를 쟀다**: `apps/web/.next` mtime 이 **2026-08-22 01:30** 이라 그 사이 메인이 FE 를 한 번도 빌드하지 않았다. **재채취로는 측정점이 늘지 않는다**(같은 표본을 다시 재는 것뿐) — 이 항목을 닫으려면 **메인에서 FE 빌드를 돌린 직후** 채취해야 한다.
