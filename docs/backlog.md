...
**게이트 현황 (2026-08-05 기계 판정 개시):** `scripts/soak-gate.sh` = **FAIL** (exit 1) — 누적 **0h / 168h**. 소크는 돌고 있다(세션 `a16aa640` · 커밋 `f5f06886` = main 조상). ★★★**이 항목의 실제 차단자는 「달력 시간」이 아니라 `phantom` 이다** — 게이트 가동 첫 5시간에 **phantom 7건 · 사망 2건**, 소크 수명 **38분 / 138분**(둘 다 `position_divergence`). 현행 MTBF 1~2.3h 로는 연속 24h 는커녕 168h 누적도 못 채운다. ⇒ 선행 조건 = **[BL-591]/[ADR-023] 축**. ★★**판별식도 흔들린다** — phantom→사망 **2/7**, 분류기 자신이 「봉경계식 vs 시간문턱식 불일치 4건」 보고, 경과가 `6.9~762.4초` **연속 분포**(직전 회차의 「37배·겹침 0」은 표본의 성질이었다). ★★★**과거 56.44h 는 소급 인정하지 않는다** — 30세션 재생 시 **귀속 가능 0.46%**(그 시절 워커는 움직이는 작업 트리를 mount 했다). 그리고 **역대 2위 8.65h(`0e15c3c0`)는 마지막 46.7분 동안 평가가 멈춰 있었다** — 「역대 최장 15.3h = 9%」는 두 겹으로 낙관이었다. **시계는 `scripts/soak-stack.sh` 가 소크를 커밋에 고정해 돌린다**(편집해도 워커가 재적재되지 않는다 — 음성 대조로 확인).
...
**상태:** 🟢 **Open — 슬라이스 1(계측)은 PR #539 OPEN(미머지, 통합 브랜치). 슬라이스 2 는 사전등록 V1 발동으로 미착수 확정. ★2026-08-04 후속 회차에서 C 안이 「예방 전용·사망 경로 구조적 미도달」로 축소되고 사망 경로의 수리 축은 [ADR-023](decisions/023-engine-state-ssot.md)(Proposed)으로 이관됐다. ★★2026-08-04 3차에서 `direction` 이 「무해 7 : 치명 4」로 갈렸고(37배 이중분포·사망 상관 2/2) 치명 갈래는 [BL-590] 수리 뒤 0건/6.12h 다(95% 상계 0.49/h — 닫혔다고 쓰지 마라). ★★2026-08-04 4차에서 **슬라이스 A 를 동결**했다(사용자 결정) — 관측 전용이라 방어를 늘리지 않고, 자동 사망이 0건이라 급하지 않다. 설계·판별식·20 테스트·오라클은 레포에 있고 **전사만 남았다**. **재개 조건 = `phantom` 이 1건이라도 재발**하면 즉시 착수 + [ADR-023] 긴급도 복원. 전향 예측은 소크 중단으로 **최종 미판정**(post-fix 누적 7.24h · phantom 0 · 95% 상계 0.41/h — 누적이라 다음 소크에서 이어 잰다)**
