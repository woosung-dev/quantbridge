...
>
> ★★**2026-08-16 수명 분리 ([BL-779]).** 원장은 이제 **파일 둘**이다 —
> 본 파일(열린 것) + [`backlog-resolved.md`](backlog-resolved.md)(**RESOLVED 118건 본문**).
> `bl-audit.sh`·`docs-audit.sh`·`bl-trigger-sweep.sh`·`context-budget.sh` 가 둘을 **한 벌로** 읽고,
...
**상태:** ⬜ Open — RESOLVED 축은 닫혔고 **감축 목표(40%)가 열려 있다** (2026-08-16 bl779-backlog-split).
...
**Title:** e2e 의 `click({ force: true })` 가 **하이드레이션 전에 발화**한다 — 게이트 부하에서만 red 이고 단독 실행은 항상 green
...
**원인 / 영향:** `force: true` 는 Playwright 의 **actionability 검사를 건너뛴다.** React 가 하이드레이션으로
onClick 을 붙이기 전에 클릭이 떨어지면 핸들러가 없어 요청이 나가지 않고, `page.waitForRequest` 가
timeout 으로 죽는다. 증상은 「제출 버튼을 눌렀는데 POST 가 없다」이고 **브라우저가 submit 을 발화조차
안 한** 2026-08-10 프로덕션 사고와 같은 모양이라 원인 오인이 쉽다.

★**레포가 이미 알고 있었다** — `apps/web/e2e/dogfood-flow.spec.ts:152` 주석이
「`force: true` click 가 React onClick handler 안 도달하는 issue 우회」라고 적는다.
**`force` 를 우회책으로 썼지만 실은 `force` 가 원인이다.** [BL-775] — 하이드레이션 경쟁, 2026-08-16 종결 — 와 같은 가족.

★★**왜 지금까지 안 보였나 — 그 레그가 안 돌았다.** `final-gates.sh:378` 의 `e2e authed` 술어는
`has_fe ∪ has_be` 다. 2026-08-17 회차의 세 브랜치 중 `apps/web`·`apps/api/src` 를 건드린 것은
[BL-773] 하나뿐이라(7파일), **#651·#652 는 이 레그가 skip 된 채 유예 원장이 지워졌다.**
영역 판정으로는 옳은 skip 이지만 「원장이 지워졌다 = 종결」이 담는 증거량은 브랜치마다 다르다.

**잔존 2곳** (이 회차는 실패한 1곳만 고쳤다 — 범위 밖):

- `apps/web/e2e/sprint46-tier3-nth.spec.ts:559` — 같은 `backtest-submit` 강제 클릭
- `apps/web/e2e/dogfood-flow.spec.ts:152` — 위 주석이 달린 자리

**권장 접근:** ⑴ 클릭 전에 **상호작용 가능**을 기다린다(`toBeEnabled` + 하이드레이션의 관측 가능한 신호)
⑵ `force: true` 를 없앤다. 정말 필요한 자리면 사유를 주석으로 남긴다
⑶ ★**대기를 다시 빼면 red 인지 변이로 확인해라** — 없으면 「대기를 넣었더니 통과했다」가 우연과 구별되지 않는다
⑷ ★**timeout 예산을 늘리지 마라** — 진짜 회귀도 못 잡게 된다([BL-775] 회차 교훈)

**Risk:** 🟡 (테스트 전용. 다만 잘못 고치면 판별력이 사라져 진짜 회귀를 놓친다)

**상태:** 🟡 부분 해결 — 2026-08-17 에 `sprint46-tier1-critical.spec.ts:133` 1곳 수리. 잔존 2곳 미착수
