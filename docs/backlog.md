...
> ★**2026-08-13 docs-diet.** RESOLVED **78건**의 본문을 접었다 — 각 섹션에 `### BL-nnn` + `**Title:**` +
> `**Priority:**` + `**상태:**` 넉 줄만 남기고 나머지는 `📦 본문 접힘` 1줄로 대체했다(−146,412자, −22.0%).
> 접힌 78건의 원문 전량 = `git show 8abd0d67:docs/backlog.md`. ★남긴 넉 줄은 취향이 아니라 **`bl-audit.sh` 가
> 4면 정합에 쓰는 입력 전부**다 — 하나라도 지우면 게이트가 red 다(음성 대조 2건으로 확인).
>
