...
5. ★**남은 축 — 40% 감축은 RESOLVED 만으로는 도달할 수 없다.** 실측 감축은 **21.5%**(704,265 → 552,666자)
   이고 ⑴ 의 상한이 그 천장이다. 더 내리려면 **열린 항목의 수명 재분류**가 필요하다:
   ⒜ RESOLVED 인덱스 표 행 60,030자(8.5%p)도 함께 내린다 — 단 ⑵ 의 「원본에 색인을 남긴다」와 상충하고,
   ★행을 `backlog-resolved.md#bl-nnn` 로 다시 가리키면 prettier 가 ID 열을 19자 넓혀 **최장 줄 993 → 1,012자**
   로 **1,000자 상한을 밟는다**(실측). ⒝ DEFERRED 178건 265,092자(37.6%)를 `backlog-deferred.md` 로 되돌린다
   — 2026-08-06 에 `_deferred.md` 를 합쳤던 것을 되돌리는 것이라 **사용자 결정 사항**이다.
