...
| [BL-724](#bl-724) | ★**수수료가 gross 를 16배로 먹는다** — 고유 청산 592건에서 gross **+74.18** vs 수수료 **−1,200.00** ⇒ 순 **−1,125.81**. `closedPnl` 은 수수료 차감 순액이고 요율은 단면 **0.055%** taker(검산 일치). 소크 게이트가 PASS 해도 이 사실은 안 바뀐다 — **수리가 아니라 판정 항목**이고 산출물은 「실자금으로 가도 되는가」의 숫자다                                                                                                                                                                                                                                                          | ★즉시 — [BL-003] cutover 판단 전                                             | M (2-3h) | 2026-08-14 money-path-attribution |
...
| [BL-726](#bl-726) | `rejected` + reduce-only 주문 **46건**에 `realized_pnl` **+55.32** 가 채워져 있는데 evaluator 2곳이 `state==filled` 로 거른다. 「값이 채워졌다(=청산으로 봤다)」와 「rejected(=발주 실패)」가 **모순**이고 어느 쪽이 옳은지 판정된 적이 없다. 부호가 + 라 게이트를 느슨하게 만드는 쪽                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | 동승 — [BL-438] 백필 수리와 같은 SUM                                                                            | S (1h)       | 2026-08-14 money-path-attribution                            |
| [BL-727](#bl-727) | `soak-gate.sh:706` 이 맨 `python3` 를 불러 **맥에서 판정을 못 낸다**(`/usr/bin/python3`=3.9.6, `itertools.pairwise` 는 3.10+). 죽고도 진행해 **`판정: ` 빈 줄**을 찍는 fail-open. ★같은 파일 `:449` 가 이미 `uv run python` 으로 고치며 「verdicts 가 늘 0 이 된다」고 적어 뒀는데 **판정 본체에는 적용 안 됐다**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | ★이미 발화 — 2026-08-14 맥 판독 사망                                                                            | XS (10분)    | 2026-08-14 money-path-attribution                            |
...
| ID                | 제목                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | Trigger                                                                                                           | Est       | 출처                                                   |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------- | --------- | ------------------------------------------------------ |
| [BL-728](#bl-728) | `classify_exit:64` 가 `"liquidation" in create_type` 인데 **Bybit 실제 enum 은 `CreateByLiq`** — 부분문자열이 안 걸려 강제청산이 `unknown` 으로 샌다(ADL 은 `"adl"` 로 걸린다). 관측 0건이라 잠복이고 실자금 전환 후엔 아니다. 처방 = 이미 있는 `_*_CREATE_TYPES` 집합 방식으로 통일                                                                                                                                                                                                                                                                                                                                                                                                  | 동승 — 강제청산 관측 또는 [BL-438] 분류 축                                                                        | XS (10분) | 2026-08-14 money-path-attribution                      |
...
**상태:** 🟡 **부분 해결 — 관측 원장은 섰고 계상 경로가 남았다. ★2026-08-14 로 진단이 교체됐다** (아래 「② 재평가가 반증됐다」 절이 정본). 구멍은 「청산이 DB 에 안 남는다」가 아니라 **「청산은 `exact` 561건으로 남아 있는데 `list_unsynced_reduce_only` 의 `reduce_only=true` 필터가 백필을 막는다」**이고, 규모는 **490건 / −1,023.87 USDT (실현손익의 90.9%)** 다. 종전 상태줄(2026-07-25) 원문은 이 줄 아래 유지한다 — 그 시점 판단의 기록이다.
...
**Title:** ★수수료가 gross 손익을 16배로 잡아먹는다 — 실자금 전환의 경제성 전제
...
**상태:** ⬜ Open — 미착수. 실측치만 확보됐다 (2026-08-14 money-path-attribution)
...
**상태:** ⬜ Open — 미착수 (2026-08-14 money-path-attribution)
**트리거 판정:** 미도래 — 동승 조건. [BL-438](#bl-438) 백필 수리 회차에 붙인다 (2026-08-14 money-path-attribution)
...
**상태:** ⬜ Open — 미착수 (2026-08-14 money-path-attribution)
**트리거 판정:** 미도래 — 동승 조건. 단독 착수 시 값이 0이라 [BL-438](#bl-438) 회차에 붙인다 (2026-08-14 money-path-attribution)
