...
| [BL-633](#bl-633) | ✅ **이중 호스트 오염 — 근인 확정** — 같은 Bybit demo 계정의 맥 로컬 체결이 서버 세션 `39484a2c` 를 죽였다. G-A4′ 소유권 6/6·G-A6 산술 닫힘 3/3·G-A5 size 51/51 이 이를 뒷받침한다. `phantom` 은 증상이며, 오염 창은 ADR-025 의 반례로 셀 수 없다                                                        | — (부검 완료 · 후속은 BL-634 · BL-641 로 이관)                               | M        | 2026-08-08 bl003-unblock    |
...
| [BL-637](#bl-637) | `bl-audit.sh` 가 우선순위 배치를 검사하지 않는다 — 수리 전 불일치 40건을 P0 1 / P1 9 / P2 56 / P3 38로 제자리에 옮겼으나, 인덱스 행이 섹션 Priority 와 같은 H2 표에 있는지는 여전히 검사하지 않는다                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 다음 백로그 인덱스를 편집할 때                                                                                  | S            | 2026-08-08 bl003-unblock                            |
...
**Est:** M
**상태:** ✅ **Resolved (2026-08-08 bl003-unblock 회차)** — G-A4′·G-A6·G-A5 로 이중 호스트 오염을 근인으로 확정했다.
...
**G-A4′ 소유권 = 6/6.** 서버 `exchange_exits` 에서 `matched_order_id IS NULL` 인 6건의
`order_link_id` 는 모두 로컬 `trading.orders.id` 에 실재한다. 내부 id 가 그대로 `order_link_id` 로
들어가는 규약이다.

- `adf667af` — 로컬 14:17:25.923 체결 / 서버 14:17:26.059 관측, buy 0.058
- `d7dd1d68` — 14:49:20.342 / 14:49:20.482, sell 0.116
- `897a71dd` — 15:09:41.517 / 15:09:41.694, buy 0.174
- `2cab1a3f` — 15:38:36.881 / 15:38:37.059, sell 0.058
- `f8ba3233` — 16:13:15.068 / 16:13:15.231, buy 0.058
- `ebf189cb` — 16:23:43.230 / 16:23:45.209, sell 0.058

6건 전부 `classification=unknown` 이고 각각 **정확히 2행** 중복이다 — BL-605 를 재확인한다.

**G-A6 산술 닫힘 = 3/3.** 서버가 본 거래소 포지션은 서버 엔진 포지션과 로컬 순포지션의 합으로
정확히 닫힌다 — `14:17:49 +0.087 = +0.029 + (+0.058)` ·
`14:49:49 -0.145 = -0.029 + (-0.116)` · `15:09:49 +0.029 = -0.029 + (+0.058)`.
서버 엔진은 세션당 고정 sizing 이므로 `0.087` 과 `0.145` 를 발주할 수 없다.

**G-A5 size 재현 = 51 정확 일치.** `category=size` 52건은 `14:17:49.558` ~
`15:08:49.945` 에 발화했고, 분 유니크는 51이다.

**실격 기록은 그대로 유효하다** — 분류기 `2026-08-05-recovery-ratchet` · `classifier_ok: true`,
게이트 **FAIL 실격**, C1 **5.31h → 0.0000h** · C2 **5.24h → 0.0000h**. 라벨이 오염된 표본에
붙었다는 것이지 리셋이 안 일어났다는 뜻이 아니다. 이 한 건이 BL-641 의 「실격은 C1 을 0 으로
되돌린다」를 실물로 보여 준다.

★★**정정.** 프롬프트와 직전 회차가 준 「사망 창 발산 관측 58」은 세션 경계를 보지 않았다.
세션 `39484a2c` 의 관측은 **55** — size 52 + direction 3 — 이다. `exchange_only` 3건인
09:35~09:37 은 이전 세션 `913c8681` 의 것이며 engine 0.0 / exchange 0.029 이다. 이는 세션 시작
시점에 계정이 이미 flat 이 아니었다는 증거다.

타임라인의 마지막 고리는 로컬 게이트 아카이브의 `log_from` 이 `2026-08-07T13:52:29.694330` 이라는
사실이다. 그것이 로컬 워커의 첫 로그 줄이자 `make up` 이 로컬 워커를 되살린 시각이고, 첫 오염 체결
14:17:26 은 그 **25분 뒤**다. 전문은 `.claude/fleet/bl003/artifacts/timeline.md` 에 있다.
...
이번 부검의 미조인 6건은 모두 `classification=unknown` 이고 각각 **정확히 2행** 중복이었다.
