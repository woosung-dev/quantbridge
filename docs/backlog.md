...
> 지금 기계로 집행되는 것은 `tools/scripts/ledger-vitals.sh` **4축뿐**이다(2026-08-31 ④ 신설 · 다음 행동 ≤1 ∧ ≥1 ·
...
**Title:** 라이트 캐논 게이트가 **텍스트×표면 몇 짝만** 본다 — 남은 미달 2건과, 잉크 계층 간격은 아무도 안 잰다
★**번호 정정 (2026-09-06).** 이 항목은 처음 `BL-855` 로 등재됐으나 그 번호를 동료 세션이 같은 날 먼저 썼다(임포트 `parse_status`). `AGENTS.md` §6 「ID 재사용 금지」에 따라 **857 로 옮긴다** — 내용은 그대로다. PR #872 의 커밋 메시지·본문은 옛 번호로 남아 있다(히스토리는 못 고친다).
**Category:** FE / 접근성 게이트 범위
**Priority:** P3
**출처:** 2026-09-06 [BL-851] 수리의 적대 검토(독립 3인, 전원 `refuted=false` 이나 잔여 지적)

**증상 (실측, `#555c65` 적용 후):**

⑴ **PAIRS 밖에 남은 미달 표면.** `light-canon-contrast.test.ts` PAIRS 는 텍스트 토큰을
`[--card, --bg, --bg-alt]` 까지만 본다. 그 밖에서 `--text-muted` 가 실제로 앉을 수 있는 표면 중
**`--destructive-subtle`/`--destructive-light`(#fae9e8) = 5.7602** 가 여전히 캐논 5.82 미달이다
(에러/철회 상태 카드의 본문 표면이다 — 예: `share-revoked-state.tsx`). 나머지 미달값
(`--border` 5.352 · `--input` 5.100 · `--primary-100` 5.055)은 **텍스트 표면이 아니라** 선·입력
테두리라 같은 잣대를 대면 통과 불가능한 래칫이 된다 — 늘릴 때 짝을 골라야 하는 이유다.

⑵ **잉크 계층 간격을 재는 것이 없다.** 모든 검사가 **텍스트 대 표면**이고 **텍스트 대 텍스트**는
0건이다. [BL-851] 로 `--text-secondary`(#4b535c) ↔ `--text-muted` 대비가 **1.2084 → 1.1539** 로
좁아졌다(다크 짝은 1.3725 — 라이트 3단 잉크 사다리가 원래 더 평평했고 더 평평해졌다).
캐논을 올리는 수리는 언제나 이 방향으로 압력을 준다. **한쪽만 재면 다른 쪽을 조용히 쓴다.**

⑶ **실화면 감사의 모델 한계**(수리 대상 아님, 기록용) — `design-canon-audit.ts:315` 의 `bgOf` 는
`backgroundColor` 만 읽고 조상만 타므로 **gradient 배경**(계산값이 투명)과 **형제 오버레이**
(`absolute inset-0`)를 못 본다. 즉 `canon=0` 은 「그 모델이 본 범위에서 0」이다.

**권장 접근:** ⑴ 은 짝을 고르는 판단이 먼저다(`--destructive-subtle` 을 밝히나, 잉크를 더
어둡게 하나 — 후자는 ⑵ 를 악화시킨다). ⑵ 는 인접 잉크 쌍의 **하한**을 새 축으로 넣는 것이
가장 싸다. ★**⑴ 과 ⑵ 는 반대 방향으로 당기므로 함께 정해야 한다.**
**Risk:** 🟢 (지금 무엇도 안 깨진다. 게이트 범위의 빚이다)
...
**Title:** `status.md` 가 **자기 크기를 1/2.9 로 적고**, 자기 상한을 12번 인용하면서 그 상한의 **1.6배**다 — 그리고 철거된 도구를 8번 부른다
**Category:** 문서 / 원장 위생
**Priority:** P3
**출처:** 2026-09-06 제로컨텍스트 재개 검증(서브에이전트)이 덤으로 잡아냄

**증상:** ★**수를 여기 박지 않는다** — 이 항목의 주제가 「낡은 수」인데 초판이 스스로 그것을 했다.
2026-09-06 초판은 `1,132줄 / 146,414 B` 와 「`bl-audit.sh` **8곳**」을 박았고 **어느 커밋 상태와도
안 맞았다**(사라진 중간 워킹트리를 잰 값이다 — 같은 날 재확인 = 1,148줄 / 148,470 B / **9곳**).
파일이 편집될 때마다 낡으므로 **재는 명령**만 남긴다.

```
wc -l docs/status.md ; wc -c docs/status.md      # 자기 크기 — 다이어트 표(:346)와 대조
grep -c '700줄' docs/status.md                    # 스스로 인용하는 상한 횟수
grep -c 'bl-audit.sh' docs/status.md              # 철거된 도구를 부르는 횟수
```

| 무엇 | 파일이 말하는 것 | 실제 |
| --- | --- | --- |
| `status.md` 크기 | `:346` 다이어트 표 = **49,845 B** | 그 **약 3배** |
| 줄 수 상한 | 「700줄 상한」을 여러 곳에서 인용 | 그 **1.6배 이상** |
| `bl-audit.sh` | 함정 3건 + 「`--list DEFERRED` **151건**」을 인용 | ADR-037 로 **철거됨** · DEFERRED 실제 **23건**. ★**취소선 없는 살아 있는 인용**이 남아 있다 |

★**세 줄 다 「집행하는 것이 없다」는 같은 원인이다.** `ledger-vitals` 4축은 `다음 행동` 수 · ⓪ 행 수 ·
역류 · §5 겨냥만 본다 — **크기도 인용 대상의 생존도 안 본다.** 그래서 다이어트 표가 낡아도,
철거된 도구를 계속 불러도 초록이다.

★**비용은 착수 비용이다.** `AGENTS.md` 는 새 세션에게 이 파일을 **필독 3종 중 하나**로 지정한다.
2026-08-10 docs-context-diet 가 착수 비용을 103k→51k tok 로 줄였는데, 그 절감이 이 파일 하나로
상당 부분 되돌아갔다.

**권장 접근:** ⑴ `:346` 표를 **실측으로 갱신**하거나 표를 지운다(낡은 수는 없는 수보다 나쁘다).
⑵ 죽은 `bl-audit.sh` 인용 8곳을 지운다 — 그 자리에 「무엇으로 대체됐나」를 적어야 다음 사람이
또 찾지 않는다. ⑶ 상한을 지킬 생각이 없으면 **12곳의 인용을 지워라** — 지켜지지 않는 상한을
12번 인용하는 것이 이 파일이 스스로 신뢰를 깎는 방식이다.
★**크기 축을 `ledger-vitals` 에 넣을지는 별도 판단이다** — 넣으면 다이어트가 강제되지만,
회차마다 red 를 만들 수 있다.
**Risk:** 🟢 (문서만. 다만 잘못 지우면 살아 있는 지시가 사라지므로 인용 8곳은 하나씩 대조)
...
**Title:** 화면 리뷰가 절반이다 — **동적 라우트 6종을 한 번도 열지 않았고**, 그중에 백테스트 리포트가 있다
**Category:** FE / UI·UX 리뷰 커버리지
**Priority:** P2
...
| 안 본 라우트 | 무엇 | 왜 중요한가 |
| --- | --- | --- |
| `/backtests/[id]` | 리포트 9섹션(성과 요약~가정 흔들기) | **이 제품의 핵심 화면.** d1 실사용 루프가 반드시 지난다 |
| `/backtests/[id]/trades` | 거래 원장 표 | `.table-wrap` 스크롤 축의 최대 소비자 |
| `/optimizer/[id]` | 히트맵·반복 이력·OOS | 차트 4종이 한 화면에 있다 |
| `/strategies/[id]/edit` | Monaco 편집기 + 4탭 | 유일한 코드 편집 표면 |
| `/invite/[token]` | 초대 5분기 | 공개 라우트인데 스윕 밖 |
| `/share/backtests/[token]` | 공개 공유 리포트 | 로그인 없이 남에게 보이는 **유일한** 화면 |

★**빠진 이유가 곧 재발 조건이다** — 다음 스윕도 정적 라우트 목록으로 만들면 같은 6개가 또 빠진다.

★**`ui-ux-pro-max` 축이 한 번도 안 돌았다.** 사용자가 명시적으로 요청했는데(2026-09-06) 스윕은
...
길은 둘:
- **e2e 로 돌 때** — `cd apps/web && pnpm exec playwright test --project=setup` 이
  `e2e/.auth/storageState.json` 을 **매 실행 재발급**한다(만료를 스스로 처리한다).
- **브라우저로 눈으로 볼 때** — `/sign-in` 에서 그 두 값을 폼에 직접 넣는다. `curl` 로 확인하지 마라
  — `Origin` 헤더를 안 보내 검사를 건너뛰고 200 을 낸다(2026-08-16 오판). **증인은 브라우저다.**

★**리뷰의 합격 기준 (이것이 없으면 「끝났다」를 스스로 정의하게 된다).**
화면당 산출 **3줄**을 남긴다 — ⑴ 이 화면이 답해야 하는 질문 한 줄 ⑵ 그 질문에 답하는가(예/아니오
+ 근거 1개) ⑶ 첫 진입에서 다음 행동이 보이는가. 여기서 나온 것만 BL 로 등재한다.
★**기계 축(가로스크롤·axe·대비·포커스링·콘솔)은 이미 초록이라 여기서 새로 얻을 것이 거의 없다** —
그것을 다시 재고 「이상 없음」으로 닫지 마라. 이 항목이 사는 것은 **기계가 못 재는 축**이다.
★**측정 가능한 산출은 ⑵ 하나뿐이다** — 라우트 목록을 `page.tsx` 글롭 + id 주입으로 만드는 것.
★★**그 스윕 스크립트는 레포에 없다**(2026-09-06 제로컨텍스트 검증이 지적, 실측 확인). PR #872 의
스윕은 scratchpad 에서 돌리고 **커밋하지 않았다** — 그래서 「목록을 고쳐라」가 **존재하지 않는 대상**을
겨냥하고 있었다. ⇒ ⑵ 는 「고치기」가 아니라 **「만들어서 커밋하기」**다. 그것이 이 축에서
**유일하게 셀 수 있는 산출**이고, 커밋되지 않으면 다음 스윕도 같은 6개를 또 빠뜨린다.
★한 번 쓰고 버린 것이 이 항목을 만든 원인이라는 점을 기억해라 — 재발 조건은 **정적 목록**이 아니라
**스크립트가 커밋 밖에 산 것**이다.

**권장 접근:** ⑴ 위 6종을 실데이터로 열고 **`ui-ux-pro-max` 렌즈**로 본다(기계 축은 이미 초록이라
새로 얻을 것이 적다 — 얻을 것은 위계·흐름·카피다). ⑵ 스윕 스크립트의 라우트 목록을
**`page.tsx` 글롭 + id 주입**으로 바꿔 재발을 막는다. ⑶ 발견은 BL 로 등재하되 하드 제약 위반은 즉시 수리.
**Risk:** 🟢 (읽기 위주. 수리는 발견 후 별도 판단)

**상태:** 🟡 PARTIAL — 2026-09-06 등재, **2026-09-08 리뷰 실행**.
실데이터로 연 것 **5/6**(`/invite/:token` 은 픽스처 0행 — 위 재개 재료 참조).
발견은 [BL-859]·[BL-860]·[BL-861]·[BL-862]·[BL-863] 으로 등재했다.
측정 가능한 산출 ⑵ 는 **커밋됐다** — `apps/web/scripts/app-routes.mjs`(글롭 + id 주입) +
`apps/web/src/__tests__/app-routes-coverage.test.ts`(분류 래칫, vitest 이므로 **CI 가 문다**).
★**남은 것은 `/invite/:token` 하나뿐이고 그것은 픽스처 작업이다** — 이 항목이 아니라 별건이다.
**트리거 판정:** 도래 (남은 1종은 픽스처가 선행)

---

### BL-859

**Title:** 결과 3화면이 **「무엇의 결과인지」를 안 싣는다** — 그리고 부모→자식 링크가 전부 없다
**Category:** FE / 정보 위계 · 내비게이션
**Priority:** P2
**출처:** 2026-09-08 [BL-856] 동적 라우트 실데이터 리뷰 (Playwright MCP · `ui-ux-pro-max` 렌즈)

**증상 (실측 · 백테스트 `a4971a38` / 전략 `0d94167b` 기준):**

| 화면 | `h1` | 전략명 | `/strategies/:id` 링크 |
| --- | --- | --- | --- |
| `/backtests/:id` | `BTC/USDT · 1h` | 0건 | 0건 |
| `/backtests/:id/trades` | `거래 내역` | 0건 | 0건 |
| `/optimizer/:id` | `그리드 탐색` | 0건 | 0건 |
| `/strategies/:id/edit` | **`EMA Crossover Demo`** | — | — |

★**이름은 있는데 안 쓴다.** 편집기의 `h1` 이 그 이름이고 Pine 원문 2행도 `strategy("EMA Crossover Demo", …)` 다.
결과 화면 셋은 그것을 한 번도 싣지 않는다. 이 계정의 백테스트 **17건**이 같은 심볼·타임프레임이면
리포트를 열어도 **어느 전략의 결과인지 알 수 없다.**

★**링크가 한 방향으로만 난다** (`main a[href]` 실측):

| 방향 | 있나 |
| --- | --- |
| 원장 → 리포트 · 옵티마이저 → 백테스트 | **있다** |
| 리포트 → 원장 | **없다** — 그런데 리포트 본문이 「전체 원장과 런업·드로다운 분해는 **상세 보기에서** 확인합니다」라고 **그 화면을 가리킨다** |
| 리포트 → 이 백테스트의 최적화 실행 | 없다 (`/optimizer` 목록으로만) |
| 전략 → 이 전략의 백테스트 | 없다 (`/backtests/new` 로만) |

⇒ **자식은 부모로 올라가는데 부모는 자식·형제로 못 내려간다.** 사용자는 매번 목록으로 나가 다시 찾는다.
문구가 가리키는 화면에 문이 없는 것은 `/onboarding` 도달 불가([BL-852] 동반 항목)와 **같은 계열의 2번째 사례**다.

**권장 접근:** ⑴ 결과 3화면 헤더 칩에 전략명 + `/strategies/:id/edit` 링크를 넣는다(칩 자리는 이미 있다).
⑵ 리포트 「거래 내역」 섹션 제목 옆에 `/backtests/:id/trades` 링크. 문구가 이미 그것을 약속한다.
⑶ 리포트 → 이 백테스트의 옵티마이저 실행 목록. 역방향 링크는 이미 있으므로 데이터는 있다.
**Risk:** 🟢 (링크·라벨 추가. 되돌리기 쉽다)

**상태:** 🔵 ACTIVE — 2026-09-08 등재, 미수리
**트리거 판정:** 도래 (단독 착수 가능)
...
**Title:** 리포트와 원장이 **같은 거래에 다른 번호**를 단다 (1 어긋남)
**Category:** FE / 데이터 표시 일관성
**Priority:** P2
**출처:** 2026-09-08 [BL-856] 리뷰 — 두 화면을 나란히 놓아야만 보였다

**증상 (실측):** 같은 거래(롱 · 진입 `2026-08-23 14:00` · 진입가 `77,172.60` · 수량 `0.0132` · 손익 `-0.70` · 「보유 중」)가

| 화면 | 번호 | 범위 |
| --- | --- | --- |
| `/backtests/:id` 리포트 | **77** | 1‥77 |
| `/backtests/:id/trades` 원장 | **76** | 0‥76 |

**컬럼명은 양쪽 다 「번호」다.** 원장은 0-based, 리포트는 1-based. 「거래 #62 를 봐라」가 화면에 따라
다른 거래를 가리킨다. CSV 내보내기도 원장 기준이라 **화면과 파일이 또 어긋난다**(미확인 — 받아서 대조할 것).

★부수 관찰 — 「완료 거래」 수는 화면마다 정당하게 다르다: 리포트 **77**(미청산 1 포함) · 공유 **76**(완료만).
라벨이 그것을 말하므로 이쪽은 결함이 아니다. **결함은 같은 라벨(「번호」)에 다른 기준을 쓴 것뿐이다.**

**권장 접근:** 한쪽으로 통일한다. 원장이 원시 인덱스(0-based)를 그대로 쓰는 쪽이 원인일 가능성이 높다 —
**표시 번호는 1-based 로 맞추고**, 원시 인덱스가 필요하면 별도 컬럼/속성으로 분리한다.
**Risk:** 🟢 (표시 층. 다만 CSV·상세 패널의 번호도 함께 봐야 한다)
...
**Title:** 공유 화면이 **가정과 데이터 출처를 통째로 뺐다** — 랜딩이 원칙 04로 약속한 바로 그것이다
**Category:** FE / 제품 원칙 정합
**Priority:** P2
**출처:** 2026-09-08 [BL-856] 리뷰 — `/share/backtests/:token`

**증상 (실측):** 공유 화면 본문 전체가 **357자**이고 `가정|수수료|슬리피지|데이터 출처` 매치 **0건**이다.
싣는 것은 심볼·기간·KPI 4개·자산 곡선 미리보기·가입 CTA 2개뿐이다.

★**같은 제품의 로그인 화면이 원칙 04로 이렇게 적는다** —
「리포트는 체결 가정과 데이터 출처를 결과 옆에 인쇄합니다. 체결 시점, 수수료율, 슬리피지, 데이터 구간,
펀딩 반영 여부를 수익률과 같은 화면에 둡니다. **가정을 모르면 수익률도 읽을 수 없기 때문입니다.**」

로그인 리포트는 그 약속을 지킨다(「가정과 데이터 출처」 섹션 존재). **로그인 없이 남에게 보이는 유일한
화면만 그것을 뺐다.** 즉 약속이 가장 필요한 대상(맥락이 0인 외부인)에게만 안 지켜진다.

★부수 — 같은 화면에서 **부호와 색이 동시에 사라진다.** 리포트 `+0.48%`(초록) → 공유 `0.48%`(검정).
KPI 4개가 전부 `rgb(23,26,30)` 이라 **손익 방향을 색으로도 기호로도 못 읽는다**([BL-862] 와 같은 뿌리).

**권장 접근:** 리포트의 「가정과 데이터 출처」 블록을 공유본에 **읽기 전용으로 그대로** 싣는다.
민감한 것이 없다 — 수수료율·슬리피지·데이터 구간은 결과를 읽는 데 필요한 값이지 비밀이 아니다.
★**이 화면은 마케팅 화면이기도 하다** — 가정을 같이 싣는 것이 이 제품의 차별점 자체다.
**Risk:** 🟢 (추가만. 공유 payload 에 필드가 이미 있는지는 확인 필요)
...
**Title:** 손익 색 규칙이 **화면마다 다르고**, 샤프 지수는 어디서도 색이 없다
**Category:** FE / 디자인 시스템 (시맨틱 색)
**Priority:** P3
**출처:** 2026-09-08 [BL-856] 리뷰 — `getComputedStyle` 실측

**증상 (실측):**

| 화면 | 수익 | 손실 | 샤프 |
| --- | --- | --- | --- |
| `/backtests/:id` | `+0.48%` 초록 `rgb(7,75,52)` | 낙폭 `-1.85%` 빨강 `rgb(173,50,42)` | `-0.15` **`rgb(23,26,30)` 본문색** |
| `/backtests/:id/trades` | 평균 수익 초록 | 평균 손실 빨강 | — |
| `/share/backtests/:token` | `0.48%` **검정** | `-1.85%` **검정** | `-0.15` **검정** |

★**색이 실려야 할 곳에만 안 실린다.** 최대 낙폭은 **항상 음수**라 빨강이 정보를 0 나른다. 반대로
샤프 `-0.15`(= 위험조정 수익이 마이너스, 이 실행의 가장 중요한 나쁜 신호)는 **중립색**이다.
⇒ 화면이 「초록 = 좋음」을 가르쳐 놓고 진짜 나쁜 신호에만 색을 안 준다.

**권장 접근:** 시맨틱 규칙을 하나로 정한다 — 「**부호가 의미를 갖는 값에만** 색을 싣는다」.
그러면 낙폭은 중립, 샤프·수익률·손익은 부호색이 된다. 공유본도 같은 규칙을 쓴다([BL-861]).
★[BL-853] 어휘 이원화와 **같은 층의 문제**지만 축이 다르다 — 저쪽은 형태(높이·반경·그림자), 이쪽은 의미(색).
**Risk:** 🟢 (토큰 적용. 화면 증거 베이스라인 갱신이 따라온다)
...
**Title:** 화면당 **주 행동이 안 보인다** — 리포트는 98% 지점에, 편집기는 활성 primary 가 0개
**Category:** FE / 행동 유도 (CTA 위계)
**Priority:** P3
**출처:** 2026-09-08 [BL-856] 리뷰

**증상 (실측):**

⑴ **리포트** — 페이지가 스스로 제시하는 다음 행동 3종(「이 결과로 무엇을 할까요?」 = 최적화 / 스트레스 /
거래소 연결)이 `y=6154`, 전체 `scrollHeight` **6549** 의 **94% 지점**에 있다. 폴드에서 유일한 코퍼 채움
버튼은 **「재실행」**(같은 계산을 반복) 이다. 게다가 코퍼 채움이 한 화면에 **3개**(재실행 · Monte Carlo 실행 ·
최적화 실행)이고 탭 선택 상태(`전체`)까지 같은 채움이라 **주 행동과 선택 상태의 시각 무게가 같다.**

⑵ **전략 편집기** — `.btn-primary` 를 가진 것은 「저장」·「변경사항 저장」·「설정 저장」 **셋뿐이고 전부
`disabled`** 다. 지금 할 수 있는 유일한 전진 행동 「백테스트 실행」은 **modifier 없는 `.btn`**(기본 변형)이다.
⇒ 첫 진입에서 **활성 primary 가 0개**다. 또한 서로 다른 대상을 저장하는 버튼 3개 중 하나만 총칭 「저장」이라
무엇이 무엇을 저장하는지 라벨로 구분되지 않는다.

⑶ **옵티마이저** — KPI 4칸 중 2칸이 **같은 값**이다(「최적 목표값 `0.20`」 = 「최적 셀 샤프 지수 `0.20`」).
목표 함수가 샤프라 **정의상 항상 같다** — 화면이 그 사실을 본문에 적어 두고도 두 칸을 쓴다.

**권장 접근:** ⑴ 리포트 헤더의 채움 버튼을 「재실행」에서 다음 행동으로 바꾸거나, 「이 결과로 무엇을
할까요?」 블록을 **가정 섹션 위**로 올린다. ⑵ 「백테스트 실행」을 primary 로. 저장 3종은 대상을 라벨에 넣는다.
⑶ 중복 KPI 칸을 다른 값(조합 수 · OOS 검증 여부 · 값별 분산)으로 교체한다.
★**폴드 위 채움 버튼은 화면당 하나로 제한**하는 것이 세 항목의 공통 처방이다.
**Risk:** 🟢 (배치·변형 변경. 기능 무변)
...
**상태:** 🟡 부분 해결 — 2026-09-10 그림자 축 종결(`html:not(.dark)` 오버라이드 · KITPORT 밖 · `design-canon-light-shadow.test.ts` 가 라이트 알파 ≤0.3 을 잰다 · 실측 라이트 카드 α .55/.8 → .06/.18, 다크 불변). 남은 것 = **버튼 높이 48/38 · 반경 6·10/12** 두 축(시각 회귀가 전 화면이라 별도 회차)
**트리거 판정:** 도래 (단독 착수 가능 — 남은 두 축)
