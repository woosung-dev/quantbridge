...
| [BL-591](#bl-591)    | ★**뿌리** — 엔진 포지션의 SSOT 가 없다. `run_live` 시뮬이 매 tick 봉을 재생해 포지션을 **도출**하고 정상 운행 중엔 현실로 **보정되지 않는다**. **설계 확정됨(2026-08-04)** = 원장 SSOT + 거래소 veto + 관망. ★착수 전제 **5건 반증**(30건 Resolved→**16** · 겨냥 대상은 `engine_only` 314 가 아니라 `exchange_only` **21** · C 는 교정이 아니라 **예방** · `_ledger_gap_seed` **재사용 불가**) | 발산 증상 BL 을 또 하나 열기 전에 · 소크가 또 죽었을 때                                                           | L            | 2026-08-03 breach-rejection-recovery                   |
...
**상태:** 🟢 **Open — 설계 확정됨 (2026-08-04), 구현 진행 중**
