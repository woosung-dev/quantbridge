...
- 축약 허용 자리는 **좁은 칸**(표 헤더 · `screen-02` 전략 성과 카드 `.perf-figure .k` 4곳)이고, 방식은 `screen-06:1248` 의 `aria-label` 패턴이다. `abbr="샤프"` 는 17벌 실측 **0건** 이라 `_KIT.md:532` 가 명시적으로 폐기했으므로 쓰지 않는다. 상세 규약은 `_KIT.md` §4.10 이 상위다.
...
#### B7. `total_return` = 총 수익률 (축약 금지)

- 탈락. `수익률` 단독 시각 텍스트, `abbr="수익률"`.
- **축약 금지 근거.** `_KIT.md:534` 가 "축약해도 되는 이름은 `MDD` 와 `샤프` 두 개뿐이고 `수익률` · `거래 수` 같은 다른 열은 축약하지 않는다" 고 못박았다. 이전 판본이 지정했던 `abbr="수익률"` 은 그래서 폐기다.
...
// 축약 매핑 상수는 두지 않는다. `_KIT.md` §4.10 이 상위 캐논이고, 축약은 지표가
// 아니라 **자리**로 판정한다 — 좁은 칸이면 축약, 표 헤더·산문이면 완전형이다.
// 축약 허용 이름은 `MDD` 와 `샤프` 둘뿐이며 `수익률` 은 금지다. 지표 단위 Record 로
// 만들면 그 금지가 무력화되므로, 축약이 필요한 좁은 칸에서 리터럴로 쓴다.
...
/**
 * 성과 지표 이름. 여기 값은 전부 완전형이다. 축약은 `_KIT.md` §4.10 이 판정하며
 * 허용 이름은 `MDD` 와 `샤프` 둘뿐이다(좁은 칸 한정 · `수익률` 축약 금지).
