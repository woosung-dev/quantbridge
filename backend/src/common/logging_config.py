...
def _render_extras(record: logging.LogRecord) -> str:
    ...
    ★키를 하드코딩하지 않는다 — `tasks/live_signal.py` 의 `live_conditional_plan_drop` 발화
    ...
    근거: `extra=` 에 dict 를 unpack 하는 곳은 위 `live_conditional_plan_drop` 발화 **한 자리뿐**이고
    ★**줄번호로 적지 않는다** — 그 앵커는 2026-08-02 BL-576 라벨 분화로 이미 한 번 밀렸다.
