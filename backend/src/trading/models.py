...
class ExchangeExitSyncState(SQLModel, table=True):
    """계정별 청산 원장 과거 적재 진행 상태.

    원장의 ``min(exchange_created_at)`` 으로는 진행을 파생할 수 없다. 청산이 한 건도
    없던 과거 구간을 만나면 삽입이 0 이라 min 이 그대로여서 같은 빈 창을 영원히 다시
    조회하고, 그 이전 역사에는 도달하지 못한다(실측 반증 — 07-05 행 7건이 영구 미적재).
    "행이 없었다" 를 표현하려면 스캔 경계를 따로 영속해야 한다.
    """

    __tablename__ = "exchange_exit_sync_state"
    __table_args__ = ({"schema": "trading"},)
    ...
    # 과거로 훑어 스캔을 마친 가장 이른 경계. 행 유무와 무관하게 전진한다.
    backfilled_from: datetime | None = Field(
        default=None, sa_column=Column(AwareDateTime(), nullable=True)
    )
    created_at: datetime = Field(
        default_factory=lambda: datetime.now(UTC),
