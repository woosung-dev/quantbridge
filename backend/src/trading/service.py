...
class LiveSignalSessionService:
    ...
    async def register(
        self, user_id: UUID, req: RegisterLiveSessionRequest
    ) -> LiveSignalSession:
        # 1. Strategy 조회 + ownership + StrategySettings.model_validate
        strategy = await self._strategy_repo.find_by_id_and_owner(
            req.strategy_id, user_id
        )
        if strategy is None:
        ...
        if current >= self._max_active_per_user:
            raise LiveSessionQuotaExceeded(
                current=current, cap=self._max_active_per_user
            )
