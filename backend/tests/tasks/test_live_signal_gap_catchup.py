...
def _patch_gap_precision(
    monkeypatch: pytest.MonkeyPatch, *, qty_step: str | None
) -> list[object]:
    """gap-resync 판정이 부르는 step 조회를 고정한다. 호출 여부도 기록한다.

    실제 `_reconcile_market_precision` 은 테스트 환경에서 ccxt 에 못 붙어 `None` 을
    돌려준다 — 그러면 판정이 **항상 종전 축으로 폴백**해 [BL-545] 를 아무것도 못 잰다.
    """
    calls: list[object] = []

    async def _fake(sess: object) -> tuple[Decimal, Decimal] | None:
        calls.append(sess)
        if qty_step is None:
            return None
        return (Decimal(qty_step), Decimal("0.1"))

    monkeypatch.setattr(live_signal_module, "_reconcile_market_precision", _fake)
    return calls


@pytest.mark.asyncio
async def test_long_gap_large_size_mismatch_survives_without_a_step(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """★[BL-545] 결함의 재현 — 5% 축에서는 0.1 BTC 어긋난 상태가 **일치로 통과**한다.

    엔진 4.9 / 거래소 5.0 이면 허용치가 `5.0 * 0.05 = 0.25` 라 차이 0.1 이 삼켜진다.
    이것이 「구 게이트가 막던 불일치를 통과시킨다」의 실체다. **음성 대조**이자
    아래 배선 테스트의 대조군이다 — 같은 입력에서 step 만 바뀐다.
    """
    t0 = datetime(2026, 5, 1, 12, tzinfo=UTC)
    t6 = t0 + timedelta(minutes=6)
    sess = _session(last_evaluated_bar_time=t0)
    sess_repo, event_repo, _ = _install_evaluation(
        monkeypatch,
        sess=sess,
        rows=_rows(t0, t6),
        run_result=_result(
            last_bar_time=t6,
            signals=[],
            open_trades=[_open_trade(trade_id="L", direction="long", qty=4.9, entry_bar=0)],
        ),
        inserted_events=[],
        ledger_fills=[],
    )
    _patch_positions(monkeypatch, positions=[_position(side="long", size="5.0")])
    _patch_gap_precision(monkeypatch, qty_step=None)
...
@pytest.mark.asyncio
async def test_long_gap_large_size_mismatch_deactivates_on_the_step_axis(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """★★[BL-545] **배선** 테스트 — 순수 함수가 아니라 「그것을 쓰는 경로」를 잰다.

    `TestStepDerivedSizeTolerance` 는 `_classify_position_divergence` 를 **직접** 부른다.
    그래서 gap-resync 호출부에서 `qty_step` 인자를 떼어내는 변이(M4)가 그 스위트에서는
    **red 를 하나도 못 낸다** — 실측으로 확인했다. 이 테스트가 그 구멍을 막는다
    (backend/AGENTS.md §10: 「순수 함수 정확성 ≠ 배선」).

    입력은 위 음성 대조와 **완전히 같고** step 하나만 주어진다.
    허용치가 `max(0.001, 5.0 * 0.001) = 0.005` 로 내려가 차이 0.1 이 `size` 가 된다.
    """
    t0 = datetime(2026, 5, 1, 12, tzinfo=UTC)
    t6 = t0 + timedelta(minutes=6)
    sess = _session(last_evaluated_bar_time=t0)
    sess_repo, _, _ = _install_evaluation(
        monkeypatch,
        sess=sess,
        rows=_rows(t0, t6),
        run_result=_result(
    ...
    _patch_positions(monkeypatch, positions=[_position(side="long", size="5.0")])
    calls = _patch_gap_precision(monkeypatch, qty_step="0.001")
    ...
    assert result == {"deactivated": "gap_resync_position_mismatch"}
    sess_repo.deactivate.assert_awaited_once()
    assert calls, "판정이 step 을 실제로 조회해야 한다 — 조회조차 안 하면 배선이 없는 것이다"
