...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE trading.orders ADD COLUMN IF NOT EXISTS trigger_price NUMERIC(18, 8)"
    )
    op.execute(
        "ALTER TABLE trading.orders ADD COLUMN IF NOT EXISTS trigger_by VARCHAR(16)"
    )
    op.execute(
        "ALTER TABLE trading.orders ADD COLUMN IF NOT EXISTS take_profit NUMERIC(18, 8)"
    )
    op.execute(
        "ALTER TABLE trading.orders ADD COLUMN IF NOT EXISTS stop_loss NUMERIC(18, 8)"
    )
