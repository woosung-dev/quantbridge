...
gate-harnesses:
	@rc=0; for h in bl-audit header-audit soak-restart soak-watch docs-audit pre-push-guard assert-main-checkout skip-ratchet signal-check final-gates; do \
	  printf '\n▶ %s-test\n' "$$h"; \
	  bash tools/scripts/$$h-test.sh || rc=$$?; \
	done; \
	if [ "$$rc" != 0 ]; then echo; echo "✗ 하네스 실패 — 게이트가 판별력을 잃었다"; exit 1; fi; \
	echo; echo "✓ 게이트 하네스 10종 전건 통과"
