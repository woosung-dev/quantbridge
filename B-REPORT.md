...
★**게이트를 한 번 더 돌렸고, 이유는 결과가 아니라 순서다.** 위 rc=0 을 낸 실행은 `2bf3e7cf`
시점의 것인데 그 뒤에 이 절을 쓰는 커밋이 하나 더 붙어 `deferred.txt` 의 `sha:` 가 레인 HEAD 와
어긋났다. CONTROL 의 통과 판정식이 **「원장의 sha == 레인 HEAD」**라 그 상태는 코드가 같아도
통과로 안 읽힌다. ⇒ **이 문단을 먼저 커밋하고 게이트를 마지막에 돌리는** 순서로 바로잡았다
(마지막 재실행의 rc 는 이 문서 밖 — 오케스트레이터는 `deferred.txt` 의 `sha:` 가 HEAD 와
같은지로 판정하면 된다). 두 실행 사이에 **코드 변경은 없다**(diff = 이 문서뿐).

⇒ 계약 §마감의 「게이트는 마지막 커밋 뒤에 돌려라」는 pre-commit 의 포매터 때문만이 아니다.
**원장이 sha 를 적기 때문에, 게이트 뒤에 커밋을 하나라도 더 하면 그 원장은 낡는다.**
