...
**활성 브랜치:** `stage/h2-sprint26-signal` (cascade base `9978d33` — Sprint 25 main `aed201f` PR #96 머지 후 새 cascade)

**활성 sprint:** H2 Sprint 26 — Pine Signal Auto-Trading (Live Session daily flow)

- Phase A + B + C 완료 (Tasks #1~#13). Phase D (codex G.2 + 라이브 dogfood + PR) pending.
- BE 1401 → +28 tests (eval 11 + dispatch 11 + ccxt limit_bars 6) / FE 251 → +6 (utils.test.ts)
- 5 Prometheus metric 신규 (`qb_live_signal_*`)
- codex G.0 P1 #3/#4/#5/#6 + P2 #1~#5 모두 코드 반영 (G.2 challenge 는 Phase D)
...
- 사용자 작업 (Phase D): `make up-isolated-build` → PUT settings → POST live-sessions → Bybit Demo 캡처 → DevTools CPU < 30%

**직전 완료:** H2 Sprint 25 — Hybrid (FE E2E Playwright + BE 강화 + codex G.0/G.2)
