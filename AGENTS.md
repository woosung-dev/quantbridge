...
- Sprint 8b Tier-1 + Tier-0 렌더링 + 6/6 corpus + Opus/Sonnet 교차 hardening ✅ 완료 (2026-04-18, `feat/sprint8b-tier1-rendering`) — VirtualStrategyWrapper + RenderingRegistry + v4 alias/iff + switch/stdev/variance + v4 strategy.entry boolean/when= + time/timestamp(month/day 반영) + 40+ Pine enum constants + deleted line 정책. **6 corpus 완주 (2/6 → 6/6)**, 224 pine_v2 tests (+55), backend 750 전체 green
- **다음:** user-defined function(=>) 지원 + 3-Track 라우터(S/A/M) 묶음 → s3_rsid/i3_drfx 실제 매매 검증 재활성화. 이후 별도 세션에 TabParse 1질문 UX (FE). Sprint 7d(OKX + Trading Sessions)는 H1 내 편성 유지
