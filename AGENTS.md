...
**활성 브랜치:** `main @ 78fb39b` (PR #152 office-hours dev-log squash merge — Sprint 35 진입 결정 영구 기록)

**활성 sprint:** Sprint 35 — **Wedge A backtest 단독 정밀화 (polish iter 3 + 4중 AND gate)**. /office-hours session 2026-05-05 결정 (Wedge A backtest 단독 정밀화 + Gated 2-step + 4중 AND gate). codex G.0 validation **iter cap 2 도달, surgery 18건 모두 plan 반영** (iter 1 = 11 / iter 2 = 7, 누적 1.34M tokens, session `019df85d-bcbf-7d63-a6cc-ce106de4ace1`). master plan: [`~/.claude/plans/quantbridge-sprint-35-anchored-oracle.md`](~/.claude/plans/quantbridge-sprint-35-anchored-oracle.md).

**Sprint 35 Mandatory + Stretch 구조 (codex P1-7 surgery):**

- **Mandatory** (1주 안 의무): Slice 1a (BL-178 root cause 6항목 진단) + 1b (BL-178 fix Path A reject/quarantine + observable reason / Path B escape + Day 7 (b) FAIL/UNKNOWN) + **1.5a engine golden oracle (BL-180 신규, hand-computed minimal 2 strategy + tiny OHLCV + checked-in JSON expected)** + 4a mid-dogfood 6항목 + 5 retro
- **Stretch** (mandatory GREEN + 시간 여유): 1.5b dogfood snapshot integration oracle / 2 BL-176 SelectWithDisplayName clear button prop (sentinel/nullable X, schema required 유지) / 3 BL-150 잔여 walk-forward = 기존 `stress_test` API/UI wiring 만 (vectorbt native premise 폐기, codex P1-4)
- **defer Sprint 36+ confirmed**: BL-174 detail / BL-177-A/B/C / BL-179 / vectorbt native walk-forward / monte-carlo

**Day 7 4중 AND gate (codex P1-2 + P2-7 surgery):**

- (a) self-assess ≥7/10 (근거 ≥3 줄)
- (b) BL-178 production BH curve 정상 (3 backtest != null + Path A quarantine log 0). **Path B escape 발동 시 = (b) FAIL/UNKNOWN, gate 통과 불가** (free pass 폐지)
- (c) BL-180 hand oracle 8 test all GREEN (2 strategy × 4 axis: entries/exits/equity/BH bar-by-bar tolerance ±0.0001)
- (d) **new P0=0 AND unresolved Sprint-35-caused P1=0** + 기존 deferred P1 명시 의무

**직전 완료:** Sprint 34 — polish iter 2 (3 PR + codex G.0 P1 surgery 6건, BL-175 본격 fix + BL-177 dense text shorten + BL-166 kill K-2 cancel + mid-dogfood Day 6.5 PASS + BL-178 신규 분리). 상세: [`docs/dev-log/2026-05-05-sprint34-master-retrospective.md`](docs/dev-log/2026-05-05-sprint34-master-retrospective.md). office-hours 분기 결정: [`docs/dev-log/2026-05-05-office-hours-sprint-35-decision.md`](docs/dev-log/2026-05-05-office-hours-sprint-35-decision.md).

**다음 분기:** Sprint 36 = **Day 7 4중 AND gate 결과 따라 결정**:

- **통과 (4 AND gate 모두)** → Sprint 36 = **BL-003 (Bybit mainnet runbook + smoke 스크립트) + BL-005 본격 (1-2주 소액 mainnet, ≠ Beta 본격 진입)**. office-hours session 2026-05-05 P3 정정 = "BL-005 자체 skip 하는 fictitious gate" 차단
- **미통과** → Sprint 36 = **polish iter 4** (새 발견 BL fix) + Day 7 재측정. Path B escape 발동 시 BL-178-BF (backfill) Sprint 36 P0 active

**Beta 본격 진입 (BL-070~075)** = Day 7 4중 AND gate **이후** BL-005 본격 (1-2주 mainnet) 통과 후 별도 trigger. **≠ Day 7 통과 즉시 Beta 진입** (Sprint 28 design 의 P3 fictitious gate detection 결과 영구 반영).

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 40+ 회고·ADR·dogfood 기록 인덱스

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 35 진입 시점 갱신 (BL-180 신규 등록 P1, BL-174 = Sprint 36+ defer confirmed, BL-070~075 trigger 정정 "Day 7 + BL-005 본격 후" → ≠ Beta 본격 즉시 진입). 총 86 → **87 BL** (BL-180 신규 +1 net)
