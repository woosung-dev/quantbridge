...
**활성 브랜치:** `stage/h2-sprint26-signal` (cascade base `4c38d5a` — Sprint 25 PR #98 + AGENTS.md 비대화 PR #99 머지 후 새 cascade)

**활성 sprint:** H2 Sprint 26 — Pine Signal Auto-Trading (Live Session daily flow)

- Phase A + B + C + D 완료 (Tasks #1~#14 + dogfood Day 0). PR #100 머지 대기.
- BE 1401 → +30 tests (eval 13 + dispatch 11 + ccxt limit_bars 6) / FE 251 → +6 (utils.test.ts)
- 5 Prometheus metric 신규 (`qb_live_signal_*`)
- codex G.0 P1 #3/#4/#5/#6 + P2 #1~#5 + G.2 P1 #10 모두 코드 반영 (P1 0건 도달)
- dogfood Day 0 evidence: **Bybit Demo 5 orders filled** (실 체결, exchange_order_id 발급)
...
- 후속 BL: BL-119/120/121 (codex G.2 P2/P3 hardening) + BL-122~125 (dogfood Day 0 발견 — 모두 fix 완료)

**직전 완료:** H2 Sprint 25 — Hybrid (FE E2E Playwright + BE 강화 + codex G.0/G.2) + AGENTS.md 비대화 (#99)

- BE 1401 / FE 251 tests · BL-110a/112/113/114/115 ✅ Resolved · BL-117~129 신규 등록
- 상세: [`docs/dev-log/2026-05-03-sprint25-hybrid.md`](docs/dev-log/2026-05-03-sprint25-hybrid.md)

**다음 분기:** Sprint 27+ Path A Beta 오픈 / Path B G.2 P2 hardening / Path C BL-110b — Sprint 26 PR 머지 + 사용자 직접 dogfood Day 3-7 (Live Session 기반) 종료 후 결정
