...
**활성 브랜치 (예정):** `stage/h2-sprint29-pine-coverage-hardening` (base `main @ 1badefd` — Sprint 28 squash merge 후)

**활성 sprint:** Sprint 29 — Pine Coverage Layer Hardening + DrFXGOD Schema (Type A, 16-22h)

- v2 plan (codex G0 high reasoning 1.6M tokens + Opus fresh ctx 2-검토 후 frame pivot — v1 "Architectural Fix" 의 핵심 가정 stale 판정)
- **진입 baseline 실측:** LuxAlgo / RsiD coverage 0 unsupported (이미 runnable). UtBot strategy 4 unsupported / DrFXGOD 39 unsupported (실제 reject 케이스). 4/6 → **5/6 (83%)** 목표
- **Slice A** (8-12h) UtBot strategy 4 unsupported 처리 (barcolor / heikinashi ADR / request.security graceful / timeframe.period)
- **Slice B** (8-12h) Coverage layer structured schema (line + workaround dict) + DrFXGOD 39 line-numbered 응답
- **Slice C** (4-6h) SSOT parity audit test 4건 (`STDLIB_NAMES ⊆ SUPPORTED_FUNCTIONS` 등) + architecture.md SSOT 명세 갱신 + baseline snapshot dev-log
- 권장 실행: (A‖C) → B 병렬 (cmux 2 워커, 16-22h)
- 메타-방법론: **baseline 재측정 preflight** 의무 (LESSON-037 후보 first validation), brainstorming + writing-plans + codex G0 + TDD + codex challenge G2 + /review
- 상세 plan: [`~/.claude/plans/quantbridge-sprint-29-sunny-origami.md`](../../../.claude/plans/quantbridge-sprint-29-sunny-origami.md)

**직전 완료:** Sprint 28 — Beta prereq 종합 (Phase B+C cleanup + Vertical Slice 3 + 메타-방법론 4종, PR #113 `1badefd` merged)

- 5 PR cascade · dual metric ✅ 통과 (self-assess 8.5/10 / 신규 P0=0 / 기존 P0 4→3) · Beta path A1 narrowest wedge 3 prereq 처리 (BL-141 / BL-140b / BL-004) · LESSON-033/034/035/036 first validation
- 상세: [`docs/dev-log/2026-05-04-sprint28-retrospective.md`](docs/dev-log/2026-05-04-sprint28-retrospective.md)

**다음 분기:** Sprint 30+ — Beta open 인프라 (BL-070~075: 도메인+DNS / Backend 프로덕션 배포 / Resend / 캠페인 / 인터뷰 / H2 게이트) 또는 BL-003 Bybit mainnet runbook. **trigger** = Sprint 29 dual metric 통과 (self-assess ≥7 + 통과율 5/6 + SSOT audit PASS).

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 38+ 회고·ADR·dogfood 기록 인덱스 (Sprint 26/27/dogfood Day 1 inline 은 INDEX 압축)

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 28 진입 67건 → 종료 시점 갱신 (BL-141/140b/004 ✅ Resolved)
...
- **Sprint kickoff 첫 step = baseline 재측정 preflight** (LESSON-037 후보, Sprint 29 v1→v2 frame change 사례에서 도출)
