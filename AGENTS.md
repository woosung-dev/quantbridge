...
**활성 브랜치:** `main @ f17421d` (Sprint 32 7 PR all merged: #134 A BL-168 / #133 B BL-169+170 / #136 D BL-156 / #135 E BL-163 / #137 F gate spec / #138 C BL-171+172 / #139 G gate §4 unblock)

**활성 sprint:** Sprint 33 진입 대기 — **dogfood Day 5 = 6~7/10 borderline** (gate ≥7 잠정 통과 + 1주 안정 검증 의무). 분기: **6.5 양다리 = polish 잔여 + Beta 인프라 BL-070 (도메인+DNS) prep 병행**. Sprint 33 끝 시 1주 안정성 종합 평가 → Beta 본격 진입 (Sprint 34) 또는 polish iter 2

**직전 완료:** Sprint 32 — Surface Trust Recovery (7 PR + retro, codex G.0 P1 4건 surgery, dogfood self-assess **5 → 6~7 (+1.5~2)**)

- **codex G.0 master plan validation** (session `019df68f` resume, 145.5k tokens) — Verdict FIX_FIRST → P1 4건 surgery → GO. P1-1 BL-168 ownership (root Makefile + migrate-isolated 신규 + dev-isolate→dev-isolated 정정) / P1-2 BL-157 live-smoke 과신 보완 (Sprint 32 전용 dogfood gate 신규) / P1-3 Worker B/C 충돌 (B 선행 merge → C MarkerLayer rebase AND 조건) / P1-4 BL-172 P1 확정
- **BL-168** root `Makefile` `migrate-isolated` + `wait-db-isolated` + sentinel + `ifndef` 가드 (sub-make 캐시 미공유 회피) + `container_name` fixed identifier (worktree-robust). fresh dev-isolated env 자동 alembic upgrade
- **BL-156** vectorbt drawdown 의미 정합 (`(equity - running_peak) / running_peak` = equity ratio) + `mdd_unit` / `mdd_exceeds_capital` 메타 추가 + FE caption 4 케이스 (leverage Nx 가정 / 자본 초과 손실)
- **BL-169+170+171+172** EquityChartV2 2-pane 분리 (별도 chart 인스턴스 2개, top USDT + bottom %) + ChartLegend (color/style 명시) + MarkerLayer (arrow entry / circle exit + 색상 long/short, win/loss) + AxisLabelBar (Y축/X축 단위). lightweight-charts 4.x native tooltip 미지원 → marker text 의미화 (가격 + PnL)
- **BL-163** friendly_message 카테고리 6 (corruption / syntax / data / drawing / math / other) + SSOT backend `_UNSUPPORTED_WORKAROUNDS` + 5xx 정보 leak 차단 + Sprint 21 BL-095 missing case (StrategyDegraded.degraded_calls) fix
- **sprint32-dogfood-gate.spec.ts** (codex G.0 P1-2 surgery) — chromium-authed §1+§2+§3+§4 통합 검증. main e2e CI 자동 실행. fresh env green path 3연속 + actionable error + dogfood gate 실측 통과
- **dual metric:** 7 PR / vitest 371+ / backend pytest 138+ / mypy 0 / ruff 0 / tsc 0 / eslint 0 / dogfood self-assess 5 → 6~7 borderline / Surface fix 효과 +1.5 점 / sprint (Sprint 31 = +1, Sprint 30 = +0)
- **자율 병렬 worker isolation 위반 lesson** — Worker C/D 가 메인 worktree branch swap (자체 worktree 에서 cd 로 메인 이동 + push). Worker prompt 명시 만으론 부족 → tooling 강화 (pre-push hook worktree path 검사) Sprint 33+ 후보
- 상세: [`docs/dev-log/2026-05-05-sprint32-master-retrospective.md`](docs/dev-log/2026-05-05-sprint32-master-retrospective.md)

**다음 분기:** Sprint 33 — 6.5 양다리 (polish 잔여 + Beta 인프라 BL-070 prep 병행):

1. **dogfood Day 6/7/.../12 일별 추적** — 1주 안정 self-assess. ≥7 안정 시 BL-005 ✅ Resolved trigger
2. **polish 잔여 BL** — BL-164 (P2 dropdown UUID) / BL-166 (P3 uvicorn cache) / BL-174 (P3 Empty/Failed/Loading) / BL-150 (P2 chart full migration recharts→lightweight-charts)
3. **Beta 인프라 prep (BL-070~072 점진)** — BL-070 도메인 + DNS + Cloudflare 설정 (사용자 manual 1-2h + 24h DNS) / BL-071 Cloud Run dockerfile + healthcheck (Sprint 30 ε prod-code 자산 활용) / BL-072 Resend 검토
4. **Sprint 33 끝 시 종합 평가** — 1주 안정성 + Day 5 ≥7 안정 → Sprint 34 Beta 본격 진입 OR polish iter 2

**Sprint 34+ 이관 (Beta 본격 진입 결정 후):** BL-070~072 본격 / BL-073~075 캠페인+인터뷰+H2 gate / BL-005 ✅ Resolved 잠정 → 안정성 1주 검증 후 final / BL-003 Bybit mainnet / BL-150 chart full migration

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 40+ 회고·ADR·dogfood 기록 인덱스

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 32 종료 시점 갱신 (Sprint 32 7 BL Resolved: BL-156/163/168/169/170/171/172. 잔여 BL-164/166/174/070~072/150 등)
