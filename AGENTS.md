...
**활성 브랜치:** `stage/h2-sprint27-beta-prereq-hotfix` (cascade base `686a71f` — PR #102 + #103 머지 후 새 cascade)

**활성 sprint:** Sprint 27 Beta prereq hotfix — BL-137 + BL-140

- BL-137 frontend Trading Settings UI (메타데이터 tab, leverage/margin_mode/position_size_pct, PUT /strategies/{id}/settings)
- BL-140 LiveSignalDetail Activity Timeline line chart (events 누적 entry/close, recharts dual-line)
- 5 파일 +295/-2 (frontend only, BE 변경 0). tsc/lint/vitest 257/257 PASS.
- BL-140b (real equity curve, BE schema), BL-141 (backtest 인프라) = 후속 sprint
- 상세: [`docs/dev-log/2026-05-04-sprint27-beta-prereq-hotfix.md`](docs/dev-log/2026-05-04-sprint27-beta-prereq-hotfix.md)

**직전 완료:** dogfood Day 1 — Sprint 27 launch (Auto-Loop §0.5 first run)

- Auto-Loop `DOGFOOD_OPTION=A` 무중단 자동 dogfood — TEST_STRATEGIES (PbR + UtBot) 등록 + LiveSession 2건 (5m + 15m) + Beat dispatch 검증 + UI 효과 + CPU smoke
- Beat scheduler `due_count` 1→3 즉시 인식 ✅ / Bybit Demo 90s window +2 filled orders
- LESSON-004 CPU smoke: Long Task 0건 / FPS 120 / blocked main 0.0% (한도 30% 보다 압도적 양호)
- BL-122 fix UI 효과 검증: 4 strategy 모두 name 으로 표시, UUID 노출 0
- dogfood Day 1 Findings (3) + LESSON L-S27-1 — 별도 hotfix PR 후속 처리 (BL 신규 등록 포함)
- 상세: [`docs/dev-log/2026-05-04-dogfood-day1-sprint27-launch.md`](docs/dev-log/2026-05-04-dogfood-day1-sprint27-launch.md)
- self-assessment **8/10** — H1→H2 gate 통과 후 추가 momentum

**직전 완료:** H2 Sprint 26 — Pine Signal Auto-Trading (Live Session daily flow, PR #100 `01481ea` merged)
...
**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — 67건 (Sprint 25 종료 시점)
