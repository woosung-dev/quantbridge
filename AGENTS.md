...
**활성 sprint:** (Sprint 31+ 진입 대기 — brainstorming 후 결정. dogfood Day 3 self-assess 결과 분기. ≥7 → Beta soft-open 인프라 / <7 → Surface polish + caplog root cause)

**직전 완료:** Sprint 30 — Surface Hardening + Beta 인프라 하이브리드 (7 PR, ADR-019 Surface Trust sub-pillar 신설, dual metric 5/7 ✅ + self-assess pending)
...
- **dual metric:** Pine v2 426 / BE 1497 / FE 264→304 (+40) / SSOT 4 / bundle 152KB / 신규 P0=0 / 기존 P0 감소 self-assess Day 3 의존
- **신규 BL:** BL-150 (P2 chart full migration) / BL-151 (P3 golden_backtest) / BL-152 ✅ Resolved (alias) / BL-153 (P3 H3 격리, Strategy DevOps 메시징)
- **Surface Trust 측정 4 기준 중 3 ✅** + dogfood Day 3 self-assess pending (사용자 본인 측정)
...
**다음 분기:** Sprint 31+ — dogfood Day 3 self-assess 결과 분기:

1. **≥7 통과 시:** Beta soft-open 인프라 (BL-070 도메인 + ε deploy 잔여 4 sub-task + BL-072 Resend) ★★★★★
2. **<7 시:** Surface polish 추가 + caplog root cause 추적 (kill_switch logger disabled=True 잔류, Sprint 31 hardening)
3. (병렬) BL-150 Equity chart full migration (recharts → lightweight-charts 통합, ~150KB bundle 절감)
4. BL-003 Bybit mainnet runbook (Bybit Demo 1주 안정 후 trigger) / BL-005 실자본 dogfood (Day 3 ≥7 시 ✅ Resolved 후보)

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 40+ 회고·ADR·dogfood 기록 인덱스

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 30 종료 시점 갱신 (BL-150 P2 chart full migration / BL-151 P3 golden_backtest / BL-152 ✅ Resolved alias / BL-153 P3 Strategy DevOps H3 격리)
