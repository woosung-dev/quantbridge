...
**활성 브랜치:** `main @ 10876a3` + Sprint 37 3 PR open (#159/#160/#161 unmerged) + PR #162 (docs)

**활성 sprint:** Sprint 37 — **polish iter 5 (Day 7 재측정 목표)**. Sprint 36 Day 7 self-assess = **≤6/10 → gate (a) FAIL** → Sprint 37 = office-hours 결정 묶음 + Day 7 재측정.

**Sprint 37 진행 현황 (2026-05-06 office-hours D 옵션 + spot-equivalent 결정):**

- **PR #159** [BL-185](docs/REFACTORING-BACKLOG.md#bl-185) — Pine 포지션 사이징 spot-equivalent (foundation, 3 commits, BE 633 tests). qty=1 hardcode → `default_qty_type/value` 3종 (percent_of_equity / cash / fixed) in-loop. **leverage / funding / liquidation EXCLUDE** (사용자 통찰 — initial_capital 배수로 우회 가능 + simple liquidation 도 trust 갉아먹음). 풀 모델은 BL-186 후속.
- **PR #161** [BL-184](docs/REFACTORING-BACKLOG.md#bl-184) — FE-only equity/BH curve PnL 정렬 (3 commits, FE 415+9 tests). `normalizeToPnlCurve` idempotent helper. BE BH curve absolute capital format 보존 (codex 권장 — metrics/MC/stress 입력 안전).
- **PR #160** [BL-183](docs/REFACTORING-BACKLOG.md#bl-183) — `MonteCarloSummaryTable` 4 통계 컴포넌트 (2 commits, FE 415+9 tests). `MonteCarloFanChart` chart 책임 분리 유지 (codex 권장).
- **office-hours design doc**: [`~/.gstack/projects/quant-bridge/woosung-main-design-20260506-084244.md`](~/.gstack/projects/quant-bridge/woosung-main-design-20260506-084244.md)
- **codex Phase 3.5 GO_WITH_FIXES** — 5 영역 검토 후 적용 (in-loop running_equity / FE-only normalize / leverage UI cleanup / SSOT explicit / 별도 MC 테이블).

**Day 7 4중 AND gate (영구 기준):**

- (a) self-assess ≥7/10 (근거 ≥3 줄) — **≤6/10 FAIL** (BL-183 MC 숫자 미노출 + 기존 6/10 후한 측정)
...
**직전 완료:** Sprint 36 — polish iter 4 (PR #157 BL-150 MC sign-flip fix + BL-176 onClear, Day 7 = ≤6/10 FAIL, BL-183 신규 발견). 상세: [`docs/dev-log/2026-05-06-dogfood-day7-sprint36.md`](docs/dev-log/2026-05-06-dogfood-day7-sprint36.md).

**다음 분기:** Sprint 38 = **Day 7 4중 AND gate 결과 따라 결정**:

- **통과 (4 AND gate 모두)** → Sprint 38 = **BL-003 (Bybit mainnet runbook + smoke 스크립트) + BL-005 본격 (1-2주 소액 mainnet)**
- **미통과** → Sprint 38 = **polish iter 6** + Day 7 재측정
...
**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 36 BL-150 / BL-176 Resolved + Sprint 37 BL-183/184/185 등록 (3 PR open) + BL-186 신규 deferred (Sprint 38+ 풀 leverage 모델). 총 **89 active BL**
