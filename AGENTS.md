...
> `apps/api/AGENTS.md`·`apps/web/AGENTS.md`, 결정 근거는 `docs/decisions/`, 반증 기록은 `docs/lessons.md` 가 정본이다.
...
- `tools/scripts/` — 게이트·감사 셸 (`final-gates` · `bl-audit` · `docs-audit` · `context-budget` · `soak-gate` · `db-backup` · `disk-guard` · `tool-pin-audit` · `api-service`) + 그 하네스 `*-test.sh` 15종
