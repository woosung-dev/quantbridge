...
**활성 브랜치:** `main @ 1badefd` (Sprint 29 stage `ae4798f` 사용자 PR squash merge 대기 중)

**활성 sprint:** (Sprint 30+ 진입 대기 — brainstorming 후 결정. 권장 = Beta open 인프라)

**직전 완료:** Sprint 29 — Pine Coverage Layer Hardening + DrFXGOD Schema (17 commits, dual metric ALL PASS)

- 진입 통과율 3/6 → **종료 5/6 (83%)** ★ (UtBot indicator + strategy 동시 PASS, lever = barcolor/heikinashi/security/timeframe.period 4 항목 공유)
- DrFXGOD response **28/28 = 100% workaround** coverage (≥80% SLO 초과). line + category structured schema + Pydantic V2 round-trip
- SSOT parity audit **4 invariant 모두 PASS** (STDLIB ⊆ SUPPORTED / RENDERING_FACTORIES ⊆ SUPPORTED / V4_ALIASES ⊆ SUPPORTED / \_ATTR_CONSTANTS prefix 화이트리스트). drift 차단 자동화
- heikinashi (a) Trust Layer 위반 + dogfood-only flag ADR 영구 기록 (Sprint 30+ ADR-009 Candle transformation layer trigger 명시)
- Pine v2 BE regression **426 passed / +17 신규** · self-assessment **9/10** · 신규 BL P0=0/P1=0 · 기존 P0 잔여 2 (BL-003/005 deferred)
- **LESSON-037 third validation 통과** — sprint kickoff baseline 재측정 preflight 영구 승격 trigger 도달 (Sprint 30+ kickoff 부터 의무 적용)
- BL-096 partial ✅ Resolved (heikinashi (a) ADR + security graceful)
- 상세: [`docs/dev-log/2026-05-04-sprint29-coverage-hardening.md`](docs/dev-log/2026-05-04-sprint29-coverage-hardening.md)

**다음 분기:** Sprint 30+ — Beta open 인프라 (BL-070~075: 도메인+DNS / Backend 프로덕션 배포 / Resend / 캠페인 / 인터뷰 / H2 게이트) ★★★★★ 권장. 또는 BL-003 Bybit mainnet runbook (Bybit Demo 1주 안정 후) / ADR-009 Candle transformation layer (heikinashi 거짓 양성 발견 시) / BL-005 실자본 dogfood (수동 trigger). **brainstorming 시 결정.**

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 40+ 회고·ADR·dogfood 기록 인덱스

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 28 종료 67건 → Sprint 29 종료 시점 갱신 (BL-096 partial ✅ Resolved 추가)
