...
**활성 브랜치:** `main @ 773424f` (Sprint 30 7 PR all merged: #116 ADR-019 / #117 γ-BE / #118 γ-FE / #119 α+δ / #120 β / #121 ε / #122 mount finalize)

**활성 브랜치:** `main @ 773424f` + hot-fix `edcfaa0` PR #124 (lightweight-charts currentColor) ✅ MERGED + retro Day 3 PR pending

**활성 sprint:** Sprint 31 진입 대기 — **dogfood Day 3 = 4/10** (gate ≥7 3점차 미달). 사용자 코멘트: "$1K~$50K 자본 의사결정 quality bar 까지 도달 안 함". 분기: **Surface polish + bug fix** (BL-154/156/157/159/161). Beta soft-open 인프라 Sprint 32+ 이관

**직전 완료:** Sprint 30 — Surface Hardening + Beta 인프라 하이브리드 (8 PR + retro, ADR-019 Surface Trust sub-pillar 신설, dual metric **5/7 ✅** + self-assess **4/10 ❌**)

- ADR-019 Trust Pillar **4 sub-pillar 분할** + Surface Trust 신설. 외부 LLM YC 브리프 v2 사실 검증 (정확 12 / 부정확 8) 후 본질적 진단 흡수
- PRD `backtests.results` JSONB **24 metric BE+FE 100%** (Sprint 29 12/24 → 24/24). γ-BE vectorbt drift 방어 (try/except None fallback) + γ-FE 17 row + monthly heatmap
- PRD `backtests.config` 5 가정 FE 100% 노출 (default 표시 graceful degrade) — α assumptions-card.tsx
- lightweight-charts 점진 마이그레이션 (Option B) — β trading-chart wrapper + equity-chart-v2 + B&H + drawdown area + 거래 마커 + 152KB chunk (게이트 <+200KB ✅)
- ε Backend prod 인프라 코드 (Cloud Run target Dockerfile + healthcheck + Alembic advisory lock + Prometheus alert wire `qb_pending_alerts > 50`) — H1→H2 게이트 1건 진전
- δ 거래 목록 정렬 5 필드 + 필터 2 + UTF-8 BOM CSV export (Excel 한글 호환)
- **dual metric:** Pine v2 426 / BE 1497 / FE 264→304 (+40) / SSOT 4 / bundle 152KB / 신규 P0=0 / **기존 P0 0 감소 (BL-005 ⏳ self-assess <7)** / **self-assess 4/10 ❌**
- **신규 BL Sprint 30 종료:** BL-150 (P2 chart full migration) / BL-151 (P3 golden_backtest) / BL-152 ✅ Resolved (alias) / BL-153 (P3 H3 격리)
- **신규 BL dogfood Day 3 발견:** BL-154 (P2 γ-BE 신규 12 metric activate) / BL-155 (P3 direction count mismatch) / BL-156 (P2 MDD 수학 모순) / BL-157 (P2 PR live smoke gate) / BL-159 (P2 array._ pre-flight 422) / BL-160 (P3 array.new\__) / BL-161 (P3 Pine v6 ADR)
- **Surface Trust 측정 4 기준 중 3 ✅** + **dogfood Day 3 = 4/10 ❌** (gate 미달, "$1K~$50K 자본 의사결정 quality bar" 미도달)
- **자율 병렬 sprint 패턴 실측** — 메인 세션 + Agent worktree 3개 + 후처리 + 회고 = 7 PR auto-merge gh CLI. allow_auto_merge=true PATCH API 활성화
- 상세: [`docs/dev-log/2026-05-05-sprint30-master-retrospective.md`](docs/dev-log/2026-05-05-sprint30-master-retrospective.md)

**다음 분기:** Sprint 31 — Surface polish + bug fix (self-assess 4 → 7+ 회복):

1. **BL-159 + BL-161** Pine v6 + Coverage Analyzer false negative fix (사용자 첫 strategy `array.new_float` runtime fail 차단)
2. **BL-154** γ-BE vectorbt 신규 12 metric extract 활성화 (drift 보수적 None silent skip 해소)
3. **BL-156** MDD -132.96% 수학 모순 (vectorbt drawdown 검증 + leverage 가정 강조)
4. **BL-157** PR pre-merge live dev smoke gate 강제 (β agent 누락 root cause, hot-fix #124 회귀 차단)
5. **BL-155** direction count mismatch (83 vs 84) minor

**Sprint 32+ 이관:** Beta soft-open 인프라 (BL-070 도메인 / ε deploy 4 sub-task / BL-072 Resend) / BL-150 chart full migration / caplog root cause / BL-003 mainnet / BL-005 (Day 4 ≥7 시 ✅ Resolved)

**전체 sprint 이력:** [`docs/dev-log/INDEX.md`](docs/dev-log/INDEX.md) — 40+ 회고·ADR·dogfood 기록 인덱스

**미해결 BL:** [`docs/REFACTORING-BACKLOG.md`](docs/REFACTORING-BACKLOG.md) — Sprint 30 종료 시점 + Day 3 dogfood 발견 갱신 (BL-150~157 + BL-159~161, P2 5건 / P3 4건 / Resolved 1건)
