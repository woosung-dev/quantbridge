...
- Sprint 7c FE 따라잡기 (Strategy CRUD UI) — scope 결정 완료 (2026-04-17, gstack `/office-hours` session 12). [ADR 008](docs/dev-log/008-sprint7c-scope-decision.md). Implementation plan은 별도 세션에서 `/superpowers:writing-plans` 호출 후 `docs/superpowers/plans/`에 배치 예정
- **다음:** Sprint 7c 구현 → Sprint 7b (OKX 멀티 거래소 + Trading Sessions) → Sprint 8+ (Binance mainnet 실거래)
