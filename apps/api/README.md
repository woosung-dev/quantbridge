...
| strategies         | `POST /strategies/parse` · `GET`·`POST /strategies` · **`GET /strategies/{id}/brief`**([ADR-040]) · `GET /strategies/{id}/brief/narrative` · **`POST /strategies/generate`**([ADR-041]) · `PUT /strategies/{id}/settings` · `POST /strategies/{id}/rotate-webhook-secret` · `POST /strategies/convert-indicator` |
