...
async def test_cancel_submitted_delegates_to_exchange_task(
    ...
    order = await _setup(
        db_session, mock_authed_user, state=OrderState.submitted, exchange_order_id="bybit-x1"
    )
    calls: list[str] = []
    ...
    assert order.state == OrderState.submitted, (
        "submitted 주문이 DB-only cancel 되면 안 됨 (orphan)"
    )


@pytest.mark.asyncio
async def test_cancel_pending_immediate_db_cancel(client, mock_authed_user, db_session):
    """pending 주문 cancel → 200 + 즉시 cancelled (거래소 미발주이므로 안전)."""
    order = await _setup(db_session, mock_authed_user, state=OrderState.pending)
