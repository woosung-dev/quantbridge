...
async def test_terminal_status_transitions_to_filled(submitted_order, db_session, monkeypatch):
...
async def test_unknown_order_state_unchanged_with_alert(submitted_order, db_session, monkeypatch):
...
async def test_cancelled_status_transitions(submitted_order, db_session, monkeypatch):
    order, acc = submitted_order
    fetcher = _make_fetcher(
        open_orders=[{"clientOrderId": str(order.id), "status": "Cancelled", "id": "EX-1"}]
    )
    monkeypatch.setattr(
        "src.trading.services.websocket_reconciliation_service.send_critical_alert",
        AsyncMock(return_value=False),
...
async def test_no_local_active_orders_skips_fetch(db_session):
...
async def test_ccxt_unified_status_closed_transitions_to_filled(
    submitted_order, db_session, monkeypatch
):
...
async def test_ccxt_unified_canceled_transitions(submitted_order, db_session, monkeypatch):
    """CCXT 'canceled' (single-l) → cancelled."""
    order, acc = submitted_order
    fetcher = _make_fetcher(
        open_orders=[{"clientOrderId": str(order.id), "status": "canceled", "id": "EX-201"}]
    )
    monkeypatch.setattr(
        "src.trading.services.websocket_reconciliation_service.send_critical_alert",
        AsyncMock(return_value=False),
...
async def test_reconciler_forwards_transport_request_to_callback():
    """Transport adapter는 DB/REST 구현 대신 조립된 reconciliation callback만 호출한다."""
    callback = AsyncMock()
    account_id = uuid4()

    await Reconciler(callback).run(account_id=account_id)

    callback.assert_awaited_once_with(account_id)
