...
class Reconciler:
    ...
    async def run(self, *, account_id: UUID) -> None:
        async with self._session_factory() as session:
            ...
            exch_recent = await self._fetcher.fetch_recent_orders(account_id, limit=50)
            ...
            for local in local_active:
                ...
                if status in _TERMINAL_STATUSES:
                    ...
                    if new_state != local.state:
                        rowcount = await self._apply_transition(session, local, new_state, exch)
                        if rowcount == 1:
    ...
    async def _list_local_active(self, session: AsyncSession, account_id: UUID) -> list[Order]:
        """pending / submitted state 의 local order 조회."""
        return await OrderRepository(session).list_active_by_exchange_account(account_id)
    ...
    def _find_match(
        ...
        for o in exch_orders:
            client_id = o.get("clientOrderId") or o.get("orderLinkId") or o.get("clOrdId")
    ...
    async def _handle_unknown(self, local: Order, account_id: UUID) -> None:
        ...
        qb_ws_reconcile_unknown_total.labels(account_id=str(account_id)).inc()
