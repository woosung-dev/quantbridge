...
- **CI** — `ci.yml` 의 backend pytest 스키마는 세션 픽스처의 `drop_all`+`create_all` 이다.
  별도 `strategy-journey` 잡은 실제 앱 기동 전에 `alembic upgrade head` 를 실행한다.
  `alembic check` 게이트 복귀는 재입힘 규칙 경유.
  ...
  「backend pytest 에 없다」와 「레포 어디에도 없다」는 다른 문장이고, 후자로 읽으면 nightly 가 깨졌을 때
