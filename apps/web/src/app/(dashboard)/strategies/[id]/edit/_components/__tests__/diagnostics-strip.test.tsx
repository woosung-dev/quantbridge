...
import { DiagnosticsStrip } from "@/app/(dashboard)/strategies/[id]/edit/_components/diagnostics-strip";
import type { ParsePreviewResponse, StrategyResponse } from "@/features/strategy/schemas";

vi.mock("@/features/strategy/edit-store", () => ({
  useEditStore: (sel: (s: { pineSource: string }) => unknown) => sel({ pineSource: "//@version=5" }),
...
describe("DiagnosticsStrip — C 이식 진짜 탭 구조 (§3-6)", () => {
  ...
  it("role=tablist + tab 3종(파싱/파라미터/지표) + tabpanel 3종을 그린다", () => {
    mockUsePreviewParse.mockReturnValue({ data: makeResult(), isFetching: false, isError: false, refetch: vi.fn() });
