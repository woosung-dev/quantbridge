...
export function TradeOutcomeDonut({ counts }: TradeOutcomeDonutProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-4 sm:flex-row" data-testid="trade-outcome-donut">
      <div ref={wrapperRef} className="relative h-[180px] w-[180px] shrink-0">
        ...
        <div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center">
          <span className="font-mono text-2xl font-bold tabular-nums">{counts.total}</span>
        ...
        <tbody>
          {legendRows.map((row) => (
            <tr key={row.name}>
              ...
              <td className="py-1.5 text-right font-mono tabular-nums">{row.count} 거래</td>
