...
describe("OptimizerSearchForm field errors", () => {
  it.each(forms)(
    "$name 폼은 빈 변수 이름 오류를 row에 표시하고 제출을 차단한다",
    async ({ Form, submitLabel, mutateAsync }) => {
  ...
  it.each(forms)(
    "$name 폼은 최소값이 최대값 이상이면 최대값 오류를 표시하고 제출을 차단한다",
    async ({ Form, submitLabel, mutateAsync }) => {
      render(<Form backtestId={BACKTEST_ID} />);
      ...
      await waitFor(() => expect(screen.getByRole("alert")).toHaveTextContent("최소값은 최대값"));
  ...
  it.each(forms)(
    "$name 폼은 파라미터 행을 전부 삭제하고 제출하면 배열 오류를 표시하고 제출을 차단한다",
    async ({ Form, submitLabel, mutateAsync }) => {
      render(<Form backtestId={BACKTEST_ID} />);
      ...
      await waitFor(() =>
        expect(screen.getByRole("alert")).toHaveTextContent("파라미터를 하나 이상 추가하세요."),
      );
      expect(mutateAsync).not.toHaveBeenCalled();
    },
  );
