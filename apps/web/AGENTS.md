...
- **React Query** — Query Key 하드코딩 금지 → 도메인별 `query-keys.ts` 팩토리(첫 인자 = `userId`). API 호출은
  `features/[domain]/api.ts` 에 모은다(★`apiFetch` 밖에서 **FastAPI 를 직접 `fetch`** 하는 예외 **5곳** — `app/share/backtests/[token]/opengraph-image.tsx` · `features/backtest/components/share/shared-backtest-page.tsx`(공개 share 본문 — ADR-035 대로 여기가 제자리다) · `features/trading/components/test-order-webhook.ts` · `features/waitlist/api.ts`(invite 검증 — 400/404 를 throw 가 아니라 분기로 받으려 우회) · `lib/auth.ts`(beforeDelete). 기준은 **`getApiBase()`/`API_BASE_URL` 를 쓰는 `fetch(`** 라 grep 으로 재현된다 — Next 내부 라우트 호출(`/api/auth/token`)은 세지 않는다. 2026-08-30 3→4 · 2026-09-06 실측으로 4→5 정정 + 위치 정정). 재는 것 없음
