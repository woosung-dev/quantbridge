...
2. **기술 스택 준수**: `docs/decisions/`(ADR)에 정의된 기술 선택을 벗어나지 않았는가? (예: pine_v2 인터프리터 — `exec`/`eval` 금지, Better Auth JWKS, Zod v4)
...
5. **표준 러너 통과**: `ruff` / `pytest`(BE) · `eslint` / `tsc` / `vitest`(FE)가 에러 없이 통과하는가?
